...
DECLARE 
    ...
    @assessment1Settings NVARCHAR(MAX) = '{"items":[{"typeName":"Multiple Choice","question":"How many apostles are there?","helpText":"","answers":["7","12","20"],"correctAnswer":"12"},{"typeName":"Section","title":"Short Answer Section","summary":"Please answer with as much detail as you can muster."},{"answerBoxRows":5,"helpText":"","maxCharacterCount":120,"pointsEarned":0,"question":"Are there any other numbers in the Bible where that match the number of apostles?","questionWeight":5,"typeName":"Short Answer"}],"assessmentTerm":"Asessment","header":"{\"time\":1716326144116,\"blocks\":[{\"id\":\"-SQIQa_9xh\",\"type\":\"paragraph\",\"data\":{\"text\":\"Please complete to the best of your ability.\"}}],\"version\":\"2.28.0\"}","multipleChoiceWeight":1,"showMissedQuestionsOnResults":true,"showResultsOnCompletion":true}',
    ...
    @checkOffSettings NVARCHAR(MAX) = '{"time":1714410593015,"blocks":[{"id":"nUecbc6V6Z","type":"paragraph","data":{"text":"I Certify that I have prepared my final assessment on my own without any significant contributions from others."}}],"version":"2.28.0"}';
...
FROM (
    SELECT 'Assessment 1' [Name], 1 [Order], @assessmentComponentId [ActivityComponentId], 0 [AssignTo], @classStartOffset [DueCalculationMethod], NULL [DueDateDefault], 2 [DueDateOffset], @alwaysAvailable [AvailableCalculationMethod], NULL [AvailableDateDefault], NULL AvailableDateOffset, 5 [Points], 1 [IsStudentCommentingEnabled], 0 [SendNotificationCommunication], @assessment1Description [Description], @assessment1Settings [SettingsJson]
    UNION SELECT 'Check Off' [Name], 2 [Order], @checkOffComponentId [ActivityComponentId], 0 [AssignTo], @neverDue [DueCalculationMethod], NULL [DueDateDefault], NULL [DueDateOffset], @alwaysAvailable [AvailableCalculationMethod], NULL [AvailableDateDefault], NULL AvailableDateOffset, 10 [Points], 0 [IsStudentCommentingEnabled], 0 [SendNotificationCommunication], @checkOffDescription, @checkOffSettings
    UNION SELECT 'Review Final' [Name], 3 [Order], @pointAssessmentComponentId [ActivityComponentId], 1 [AssignTo], @classStartOffset [DueCalculationMethod], NULL [DueDateDefault], 12 [DueDateOffset], @availableAfterPreviousComplete [AvailableCalculationMethod], NULL [AvailableDateDefault], NULL AvailableDateOffset, 0 [Points], 0 [IsStudentCommentingEnabled], 0 [SendNotificationCommunication], @presentationDescription, @presentationSettings
) seed
JOIN [Group] g ON 1 = 1
