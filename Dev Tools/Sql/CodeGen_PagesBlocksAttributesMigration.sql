...
****************************************************************************************************************************************************************************************
****************************************************************************************************************************************************************************************/
-- Use all available 128 characters for the column name so that the results will be easier to read without resizing columns in SSMS.
SELECT InstructionNote AS [******************************************************WARNING READ THIS!!******************************************************]
FROM (
    SELECT 'Instructions and Parameters' InstructionNote, 0 DisplayOrder
    UNION SELECT '1. Only run this script on a clean database that only has your changes in it ', 10
    UNION SELECT '    (e.g. new block type, page, block instance, etc...)', 20
    UNION SELECT '2. This script was originally intended for core code (e.g. not plugins);', 30
    UNION SELECT '  however some adaptations have since been made which might be beneficial for plugins.', 40
    UNION SELECT '  USE WITH CAUTION', 50
    UNION SELECT '3. To copy and paste directly from the grid results ', 60
    UNION SELECT '  turn on Options > Query Results > Sql Server > Results to Grid > ''Retain CR/LF on copy or save''', 70
    UNION SELECT '4. The results should not be blindly accepted. Make sure the output is correct.', 80
    UNION SELECT '', 90
    UNION SELECT 'Available Parameters:', 100
    UNION SELECT '  @forceIncludeRecentlyUpdatedBlockAttributeValues:', 105
    UNION SELECT '      Set to 1 to include any block attribute values updated in the last 60 minutes, ', 107
    UNION SELECT '      even if their BlockType Attribute is IsSystem = 1', 110
    UNION SELECT '  @forceIncludeRecentlyUpdatedBlockAttributeValuesMinutes:', 115
    UNION SELECT '      Used in conjunction with the parameter ''@forceIncludeRecentlyUpdatedBlockAttributeValues''.', 117
    UNION SELECT '      Changes the timeframe.', 120
    UNION SELECT '  @useSkipIfAlreadyExistsForAddBlockAttributeValue:', 125
    UNION SELECT '      Set to 1 to only add/update the attribute value if it doesn''t already exists, ', 128
    UNION SELECT '      otherwise the current attribute value will be overwritten', 130
    UNION SELECT '  @includeEntityTypesForPluginEntityTypeNamesLike:', 150
    UNION SELECT '      If developing a plugin and migrations for EntityTypes, BlockTypes, etc. are necessary ', 160
    UNION SELECT '      the reverse domain can be added here and will be used with a wildcard to include', 170
    UNION SELECT '      [EntityType] records whose [Name] begins with the specified value.', 180
    UNION SELECT '', 190
    UNION SELECT '  @forceIncludeMigrationsForPageId: ', 200
    UNION SELECT '      Set to a specific pageId if you want to generate ''AddBlockAttributeValue'' migrations', 205
    UNION SELECT '      for all the block attribute values and page migrations on the page, not just the new ones', 210
) instructionLines ORDER BY DisplayOrder
/***************************************************************************************************************************************************************************************/
...
@forceIncludeRecentlyUpdatedBlockAttributeValuesMinutes int = 60,

-- set to 1 to only add/update the attribute value if it doesn't already exists, otherwise it'll overwrite the current attribute value
@useSkipIfAlreadyExistsForAddBlockAttributeValue bit = 0,

-- If developing a plugin and migrations for EntityTypes, BlockTypes, etc. are necessary 
-- the reverse domain can be added here and will be used with a wildcard
-- to include [EntityType] records whose [Name] begins with the specified value.
-- For example if RockSolidChurch.com is developing plugins then set 
-- @includeEntityTypesForPluginEntityTypeNamesLike = 'com.RockSolidChurch' 
-- Resulting in SQL Like: EntityType.Name LIKE 'com.RockSolidChurch%'
@includeEntityTypesForPluginEntityTypeNamesLike NVARCHAR(200) = '',

-- set to a specific pageId if you want to generate 'AddBlockAttributeValue' migrations for all the block attribute values and page migrations on the page, not just the new ones
@forceIncludeMigrationsForPageId int = null;
...
('0C828414-8AEF-4B43-AAEF-B200544A2197'), -- External WebSite
('70DCBB50-0978-4B24-9382-CDD9BEED5ADB'), -- External WebSite
('847E12E0-A7FC-4BD5-BD7E-1E9D435510E7'), -- External WebSite
('71D998C7-9F27-4B8A-937A-64C5EFC4783A') -- External WebSite
...
create table #codeTable (
    ...
    SELECT 
        @crlf + '            // Add Page ' +
        @crlf + '            //  Internal Name: '  + p.InternalName +
        @crlf + '            //  Site: '  + s.Name +
        @crlf + '            RockMigrationHelper.AddPage(' +
		' true, "' +
	...
	join [site] [s] on [s].[Id] = [l].[siteId]
    where ([p].[IsSystem] = 0 or @forceIncludeMigrationsForPageId = p.Id )
    ...
    -- Set PageTitle and BrowserTitle on pages if they are different than the internal name
    insert into #codeTable
    SELECT concat('            Sql("UPDATE [Page] set PageTitle = ''', p.PageTitle, ''', InternalName = ''', p.InternalName, ''', BrowserTitle = ''', p.BrowserTitle, ''' where [Guid] =''', p.[Guid], '''");'  )
    FROM [Page] [p]
    where ([p].[IsSystem] = 0 or @forceIncludeMigrationsForPageId = p.Id )
    and (p.PageTitle != p.InternalName or p.BrowserTitle != p.InternalName )
    and [p].[Guid] not in (select [Guid] from #knownGuidsToIgnore)
    order by [p].[Id]
    ...
    insert into #codeTable
    SELECT 
        @crlf + '           // Add/Update BlockType ' +
        @crlf + '           //   Name: ' + isnull(bt.[Name], '???') + 
        @crlf + '           //   Category: ' + isnull(bt.[Category], '') + 
        @crlf + '           //   Path: ' + isnull(bt.[Path], '-') + 
        @crlf + '           //   EntityType: ' + isnull(et.[FriendlyName], '-') + 
        @crlf +'            RockMigrationHelper.UpdateBlockType("'+
        bt.[Name]+ '","'+  
        ISNULL(bt.[Description],'')+ '","'+  
        [Path]+ '","'+  
        ISNULL(bt.[Category],'')+ '","'+  
        CONVERT( nvarchar(50),bt.[Guid])+ '");'
    from [BlockType] bt
    left outer join [EntityType] et on et.Id = bt.EntityTypeId
    where bt.[IsSystem] = 0
    and bt.Path is not null
    and bt.[Guid] not in (select [Guid] from #knownGuidsToIgnore) -- shouldn't happen
    order by bt.[Id]

    INSERT INTO #codeTable SELECT @crlf
	...
	FROM [BlockType] bt
	JOIN [EntityType] et ON bt.[EntityTypeId] = et.[Id]
	WHERE bt.[IsSystem] = 0
		AND bt.[Path] IS NULL
		AND bt.[Category] LIKE 'Mobile%'
		AND bt.[Guid] NOT IN (SELECT [Guid] FROM #knownGuidsToIgnore) -- shouldn't happen
	...
	INSERT INTO #codeTable
	SELECT 
        @crlf + '            // Add/Update Mobile Block Type' +
        @crlf + '            //   Name:' + isnull(bt.[Name], '') + 
        @crlf + '            //   Category:' + isnull(bt.[Category], '') + 
        @crlf + '            //   EntityType:' + isnull(et.[Name], '') + 
        @crlf +'            RockMigrationHelper.AddOrUpdateEntityBlockType('
		+ '"' + bt.[Name] + '", '
		+ '"' + ISNULL(bt.[Description], '') + '", '
		+ '"' + et.[Name] + '", '
		+ '"' + bt.Category + '", '
	...
	FROM [BlockType] bt
	JOIN [EntityType] et ON bt.[EntityTypeId] = et.[Id]
	WHERE bt.[Path] IS NULL
		AND bt.[Category] NOT LIKE 'Mobile%'
		AND (
            -- Non-system, but is a Rock block
			bt.[IsSystem] = 0 AND et.[Name] LIKE 'Rock.Blocks.%'
            -- Or the parameters are set to include a specific plugin developer.
			OR (LEN(@includeEntityTypesForPluginEntityTypeNamesLike) > 0 AND et.[Name] LIKE @includeEntityTypesForPluginEntityTypeNamesLike + '%')
		)
		AND bt.[Guid] NOT IN (SELECT [Guid] FROM #knownGuidsToIgnore) -- shouldn't happen
	...
	INSERT INTO #codeTable
	SELECT 
        @crlf + '            // Add/Update Obsidian Block Type' +
        @crlf + '            //   Name:' + isnull(bt.[Name], '') + 
        @crlf + '            //   Category:' + isnull(bt.[Category], '') + 
        @crlf + '            //   EntityType:' + isnull(et.[Name], '') + 
        @crlf +'            RockMigrationHelper.AddOrUpdateEntityBlockType('
    ...
    SELECT 
        @crlf + '            // Add Block ' +
        @crlf + '            //  Block Name: '  + [b.Name] +
        @crlf + '            //  Page Name: '  + isnull([p.InternalName], '-') +
        @crlf + '            //  Layout: '  + isnull([l.Name], '-') +
        @crlf + '            //  Site: '  + isnull([s.Name], '-') +
        @crlf + '            RockMigrationHelper.AddBlock(' +
		' true, ' +
        ISNULL(CONVERT(nvarchar(50), '"' + convert(varchar(50),[p.Guid]) + '".AsGuid()'),'null') + ','+ 
        ISNULL(CONVERT(nvarchar(50), '"' + convert(varchar(50),[l.Guid]) + '".AsGuid()'),'null') + ','+
		ISNULL(CONVERT(nvarchar(50), '"' + convert(varchar(50),[s.Guid]) + '".AsGuid()'),'null') + ','+
        CONVERT(nvarchar(50), '"' + convert(varchar(50),[bt.Guid]) + '".AsGuid()')+ ', "'+
        [b.Name]+ '","'+
        [b.Zone]+ '",@"'+
	...
	where b.PageId in (select PageId from (
		...
		insert into #codeTable select @crlf + '            // update block order for pages with new blocks if the page,zone has multiple blocks'
	end
	...
	select concat(
        @crlf, 
        '            // Update Order for Page: ', bt.[p.InternalName]
	    , ',  Zone: ', b.Zone
	    , ',  Block: ', b.Name, 
        @crlf, 
        '            Sql( @"', 'UPDATE [Block] SET [Order] = ', b.[Order], ' WHERE [Guid] = ''', b.[Guid]
        , '''"  );'
	) 
	from #blocksTemp bt 
	join [Block] b on b.PageId = bt.[p.Id] and b.Zone = bt.[b.Zone]
    ...
    insert into #codeTable
    SELECT
        @crlf + '            // Add/Update HtmlContent for ' + ISNULL('Block: ' + b.Name,'') +
        @crlf + 
	...
	from (
		...
		where [A].[Guid] not in (select [Guid] from #knownGuidsToIgnore)
        ...
        and ( 
            ...
            (@forceIncludeRecentlyUpdatedBlockAttributeValues = 1 and a.Id in ( select AttributeId from AttributeValue av where  DATEDIFF(minute, SYSDATETIME(),av.ModifiedDateTime) <= @forceIncludeRecentlyUpdatedBlockAttributeValuesMinutes) )
        )
    ...
    SELECT 
        ...
        @crlf + '            RockMigrationHelper.AddOrUpdateBlockTypeAttribute( '
    ...
    WHERE EntityTypeQualifierColumn = 'BlockTypeId'
		...
		-- only include CustomGridEnableStickyHeaders if it has an AttributeValue for any of its blocks
		AND ([a].[Key] != 'core.CustomGridEnableStickyHeaders' OR [a].[Id] IN (SELECT [AttributeId] FROM [AttributeValue] av WHERE av.[AttributeId] = a.[Id] AND av.[ValueAsBoolean] = 1 ))
		-- only include CustomGridColumnsConfig if it has an AttributeValue for any of its blocks
		AND ([a].[Key] != 'core.CustomGridColumnsConfig' OR [a].[Id] IN (SELECT [AttributeId] FROM [AttributeValue] [av] WHERE [av].[AttributeId] = a.[Id] AND [av].[Value] != '' ))
    ...
    insert into #codeTable
    SELECT 
        @crlf + '            // Add Block Attribute Value' + 
        @crlf + '            //   Block: ' + b.Name + 
        @crlf + '            //   BlockType: ' + bt.Name + 
        @crlf + '            //   Category: ' + isnull(bt.Category, '-') + 
        @crlf + '            //   Block Location: ' + ltrim(isnull('Page=' + p.InternalName,'') +  isnull(' Layout=' + l.Name, '') + ISNULL(', Site=' + s.Name, '')) + 
        @crlf + '            //   Attribute: ' + a.Name +
        @crlf + '            /*   Attribute Value: ' + (case when av.value like '%' + char(10) + '%' then '...' else av.Value end) + ' */' +
        (case when @useSkipIfAlreadyExistsForAddBlockAttributeValue = 1  then @crlf + '            //   Skip If Already Exists: true' else '' end) + 
        @crlf +
        '            RockMigrationHelper.AddBlockAttributeValue(' +
        (case when @useSkipIfAlreadyExistsForAddBlockAttributeValue = 1  then ' true ,' else '' end) + 
        '"'+     
        CONVERT(nvarchar(50), b.Guid)+ '","'+ 
    ...
    and ( 
        -- Get Block Attribute Values for the #attributeIds
        ([av].[AttributeId] in (select [Id] from #attributeIds))
        or 
        -- also include AttributeValues for non-system/non-shipping blocks
        ( b.IsSystem = 0 and [b].[Guid] not in (select [Guid] from #knownGuidsToIgnore) )
        or   
     	-- if enabled, include all block attribute values updated in the last 60 minutes, even if their BlockType Attribute is already IsSystem=1
		(@forceIncludeRecentlyUpdatedBlockAttributeValues = 1 and DATEDIFF(minute, SYSDATETIME(),av.ModifiedDateTime) <= @forceIncludeRecentlyUpdatedBlockAttributeValuesMinutes)
    )
    and [av].[Guid] not in (select [Guid] from #knownGuidsToIgnore)
	or (@forceIncludeMigrationsForPageId is not null and b.[Id] in (
		SELECT b.[Id]
		FROM [Block] b
		WHERE b.[PageId] = @forceIncludeMigrationsForPageId
        union 
        SELECT b.[Id]
		FROM [Block] b
		WHERE b.[LayoutId] is not null and b.[LayoutId] in ( select p.[LayoutId] from [Page] p where p.[Id] = @forceIncludeMigrationsForPageId and p.[LayoutId] is not null)
	))
    order by b.Id
    ...
    -- Page Contexts
    insert into #codeTable
      SELECT @crlf + '            // Add/Update PageContext for Page:' + p.InternalName + ', Entity: ' + pc.Entity + ', Parameter: ' + pc.IdParameter  
      + @crlf +
    ...
    insert into #codeTable SELECT         
        ...
        @crlf + '            //   Category: ' + isnull(bt.Category, '-') + 
        @crlf + '            //   Attribute: ' + a.Name +
        @crlf + '            RockMigrationHelper.DeleteAttribute("'+ 
		CONVERT(nvarchar(50), [A].[Guid]) + '");' 
		from [Attribute] [A]
		inner join [EntityType] E 
			ON E.[Id] = A.[EntityTypeId]
	...
	SELECT 
        @crlf + '            // Remove Block' +
        @crlf + '            //  Name: ' + b.Name + ', from ' + ISNULL('Page: ' + p.InternalName,'') + ISNULL(', Layout: ' + l.Name, '')  +  ISNULL(', Site: ' + s.Name, '') +
        @crlf + '            //  from ' + ISNULL('Page: ' + p.InternalName,'') + ISNULL(', Layout: ' + l.Name, '')  +  ISNULL(', Site: ' + s.Name, '') +
        @crlf + '            RockMigrationHelper.DeleteBlock("'+ CONVERT(nvarchar(50), [b].[Guid])+ '");'
        from [Block] [b]
		left outer join [Page] [p] on [p].[Id] = [b].[PageId]
		left outer join [Layout] [l] on [l].[Id] = [b].[LayoutId]
		left outer join [Layout] [pl] on [pl].[Id] = [p].[LayoutId]
    ...
    insert into #codeTable 
	SELECT 
		@crlf + '            // Delete BlockType ' +
        @crlf + '            //   Name: ' + bt.[Name] + 
        @crlf + '            //   Category: ' + bt.[Category] + 
        @crlf + '            //   Path: ' + isnull(bt.[Path], '-') + 
        @crlf + '            //   EntityType: ' + isnull(et.[FriendlyName], '-') + 
        @crlf + '            RockMigrationHelper.DeleteBlockType("'+ CONVERT(nvarchar(50), bt.[Guid])+ '");'
	from [BlockType] bt
    left outer join [EntityType] et on et.Id = bt.EntityTypeId
	where bt.[IsSystem] = 0
    and bt.[Guid] not in (select [Guid] from #knownGuidsToIgnore) 
	order by bt.[Id] desc
	...
	SELECT 
		@crlf + '            // Delete Page ' +
        @crlf + '            //  Internal Name: '  + isnull(p.InternalName, '-') +
        @crlf + '            //  Site: '  + isnull(s.Name, '-') +
        @crlf + '            //  Layout: '  + isnull(l.Name, '-') +
        @crlf + '            RockMigrationHelper.DeletePage("'+ CONVERT(nvarchar(50), [p].[Guid])+ '");' +
        @crlf
	from [Page] [p]
	join [Layout] [l] on [l].[Id] = [p].[layoutId]
	join [site] [s] on [s].[Id] = [l].[siteId]
    ...
    insert into #codeTable
    SELECT @crlf + '            // Delete PageContext for Page:' + p.InternalName + ', Entity: ' + pc.Entity + ', Parameter: ' + pc.IdParameter  + @crlf +
    + '            RockMigrationHelper.DeletePageContext( "' + convert(nvarchar(max), pc.Guid) + '");'
    + @crlf  
    FROM [dbo].[PageContext] [pc]
    join [Page] [p]
    on [p].[Id] = [pc].[PageId]
    where [pc].[IsSystem] = 0 or ( @forceIncludeMigrationsForPageId = p.Id )
