...
declare
  ...
  @maxPersonAliasIdForTransactions int = (select max(Id) from (select top 100000 Id from PersonAlias order by Id) x),  /* limit to first 100000 persons in the database */ 
...
set @startTransactionDate = DATEADD(yy, DATEDIFF(yy, 0, GETDATE()) - @yearsBack, 0)

set @transactionDateTime = @startTransactionDate
declare 
