//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the T4\Model.tt template.
//
//     Changes to this file will be lost when the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
//
// THIS WORK IS LICENSED UNDER A CREATIVE COMMONS ATTRIBUTION-NONCOMMERCIAL-
// SHAREALIKE 3.0 UNPORTED LICENSE:
// http://creativecommons.org/licenses/by-nc-sa/3.0/
//
using System.ServiceModel;

namespace Rock.Api.Cms
{
	/// <summary>
	/// Represents a REST WCF service for BlogPosts
	/// </summary>
	[ServiceContract]
    public partial interface IBlogPostService
    {
		/// <summary>
		/// Gets a BlogPost object
		/// </summary>
		[OperationContract]
        Rock.Models.Cms.BlogPostDTO Get( string id );

		/// <summary>
		/// Updates a BlogPost object
		/// </summary>
        [OperationContract]
        void UpdateBlogPost( string id, Rock.Models.Cms.BlogPostDTO BlogPost );

		/// <summary>
		/// Creates a new BlogPost object
		/// </summary>
        [OperationContract]
        void CreateBlogPost( Rock.Models.Cms.BlogPostDTO BlogPost );

		/// <summary>
		/// Deletes a BlogPost object
		/// </summary>
        [OperationContract]
        void DeleteBlogPost( string id );
    }
}
