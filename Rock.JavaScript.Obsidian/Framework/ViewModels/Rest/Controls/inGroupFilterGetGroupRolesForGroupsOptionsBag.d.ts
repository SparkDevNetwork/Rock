//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the Rock.CodeGeneration project
//     Changes to this file will be lost when the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
// <copyright>
// Copyright by the Spark Development Network
//
// Licensed under the Rock Community License (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.rockrms.com/license
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// </copyright>
//

import { Guid } from "@Obsidian/Types";

/** Options bag for the InGroupFilter's GetGroupRolesForGroups method. */
export type InGroupFilterGetGroupRolesForGroupsOptionsBag = {
    /** Guids of the groups that are of the group types we wish to get roles for. */
    groupGuids?: Guid[] | null;

    /**
     * If Rock.ViewModels.Rest.Controls.InGroupFilterGetGroupRolesForGroupsOptionsBag.IncludeChildGroups is true and this is true, then also include the child groups
     * all the way down the hierarchy when determining the group types.
     */
    includeAllDescendants: boolean;

    /**
     * Whether or not to include the direct child groups of the groups listed in GroupGuids.
     * If this is true and IncludeSelectedGroups is false, then the groups in GroupGuids will be excluded.
     */
    includeChildGroups: boolean;

    /** If Rock.ViewModels.Rest.Controls.InGroupFilterGetGroupRolesForGroupsOptionsBag.IncludeChildGroups is true and this is true, then also include inactive groups */
    includeInactiveGroups: boolean;

    /**
     * If Rock.ViewModels.Rest.Controls.InGroupFilterGetGroupRolesForGroupsOptionsBag.IncludeChildGroups is true and this is true, then the groups in GroupGuids
     * will be included along with the child groups when determining the group types.
     */
    includeSelectedGroups: boolean;

    /** The security grant token. */
    securityGrantToken?: string | null;
};
