...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const loadOptions = async (): Promise<ListItemBag[]> => {
            const options: Partial<BinaryFilePickerGetBinaryFilesOptionsBag> = {
                binaryFileTypeGuid: props.binaryFileTypeGuid
            };
            const result = await post<ListItemBag[]>("/api/v2/Controls/BinaryFilePickerGetBinaryFiles", undefined, options);
