...
/*
    ...
    The single shared compiler for Obsidian Content authored source. This lib is
    consumed by TWO hosts: the block's in-browser editor (loaded on demand through
    the import map, edit mode only) and Rock's server-side compile service, which
    runs this exact bundle inside a Jint JavaScript engine so repo-less MCP clients
    get real compile results. Because both hosts run the same bundle, the same
    source always produces the same module. Do not fork this logic per host.

    Two constraints exist specifically for the Jint host (see
    specs/260806-jint-in-process-obsidian-compile-plan.md, Phase 0 result):

    1. Source map generation is disabled in the compileScript call. Jint's
       Function.prototype.toString returns "[native code]" rather than source
       text, which breaks source-map-js regenerating its own sort function via
       new Function(fn.toString()) during map merging. Maps were always discarded
       anyway, so this costs nothing in the browser.
    ...
    2. The Vue version comes from @vue/compiler-sfc's own version export (the
       ...
       keeps "vue" external, and the Jint host has no import map to resolve it;
       taking the version from the bundled compiler keeps this bundle's
       System.register dependency array empty and the bundle self-contained.
    ...
    Reason: One compiler implementation shared by the browser editor and the server.
*/
...
export function compileSource(source: string): ObsidianContentCompileResult {
    const parsed = parse(source, { filename: "ObsidianContent.vue" });
