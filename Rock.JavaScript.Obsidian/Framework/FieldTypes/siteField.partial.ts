...
const editComponent = defineAsyncComponent(async () => {
    return (await import("./siteFieldComponents")).EditComponent;
});

// Load the configuration component only as needed.
const configurationComponent = defineAsyncComponent(async () => {
    return (await import("./siteFieldComponents")).ConfigurationComponent;
});
...
export class SiteFieldType extends FieldTypeBase {
    public override getTextValue(value: string, configurationValues: Record<string, string>): string {
        ...
        try {
            ...
            const textValue = values.find(o => o.value === value)?.text ?? "";
            return textValue;
        }
        catch {
            return "";
