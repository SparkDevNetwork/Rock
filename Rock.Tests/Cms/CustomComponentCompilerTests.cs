...
{
    /// <summary>
    /// Tests for the server-side Custom Component compiler, which runs the shared
    /// compiler bundle in a headless Chromium page. These need two things present:
    /// the built bundle at RockWeb/Obsidian/Libs/customComponentCompiler.js, and
    ...
    /// </summary>
    [TestClass]
    public class CustomComponentCompilerTests
    {
        #region Fixtures
...
";
        ...
        {
            var compiler = new CustomComponentCompiler( GetBundlePathOrInconclusive(), GetBrowserPathOrInconclusive() );
        ...
        */
        ...
        {
            var compiler = new CustomComponentCompiler( Path.Combine( Path.GetTempPath(), "does-not-exist-" + Guid.NewGuid() + ".js" ) );
        ...
        private static string GetBundlePathOrInconclusive()
        {
            ...
            while ( directory != null )
            {
                var candidate = Path.Combine( directory.FullName, "RockWeb", "Obsidian", "Libs", "customComponentCompiler.js" );
            ...
            Assert.Inconclusive( "The compiler bundle was not found. Run the Rock.JavaScript.Obsidian build to produce RockWeb/Obsidian/Libs/customComponentCompiler.js." );
            return null;
        }
