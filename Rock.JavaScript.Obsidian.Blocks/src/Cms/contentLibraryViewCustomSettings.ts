...
import { CustomSettingsBag } from "@Obsidian/ViewModels/Blocks/Cms/ContentLibraryView/customSettingsBag";
import { FilterOptionsBag } from "@Obsidian/ViewModels/Blocks/Cms/ContentLibraryView/filterOptionsBag";
import { CustomSettingsOptionsBag } from "@Obsidian/ViewModels/Blocks/Cms/ContentLibraryView/customSettingsOptionsBag";
import { ListItemBag } from "@Obsidian/ViewModels/Utility/listItemBag";
import FilterGrid from "./ContentLibraryView/filterGrid.partial";
import { SortOrdersKey } from "./ContentLibraryView/types";
import { alert } from "@Obsidian/Utility/dialogs";
import { areEqual } from "@Obsidian/Utility/guid";
import { ContentLibraryListItemBag } from "@Obsidian/ViewModels/Blocks/Cms/ContentLibraryView/contentLibraryListItemBag";
...
export default defineComponent({
    name: "Cms.ContentLibraryView.CustomSettings",
    ...
    setup(props, { emit }) {
        ...
        const contentLibraryItems = ref<ContentLibraryListItemBag[]>([]);
        ...
        const startLoading = async (): Promise<void> => {
            ...
            if (result.isSuccess && result.data && result.data.settings && result.data.options) {
                ...
                contentLibrary.value = result.data.settings.contentLibrary ?? "";
                ...
                contentLibraryItems.value = result.data.options.contentLibraryItems ?? [];
            }
            else {
        ...
        const onSave = async (): Promise<void> => {
            ...
            setCustomSettingsBoxValue(box, "contentLibrary", contentLibrary.value);
        ...
        watch(contentLibrary, () => {
            const library = contentLibraryItems.value.find(l => areEqual(l.value, contentLibrary.value));
            ...
            // Check for any filters on the selected library that don't already
            // exist in the array of filters.
            for (const f of libraryFilters) {
            ...
            // Check for any filters that are not in the selected library
            // that should be removed.
            for (let filterIndex = 0; filterIndex < newFilters.length;) {
                if (!libraryFilters.some(f => f.value === newFilters[filterIndex].sourceKey)) {
...
<Modal v-model="isModalOpen"
    title="Content Library View Settings"
    ...
    <div v-else>
        ...
        <div class="row">
            <div class="col-md-6">
                <DropDownList v-model="contentLibrary"
