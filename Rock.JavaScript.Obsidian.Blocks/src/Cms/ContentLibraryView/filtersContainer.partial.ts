...
import { SearchFilterBag } from "@Obsidian/ViewModels/Blocks/Cms/ContentLibraryView/searchFilterBag";
...
export const Filter = defineComponent({
    name: "Cms.ContentLibraryView.Filter",
    ...
    setup(props, { emit }) {
        ...
        const isDropDownList = computed((): boolean => {
            return props.filter.control === ContentLibraryFilterControl.Dropdown;
        });

        const isPills = computed((): boolean => {
            return props.filter.control === ContentLibraryFilterControl.Pills;
        });

        const isBoolean = computed((): boolean => {
            return props.filter.control === ContentLibraryFilterControl.Boolean;
        });
...
export default defineComponent({
    name: "CMS.ContentLibraryView.FilterContainer",
