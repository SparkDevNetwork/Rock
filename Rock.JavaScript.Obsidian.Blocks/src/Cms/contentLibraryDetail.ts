...
import ContentSources from "./ContentLibraryDetail/contentSources.partial";
import EditPanel from "./ContentLibraryDetail/editPanel.partial";
import SearchFilters from "./ContentLibraryDetail/searchFilters.partial";
import ViewPanel from "./ContentLibraryDetail/viewPanel.partial";
import { getSecurityGrant, provideSecurityGrant, refreshDetailAttributes, useConfigurationValues, useInvokeBlockAction } from "@Obsidian/Utility/block";
import { debounce } from "@Obsidian/Utility/util";
import { NavigationUrlKey } from "./ContentLibraryDetail/types";
...
export default defineComponent({
    ...
    setup() {
        const config = useConfigurationValues<DetailBlockBox<ContentLibraryBag, ContentLibraryDetailOptionsBag>>();
        ...
        const contentLibraryViewBag = ref(config.entity);
        const contentLibraryEditBag = ref<ContentLibraryBag | null>(null);
        ...
        const panelName = computed((): string => {
            return contentLibraryViewBag.value?.name ?? "";
        });
        ...
        const entityKey = computed((): string => {
            return contentLibraryViewBag.value?.idKey ?? "";
        });
        ...
        const onCancelEdit = async (): Promise<boolean> => {
            if (!contentLibraryEditBag.value?.idKey) {
        ...
        const onDelete = async (): Promise<void> => {
            ...
            const result = await invokeBlockAction<string>("Delete", {
                key: contentLibraryViewBag.value?.idKey
            });
            ...
            if (result.isSuccess && result.data) {
                ...
                errorMessage.value = result.errorMessage ?? "Unknown error while trying to delete content library.";
            }
        };
        ...
        const onEdit = async (): Promise<boolean> => {
            const result = await invokeBlockAction<DetailBlockBox<ContentLibraryBag, ContentLibraryDetailOptionsBag>>("Edit", {
                key: contentLibraryViewBag.value?.idKey
            });

            if (result.isSuccess && result.data && result.data.entity) {
                contentLibraryEditBag.value = result.data.entity;
        ...
        const onSave = async (): Promise<boolean> => {
            ...
            const data: DetailBlockBox<ContentLibraryBag, ContentLibraryDetailOptionsBag> = {
                entity: contentLibraryEditBag.value,
                isEditable: true,
                validProperties: validProperties
            };

            const result = await invokeBlockAction<ContentLibraryBag | string>("Save", {
                box: data
            });

            if (result.isSuccess && result.data) {
                if (result.statusCode === 200 && typeof result.data === "object") {
                    contentLibraryViewBag.value = result.data;
            ...
            errorMessage.value = result.errorMessage ?? "Unknown error while trying to save content library.";
        ...
        if (config.errorMessage) {
            ...
            blockError.value = "The specified content library could not be viewed.";
        }
        else if (!config.entity.idKey) {
            contentLibraryEditBag.value = config.entity;
