...
<template>
    <fieldset>
        <Alert v-if="isSystem" alertType="info">
            <strong>Note</strong> Because this lava shortcode is used by Rock, editing is not enabled.
        </Alert>
...
<script setup lang="ts">
    ...
    const props = defineProps({
        modelValue: {
            type: Object as PropType<LavaShortcodeBag | null>,
            required: false
        },
    ...
    const topValues = computed((): ValueDetailListItem[] => {
        ...
        if (props.modelValue.tagName) {
            valueBuilder.addTextValue("TagName", props.modelValue.tagName);
        }
    ...
    const leftSideValues = computed((): ValueDetailListItem[] => {
        ...
        if (props.modelValue?.categories != null)
        {
            valueBuilder.addTextValue("Categories", props.modelValue?.categories.map(c => c.text).join(", "));
        }
