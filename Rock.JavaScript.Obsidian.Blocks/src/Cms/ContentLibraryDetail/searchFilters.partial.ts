...
const editFilterControlStandardItems: ListItemBag[] = [
    {
        "value": ContentLibraryFilterControl.Pills.toString(),
        "text": "Pills"
    },
    {
        "value": ContentLibraryFilterControl.Dropdown.toString(),
        "text": "Dropdown"
    }
];
 ...
 */
 const editFilterControlBooleanItems: ListItemBag[] = [
    {
        "value": ContentLibraryFilterControl.Boolean.toString(),
        "text": "Boolean"
    }
];
    ...
    setup(props, { emit }) {
        ...
        const onEditAttributeFilter = (filter: AttributeFilterBag): void => {
            ...
            if (areEqual(filter.fieldTypeGuid, FieldType.Boolean)) {
                editFilterControl.value = ContentLibraryFilterControl.Boolean.toString();
        ...
        const onModalSave = async (): Promise<void> => {
            ...
            if (isEditFullTextSearch.value) {
                ...
                bag.yearSearchFilterControl = toNumberOrNull(editFilterControl.value) ?? ContentLibraryFilterControl.Pills;
                ...
                if (areEqual(filter.fieldTypeGuid, FieldType.Boolean)) {
                    filter.filterControl = ContentLibraryFilterControl.Boolean;
                    filter.isMultipleSelection = false;
                }
                else {
                    filter.filterControl = toNumberOrNull(editFilterControl.value) ?? ContentLibraryFilterControl.Pills;
            ...
            const result = await invokeBlockAction<DetailBlockBox<ContentLibraryBag, ContentLibraryDetailOptionsBag>>("SaveFilterSettings", data);
