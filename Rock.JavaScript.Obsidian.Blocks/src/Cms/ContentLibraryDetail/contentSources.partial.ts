...
export default defineComponent({
    ...
    props: {
        /** The library that contains the sources to display. */
        modelValue: {
    ...
    emits: {
        "update:modelValue": (_value: ContentLibraryBag) => true
    },
    ...
    setup(props, { emit }) {
        ...
        const librarySources = ref(props.modelValue.sources ?? []);
        ...
        const onAddCalendarSource = async (): Promise<void> => {
            ...
            sourceEntityItems.value = result.data
                .filter(e => !librarySources.value.some(s => areEqual(s.entityGuid, e.guid)))
                .map(e => ({
                    value: e.guid,
                    text: e.name
                }));
        ...
        const onSourceSave = async (): Promise<void> => {
            ...
            const result = await invokeBlockAction<DetailBlockBox<ContentLibraryBag, ContentLibraryDetailOptionsBag>>("SaveLibrarySource", data);
         ...
         * 
         * @param source The source that should be deleted from the library.
         */
        const onDeleteSource = async (source: ContentSourceBag): Promise<void> => {
            if (!(await confirmDelete("Library Source"))) {
                return;
            }
            ...
            const result = await invokeBlockAction<DetailBlockBox<ContentLibraryBag, ContentLibraryDetailOptionsBag>>("DeleteLibrarySource", data);

            if (!result || !result.isSuccess || !result.data?.entity) {
                alert(result.errorMessage || "Unable to delete the library source.");
                return;
            }
        ...
        /**
         * Event handler for when the person attempts to edit a library source.
         * 
    ...
    template: `
<Panel title="Sources">
    <SectionHeader title="Content Sources" description="Content for this library will be pulled from the following sources within Rock.">
        <template #actions>
    ...
    <div class="row" v-if="sourceSelectedEntity">
        <div class="col-md-6">
            <CheckBoxList v-model="sourceSelectedEntityAttributes"
                ...
                help="Determines which attributes should be added to the library index for search and retrieval."
