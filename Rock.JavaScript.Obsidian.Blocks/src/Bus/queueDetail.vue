...
<template>
    ...
    <DetailBlock v-if="!blockError"
                 ...
                 @save="onSave">
        <template #view>
            <ViewPanel :modelValue="queueViewBag" :options="options" />
        </template>

        <template #edit>
            <EditPanel v-model="queueEditBag" :options="options" @propertyChanged="onPropertyChanged" />
        </template>
    </DetailBlock>
</template>
...
<script setup lang="ts">
    ...
    import EditPanel from "./QueueDetail/editPanel.partial.vue";
    import ViewPanel from "./QueueDetail/viewPanel.partial.vue";
    import { getSecurityGrant, provideSecurityGrant, useConfigurationValues, useInvokeBlockAction } from "@Obsidian/Utility/block";
    import { NavigationUrlKey } from "./QueueDetail/types";
    ...
    const config = useConfigurationValues<DetailBlockBox<QueueBag, QueueDetailOptionsBag>>();
    ...
    const queueViewBag = ref(config.entity);
    const queueEditBag = ref<QueueBag>({} as QueueBag);
    ...
    const options = computed((): QueueDetailOptionsBag => {
    ...
    async function onSave(): Promise<boolean | string> {
        ...
        const data: DetailBlockBox<QueueBag, QueueDetailOptionsBag> = {
            entity: queueEditBag.value,
            isEditable: true,
            validProperties: validProperties
        };

        const result = await invokeBlockAction<QueueBag | string>("Save", {
            box: data
        });
