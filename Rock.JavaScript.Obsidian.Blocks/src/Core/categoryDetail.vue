...
<template>
    ...
    <DetailBlock v-if="!blockError"
        ...
        <template #edit>
            <EditPanel v-model="categoryEditBag" :options="options" @propertyChanged="onPropertyChanged" />
        </template>
    </DetailBlock>
</template>
...
<script setup lang="ts">
    ...
    import EditPanel from "./CategoryDetail/editPanel.partial.vue";
    import ViewPanel from "./CategoryDetail/viewPanel.partial.vue";
    import { getSecurityGrant, provideSecurityGrant, refreshDetailAttributes, useConfigurationValues, useInvokeBlockAction } from "@Obsidian/Utility/block";
    import { debounce } from "@Obsidian/Utility/util";
    import { NavigationUrlKey } from "./CategoryDetail/types";
    ...
    const categoryEditBag = ref<CategoryBag>({} as CategoryBag);
    ...
    const refreshAttributesDebounce = debounce(() => refreshDetailAttributes(categoryEditBag, validProperties, invokeBlockAction), undefined, true);
    ...
    async function onCancelEdit(): Promise<boolean | string> {
        if (!categoryEditBag.value?.idKey) {
    ...
    async function onEdit(): Promise<boolean> {
        const result = await invokeBlockAction<DetailBlockBox<CategoryBag, CategoryDetailOptionsBag>>("Edit", {
            key: categoryViewBag.value?.idKey
        });
    ...
    async function onSave(): Promise<boolean | string> {
        ...
        const data: DetailBlockBox<CategoryBag, CategoryDetailOptionsBag> = {
            entity: categoryEditBag.value,
            isEditable: true,
            validProperties: validProperties
        };

        const result = await invokeBlockAction<CategoryBag | string>("Save", {
            box: data
        });
