 ...
 * numerically. The Data View Results block emits numbers as pre-formatted strings
 * (to preserve the legacy grid's separator-free display, including decimal scale
 * such as "100.00"), so the numeric value has to be parsed back out of that string
 * for the grid's numeric comparisons.
 *
 ...
 * numerically. The Data View Results grid uses this so numeric columns such as Id,
 * Count, and Rank behave like numbers without the locale formatting that the
 * standard number column would apply.
 */
