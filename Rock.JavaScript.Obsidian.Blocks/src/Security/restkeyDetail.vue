...
<template>
    ...
    <DetailBlock v-if="!blockError"
        ...
        <template #edit>
            <EditPanel v-model="restKeyEditBag" :options="options" @propertyChanged="onPropertyChanged" />
        </template>
    </DetailBlock>
</template>
...
<script setup lang="ts">
    ...
    import EditPanel from "./RestKeyDetail/editPanel.partial.vue";
    import ViewPanel from "./RestKeyDetail/viewPanel.partial.vue";
    ...
    import { NavigationUrlKey } from "./RestKeyDetail/types";
    ...
    const restKeyEditBag = ref<RestKeyBag>({} as RestKeyBag);
    ...
    const panelName = computed((): string => {
        return restKeyViewBag.value?.name ?? "Rest Key Detail";
    });
    ...
    async function onEdit(): Promise<boolean> {
        const result = await invokeBlockAction<DetailBlockBox<RestKeyBag, RestKeyDetailOptionsBag>>("Edit", {
            key: restKeyViewBag.value?.idKey
        });
    ...
    async function onSave(): Promise<boolean | string> {
        ...
        const data: DetailBlockBox<RestKeyBag, RestKeyDetailOptionsBag> = {
            entity: restKeyEditBag.value,
            isEditable: true,
            validProperties: validProperties
        };

        const result = await invokeBlockAction<RestKeyBag | string>("Save", {
            box: data
        });
