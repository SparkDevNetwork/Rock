 ...
 * 
 * Additional Features:
 * - FE1) Added Ability to override the Campus with a Root Location
 */

namespace RockWeb.Plugins.com_bemaservices.CheckIn.Manager
{
    /// <summary>
    ...
    [LocationField(
        ...
        Key = AttributeKey.RootLocationOverride,
        Description = "If filled, Rock will use this as the root location for the block instead of the selected campus's associated location.",
        IsRequired = false,
        Order = 6,
...
",
        ...
        private NavigationData GetNavigationData( CampusCache campus, int? scheduleId )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                var locationGuid = GetAttributeValue( AttributeKey.RootLocationOverride ).AsGuidOrNull();
                if ( locationGuid.HasValue )
                {
                    var location = new LocationService( rockContext ).Get( locationGuid.Value );
                    if ( location != null )
                    {
                        validLocationids = new List<int>();
