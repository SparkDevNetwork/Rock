...
namespace RockWeb.Plugins.org_hfbc.Legacy685
{
    ...
    /// </summary>
    [DisplayName( "Family Attribute Values" )]
    [Category( "org_hfbc > Legacy 685" )]
    [Description( "Allows for editing the value(s) of a set of attributes for a group." )]

    [AttributeCategoryField( "Category", "The Attribute Category to display attributes from", false, "Rock.Model.Group", true, "", "", 0 )]
    ...
    public partial class FamilyAttributeValues : RockBlock
    {
                        ...
                        if ( group != null )
                        {
                            ...
                            foreach ( int attributeId in AttributeList )
                            {
                                    ...
                                    attribute.IsAuthorized( Authorization.EDIT, CurrentPerson ) )
                                {
                                    ...
                                    {
                                        string originalValue = group.GetAttributeValue( attribute.Key );
                                        string newValue = attribute.FieldType.Field.GetEditValue( attributeControl, attribute.QualifierValues );
                                        Rock.Attribute.Helper.SaveAttributeValue( group, attribute, newValue, rockContext );
                            ...
                            if ( changes.Any() )
                            {
                                foreach ( var personId in group.Members.Where( m => m.GroupMemberStatus == GroupMemberStatus.Active ).Select( m => m.PersonId ).ToList() )
                                {
        ...
        private void CreateControls( bool setValues )
        {
            ...
            if ( groupId != null )
            {
                ...
                if ( group != null )
                {
                    ...
                    {
                        var attribute = AttributeCache.Read( attributeId );
                        string attributeValue = group.GetAttributeValue( attribute.Key );
