...
{
    [DisplayName( "Family Group List Personalized Lava" )]
    [Category( "org_hfbc > Legacy 685" )]
    [Description( "Lists all group that the family is a member of using a Lava template." )]
    ...
    public partial class FamilyGroupListPersonalizedLava : RockBlock
    {
        ...
        private void ListGroups()
        {
            ...
            if ( GetGroupId().HasValue )
            {
                ...
                if ( group != null )
                {
                    var personIdList = group.Members.Where( m => m.GroupMemberStatus == GroupMemberStatus.Active ).Select( m => m.PersonId ).Distinct().ToList();
                   ...
                   .Where( m => personIdList.Contains( m.PersonId )
                    ...
                    qry = qry.DistinctBy( gm => gm.GroupId ).AsQueryable();
