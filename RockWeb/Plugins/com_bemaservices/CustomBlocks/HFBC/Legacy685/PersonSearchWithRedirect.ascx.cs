...
namespace RockWeb.Plugins.org_hfbc
{
    [DisplayName( "Person Search with Redirect" )]
    [Category( "org_hfbc > Legacy 685" )]
    [Description( "Displays list of people that match a given search type and term. If only a single result exists then it will redirect to the Person Detail Page attribute" )]
    ...
    public partial class PersonSearchWithRedirect : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        private void BindGrid()
        {
            ...
            if ( !string.IsNullOrWhiteSpace( type ) && !string.IsNullOrWhiteSpace( term ) )
            {
                ...
                {
                    NavigateToLinkedPage( "PersonDetailPage", "PersonId", personList[0].Id );
                }
                else
                {
