...
/*
 * BEMA Modified Core Block ( v7.6.1)
...
namespace RockWeb.Plugins.com_bemaservices.Finance
{
    ...
    [AttributeField( Rock.SystemGuid.EntityType.PERSON, "SSN Person Attribute","Person Attribute that stores SSN", false, false )]
    [AttributeField( Rock.SystemGuid.EntityType.PERSON, "Other Id Person Attribute", "Person Attribute that stores other IDs", false, false )]
    public partial class BenevolenceRequestDetail : Rock.Web.UI.RockBlock
    {
            ...
            if ( Page.IsValid )
            {
                ...
                if ( benevolenceRequest.IsValid )
                {
                    ...
                    if ( GetAttributeValue( "SSNPersonAttribute" ).AsGuidOrNull() != null )
                    {
                        ...
                        {
                            var ssnObject = AttributeCache.Read( GetAttributeValue( "SSNPersonAttribute" ).AsGuid() );
                            var person = benevolenceRequest.RequestedByPersonAlias.Person;
                            person.LoadAttributes();

                            person.SetAttributeValue( ssnObject.Key, ssnbSSN.TextEncrypted );
                            person.SaveAttributeValues( rockContext );
                        }
                    }

                    // Setting Person Other Id in Person Attribute
                    if( GetAttributeValue("OtherIdPersonAttribute").AsGuidOrNull() != null )
                    {
                        if( benevolenceRequest.RequestedByPersonAlias != null)
                        {
                            var otherIdObject = AttributeCache.Read( GetAttributeValue( "OtherIdPersonAttribute" ).AsGuid() );
                            var person = benevolenceRequest.RequestedByPersonAlias.Person;
                            person.LoadAttributes();

                            person.SetAttributeValue( otherIdObject.Key, rtbGovernmentId.Text );
                            person.SaveAttributeValues( rockContext );
                        }
                    }
            ...
            if ( ppPerson.PersonId != null )
            {
                ...
                if ( person != null )
                {
                    ...
                    if ( GetAttributeValue( "SSNPersonAttribute" ).AsGuidOrNull() != null )
                    {
                        var ssnObject = AttributeCache.Read( GetAttributeValue( "SSNPersonAttribute" ).AsGuid() );
                        person.LoadAttributes();
                        var ssnValue = person.GetAttributeValue( ssnObject.Key );
                        try {
                            if ( !string.IsNullOrEmpty(ssnValue) )
                            {
                                ssnbSSN.TextEncrypted = ssnValue;
                            }
                        }
                        catch(Exception ex) {
                            //
                    ...
                    // Loading Person OtherId from Person Attribute
                    if ( GetAttributeValue( "OtherIdPersonAttribute" ).AsGuidOrNull() != null )
                    {
                        var otherIdObject = AttributeCache.Read( GetAttributeValue( "OtherIdPersonAttribute" ).AsGuid() );
                        person.LoadAttributes();
                        var otherIdValue = person.GetAttributeValue( otherIdObject.Key );
        ...
        public void ShowDetail( int benevolenceRequestId )
        {
            ...
            // Loading Person Other Id from Person Attribute
            if ( GetAttributeValue( "OtherIdPersonAttribute" ).AsGuidOrNull() != null )
            {
                if ( benevolenceRequest.RequestedByPersonAlias != null )
                {
                    ...
                    var person = benevolenceRequest.RequestedByPersonAlias.Person;
                    person.LoadAttributes();
                    var ssnValue = person.GetAttributeValue( otherIdObject.Key );
                    if ( ssnValue != null )
                    {
                        rtbGovernmentId.Text = ssnValue;
                    }
                }
            }
