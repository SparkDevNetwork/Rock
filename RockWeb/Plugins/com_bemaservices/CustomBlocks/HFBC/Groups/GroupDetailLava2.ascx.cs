...
namespace RockWeb.Plugins.org_hfbc.Groups
{
    ...
    /// </summary>
    [DisplayName( "Group Detail Lava (2)" )]
    [Category( "HFBC > Groups" )]
	...
	[LinkedPage( "Additional Page", "Info.", true, "", "", 2)]
    ...
    [LinkedPage("Communication Page SMS", "The communication page to use for sending SMS to the group members.", true, "", "", 4)]
    ...
    [AttributeField(Rock.SystemGuid.EntityType.GROUP, "Hidden Attributes", "Attributes that will not be displayed.", true, true)]
	public partial class GroupDetailLava : Rock.Web.UI.RockBlock
    {
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            if ( IsEditingGroup == true )
            {
				...
				var guidList = GetAttributeValues( "HiddenAttributes" );
				var keyList = new List<string>();
				foreach ( var guid in guidList )
				{
					keyList.Add( AttributeCache.Read( Guid.Parse( guid ) ).Key );
				}

                phAttributes.Controls.Clear();
                Rock.Attribute.Helper.AddEditControls( group, phAttributes, false, BlockValidationGroup, keyList );
            }
        ...
        private void DisplayViewGroup()
        {
            if ( _groupId > 0 )
            {
				...
				linkedPages.Add( "AdditionalPage", LinkedPageRoute("AdditionalPage"));
        ...
        private void DisplayEditGroup()
        {
            ...
            if ( _groupId != -1 )
            {
                ...
                if ( group.IsAuthorized( Authorization.EDIT, CurrentPerson ) )
                {
                    ...
                    group.LoadAttributes();
                    phAttributes.Controls.Clear();
                    Rock.Attribute.Helper.AddEditControls( group, phAttributes, true, BlockValidationGroup, keyList );
        ...
        /// </summary>
        private void SendCommunication( string GroupMemberIds )
        {
			var rockContext = new RockContext();
			List<int?> personAliasIds = null;

            // checking if GroupMemberIds were passed in
			if( !string.IsNullOrEmpty( GroupMemberIds ) )
			{
				// Getting PersonAliasIds for passed GroupMembersIds
				var groupMemberIdList = GroupMemberIds.Split( ',' ).ToList();
				personAliasIds = new GroupMemberService( rockContext ).Queryable()
									.Where( m => groupMemberIdList.Contains( m.Id.ToString() ) )
									.ToList()
			...
			{
				// getting PersonAliasIds for Group Members
				personAliasIds = new GroupMemberService( rockContext ).Queryable()
        ...
        private void SendCommunicationSMS()
        {
            // create communication
            if (this.CurrentPerson != null && _groupId != -1 && !string.IsNullOrWhiteSpace(GetAttributeValue("CommunicationPageSMS")))
            {
                var rockContext = new RockContext();
                var service = new Rock.Model.CommunicationService(rockContext);
                var communication = new Rock.Model.Communication();
                ...
                NavigateToLinkedPage("CommunicationPageSMS", queryParameters);
            }
        }
        #endregion
    }
