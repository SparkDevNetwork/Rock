...
namespace RockWeb.Plugins.org_bemaservices.PastoralCare
{
    [ContextAware( typeof( Person ) )]
    [DisplayName( "Hope House List" )]
    [Category( "BEMA > Pastoral Care" )]
    [Description( "A summary of all the current Hope House Guests that have been reported to Pastoral Care." )]
    [WorkflowTypeField( "Hope House Admission Workflow" )]
    public partial class HopeHouseList : RockBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( string.IsNullOrWhiteSpace( GetAttributeValue( "HopeHouseAdmissionWorkflow" ) ) )
            {
            ...
            gReport.Actions.AddButton.Text = "<i class=\"fa fa-plus\" Title=\"Add Hope House Guest\"></i>";
            gReport.Actions.AddButton.Enabled = true;
            gReport.Actions.AddClick += addHopeHouseGuest_Click;
        ...
        protected void addHopeHouseGuest_Click( object sender, EventArgs e )
        {
            string url = "/Pastoral/HopeHouse/";
...
/*
        private void GenerateExcel( object sender, EventArgs e )
        {
            using ( RockContext rockContext = new RockContext() )
            {
                ...
                string title = "Hope House Guest List Report";
...
*/
        ...
        {
            Response.Redirect( "~/Pastoral/HopeHouse/" + e.RowKeyId );
        }
        ...
        private IQueryable<HopeHouseRow> GetQuery( RockContext rockContext )
        {
            ...
            Guid hopeHouseWorkflow = GetAttributeValue( "HopeHouseAdmissionWorkflow" ).AsGuid();

            var workflowType = new WorkflowTypeService( rockContext ).Get( hopeHouseWorkflow );
            ...
            var wfTmpqry = workflowService.Queryable().AsNoTracking()
                    .Where( w => ( w.WorkflowType.Guid == hopeHouseWorkflow ) && ( w.Status == "Active" || w.Status == status ) );
            ...
            if ( contextEntity != null )
            {
                ...
                var validWorkflowIds = new AttributeValueService( rockContext ).Queryable()
                    .Where( av => av.Attribute.Key == "Patient" && personGuid.Contains( av.Value ) ).Select( av => av.EntityId );
            ...
            if ( contextEntity == null )
            {
                ...
                qry = qry.AsQueryable().Where( w => !
                    ( personAliasService.Get( w.AttributeValues.Where( av => av.AttributeKey == "Patient" ).Select( av => av.Value ).FirstOrDefault().AsGuid() ) != null ?
                    personAliasService.Get( w.AttributeValues.Where( av => av.AttributeKey == "Patient" ).Select( av => av.Value ).FirstOrDefault().AsGuid() ).Person.IsDeceased :
                    false ) ).ToList();
            }

            var newQry = qry.Select( w => new HopeHouseRow
            {
                Id = w.Workflow.Id,
                Workflow = w.Workflow,
                Name = w.Workflow.Name,
                Patient = new Func<Person>( () =>
                {
                    PersonAlias pa = personAliasService.Get( w.AttributeValues.Where( av => av.AttributeKey == "Patient" ).Select( av => av.Value ).FirstOrDefault().AsGuid() );
                    if ( pa != null )
                    {
                        return pa.Person;
                    }
                ...
                Contact = new Func<Person>( () =>
                {
                    PersonAlias pa = personAliasService.Get( w.AttributeValues.Where( av => av.AttributeKey == "Contact" ).Select( av => av.Value ).FirstOrDefault().AsGuid() );
                    if ( pa != null )
                    {
                        return pa.Person;
                    }
                ...
                CareGiver = new Func<Person>( () =>
                {
                    PersonAlias pa = personAliasService.Get( w.AttributeValues.Where( av => av.AttributeKey == "CareGiver" ).Select( av => av.Value ).FirstOrDefault().AsGuid() );
                    if ( pa != null )
                    {
                        return pa.Person;
                    }
                ...
                DateActive = w.AttributeValues.Where( av => av.AttributeKey == "DateActive" ).Select( av => av.ValueAsDateTime ).FirstOrDefault(),
                Illness = w.AttributeValues.Where( av => av.AttributeKey == "Illness" ).Select( av => av.ValueFormatted ).FirstOrDefault(),
            ...
            } ).ToList().AsQueryable().OrderBy( p => p.Id ).ThenBy( p => p.Patient.LastName );

            return newQry;
        }

        public class HopeHouseRow {
