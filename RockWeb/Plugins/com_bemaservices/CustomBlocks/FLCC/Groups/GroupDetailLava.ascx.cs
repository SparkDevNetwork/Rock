...
namespace RockWeb.Plugins.com_bemaservices.Groups
{
    ...
    [SecurityAction( "Group Leader", "This gives access to the Group Leader Fields in the Block" )]
	...
	[AttributeField(Rock.SystemGuid.EntityType.GROUP, "Hidden Attributes", "Attributes that will be hidden", true, true, "" )]
    public partial class GroupDetailLava : Rock.Web.UI.RockBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            rbCloseGroup.Click += rbCloseGroup_Click;
            rnbOpenSpotsAvailable.TextChanged += rnbOpenSpotsAvailable_TextChanged;
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            if ( IsEditingGroup == true )
            {
                ...
                if ( !string.IsNullOrWhiteSpace( GetAttributeValue( "HiddenAttributes" ) ) )
				{
					var excludeGuidList = GetAttributeValue( "HiddenAttributes" ).Split(',').AsGuidList();
					List<string> exludeKeyList = new List<string>();
					foreach ( var guid in excludeGuidList )
					{
						exludeKeyList.Add( AttributeCache.Get( guid ).Key );
					}

					Rock.Attribute.Helper.AddEditControls( group, phAttributes, true, BlockValidationGroup, exludeKeyList );
        ...
        protected void btnSaveGroup_Click( object sender, EventArgs e )
        {
            ...
            if ( group != null && group.IsAuthorized( Authorization.EDIT, CurrentPerson ) )
            {
                ...
                // Saving Capacity
                if( rnbOpenSpotsAvailable.Text.AsIntegerOrNull() != null )
                {
                    // if user selects less than 0
                    if( rnbOpenSpotsAvailable.Text.AsInteger() < 0 )
                    {
                        rnbOpenSpotsAvailable.Text = 0.ToString();
                    }
                    group.GroupCapacity = rnbOpenSpotsAvailable.Text.AsIntegerOrNull() + group.Members.Count();
                }

                if ( group.GroupLocations.Any() )
                {
                    // Updating
                    group.GroupLocations.FirstOrDefault().Location.Street1 = aGroupLocation.Street1;
                    if ( aGroupLocation.Street2 != null )
                    {
                        group.GroupLocations.FirstOrDefault().Location.Street2 = aGroupLocation.Street2;
                    }
                    group.GroupLocations.FirstOrDefault().Location.City = aGroupLocation.City;
                    group.GroupLocations.FirstOrDefault().Location.State = aGroupLocation.State;
                    group.GroupLocations.FirstOrDefault().Location.PostalCode = aGroupLocation.PostalCode;
                }
                else
                {
                    // Creating
                    ...
                    location.Street1 = aGroupLocation.Street1;
                    if ( aGroupLocation.Street2 != null )
                    {
                        location.Street2 = aGroupLocation.Street2;
                    }
                    location.City = aGroupLocation.City;
                    location.State = aGroupLocation.State;
                    location.PostalCode = aGroupLocation.PostalCode;
        ...
        /// <summary>
        /// Handles the Click event of the rbCloseGroup control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void rbCloseGroup_Click( object sender, EventArgs e )
        {
            rnbOpenSpotsAvailable.Text = 0.ToString();
            rbCloseGroup.Enabled = false;
        }

        /// <summary>
        /// Handles the TextChange event of the rnbOpenSpotsAvailable control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void rnbOpenSpotsAvailable_TextChanged( object sender, EventArgs e )
        {
            if( rnbOpenSpotsAvailable.Text.AsIntegerOrNull() != null )
            {
                if ( rnbOpenSpotsAvailable.Text.AsInteger() < 1 )
                {
                    rbCloseGroup.Enabled = false;
                }
                else
                {
                    rbCloseGroup.Enabled = true;
                }
            }
        }
        ...
        private void DisplayViewGroup()
        {
            if ( _groupId > 0 )
            {
                ...
                securityActions.Add( "GroupLeader", group != null && IsUserAuthorized( "GroupLeader" ) );
        ...
        private void DisplayEditGroup()
        {
            ...
            if ( _groupId != -1 )
            {
                ...
                if ( group.IsAuthorized( Authorization.EDIT, CurrentPerson ) )
                {
                    ...
                    if ( group.GroupLocations.Any() )
                    {
                        aGroupLocation.Street1 = group.GroupLocations.FirstOrDefault().Location.Street1;
                        if ( group.GroupLocations.FirstOrDefault().Location.Street2 != null )
                        {
                            aGroupLocation.Street2 = group.GroupLocations.FirstOrDefault().Location.Street2;
                        }
                        aGroupLocation.City = group.GroupLocations.FirstOrDefault().Location.City;
                        aGroupLocation.State = group.GroupLocations.FirstOrDefault().Location.State;
                        aGroupLocation.PostalCode = group.GroupLocations.FirstOrDefault().Location.PostalCode;
                    }
                    ...
                    // mapping open spots available
                    rnbOpenSpotsAvailable.Text = ( group.GroupCapacity - group.Members.Count() ).ToString();

                    rbCloseGroup.Enabled = group.GroupCapacity - group.Members.Count() == 0 ? false : true;
                }
                else
                {
