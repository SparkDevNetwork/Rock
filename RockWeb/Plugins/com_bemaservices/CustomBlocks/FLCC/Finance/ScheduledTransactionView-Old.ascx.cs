...
{
    /*  This block was modified to add the ability to specify a Cancelation Reason
    *   It has been tested on Rock 8.9
    */
    ...
    [AttributeField( "76824E8A-CCC4-4085-84D9-8AF8C0807E20", "Reason For Cancelation", "Stores the Attribute that hold the Reason For Cancelation" )]
...
";
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                // Displaying Reason For Cancelation
        ...
        protected void lbReactivateSchedule_Click( object sender, EventArgs e )
        {
            ...
            if ( txnId.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    // Updating Cancelation Reason Attribute
                    if ( GetAttributeValue( "ReasonForCancelation" ).AsGuidOrNull() != null && GetAttributeValue( "ReasonForCancelation" ).AsGuidOrNull() != null )
                    {
                        txn.LoadAttributes();
                        var attributeGuid = GetAttributeValue( "ReasonForCancelation" ).AsGuid();
                        var attribute = AttributeCache.Get( attributeGuid );
                        txn.SetAttributeValue( attribute.Key, "" );
        ...
        /// <summary>
        /// Handles the CancelClick event of the mdCancelSchedule control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void mdCancelSchedule_CancelClick( object sender, EventArgs e )
        {
        ...
        private void ShowView( FinancialScheduledTransaction txn )
        {
            if ( txn != null )
            {
                ...
                if ( noteType != null )
                {
                    ...
                    // TAS 08/02/2018 - Fix to show Entity Information
                    if ( txn.ScheduledTransactionDetails.Where( x => x.EntityId != null && x.EntityId > 0 ).Any() )
                    {
                        lEntityInfo.Text = "<strong>Linkages</strong><br/>";
                        GroupMemberService groupMemberService = new GroupMemberService( rockContext );
                        var groupMemberList = txn.ScheduledTransactionDetails.Where( x => x.EntityId > 0 ).ToList();
                ...
                if ( GetAttributeValue( "ReasonForCancelation" ).AsGuidOrNull() != null )
                {
                    txn.LoadAttributes();
                    var attributeGuid = GetAttributeValue( "ReasonForCancelation" ).AsGuid();
                    var attribute = AttributeCache.Get( attributeGuid );
                    var attributeValue = txn.GetAttributeValue( attribute.Key );
                    if ( !string.IsNullOrEmpty( attributeValue ) )
                    {
                        lCancelationInformation.Text = string.Format( @"<dt>Reason For Cancelation</dt><dd>{0}</dd>",
                            attributeValue
                        );
                    }
                }
