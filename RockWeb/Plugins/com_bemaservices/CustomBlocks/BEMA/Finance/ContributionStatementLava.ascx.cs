// <copyright>
// Copyright by the BEMA Information Technologies
//
 ...
 * 
 * Additional Features:
 * - FE1) Added Ability to filter which accounts are used for pledges
 * - FE2) Added Ability to sort transactions by ascending date time
...
<p class=""text-center"">
    ...
    public partial class ContributionStatementLava : Rock.Web.UI.RockBlock
    {
        ...
        private static class BemaAttributeKey
        {
            ...
            public const string IsFilteredByValidDateTime = "IsFilteredByValidDateTime";
            public const string IsChildAccountGivingRolledUp = "IsChildAccountGivingRolledUp";
        }
        ...
        private void DisplayResults()
        {
            ...
            /* BEMA.FE3.Start */
            if ( targetPerson.RecordTypeValueId == DefinedValueCache.Get( Rock.SystemGuid.DefinedValue.PERSON_RECORD_TYPE_BUSINESS.AsGuid() ).Id )
            {
                mergeFields.Add( "Business", targetPerson.Id );
            }
            /* BEMA.FE3.End */
        ...
        {           
            ...
            var pledges = new FinancialPledgeService( rockContext ).Queryable().AsNoTracking()
                                ...
                                /* BEMA.FE4.Start */
                                .Where( p => !isFilteredByValidDateTime || ( p.StartDate > DateTime.MinValue && p.EndDate < DateTime.MaxValue ) )
                                /* BEMA.FE4.End */

                                /* BEMA.FE1.Start */
                                .Where( p => !accountIds.Any() || accountIds.Contains( p.AccountId.Value ) )
                                /* BEMA.FE1.End */
            ...
            foreach ( var pledge in pledges )
            {
                ...
                pledge.AmountGiven = new FinancialTransactionDetailService( rockContext ).Queryable()
                                            .Where( t =>
                                                /* BEMA.FE5.Start */
                                                ( t.AccountId == pledge.AccountId || ( isChildAccountGivingRolledUp && accounts.Contains( t.AccountId ) ) )
                                                 /* BEMA.FE5.End */
