 ...
 * 
 * Additional Features:
 * - FE1) Added Ability to Reassign transactions to accounts
 * - FE2) Added Ability to launch a workflow after an transaction has been reassigned to another account
...
namespace RockWeb.Plugins.com_bemaservices.Finance
{
    ...
    /* BEMA.UI1.Start */
    [BooleanField(
        "Is Account Reassignment Allowed?",
        Key = BemaAttributeKey.IsAccountReassignmentAllowed,
        DefaultValue = "False",
    ...
    [WorkflowTypeField(
            ...
            description: "Workflow to be triggered when transactions are reassigned to another account",
            allowMultiple: true,
            required: false,
    ...
    public partial class TransactionList : Rock.Web.UI.RockBlock, ISecondaryBlock, IPostBackEventHandler, ICustomGridColumns
    {
        ...
        {
            public const string IsAccountReassignmentAllowed = "IsAccountReassignmentAllowed";
...
", _ddlMove.ClientID, gTransactions.ClientID, Page.ClientScript.GetPostBackEventReference( this, "MoveTransactions" ), hfMoveToBatchId.ClientID);
            ...
            /* BEMA.FE1.Start */
            apAccount.Visible = GetAttributeValue( BemaAttributeKey.IsAccountReassignmentAllowed ).AsBoolean();
            /* BEMA.FE1.End */
        }
            ...
            if ( _canEdit && _person != null )
            {
                ...
                if ( txnsSelected.Any() && personAliasId.HasValue )
                {
                    using ( var rockContext = new RockContext() )
                    {
                        ...
                        IList<string> idsToUpdate = new List<string>();

                        int accountId = apReassign.SelectedValue.AsInteger();
                        foreach ( var txn in txnsDetailsToUpdate )
                        {
                            if ( accountId != 0 )
                            {

                                var account = new FinancialAccountService( rockContext ).GetByIds( new List<int> { accountId } ).FirstOrDefault();
