...
namespace RockWeb.Plugins.com_bemaservices.CustomBlocks.BEMA
{
    ...
    /* BEMA.UI1.Start */
    [TextField(
        "Weekend Dropdown Label",
        Key = BemaAttributeKey.WeekendDropdownLabel,
        Description = "Optional text to replace 'Sunday' with.",
        DefaultValue = "",
    ...
    /* BEMA.UI2.Start */
    [BooleanField(
        "Are Schedule and Weekend Dropdowns Switched?",
        Key = BemaAttributeKey.AreScheduleAndWeekendDropdownsSwitched,
        Description = "Should the schedule and weekend dropdowns be switched?",
        DefaultValue = "False",
        Category = "BEMA Additional Features" )]
    /* BEMA.UI2.End */
    ...
    public partial class ServiceMetricsEntry : Rock.Web.UI.RockBlock
    {
        ...
        private bool CheckSelection()
        {
            ...
            if ( !options.Any() && !_selectedWeekend.HasValue )
            {
                ...
                foreach ( var weekend in GetWeekendDates( 1, 0 ) )
                {
                    ...
                    /* BEMA.UI1.Start */
                    if ( GetAttributeValue( BemaAttributeKey.WeekendDropdownLabel ).IsNotNullOrWhiteSpace() )
                    {
                        options.Add( new ServiceMetricSelectItem( "Weekend", weekend.ToString( "o" ), GetAttributeValue( BemaAttributeKey.WeekendDropdownLabel ) + " " + weekend.ToShortDateString() ) );
                    }
                    else
                    {
        ...
        private void LoadDropDowns()
        {
            ...
            foreach ( var date in GetWeekendDates( weeksBack, weeksAhead ) )
            {
                ...
                if ( GetAttributeValue( BemaAttributeKey.WeekendDropdownLabel ).IsNotNullOrWhiteSpace() )
                {
                    bddlWeekend.Items.Remove( new ListItem( "Sunday " + date.ToShortDateString(), date.ToString( "o" ) ) );
                    bddlWeekend.Items.Add( new ListItem( GetAttributeValue( BemaAttributeKey.WeekendDropdownLabel ) + " " + date.ToShortDateString(), date.ToString( "o" ) ) );
                }
                /* BEMA.UI1.End */
            }
            ...
            /* BEMA.UI2.Start */
            var areScheduleAndWeekendDropdownsSwitched = GetAttributeValue( BemaAttributeKey.AreScheduleAndWeekendDropdownsSwitched ).AsBoolean();
            if ( areScheduleAndWeekendDropdownsSwitched )
            {
                foreach ( var service in GetServices() )
                {
        ...
        /* BEMA.UI2.Start */
        protected void bddlPreWeekendService_SelectionChanged( object sender, EventArgs e )
        {
            bddlService.SetValue( bddlPreWeekendService.SelectedValue );
            BindMetrics();
        }
        /* BEMA.UI2.End */
    }
