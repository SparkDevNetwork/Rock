...
namespace RockWeb.Plugins.com_bemadev.Checkin
{
    ...
    [BooleanField ( "Only Display Active Groups", "Should the Check-In Groups selector only display active groups?", false, "", 4 )]
    [BooleanField ( "Require Check-In Group for Child", "Should the Check-In Group be required for child?", false, "", 4 )]
    [BooleanField ( "Require Adult in New Family", "When adding a family, is an adult required in the family?", false, "", 5 )]
    [BooleanField ( "Require Mobile for Adult", "When adding an adult, is Mobile Phone required?", true, "", 4 )]
    [BooleanField ( "Require Email for Adult", "When adding an adult, is email required?", true, "", 4 )]
    [BooleanField ( "Require Birthdate for Child", "When adding a child, is Birthdate required?", true, "", 4 )]
    [BooleanField ( "Require Grade for Child", "When adding a child is Grade required?", false, "", 4 )]
    [BooleanField ( "Show Address for New Family", "When adding a family is Address required?", false, "", 6 )]
    [BooleanField ( "Require Address for New Family", "When adding a family is Address required?", false, "", 6 )]
    ...
    public partial class EditFamily : CheckInEditFamilyBlock
    {
        ...
        private void CreateDynamicPersonControls( FamilyRegistrationState.FamilyPersonState familyPersonState, bool setValues )
        {
            ...
            // add check-in group attribute if note there
            var attributeCheckInGroup = GetAttributeValue ( "Check-InGroupAttribute" );
            var groupAttribute = AttributeCache.Get ( GetAttributeValue ( "Check-InGroupAttribute" ).AsGuid () );
            ...
            foreach (Control attributeControl in phChildAttributes.ControlsOfTypeRecursive<Control>().OfType<Control>())
            {
                ...
                {
                    ( attributeControl as IHasRequired ).Required = GetAttributeValue ( "RequireCheck-InGroupforChild" ).AsBoolean ();

                    var limitCheckInGroup = GetAttributeValue("OnlyDisplayActiveGroups").AsBoolean();
                    if ((attributeCheckInGroup.AsGuidOrNull() != null) && (limitCheckInGroup == true))
                    {

                        // populate the drop down list for active groups
                        var GroupList = CurrentCheckInState.Kiosk.KioskGroupTypes.Where(t => t.IsCheckInActive == true)
                        .Where(t => CurrentCheckInState.ConfiguredGroupTypes.Contains(t.GroupType.Id))
                        .SelectMany(t => t.KioskGroups).ToList();
                        var ResultList = GroupList.Select(g => g.Group).ToList();
        ...
        private void ShowFamilyDetail( CheckInFamily checkInFamily )
        {
            ...
            else
            {
                ...
                acFamilyAddress.Visible = GetAttributeValue ( "ShowAddressforNewFamily" ).AsBoolean () || GetAttributeValue ( "RequireAddressforNewFamily" ).AsBoolean ();
                acFamilyAddress.Required = GetAttributeValue ( "RequireAddressforNewFamily" ).AsBoolean ();
        ...
        protected void btnSaveFamily_Click( object sender, EventArgs e )
        {
            ...
            if (GetAttributeValue( "RequireAdultinNewFamily" ).AsBoolean() && EditFamilyState.FamilyPersonListState.AsQueryable ().Where ( a => a.IsAdult == true ).Count () == 0)
            {
                // Error message
        ...
        private void ShowControlsForRole( bool isAdult )
        {
            ...
            pnMobilePhone.Required = isAdult & GetAttributeValue ( "RequireMobileforAdult" ).AsBoolean ();
            tbEmail.Required = isAdult & GetAttributeValue ( "RequireEmailforAdult" ).AsBoolean ();
            tbEmail.Visible = isAdult;


            tglAdultMaritalStatus.Visible = isAdult;
            dpBirthDate.Required = !isAdult & GetAttributeValue ( "RequireBirthdateforChild" ).AsBoolean ();
            dpBirthDate.Visible = !isAdult;
            gpGradePicker.Required = !isAdult & GetAttributeValue ( "RequireGradeforChild" ).AsBoolean ();
