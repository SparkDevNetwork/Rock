...
namespace RockWeb.Plugins.com_bemaservices.Event
{
    ...
    [BooleanField( "Enable ACH", "If true, Event Registrations can be paid via ACH", false, "", 15 )]
    [BooleanField( "ACH Default", "If true, ACH will automatically be selected as the default payment type", false, "", 16 )]

    public partial class CustomRegistrationEntry : RockBlock
    {
        #region Fields
        ...
        private PaymentInfo GetPaymentInfo( GatewayComponent gateway )
        {
            ...
            var homeLocation = CurrentPerson.PrimaryFamily.GroupLocations.Where( x => x.GroupLocationTypeValueId == homeAddressDv.Id ).First().Location;
        ...
        private void ShowSuccess( int registrationId )
        {
            ...
            try
            {
                    ...
                    ( rblSavedCC.Items.Count == 0 || ( rblSavedCC.SelectedValueAsId() ?? 0 ) == 0 ) )
                {
                    ...
                    // We are hiding Saving accounts because it makes this really complicated.
