...
namespace RockWeb.Plugins.com_bemaservices.Event
{
    ...
    [EventCalendarField( "Event Featured Calendar", "The event calendar to be displayed for Featured events", true, "1", order: 0 )]
    [CustomDropdownListField( "Default View Option", "Determines the default view option", "Day,Week,Month,Year", true, "Week", order: 1 )]
    [CustomDropdownListField( "Default View Featured Option", "Determines the default view option", "List,Featured", true, "List", order: 1 )]
    ...
    [BooleanField( "Show Year View", "Determines whether the Year view option is shown", true, order: 10 )]
    [BooleanField( "Show Audience Filter", "Determines whether the Year view option is shown", true, order: 10 )]
    [BooleanField( "Show Topic Filter", "Determines whether the Year view option is shown", true, order: 10 )]
    [BooleanField( "Show Community Filter", "Determines whether the Year view option is shown", true, order: 10 )]
    ...
    [CodeEditorField( "Lava Template Featured", "Lava template to use to display the list of Featured events.", CodeEditorMode.Lava, CodeEditorTheme.Rock, 400, true, @"{% include '~~/Assets/Lava/CalendarFeatured.lava' %}", "", 12 )]
    ...
    [TextField( "Column Class", "", true, "col-md-4", order: 16 )]
    ...
    public partial class CalendarLava : Rock.Web.UI.RockBlock
    {
        ...
        private int _calendarFeaturedId = 0;
        ...
        protected string colClass { get; set; }
        ...
        private String ViewFeaturedMode { get; set; }
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            ViewFeaturedMode = ViewState["ViewFeaturedMode"] as String;
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            var eventFeaturedCalendar = new EventCalendarService( new RockContext() ).Get( GetAttributeValue( "EventFeaturedCalendar" ).AsGuid() );
            if ( eventFeaturedCalendar != null )
            {
                _calendarFeaturedId = eventFeaturedCalendar.Id;
            }
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                if ( SetFilterControls() )
                {
                    ...
                    if ( Request["au"] != null )
                    {
                        int? ctId = Request["au"].AsIntegerOrNull();
                        ListItem test = ddlCatPicker.Items.FindByValue( ctId.ToString() );
                        if ( test != null )
                            ddlCatPicker.Items.FindByValue( ctId.ToString() ).Selected = true;
                    }

                    if ( Request["topic"] != null )
                    {
                        int? ctId = Request["topic"].AsIntegerOrNull();
                        ListItem test = ddlTopicPicker.Items.FindByValue( ctId.ToString() );
                        if ( test != null )
                            ddlTopicPicker.Items.FindByValue( ctId.ToString() ).Selected = true;
                    }

                    if ( Request["community"] != null )
                    {
                        int? ctId = Request["community"].AsIntegerOrNull();
                        ListItem test = ddlCommunityPicker.Items.FindByValue( ctId.ToString() );
                        if ( test != null )
                            ddlCommunityPicker.Items.FindByValue( ctId.ToString() ).Selected = true;
                    }
        ...
        protected override object SaveViewState()
        {
            ...
            ViewState["ViewFeaturedMode"] = ViewFeaturedMode;
        ...
        protected override void OnPreRender( EventArgs e )
        {
            ...
            btnDay.CssClass = "btn btn-event" + ( ViewMode == "Day" ? " active" : "" );
            btnWeek.CssClass = "btn btn-event" + ( ViewMode == "Week" ? " active" : "" );
            btnMonth.CssClass = "btn btn-event" + ( ViewMode == "Month" ? " active" : "" );
            btnYear.CssClass = "btn btn-event" + ( ViewMode == "Year" ? " active" : "" );

            btnList.CssClass = "btn btn-event" + ( ViewFeaturedMode == "List" ? " active" : "" );
            btnFeatured.CssClass = "btn btn-event" + ( ViewFeaturedMode == "Featured" ? " active" : "" );
        ...
        protected void cblFeatured_SelectedIndexChanged( object sender, EventArgs e )
        {
            BindData();
        }
        ...
        protected void ddlCommunityPicker_SelectedIndexChanged( object sender, EventArgs e )
        {
            BindData();
        }
        ...
        protected void btnViewFeaturedMode_Click( object sender, EventArgs e )
        {
            var btnViewMode = sender as BootstrapButton;
            if ( btnViewMode != null )
            {
                ViewFeaturedMode = btnViewMode.Text;
                ResetCalendarSelection();
                BindData();
            }
        }
        ...
        private void BindData()
        {
            ...
            //List<int> categoriesNew = ddlCatPicker.SelectedValue.
            int catNew = ddlCatPicker.SelectedValue.AsInteger();
            if ( catNew != 0 )
            {
                qry = qry
                    .Where( i => i.EventItem.EventItemAudiences
                        .Any( c => c.DefinedValueId == catNew ) );
            }


            // Community
            if ( ddlCommunityPicker.SelectedValue != "" )
            {
                var attribute = AttributeCache.Get( 13935 );

                List<int> groupListIds = new List<int>();
                List<string> groupAttributes = new List<string>();

                foreach ( var eventItem in qry )
                {

                    eventItem.LoadAttributes();

                    groupAttributes = eventItem.GetAttributeValues( attribute.Key );

                    foreach ( var attributeGuid in groupAttributes )
                    {
                        var dTC = DefinedValueCache.Get( attributeGuid );

                        if ( dTC != null )
                        {
                            if ( dTC.Id == ddlCommunityPicker.SelectedValue.AsIntegerOrNull() )
                            {
                                groupListIds.Add( eventItem.Id );
                            }
                        }
                    }
                }
                ...
                qry = qry.Where( g =>
                   groupListIds.Contains( g.Id ) );

            }

            // Topic
            if ( ddlTopicPicker.SelectedValue != "" )
            {
                var attribute = AttributeCache.Get( 15549 );
                ...
                foreach ( var eventItem in qry )
                {
                    ...
                    foreach ( var attributeGuid in groupAttributes )
                    {
                        ...
                        if ( dTC != null )
                        {
                            if ( dTC.Id == ddlTopicPicker.SelectedValue.AsIntegerOrNull() )
                            {
                                groupListIds.Add( eventItem.Id );
                            }
                        }
            ...
            {
                int? definedValueId = Request["filter"].AsIntegerOrNull();
                var attribute = AttributeCache.Get( 19420 );
            ...
            //List<int> featured = cblFeatured.Items.OfType<ListItem>().Where(l => l.Selected).Select(a => a.Value.AsInteger()).ToList();
            if ( ViewFeaturedMode == "Featured" )
            {
                qry = qry
                 .Where( m => m.EventItem.EventCalendarItems.Any( i => i.EventCalendarId == _calendarFeaturedId ) );

            }

            // add active tags
            var categoriesActive = cblCategory.Items.OfType<ListItem>().Where( l => l.Selected ).ToList();
            foreach ( var category in categoriesActive )
            {
                category.Attributes["class"] = "ActiveInput";
            ...
            var occurrencesWithDates = occurrences
                .Select( o => new EventOccurrenceDate
                {
                    ...
                    Dates = GetEventTimes( o.Schedule, beginDate.AddMonths( -1 ), endDate.AddMonths( 1 ) ).ToList()
                } )
                .Where( d => d.Dates.Any() )
                .ToList();
            ...
            string script = @" 
                window.console&&console.log('foo');
                    ...
                    $('input[type=""checkbox""][checked=""checked""]').parent().addClass('checkedbg');

                  $('input:checkbox').change(function(event){
                      if($(this).is(':checked')) {
                          $(event.target.parentElement).addClass('checkedbg');
                      } else {
                          $(event.target.parentElement).removeClass('checkedbg');
                      }
                  });
            ";

            ScriptManager.RegisterStartupScript( upnlContent, upnlContent.GetType(), "search_buttons", script, true );
            ...
            foreach ( var occurrenceDates in occurrencesWithDates )
            {
                ...
                foreach ( var datetime in occurrenceDates.Dates )
                {
                    ...
                    {
                        CalendarEventDates.Add( datetime.StartDateTime );
                    }

                    if (
                        ( ( datetime.StartDateTime >= beginDate ) || ( datetime.EndDateTime >= beginDate ) ) &&
                        ( ( datetime.StartDateTime < endDate ) || ( datetime.EndDateTime < endDate ) ) )
                    {
                        eventOccurrenceSummaries.Add( new EventOccurrenceSummary
                        {
                            ...
                            Date = datetime.StartDateTime.ToShortDateString(),
                            Time = datetime.StartDateTime.ToShortTimeString(),
            ...
            if ( ViewFeaturedMode == "Featured" )
                lOutput.Text = GetAttributeValue( "LavaTemplateFeatured" ).ResolveMergeFields( mergeFields );
            else
        ...
        private bool SetFilterControls()
        {
            ...
            ViewFeaturedMode = GetAttributeValue( "DefaultViewFeaturedOption" );

            if ( Request.QueryString["featured"] != null )
            {
                ViewFeaturedMode = "Featured";
            }

            //if( GetAttributeValue( "ShowAudienceFilter" ) == "false")
            //ddlCatPicker.visible = false;

            //GetAttributeValue( "ShowTopicFilter" )
            //GetAttributeValue( "ShowCommunityFilter" )

            ddlCatPicker.Items.Remove( ddlCatPicker.Items.FindByValue( "" ) );
            ddlCatPicker.Items.Remove( ddlCatPicker.Items.FindByValue( "982" ) );
            ddlCatPicker.Items.Remove( ddlCatPicker.Items.FindByValue( "25" ) );
            ddlCatPicker.Items.Remove( ddlCatPicker.Items.FindByValue( "1017" ) );
            ddlCatPicker.Items.Insert( 0, new ListItem( "Audience", "" ) );
            ddlCatPicker.DataBind();

            ddlCommunityPicker.Items.Remove( ddlCommunityPicker.Items.FindByValue( "" ) );
            ddlCommunityPicker.Items.Remove( ddlCommunityPicker.Items.FindByValue( "745" ) );
            ddlCommunityPicker.Items.Remove( ddlCommunityPicker.Items.FindByValue( "746" ) );
            ddlCommunityPicker.Items.Insert( 0, new ListItem( "Community", "" ) );
            ddlCommunityPicker.DataBind();

            ddlTopicPicker.Items.Remove( ddlTopicPicker.Items.FindByValue( "" ) );
            ddlTopicPicker.Items.Insert( 0, new ListItem( "Topic", "" ) );
            ...
            {
                FilterStartDate = today;
                FilterEndDate = FilterStartDate.Value.AddMonths( 6 ).AddDays( -1 );
            }
            ...
            cblCampus.DataSource = CampusCache.All().Where( c => c.IsActive == true ); //Changed 8/3/2018 to only show active campuses - JM
            cblCampus.DataBind();
            ...
            //  pnlList.CssClass = showFilter ? "col-md-12" : "col-md-12";
        ...
        public virtual List<EventDateTime> GetEventTimes( Schedule schedule, DateTime beginDateTime, DateTime endDateTime )
        {
            if ( schedule != null )
            {
                var result = new List<EventDateTime>();
        ...
        {
            public EventItemOccurrence EventItemOccurrence { get; set; }
            public List<EventDateTime> Dates { get; set; }
        }
