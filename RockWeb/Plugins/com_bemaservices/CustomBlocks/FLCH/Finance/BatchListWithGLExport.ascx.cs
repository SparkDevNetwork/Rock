    ...
    $('#{0}').change(function( e ){{
        ...
        { 
            ...
            if ( batchesSelected.Any() )
            {
                ...
                record.JournalReference = "JournalReference";
        ...
        List<GLRecord> GLRecordsForBatch( FinancialBatch batch, DateTime date, string accountingPeriod, string journalType )
        {
            ...
            var accounts = transactions.GroupBy( d => new { TransactionDate = d.Transaction.TransactionDateTime.Value.Date, AccountNumber = string.IsNullOrEmpty(d.GetAttributeValue("AccountNumber")) ? d.Account.GetAttributeValue("AccountNumber") : d.GetAttributeValue("AccountNumber"), ProjectId = string.IsNullOrEmpty(d.GetAttributeValue("ProjectCode")) ? d.Account.GetAttributeValue("ProjectCode") : d.GetAttributeValue("ProjectCode"), AccountName = d.Account.Name }, d => d ).OrderBy( d => d.Key.AccountNumber ).ThenBy( d => d.Key.TransactionDate );
            var depostis = transactions.GroupBy( d => new { TransactionDate = d.Transaction.TransactionDateTime.Value.Date, AccountNumber = string.IsNullOrEmpty(d.GetAttributeValue("AccountNumber")) ? d.Account.GetAttributeValue("AccountNumber").Left(2) : d.GetAttributeValue("AccountNumber").Left(2) }, d => d).OrderBy( d => d.Key.AccountNumber ).ThenBy( d => d.Key.TransactionDate );
            GLRecord record = new GLRecord();
            ...
            foreach ( var grp in accounts)
            {
                ...
                /* Old Code
                var accountNumber = grp.GetAttributeValue("AccountNumber");
                if (String.IsNullOrEmpty(accountNumber)){
                    accountNumber = grp.Account.GetAttributeValue("AccountNumber");
                }

                var projectId = grp.GetAttributeValue("ProjectCode");
                if (String.IsNullOrEmpty(projectId)){
                    projectId = grp.Account.GetAttributeValue("ProjectCode");
                }

                record.PostDate = grp.Transaction.TransactionDateTime.Value.ToString("M/d/yyyy");
                record.AccountNumber = accountNumber;
                record.ProjectId = projectId;
                record.Type = grp.Transaction.FinancialPaymentDetail.CurrencyTypeValue.Value;
                record.Amount = grp.Amount.ToString("0.##");
                record.JournalType = "Journal";
                record.JournalReference = grp.Account.Name;

                */
                record.PostDate = grp.Key.TransactionDate.ToString("M/d/yyyy");
                record.AccountNumber = grp.Key.AccountNumber;
                record.ProjectId = grp.Key.ProjectId;
                record.Type = grp.Sum( d => d.Amount ) >= 0 ? "Credit" : "Debit";
                record.Amount = Math.Abs(grp.Sum( d => d.Amount )).ToString("0.00");
                record.JournalType = "Journal Entry";
                record.JournalReference = "\"Batch Ids: " + string.Join(",", grp.Select( x => x.Transaction.BatchId ).Distinct()) + " - " + grp.Key.AccountName + "\"";

                records.Add(record);
            }

            foreach( var grp in depostis)
            {
                record = new GLRecord();
                int rowNumber = 0;

                record.PostDate = grp.Key.TransactionDate.ToString("M/d/yyyy");
                record.AccountNumber = grp.Key.AccountNumber + "-1010-00-000-0000";
                record.ProjectId = "";
                record.Type = grp.Sum( d => d.Amount ) >= 0 ? "Debit" : "Credit";
                record.Amount = Math.Abs(grp.Sum( d => d.Amount )).ToString("0.00");
                record.JournalType = "Journal Entry";
                record.JournalReference =journalType;
