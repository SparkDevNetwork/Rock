...
{
    /// <summary>
    /// Builds a list of all financial transactions which can be filtered by date, account, transaction type, etc.
    /// </summary>
    /// <remarks>
    /// This is a copy of Core V8.8 with the following changes
    /// 1. When the Transaction Details is selected the acount info doesn't show amount
    /// 2. The transaction and transaction detail also show the batch name next to Id
    /// </remarks>
    [DisplayName( "Transaction List" )]
    [Category("Grace Church > Finance")]
    ...
    [BooleanField( "Show Foreign Key", "Should the transaction foreign key column be displayed?", false, order: 8 )]
    [BooleanField( "Show Account Summary", "Should the account summary be displayed at the bottom of the list?", false, order: 9 )]
    ...
    public partial class TransactionList : Rock.Web.UI.RockBlock, ISecondaryBlock, IPostBackEventHandler, ICustomGridColumns
    {
        ...
        private List<AttributeValue> _softCreditDetails;
...
", _ddlMove.ClientID, gTransactions.ClientID, Page.ClientScript.GetPostBackEventReference( this, "MoveTransactions" ), hfMoveToBatchId.ClientID);
        ...
        protected void gfTransactions_ApplyFilterClick( object sender, EventArgs e )
        {
            ...
            //Soft Credit
            gfTransactions.SaveUserPreference( "Has Soft Credit", ddlSoftCredit.SelectedValue );
        ...
        protected void gTransactions_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            if ( e.Row.RowType == DataControlRowType.DataRow )
            {
                ...
                if ( txn != null )
                {
                    ...
                    var lPrimaryFamilyId = e.Row.FindControl( "lPrimaryFamilyId" ) as Literal;
                    if ( lPersonFullNameReversed != null && lPersonId != null && lPrimaryFamilyId != null && txn.AuthorizedPersonAliasId.HasValue )
                    {
                        ...
                        if ( personDetail != null )
                        {
                            ...
                            lPrimaryFamilyId.Text = personDetail.PrimaryFamilyId.ToString();
                        }
                    }
                    ...
                    if ( lSummary != null )
                    {
                        var softCredit = new FinancialTransactionService( new RockContext() ).Get( txn.Id ).TransactionDetails.First().GetAttributeValue( "SoftCreditFor" );
                        if ( softCredit.IsNotNullOrWhiteSpace() )
                        {
                            lSummary.Text += "<br/>( Soft Credit For: " + softCredit + ")";
                        }
                        
                    }

                    var lSoftCreditPersonId = e.Row.FindControl( "lSoftCreditPersonId" ) as Literal;
                    var lSoftCreditFamilyId = e.Row.FindControl( "lSoftCreditFamilyId" ) as Literal;
                    if ( lSoftCreditPersonId != null || lSoftCreditFamilyId != null )
                    {
                        AttributeValue softCreditAttributeValue = null;
                        if ( hfTransactionViewMode.Value == "Transaction Details" )
                        {
                            softCreditAttributeValue = _softCreditDetails.FirstOrDefault( sc => txn.TransactionDetail.Id == sc.EntityId );
                        }
                        else
                        {
                            softCreditAttributeValue = _softCreditDetails.FirstOrDefault( sc => txn.TransactionDetails.Any( td => td.Id == sc.EntityId ) );
                        }

                        if ( softCreditAttributeValue != null && softCreditAttributeValue.ValueAsPersonId.HasValue )
                        {
                            //Find Person
                            var person = new PersonService( new RockContext() ).Get( softCreditAttributeValue.ValueAsPersonId.Value );

                            if ( lSoftCreditPersonId != null )
                            {
                                lSoftCreditPersonId.Text = person.Id.ToString();
                            }
                            if ( lSoftCreditFamilyId != null )
                            {
                                lSoftCreditFamilyId.Text = person.PrimaryFamilyId.ToString();
                            }
                        }
                    }
                    ...
                    var lBatchName = e.Row.FindControl("lBatchName") as Literal;
                    if (lBatchName != null)
                    {
                        if (_batchPageRoute.IsNotNullOrWhiteSpace() && txn.BatchName != string.Empty && !isExporting)
                        {
                            var cell = e.Row.Cells.OfType<DataControlFieldCell>().Where(a => a == lBatchName.FirstParentControlOfType<DataControlFieldCell>()).First();
                            cell.RemoveCssClass("grid-select-cell");
                            lBatchName.Text = string.Format("<a href='{0}?BatchId={1}'>{2}</a>", _batchPageRoute, txn.BatchId, txn.BatchName);
                        }
                        else
                        {
                            lBatchName.Text = txn.BatchName;
                        }
                    }
        ...
        private void BindFilter()
        {
            ...
            if ( GetAttributeValue( "ShowForeignKey" ).AsBoolean() )
            {
            ...
            //Soft Credit
            ddlSoftCredit.Items.Add( "Yes" );
            ddlSoftCredit.Items.Add( "No" );
            ddlSoftCredit.Items.Insert( 0, new ListItem( string.Empty, string.Empty ) );
            ddlSoftCredit.SelectedValue = gfTransactions.GetUserPreference( "Has Soft Credit" );
        ...
        {
            listControl.BindToDefinedType( DefinedTypeCache.Get( definedTypeGuid ) );
        ...
        private void BindGrid( bool isExporting = false )
        {
            ...
            if ( hfTransactionViewMode.Value == "Transaction Details" )
            {
                ...
                qry = financialTransactionDetailQry.Select( a => new FinancialTransactionRow
                {
                    ...
                    BatchName = a.Transaction.Batch.Name,
                    ...
                    TransactionDetail = new DetailInfo { Id = a.Id, AccountId = a.AccountId, Amount = a.Amount, EntityId = a.EntityId, EntityTypeId = a.EntityId },
            ...
            else
            {
                ...
                qry = financialTransactionQry
                    ...
                    .Select( a => new FinancialTransactionRow
                    {
                        ...
                        TransactionDetails = a.TransactionDetails.Select( d => new DetailInfo {Id = d.Id, AccountId = d.AccountId, Amount = d.Amount, EntityId = d.EntityId, EntityTypeId = d.EntityTypeId } ),
            ...
            } ).AsNoTracking().ToList().Select( k => new PersonDetail
            {
                ...
                PrimaryFamilyId = k.PrimaryFamilyId
            } ).ToList();

            if ( hfTransactionViewMode.Value == "Transaction Details" )
            {
                var qryAttribute = new AttributeValueService( rockContext ).Queryable().AsNoTracking();
                _softCreditDetails = qryAttribute.Where( av => av.Attribute.Key == "SoftCreditFor" && qry.Any( q => q.TransactionDetail.Id == av.EntityId ) ).ToList(); //get soft credit details


                // Soft Credit
                if ( gfTransactions.GetUserPreference( "Has Soft Credit" ) == "Yes" )
                {
                    List<int> hasSoftCredit = _softCreditDetails.Select( sc => sc.EntityId ?? -1 ).ToList();
                    qry = qry.Where( t => hasSoftCredit.Contains( t.TransactionDetail.Id ) );
                }
                else if ( gfTransactions.GetUserPreference( "Has Soft Credit" ) == "No" )
                {
                    List<int> hasSoftCredit = _softCreditDetails.Select( sc => sc.EntityId ?? -1 ).ToList();
                    qry = qry.Where( t => !hasSoftCredit.Contains( t.TransactionDetail.Id ) );
                }
            ...
            {
                var qryAttribute = new AttributeValueService( rockContext ).Queryable().AsNoTracking();
                _softCreditDetails = qryAttribute.Where( av => av.Attribute.Key == "SoftCreditFor" && qry.Any( q => q.TransactionDetails.Any( td => td.Id == av.EntityId ) ) ).ToList(); //get soft credit details
                ...
                {
                    List<int> hasSoftCredit = _softCreditDetails.Select( sc => sc.EntityId ?? -1 ).ToList();
                    qry = qry.Where( t => t.TransactionDetails.Any( td => hasSoftCredit.Contains( td.Id ) ) );
                }
                else if ( gfTransactions.GetUserPreference( "Has Soft Credit" ) == "No" )
                {
                    List<int> hasSoftCredit = _softCreditDetails.Select( sc => sc.EntityId ?? -1 ).ToList();
        ...
        private string GetAccounts( FinancialTransactionRow txn, bool isExporting )
        {
            if ( txn != null )
            {
                ...
                if ( txn.TransactionDetail != null )
                {
                    ...
                    string summaryLine = string.Format( "{0}", _financialAccountLookup[txn.TransactionDetail.AccountId].Name );
