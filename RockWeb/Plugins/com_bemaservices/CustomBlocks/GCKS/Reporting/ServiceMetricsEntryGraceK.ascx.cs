...
namespace RockWeb.Plugins.com_visitgracechurch.Reporting
{
    /// <summary>
    ...
    [CampusField( "Campus Override", "Select one campus instead of giving user a selection", false, "", "", 4)]
    public partial class ServiceMetricsEntryGraceK : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                //If Campus Override selected, convert
                if ( GetAttributeValue( "CampusOverride" ).AsGuidOrNull().HasValue )
                {
                    _selectedCampusId = CampusCache.Get( GetAttributeValue( "CampusOverride" ) ).Id;
                }
        ...
        private List<CampusCache> GetCampuses()
        {
            ...
            if ( GetAttributeValue( "CampusOverride" ).AsGuidOrNull().HasValue )
            {
                campuses.Add( CampusCache.Get( GetAttributeValue( "CampusOverride" ) ) );
            }
            else
            {
                foreach ( var campus in CampusCache.All()
