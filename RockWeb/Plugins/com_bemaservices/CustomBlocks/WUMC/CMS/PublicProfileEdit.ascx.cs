...
namespace RockWeb.Plugins.com_bemadev.Cms
{
    ...
    public partial class PublicProfileEdit : RockBlock
    {
        ...
        protected void rptGroupMembers_ItemDataBound( object sender, RepeaterItemEventArgs e )
        {
            ...
            {
				// David Bensen - BEMA - new age format
             
				string monthName = person.BirthDate.Value.ToString("MMMM");
				lAge.Text = string.Format( "Born on {0}<small>({1})</small><br/>", monthName, person.BirthYear != DateTime.MinValue.Year ? person.BirthDate.Value.ToShortDateString() : person.BirthDate.Value.ToMonthDayString() );
                ...
                if ( group != null )
                {
                    rockContext.WrapTransaction( () =>
                    {
                        ...
                        if ( personId == 0 )
                        {
                            ...
                            groupMember.Person.MiddleName = tbMiddleName.Text;
                            ...
                            {
                                DefinedValueCache dvcConnectionStatus = DefinedValueCache.Read( 680 );
        ...
        private void ShowDetail()
        {
            ...
            if ( CurrentPerson != null )
            {
                ...
                // Setup Image
                string imgTag = Rock.Model.Person.GetPersonPhotoImageTag( CurrentPerson, 150, 150 );
                ...
                {
					string monthName = CurrentPerson.BirthDate.Value.ToString("MMMM");
					string monthDate = CurrentPerson.BirthDate.Value.ToString("dd");
					string yearDate= CurrentPerson.BirthDate.Value.ToString("yyyy");
					lAge.Text = string.Format( "Born on {0} {1}, {2}<br/>", monthName, monthDate, yearDate );
				...
				if (CurrentPerson.ConnectionStatusValue.ToString() == "Member")
				{
					lConnection.Text = CurrentPerson.ConnectionStatusValue.ToString() + " at The Woodlands UMC";
				}
				else 
				{
					lConnection.Text = "Nonmember at the Woodlands UMC";
				}
				
				CurrentPerson.LoadAttributes();
				
				if ( CurrentPerson.GetAttributeValue("EmployerNew") != null )
				{
					lEmployer.Text =  "Works at " + CurrentPerson.GetAttributeValue("EmployerNew").ToString();
				}
				
				if ( CurrentPerson.GetAttributeValue("PositionNew") != null )
				{
					lPosition.Text =  "Position: " + CurrentPerson.GetAttributeValue("PositionNew").ToString();
				}
                        ...
                        if ( group != null )
                        {
                            ...
                            if ( locationTypeGuid.HasValue )
                            {
                                ...
                                if ( familyGroupTypeGuid.HasValue )
                                {
                                    ...
                                    if ( address != null )
                                    {
                                        lAddress.Text = string.Format( "<div class='margin-b-md'>{0}</div>", address.FormattedHtmlAddress );
                                    }
                                }
                            }
