...
", "", 5 )]
    ...
    /* BEMA.FE2.Start */
    [BooleanField( "Enable Continue As Guest For Registrations", "This will enable users to continue back to a registration as a guest utilizing a redirect on the registration page looking for a parameter of Guest=True",false,"BEMA Addditional Features",18)]
    ...
    public partial class Login : Rock.Web.UI.RockBlock
    {
        ...
        private void ApplyBlockSettings()
        {
            ...
            /* BEMA.FE2.Start */
            //Grabs the url redirect string to determin if registration source and if not blank and Allow Guest is True show the guest button.
            string stringUrl = Request.QueryString["returnurl"];
            
			if(GetAttributeValue("EnableContinueAsGuestForRegistrations").AsBoolean() && !string.IsNullOrWhiteSpace(stringUrl))
			{			
				if (stringUrl.Contains("RegistrationInstanceId") || stringUrl.Contains("/Registration/") )
				{
        ...
        /// <summary>
        /// Handles the Click event of the btnGuest control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="System.EventArgs"/> instance containing the event data.</param>
        protected void btnGuest_Click1(object sender, EventArgs e)
        {
            ...
            if(returnUrl.Contains("?")){
              url = returnUrl + "&Guest=true";
            }else{
              url = returnUrl + "?Guest=true";
            }

            string redirectUrl = Server.UrlDecode(url);
