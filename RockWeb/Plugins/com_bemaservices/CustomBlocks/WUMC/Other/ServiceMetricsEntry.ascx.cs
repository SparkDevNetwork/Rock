...
namespace com_BEMAServices.Reporting
{
    /// <summary>
    ...
    [Category("com_bemaservices > Reporting")]
    ...
    public partial class ServiceMetricsEntry : Rock.Web.UI.RockBlock
    {
        ...
        private int? _selectedLocationId { get; set; }
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            _selectedLocationId = ViewState["SelectedLocationId"] as int?;
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                _selectedLocationId = GetBlockUserPreference( "LocationId" ).AsIntegerOrNull();
        ...
        {
            ViewState["SelectedLocationId"] = _selectedLocationId;
        ...
        protected void rptrSelection_ItemCommand( object source, RepeaterCommandEventArgs e )
        {
            switch( e.CommandName )
            {
                ...
                //    break;
                case "Location":
                    _selectedLocationId = e.CommandArgument.ToString().AsIntegerOrNull();
                    break;
                case "Weekend":
        ...
        protected void btnSave_Click( object sender, EventArgs e )
        {
           ...
           // int? campusId = bddlCampus.SelectedValueAsInt();
            int? locationId = bddlLocation.SelectedValueAsInt();
            int? scheduleId = bddlService.SelectedValueAsInt();
            DateTime? weekend = bddlWeekend.SelectedValue.AsDateTime();

            if (locationId.HasValue && scheduleId.HasValue && weekend.HasValue )
            {
                    ...
                    foreach ( RepeaterItem item in rptrMetric.Items )
                    {
                        ...
                        if ( hfMetricIId != null && nbMetricValue != null  )
                        {
                            ...
                            if ( metric != null )
                            {
                                ...
                                int locationPartitionId = metric.MetricPartitions.Where( p => p.EntityTypeId.HasValue && p.EntityTypeId.Value == 31 ).Select( p => p.Id ).FirstOrDefault();
                                ...
                                var metricValue = metricValueService
                                    ...
                                    .Where( v =>
                                       ...
                                       // v.MetricValuePartitions.Count == 2 &&
                                        v.MetricValuePartitions.Any( p => p.MetricPartitionId == locationPartitionId && p.EntityId.HasValue && p.EntityId.Value == locationId.Value ) &&
                                ...
                                if ( metricValue == null )
                                {
                                    ...
                                    var locationValuePartition = new MetricValuePartition();
                                    locationValuePartition.MetricPartitionId = locationPartitionId;
                                    locationValuePartition.EntityId = locationId.Value;
                                    metricValue.MetricValuePartitions.Add(locationValuePartition);
                                }
                ...
                nbMetricsSaved.Text = string.Format( "Your metrics for the {0} service on {1} at the {2} Campus have been saved.",
                    bddlService.SelectedItem.Text, bddlWeekend.SelectedItem.Text, bddlLocation.SelectedItem.Text );
        ...
        private bool CheckSelection()
        {
            ...
            if ( _selectedLocationId.HasValue && _selectedServiceId.HasValue && !_selectedWeekend.HasValue )
            {
            ...
            {
                var dfLocations = new DefinedValueService(new RockContext()).Queryable().Where(a => a.DefinedType.Id == 66).ToList();
                lSelection.Text = "Select Location:";
                foreach (var location in dfLocations)
                {
                    options.Add(new ServiceMetricSelectItem("Location", location.Id.ToString(), location.Value));
                }
            }
        ...
        private void LoadDropDowns()
        {
            ...
            var dfLocations = new DefinedValueService(new RockContext()).Queryable().Where(a => a.DefinedType.Id == 66).OrderBy(a => a.Order).ToList();

            foreach (var location in dfLocations)
            {
                bddlLocation.Items.Add(new ListItem(location.Value, location.Id.ToString()));
            }
            bddlLocation.SetValue(_selectedLocationId.Value);
        ...
        private void BindMetrics()
        {
           ...
           // int? campusId = bddlCampus.SelectedValueAsInt();
            ...
            if (  scheduleId.HasValue && weekend.HasValue )
            {

                SetBlockUserPreference( "LocationId", locationId.HasValue ? locationId.Value.ToString() : "" );
                ...
                using ( var rockContext = new RockContext() )
                {
                    ...
                    foreach ( var metric in new MetricService( rockContext )
                        ...
                        .Where( m =>
                           //m.MetricPartitions.Count == 2 &&
                            m.MetricPartitions.Any( p => p.EntityTypeId.HasValue && p.EntityTypeId.Value == 31 ) &&
                        ...
                        .Select( m => new
                        {
                            m.Id,
                            m.Title,
                            LocationPartitionId = m.MetricPartitions.Where( p => p.EntityTypeId.HasValue && p.EntityTypeId.Value == 31 ).Select( p => p.Id ).FirstOrDefault(),
                        ...
                        } ) )
                    {
                        ...
                        if (  weekend.HasValue && scheduleId.HasValue )
                        {
                            var metricValue = metricValueService
                                .Queryable().AsNoTracking()
                                .Where( v =>
                                    ...
                                    v.MetricValueDateTime.HasValue && v.MetricValueDateTime.Value == weekend.Value &&
                                   // v.MetricValuePartitions.Count == 2 &&
                                    v.MetricValuePartitions.Any( p => p.MetricPartitionId == metric.LocationPartitionId && p.EntityId.HasValue && p.EntityId.Value == locationId.Value ) &&
