  ...
  },
  {
    ...
    public partial class CustomTransactionEntry : Rock.Web.UI.RockBlock
    {
        ...
        private void GetAccounts()
        {
            ...
            {
				if(GetAttributeValue( "Impersonation" ).AsBooleanOrNull() ?? true)
				{
        ...
        private void ShowPersonal( Person person )
        {
            if ( person != null )
            {
                txtCurrentName.Text = person.FirstName + " " + person.LastName;
        ...
        private bool ProcessPaymentInfo( out string errorMessage )
        {
            ...
            if ( schedule != null )
            {
				...
				// Make sure a repeating payment ends in the future
                if ( schedule.EndDate <= schedule.StartDate )
                {
                    errorMessages.Add( "When scheduling a repeating payment, make sure the End Gift date is after your start day.)" );
                }
