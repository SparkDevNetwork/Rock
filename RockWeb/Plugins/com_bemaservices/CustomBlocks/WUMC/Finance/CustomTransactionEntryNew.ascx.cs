/*
Core Block: TransactionEntry

Custom Changes:
- Changed personal info to show firstname & lastname instead of nickname & lastname
...
namespace RockWeb.Plugins.org_thewoodlandsumc.Finance
{
    ...
    /// </summary>
    [DisplayName( "Custom Transaction Entry" )]
...
TransactionAccountDetails: [
  ...
  },
  {
    ...
    public partial class CustomTransactionEntry : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void dllGiftIndexChange( object sender, EventArgs e )
        {
            if (ddlGift.SelectedValue != "")
            {
                txtGiftFor.Visible = true;
                txtNotified.Visible = true;
            }
            else
            {
                txtGiftFor.Visible = false;
                txtNotified.Visible = false;
            }
        }
        ...
        private bool ProcessStep3( string resultQueryString, out string errorMessage )
        {
            ...
            if ( txtCommentEntry.Text.IsNotNullOrWhiteSpace() )
            {
                sb.AppendFormat( "\n Comment: {0}", txtCommentEntry.Text );
            }
            if ( ddlGift.SelectedValue.IsNotNullOrWhiteSpace() )
            {
                sb.AppendFormat( "\n Type: {0}", ddlGift.SelectedValue );
            }
            if ( txtGiftFor.Text.IsNotNullOrWhiteSpace() )
            {
                sb.AppendFormat( "\n Who is the gift in memory or honor of?: {0}", txtGiftFor.Text );
            }
            if ( txtNotified.Text.IsNotNullOrWhiteSpace() )
            {
                sb.AppendFormat( "\n Who should be notified of the gift?: {0}", txtNotified.Text );
            }
            
            if ( GetAttributeValue( "EnableCommentEntry" ).AsBoolean() )
            {
                paymentInfo.Comment1 = !string.IsNullOrWhiteSpace( GetAttributeValue( "PaymentComment" ) ) ? string.Format( "{0}: {1}", GetAttributeValue( "PaymentComment" ), sb.ToString() ) : sb.ToString();
            }
            else
            {
        ...
        private PaymentInfo GetTxnPaymentInfo( Person person, out string errorMessage )
        {
            ...
            if ( GetAttributeValue( "EnableCommentEntry" ).AsBoolean() )
            {
                paymentInfo.Comment1 = !string.IsNullOrWhiteSpace( paymentComment ) ? string.Format( "{0}: {1}", paymentComment, sb.ToString() ) : sb.ToString();
            }
            else
            {
