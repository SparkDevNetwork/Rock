...
namespace RockWeb.Plugins.com_bemaservices.Groups
{
    ...
    [AttributeField(Rock.SystemGuid.EntityType.GROUP, "Attribute Filters 3", "", false, true, "", "CustomSetting")]
    [AttributeField(Rock.SystemGuid.EntityType.GROUP, "Attribute Filters 4", "", false, true, "", "CustomSetting")]
	...
	[TextField( "Column Class", "", true, "col-md-4")]
...
", "CustomSetting" )]
    ...
    public partial class GroupFinder : RockBlockCustomSettings
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
			...
			colClass = GetAttributeValue( "ColumnClass" );
        ...
        private void BuildDynamicControls()
        {
            ...
            cpCampus.Campuses = CampusCache.All().Where(c=> c.IsActive == true && c.Id != 4).ToList();
            cpCampus.DataBind();

            string campusParam = PageParameter( "campus" );

            if ( !string.IsNullOrWhiteSpace( campusParam ) )
            {
                int? campusId = null;
                if (campusParam == "colleyville")
                {
                  campusId = 2;
                }
                else if (campusParam == "roanoke")
                {
                  campusId = 3;
                }
                else if (campusParam == "northfortworth")
                {
                  campusId = 7;
                }
                else if (campusParam == "northrichlandhills")
                {
                  campusId = 6;
                }
                ...
                if ( campus != null )
                {
                    cpCampus.SetValue( campusId.Value );
                }
            }

            ddlChildcarePicker.Items.Remove(ddlChildcarePicker.Items.FindByValue(""));
            ddlChildcarePicker.Items.Insert(0, new ListItem("Childcare", ""));
            ddlChildcarePicker.DataBind();

            ddlAgePicker.Items.Remove(ddlAgePicker.Items.FindByValue(""));
            ddlAgePicker.Items.Insert(0, new ListItem("Age", ""));
            ddlAgePicker.DataBind();

            ddlCatPicker.Items.Remove(ddlCatPicker.Items.FindByValue(""));
            ddlCatPicker.Items.Insert(0, new ListItem("Group Type", ""));
            ddlCatPicker.DataBind();

            ddlCommunityPicker.Items.Remove(ddlCommunityPicker.Items.FindByValue(""));
            ddlCommunityPicker.Items.Insert(0, new ListItem("Region", ""));
            ddlCommunityPicker.DataBind();

            ddlDayOfTheWeekPicker.Items.Remove(ddlDayOfTheWeekPicker.Items.FindByValue(""));
            ddlDayOfTheWeekPicker.Items.Insert(0, new ListItem("Day of the Week", ""));
            ddlDayOfTheWeekPicker.DataBind();

			      ddlHourPicker.Items.Remove(ddlHourPicker.Items.FindByValue(""));
            ddlHourPicker.Items.Insert(0, new ListItem("Time", ""));
            ddlHourPicker.DataBind();
        ...
        private void ShowResults()
        {
            ...
            // day of the week
            if (ddlDayOfTheWeekPicker.SelectedDayOfWeek.HasValue)
            {
                groupQry = groupQry.Where(g =>
                    g.Schedule.WeeklyDayOfWeek.HasValue &&
                    ddlDayOfTheWeekPicker.SelectedDayOfWeek.Value == g.Schedule.WeeklyDayOfWeek.Value);
            }
            ...
            var campusId = cpCampus.SelectedCampusId;
             ...
             {
                 groupQry = groupQry.Where( g => !g.CampusId.HasValue || g.CampusId.Value == campusId );
             }


            // Childcare
            if (ddlChildcarePicker.SelectedValue != "")
            {
                var attribute = AttributeCache.Get(6596);
                ...
                {
                    group.LoadAttributes();
                    groupAttributes = group.GetAttributeValues(attribute.Key);
                    ...
                    foreach (var attributeGuid in groupAttributes)
                    {
                        ...
                        if (dTC != null)
                        {
                            if (dTC.Id == ddlChildcarePicker.SelectedValue.AsIntegerOrNull())
                            {
                                groupListIds.Add(group.Id);
                            }
                        }
                    }
                }
            ...
            // Age Range
            if (ddlAgePicker.SelectedValue != "")
            {
                var attribute = AttributeCache.Get(6597);
                ...
                foreach (var group in groupQry)
                {
                    ...
                    foreach (var attributeGuid in groupAttributes)
                    {
                        ...
                        if (dTC != null)
                        {
                            if (dTC.Id == ddlAgePicker.SelectedValue.AsIntegerOrNull())
                            {
                                groupListIds.Add(group.Id);
                            }
                        }
            ...
            // Category
            if (ddlCatPicker.SelectedValue != "")
            {
                var attribute = AttributeCache.Get(6595);
                ...
                foreach (var group in groupQry)
                {
                    ...
                    foreach (var attributeGuid in groupAttributes)
                    {
                        ...
                        if (dTC != null)
                        {
                            if (dTC.Id == ddlCatPicker.SelectedValue.AsIntegerOrNull())
                            {
                                groupListIds.Add(group.Id);
                            }
                        }
            ...
            {
                var attribute = AttributeCache.Get(7581);
			  ...
			  // Hour
            if (ddlHourPicker.SelectedValue != "")
            {
                var attribute = AttributeCache.Get(15567);
                ...
                foreach (var group in groupQry)
                {
                    ...
                    foreach (var attributeGuid in groupAttributes)
                    {
                        ...
                        if (dTC != null)
                        {
                            if (dTC.Id == ddlHourPicker.SelectedValue.AsIntegerOrNull())
                            {
                                groupListIds.Add(group.Id);
                            }
                        }
            ...
            if ( GetAttributeValue( "HideOvercapacityGroups" ).AsBoolean() )
            {
                groupQry = groupQry.Where(
                    ...
                    g.Members.Where( m => m.GroupMemberStatus == GroupMemberStatus.Active && m.IsArchived == false ).Count() < g.GroupCapacity );
...
";
        ...
        protected void ddlDayOfTheWeekPicker_SelectedIndexChanged(object sender, EventArgs e)
        {
            ShowResults();
        }

        protected void ddlAgePicker_SelectedIndexChanged(object sender, EventArgs e)
        {
            ShowResults();
        }

        protected void ddlChildcarePicker_SelectedIndexChanged(object sender, EventArgs e)
        {
            ShowResults();
        }
        ...
        protected void lbShowHideMap_Click(object sender, EventArgs e)
        {
