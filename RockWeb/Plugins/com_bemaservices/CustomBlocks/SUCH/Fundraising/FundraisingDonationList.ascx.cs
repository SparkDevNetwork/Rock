...
namespace RockWeb.Blocks.Fundraising
{
    ...
    [Category( "com_bemadev > Fundraising" )]
    ...
    public partial class FundraisingDonationList : RockBlock
    {
        ...
        protected void BindGrid( bool isExporting = false )
        {
            ...
            var donations = financialTransactionDetailService.Queryable()
                ...
                .Select( d => new
                {
                    ...
                    PaymentType = d.Transaction.FinancialPaymentDetail.CurrencyTypeValue
                } ).AsQueryable();
        ...
        protected void gDonations_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            if ( e.Row.RowType == DataControlRowType.DataRow )
            {
                ...
                mergeFields.AddOrReplace( "PaymentType", item.GetPropertyValue( "PaymentType" ) );

                //
                // Set the Donor column value.
                //
                var column = gDonations.ColumnsOfType<RockLiteralField>().First( c => c.HeaderText == "Payment Type" );
                string paymentString = "{{ PaymentType }}";
                if ( column.Visible )
                {
                    var literal = ( Literal ) e.Row.Cells[gDonations.Columns.IndexOf( column )].Controls[0];
                    var donorText = paymentString.ResolveMergeFields( mergeFields );
                    if ( isExporting )
                    {
                        donorText = donorText.ScrubHtmlAndConvertCrLfToBr();
                    }
                    literal.Text = item.GetPropertyValue( "PaymentType" ).ToString();
                }

                //
                // Set the Donor column value.
                //
                column = gDonations.ColumnsOfType<RockLiteralField>().First( c => c.HeaderText == "Donor" );
                if ( column.Visible )
                {
                    var literal = ( Literal ) e.Row.Cells[gDonations.Columns.IndexOf( column )].Controls[0];
