...
namespace com_bemaservices.Event
{
    ...
    [CustomRadioListField( "Type Filter Display Mode", "", "1^Hidden, 2^Plain, 3^Panel Open, 4^Panel Closed", true, "1", key: "TypeFilterDisplayMode", order: 4 )]
    ...
    [DefinedValueField( "7BC43C20-4D98-4105-8425-3BC2C883D0BC", "Filter Type", "Determines which types should be displayed in the filter.", false, true, key: "FilterType", order: 5 )]
    ...
    public partial class CalendarLava : Rock.Web.UI.RockBlock
    {
        ...
        protected bool TypePanelOpen { get; set; }
        protected bool TypePanelClosed { get; set; }
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            TypePanelOpen = !String.IsNullOrWhiteSpace( GetAttributeValue( "FilterCategories" ) ) && GetAttributeValue( "TypeFilterDisplayMode" ) == "3";
            TypePanelClosed = !String.IsNullOrWhiteSpace( GetAttributeValue( "FilterCategories" ) ) && GetAttributeValue( "TypeFilterDisplayMode" ) == "4";
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                if ( SetFilterControls() )
                {
                    ...
                    {
                        int? ctId = Request["ca"].AsIntegerOrNull();
                        ListItem test = cblCategory.Items.FindByValue(ctId.ToString());
                        if (test != null)
                            cblCategory.Items.FindByValue(ctId.ToString()).Selected = true;
                    }

                    if (Request["campus"] != null)
                    {
                        int? ctId = Request["campus"].AsIntegerOrNull();
                        ListItem test = cblCampus.Items.FindByValue(ctId.ToString());
                        if (test != null)
                            cblCampus.Items.FindByValue(ctId.ToString()).Selected = true;
                    }

                    if (Request["type"] != null)
                    {
                        int? ctId = Request["type"].AsIntegerOrNull();
                        ListItem test = cblType.Items.FindByValue(ctId.ToString());
                        if (test != null)
                            cblType.Items.FindByValue(ctId.ToString()).Selected = true;
                    }

                    if (CurrentPerson != null)
                    {
                        int? ctId = CurrentPerson.GetCampus().Id;
                        ListItem test = cblCampus.Items.FindByValue(ctId.ToString());
                        if (test != null)
                            cblCampus.Items.FindByValue(ctId.ToString()).Selected = true;
                    }
        ...
        private void BindData()
        {
            ...
            if ( campusIds.Any() )
            {
                qry = qry
                    .Where( c => 
                        c.CampusId == null ||    // All
                        campusIds.Contains( c.CampusId.Value )); // All church
            }
            ...
            if ( categories.Any() )
            {
              ...
              // Filter by Type
            List<int> types = cblType.Items.OfType<ListItem>().Where( l => l.Selected ).Select( a => a.Value.AsInteger() ).ToList();
            if ( types.Any() )
            {
                
                var attribute = AttributeCache.Get(5918); //Event Type

                List<int> eventsIds = new List<int>();
                List<string> eventAttributes = new List<string>();
                ...
                foreach (var eventItem in qry)
                {
                    ...
                    eventAttributes = eventItem.GetAttributeValues(attribute.Key);

                    foreach (var attributeGuid in eventAttributes)
                    {
        ...
        private bool SetFilterControls()
        {
            ...
            if ( definedType != null )
            {
             ...
             // Setup Type Filter
            var selectedTypeGuids = GetAttributeValue( "FilterType" ).SplitDelimitedValues( true ).AsGuidList();
            rcwType.Visible = selectedTypeGuids.Any() && GetAttributeValue( "TypeFilterDisplayMode" ).AsInteger() > 1;
            var definedTypeType = DefinedTypeCache.Read( Guid.Parse("7BC43C20-4D98-4105-8425-3BC2C883D0BC") ); // Event Type Guid
            if ( definedTypeType != null )
            {
                cblType.DataSource = definedTypeType.DefinedValues.Where( v => selectedTypeGuids.Contains( v.Guid ) );
            ...
            pnlList.CssClass = showFilter ? "col-md-8 col-md-offset-1" : "col-md-12";
