...
",
            ...
            if ( ValidateInfo() )
            {
                ...
                foreach ( var child in Children )
                {
                    ...
                    if ( pnlPlannedDate.Visible )
                    {
                        ...
                        {
                            person.SetAttributeValue( "FirstVisit", visitDate );
                        }
                    }

                    if ( ddlServiceTimes.SelectedValue != null )
                    {
                            person.SetAttributeValue( "ServiceTime", ddlServiceTimes.SelectedValue );
                    }
        ...
        private void SetControls()
        {
            ...
            dpPlannedDate.SelectedDate = RockDateTime.Now;
            ...
            {
                var defaultCampus = CampusCache.Get( Request.QueryString["CampusId"].AsIntegerOrNull() ?? 0 );
            ...
            dllGuest.Items.Clear();
            dllGuest.Items.Add(new ListItem("", ""));
			dllGuest.Items.Add(new ListItem("Yes", "1"));
            dllGuest.Items.Add(new ListItem("No", "0"));
            ...
            {
                ddlServiceTimes.Items.Add(new ListItem(time.Value, time.Value));
            }
            ...
            DynamicPlaceholder phAttributes )
        {
            ...
            if ( adult != null || ( tbFirstName.Text.IsNotNullOrWhiteSpace() && tbLastName.Text.IsNotNullOrWhiteSpace() ) )
            {
                ...
                _rockContext.SaveChanges();
                    ...
                    if (tbNewNote.Text != "")
                    {
                      // Save Note BEMA
                        ...
                        var service = new NoteService( _rockContext );
                ...
                if ( pnlPlannedDate.Visible )
                {
                    ...
                    if ( visitDate.HasValue )
                    {
                        adult.SetAttributeValue( "FirstVisit", visitDate );
                    }
                }

                if ( ddlServiceTimes.SelectedValue != null )
                {
                        adult.SetAttributeValue( "ServiceTime", ddlServiceTimes.SelectedValue );
                }
