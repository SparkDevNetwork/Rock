...
namespace RockWeb.Plugins.com_bemadev.Finance
{
        ...
        private List<KindridTransaction> ValidateTransactions( List<KindridTransaction> transactions, RockContext rockContext )
        {
            ...
            for ( var i = 0; i < transactions.Count; i++ )
            {
                ...
                //{
                    ...
                    //var personAttEnvelope = attributeValueService.Queryable().Where(a => a.AttributeId == 8558 && a.Value == test).FirstOrDefault();
                ...
                int accountId = _defaultAccountId;

                string transactionNonDeductible = transactions[i].TransactionType.Contains("Non-Deductible").ToTrueFalse();
                string accountName = transactions[i].Account;

                transactions[i].TransactionType = transactionNonDeductible;
                
                if (accountName != "")
                {
                    var accounts = new FinancialAccountService(new RockContext()).Queryable().Where(a => a.Name.Contains(accountName)).FirstOrDefault();

                    if (accounts == null)
                    {
                        accountName = accountName + " Non-Deductible";
                        accounts = new FinancialAccountService(new RockContext()).Queryable().Where(a => a.Name.Contains(accountName)).FirstOrDefault();
                    }
                    ...
                    if (accounts == null)
                    {
                        ...
                        accounts = new FinancialAccountService(new RockContext()).Queryable().Where(a => a.Id == _defaultAccountId).FirstOrDefault();
                        transactions[i].MatchedAccount = accounts.Name.ToString() + " (Default Account - No Account Matched)";
                    }
                    else
                    {
                        accountId = accounts.Id;
                        transactions[i].MatchedAccount = accounts.Name.ToString();
                    }
                }
                else
                {
                     accountId = _defaultAccountId;
                     var accounts = new FinancialAccountService(new RockContext()).Queryable().Where(a => a.Id == _defaultAccountId).FirstOrDefault();
                     transactions[i].MatchedAccount = accounts.Name.ToString() + " (Default Account - No Account Matched)";

                }
                        

                transactions[i].ImportedName = transactions[i].FirstName + " " + transactions[i].LastName;
                
                // Checking to see if person with same name exists
                Person person = null;
                PersonAlias personAlias = null;
               
                var personQuery = new PersonService.PersonMatchQuery(transactions[i].FirstName, transactions[i].LastName, transactions[i].Email, null, null, null, null);
        ...
        protected void btnImport_Click( object sender, EventArgs e )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( importableTransactions > 0 )
                {
                    ...
                    financialBatch.Name = string.Format( "ServiceU Import - {0}", DateTime.Now.ToString( "MM/dd/yyyy HH:mm" ) );
            ...
            using ( var rockContext = new RockContext() )
            {
                if ( !transaction.PreviouslyImported )
                {
                   ...
                   // string accountName = transaction.FundDescription;
                    ...
                    //{
                    //    accountName = transaction.FundDescription + " Non-Deductible";
                    //}
                   ...
                   // var accounts = new FinancialAccountService(new RockContext()).Queryable().Where(a => a.Name.Contains(accountName)).FirstOrDefault();
                    ...
                    var currency = new DefinedValueService(new RockContext()).Queryable().Where(a => a.DefinedTypeId == 10 && a.Value.Contains(transaction.Type)).FirstOrDefault();
                    ...
                    newTransaction.Summary = string.Format( "{0} {1}, {2}, Supporting: {3}",
    ...
    public sealed class KindridTransactionsMap : CsvClassMap<KindridTransaction>
    {
        public KindridTransactionsMap()
        {
            ...
            Map(m => m.PostDate).Name("Date Entered");
            Map(m => m.TransactionType).Name("Payment Method");
            Map(m => m.FirstName).Name("First Name");
            Map(m => m.LastName).Name("Last Name");
            Map(m => m.Email).Name("Email Address");
            Map(m => m.Supporting).Name("Supporting");
            Map(m => m.Account).Name("Account");
            Map(m => m.Amount).Name("Amount");
            //Map(m => m.SortField).Name("Sortfield");
            //Map(m => m.Envelope).Name("Envelope");
