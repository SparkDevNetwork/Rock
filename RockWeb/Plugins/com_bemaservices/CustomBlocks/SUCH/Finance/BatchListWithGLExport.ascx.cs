    ...
    $('#{0}').change(function( e ){{
        ...
        { 
            ...
            if ( batchesSelected.Any() )
            {
                ...
                record.EmployeeId = "GLENTRY_EMPLOYEEID";
                record.ClassId = "GLENTRY_CLASSID";
                record.ProjectId = "GLENTRY_PROJECTID";
        ...
        List<GLRecord> GLRecordsForBatch( FinancialBatch batch, DateTime date, string accountingPeriod, string journalType )
        {
            ...
            foreach ( var grp in accounts )
            {
                ...
                record.AccountNumber = "1004";
				record.Location = "E-1000";
				record.AmountCredit = string.Empty;
				
				if(grp.Sum( d => d.Amount ) > 0)
					record.AmountDebit = grp.Sum( d => d.Amount ).ToString("0.##");
				else
					record.AmountCredit = grp.Sum( d => d.Amount * -1 ).ToString("0.##");
                ...
                record.Memo = batch.Name + " (" + batch.Id.ToString() + ")";
                ...
                // Build each of the revenue fund withdrawls.
				 ...
				 var fundAccounts = grp.GroupBy(d => new { GLLocation = d.Account.GetAttributeValue("GlLocation"), GlAccount = d.Account.GetAttributeValue("GLAccount"), GlDepartment = d.Account.GetAttributeValue("GlDepartment"), GlFund = d.Account.GetAttributeValue("GlFund"), RockAccount = d.Account.Name }, d => d).OrderBy(g => g.Key.GLLocation);
                ...
                foreach (var grpTransactions in fundAccounts)
                {
                    record = new GLRecord();
						...
						if (grpTransactions.First().Account.Campus != null)
                        {
                            var campusShortCode = grpTransactions.First().Account.Campus.ShortCode;
                            if ( campusShortCode == "GRC")
                            {
							    record.Location = "Garner";
                            }
                            else if ( campusShortCode == "CPC")
                            {
                                 record.Location = "Capital Hills";
                            }
                            else 
                            {
                                 record.Location = campusShortCode;
                            }
                        }
						
						if (!string.IsNullOrWhiteSpace(grpTransactions.Key.GlAccount))
							record.AccountNumber = grpTransactions.Key.GlAccount;
						
						if (!string.IsNullOrWhiteSpace(grpTransactions.First().Account.GlCode))
							record.Department = grpTransactions.First().Account.GlCode;
						
						if (!string.IsNullOrWhiteSpace(grpTransactions.First().Account.GetAttributeValue("GlAccount")))
							record.AccountNumber = grpTransactions.First().Account.GetAttributeValue("GlAccount");
						...
						{
							record.AmountCredit = string.Empty;
							record.AmountDebit = grpTransactions.Sum(d => d.Amount * -1).ToString("0.##");
						}
						
						record.Company = string.Empty;
						
						if (!string.IsNullOrWhiteSpace(grpTransactions.First().Account.GetAttributeValue("PersonFinancialAccount")))
							record.EmployeeId = grpTransactions.First().Account.GetAttributeValue("PersonFinancialAccount");
						
						if (!string.IsNullOrWhiteSpace(grpTransactions.First().Account.GetAttributeValue("GlProjectID")))
							record.ProjectId = grpTransactions.First().Account.GetAttributeValue("GlProjectID");
