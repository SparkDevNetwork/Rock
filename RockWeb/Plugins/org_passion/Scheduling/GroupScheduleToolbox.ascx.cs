...
namespace RockWeb.Blocks.GroupScheduling
{
    ...
    /// </summary>
    [DisplayName( "Passion | My Serving Schedule" )]
...
",
        ...
        private void BindTabs()
        {
            ...
            {
                tabs = tabs.Where( a => a != GroupScheduleToolboxTab.SignUp ).Where(a => a != GroupScheduleToolboxTab.Preferences).ToList();
            }
        ...
        protected string GetOccurrenceDetails( Attendance attendance )
        {
            ...
            {
                return "<b>" + attendance.Occurrence.OccurrenceDate.ToShortDateString() + "</b>";
            }

            if (attendance.Occurrence.GroupId == null)
            {
                return string.Format("{0} - {1}", attendance.Occurrence.OccurrenceDate.ToShortDateString(), attendance.Occurrence.Location.Name);
            }
            ...
            startTime = (DateTime)attendance.Occurrence.Schedule.FirstStartDateTime;
            var schStartTime = startTime.ToString("hh:mm tt");

            //return "Testing";
            return string.Format("<b>{0}</b> | <span class='alert-info' style='padding: 5px;'>{1}</span> for the <span class='alert-info' style='padding: 5px;'>{2}</span> Team at <b>{3}</b>", attendance.Occurrence.OccurrenceDate.ToShortDateString(), attendance.Occurrence.Group.Name, schStartTime, attendance.Occurrence.Location.Name);
        }
        ...
        /// Binds the Pending Confirmations grid.
        /// </summary>
        private void BindDeclinedConfirmationsGrid()
        {
            ...
            pnlDeclinedSchedules.Visible = false;

            using (var rockContext = new RockContext())
            {
                var declinedConfirmations = passionAttendanceService.GetDeclinedScheduled(new AttendanceService(rockContext))
                    .Where(a => a.PersonAlias.PersonId == this.SelectedPersonId)
                    .Where(a => a.Occurrence.OccurrenceDate >= currentDateTime)
                    .OrderBy(a => a.Occurrence.OccurrenceDate >= currentDateTime)
                    .ToList();

                if (declinedConfirmations.Any())
                {
                    pnlDeclinedSchedules.Visible = true;
                    rptDeclined.DataSource = declinedConfirmations;
                    rptDeclined.DataBind();
                }
            }
        }
        ...
        protected void rptDeclined_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            var lDeclinedOccurrenceDetails = e.Item.FindControl("lDeclinedOccurrenceDetails") as Literal;
            var lDeclinedOccurrenceTime = e.Item.FindControl("lDeclinedOccurrenceTime") as Literal;
            var btnConfirmAttending = e.Item.FindControl("btnConfirmAttending") as LinkButton;
            var btnDeclineAttending = e.Item.FindControl("btnDeclineAttending") as LinkButton;
            var attendance = e.Item.DataItem as Attendance;

            lDeclinedOccurrenceDetails.Text = GetOccurrenceDetails( attendance );
            lDeclinedOccurrenceTime.Text = GetOccurrenceScheduleName(attendance);
            btnConfirmAttending.CommandName = "AttendanceId";
            btnConfirmAttending.CommandArgument = attendance.Id.ToString();
