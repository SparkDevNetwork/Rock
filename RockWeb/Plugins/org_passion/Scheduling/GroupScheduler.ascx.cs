...
namespace RockWeb.Blocks.GroupScheduling
{
    ...
    ///
    /// </summary>
    [DisplayName( "Passion Group Scheduler" )]
...
btnCopyToClipboard.ClientID );
        ...
        {
            bgResourceListSource.BindToEnum<GroupSchedulerResourceListSourceType>();
        ...
        /// <summary>
        /// Updates the group name in the header of the block
        /// </summary>
        private void UpdateGroupNameHeader(Literal lbGroupHeader)
        {
            ...
            if (group != null)
            {
                ...
                {
                    lbGroupHeader.Text = group.ParentGroup.Name + " - " + group.Name;
                } else
                {
                    lbGroupHeader.Text = group.Name;
                }
                
            } else
            {
                lbGroupHeader.Text = null;
            }
        ...
        private void LoadFilterFromUserPreferencesOrURL()
        {
            ...
            var resouceListSourceType = this.GetUrlSettingOrBlockUserPreference( UserPreferenceKey.SelectedResourceListSourceType ).ConvertToEnumOrNull<GroupSchedulerResourceListSourceType>() ?? GroupSchedulerResourceListSourceType.AlternateGroup;
        ...
        private void ApplyFilter()
        {
            ...
            if ( group != null && group.SchedulingMustMeetRequirements )
            {
                ...
                bgResourceListSource.SetValue( ( int ) GroupSchedulerResourceListSourceType.GroupMembers );
            ...
            var resourceListSourceType = bgResourceListSource.SelectedValueAsEnumOrNull<GroupSchedulerResourceListSourceType>();
            ...
            pnlResourceFilterGroup.Visible = resourceListSourceType == GroupSchedulerResourceListSourceType.GroupMembers;
        ...
        private void InitResourceList()
        {
            ...
            var resourceListSourceType = bgResourceListSource.SelectedValueAsEnum<GroupSchedulerResourceListSourceType>();
            ...
            if ( resourceListSourceType == GroupSchedulerResourceListSourceType.GroupMembers )
            {
        ...
        protected void rptAttendanceOccurrences_ItemDataBound( object sender, RepeaterItemEventArgs e )
        {
            ...
            var lGroupTitle = e.Item.FindControl("lbGroupHeader2") as Literal;
            ...
            UpdateGroupNameHeader(lGroupTitle);
        ...
        protected void btnAutoSchedule_Click( object sender, EventArgs e )
        {
            ...
            // create Passion version for more flexible Auto Schedule option
            var passionAttendanceService = new com.passioncitychurch.Model.AttendanceService();

            // create boolean for whether the resource Type is coming from a data view
            var resourceListSourceType = bgResourceListSource.SelectedValueAsEnumOrNull<GroupSchedulerResourceListSourceType>();
            bool isDataView = resourceListSourceType == GroupSchedulerResourceListSourceType.DataView;

            // create holder for the id to pass to the Passion Auto Schedule service
            int resourceId = -1;

            // if the resource grouping is from a DataView, update the resourceId from that source
            if (isDataView)
            {
                resourceId = dvpResourceListDataView.SelectedValue.AsInteger();
            }
            else
            {
                resourceId = int.Parse(hfResourceGroupId.Value);
            }

            // AutoSchedule the occurrences that are shown
            passionAttendanceService.SchedulePersonsAutomaticallyForAttendanceOccurrences( attendanceOccurrenceIdList, resourceId, isDataView, this.CurrentPersonAlias, attendanceService );
            rockContext.SaveChanges();
        }
