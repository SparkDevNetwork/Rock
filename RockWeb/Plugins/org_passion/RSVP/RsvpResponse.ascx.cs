...
namespace RockWeb.Blocks.RSVP
{
    ...
    /// </summary>
    [DisplayName( "Passion | RSVP Response" )]
    [Category( "RSVP" )]
...
";
        ...
        private void ShowSingleOccurrence_Choice( int occurrenceId, Person person )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                else if ( occurrence.OccurrenceDate.EndOfDay() < DateTime.Now || acceptedRSVPs >= group.GroupCapacity )
                {
        ...
        private void ShowSingleOccurrence_Accept( int occurrenceId, Person person )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                /* The below currently isn't ideal, pausing to ensure this is our best route before moving forward */
                mergeFields.Add("OccurrenceStartDate", occurrence.OccurrenceDate.ToShortDateString());
                mergeFields.Add("OccurrenceStartTime", attendance.StartDateTime.ToShortTimeString());
                ...
                var emailTemplate = occurrence.Group.GetAttributeValue("EmailTemplate");
                emailTemplate = emailTemplate.ResolveMergeFields(mergeFields);

                if (!string.IsNullOrEmpty(emailTemplate))
                {
                    var message = new RockEmailMessage();
                    message.AddRecipient(new RockEmailMessageRecipient(person, mergeFields));
                    message.Message = emailTemplate;
                    message.FromEmail = "connect@passioncitychurch.com";
                    message.FromName = "Passion City Church";
                    message.Subject = occurrence.Group.GetAttributeValue("EmailSubjectLine");
        ...
        private void ShowMultipleOccurrence_Choice( List<int> occurrenceIds, Person person )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                foreach ( int occurrenceId in occurrenceIds )
                {
                    ...
                    var activeMembers = group.ActiveMembers().Count();
                    var acceptedRSVPs = occurrence.Attendees.Where(a => a.RSVP == Rock.Model.RSVP.Yes).Count();
                    if ( occurrence.OccurrenceDate.EndOfDay() < DateTime.Now || acceptedRSVPs >= group.GroupCapacity)
                    {
                        // This event has expired.
                        isExpired = true;
                        continue;
        ...
        private void UpdatePersonRecord()
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                CurrentPerson.UpdatePhoneNumber(12, "US", pnbPhone.Number, true, false, rockContext);
                rockContext.SaveChanges();
            }
        }
        ...
        private void SetButtonProperties()
        {
            ...
            {
                acceptButtonStyle = "background-color: " + acceptButtonColor + "; margin-bottom: 15px;";
            }
            ...
            {
                declineButtonStyle = "background-color: " + declineButtonColor + "; margin-bottom: 15px;";
            }
