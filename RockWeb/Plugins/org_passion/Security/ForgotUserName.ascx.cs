...
namespace RockWeb.Blocks.Security
{
    ...
    /// </summary>
    [DisplayName( "Passion | Forgot Password" )]
    [Category( "Security" )]
    ...
    public partial class ForgotUserName : Rock.Web.UI.RockBlock
    {
        ...
        protected void btnSend_Click( object sender, EventArgs e )
        {
            ...
            //foreach ( Person person in personService.GetByUserLoginId( tbEmail.Text )
            //    .Where( p => p.Users.Any()))
            //{
            
            //foreach ( UserLogin user in userLoginService.GetByUserName( tbEmail.Text ) )
            //{
            //    if ( user.EntityType != null && user.UserName == tbEmail.Text )
            //    {
            ...
            var user = userLoginService.GetByUserName(tbEmail.Text);
            var users = new List<UserLogin>();
            users.Add( user );
            var resultsDictionary = new Dictionary<string, object>();
            resultsDictionary.Add("Person", user.Person);
            resultsDictionary.Add("Users", users);
            results.Add(resultsDictionary);
