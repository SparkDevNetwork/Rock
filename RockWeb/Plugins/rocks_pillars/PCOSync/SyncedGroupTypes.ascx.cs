...
namespace RockWeb.Plugins.rocks_pillars.PCOSync
{
    ...
    public partial class SyncedGroupTypes : RockBlock, ISecondaryBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            {
                _pcoAccountFieldType = FieldTypeCache.Get( rocks.pillars.PCOSync.SystemGuid.FieldType.PCO_ACCOUNT.AsGuid(), rockContext );
                _groupEntityType = EntityTypeCache.Get<Rock.Model.Group>( false, rockContext );
            }
        ...
        private void MdlgGroupType_SaveClick( object sender, EventArgs e )
        {
            ...
            if ( !string.IsNullOrWhiteSpace( qualifierValue ) && _pcoAccountFieldType != null && _groupEntityType != null )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    var definedValueFieldType = FieldTypeCache.Get( Rock.SystemGuid.FieldType.DEFINED_VALUE.AsGuid(), rockContext );
            ...
            if ( _pcoAccountFieldType != null && _groupEntityType != null )
            {
                using ( var rockContext = new RockContext() )
                {
                        ...
                        .ToList() )
                    {
                        ...
                        rockContext.SaveChanges();
                        AttributeCache.Remove( attributeId );
                    }
