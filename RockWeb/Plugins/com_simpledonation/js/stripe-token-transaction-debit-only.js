(function () {
  ...
  var selectors = {
    cardNumber: '[id$="txtCreditCard"]',
    cvv: '[id$="txtCVV"]',
    expMonth: '[id$="monthDropDownList"]',
    expYear: '[id*="yearDropDownList"]',
    routing: '[id$="txtRoutingNumber"]',
    bankAccount: '[id$="txtAccountNumber"]',
    giveAs: '[id$="tglGiveAsOption_hfChecked"]',
    firstName: '[id$="txtCardFirstName"]',
    lastName: '[id$="txtCardLastName"]',
    zipCode: '[id$="tbPostalCode"]',
    nextButton: '[id$="btnPaymentInfoNext"]',
  ...
  function handleStripeResponse(status, response) {
    ...
    if (response.card && response.card.funding == "debit") {
      var error = {message: "We only accept debit cards (not credit cards)"}
