    ...
    public partial class NextStepsGroupMemberDetail : RockBlock, IDetailBlock
    {
        ...
        private void SaveGroupMember()
        {
            if ( Page.IsValid )
            {
                ...
                {
                    case church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_UNABLE_TO_REACH:
                    {
                        StartWorkflow( "OptOutUnableToReachWorkflow", groupMember.Person, rockContext );
                        break;
                    }
                    case church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_DO_NOT_CONTACT:
                    {
                        StartWorkflow( "OptOutDoNotContactWorkflow", groupMember.Person, rockContext );
                        break;
                    }
                    case church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_REASSIGN_TO_NEW_COACH:
                    {
                        HandleOptOut_Reassign( personService, groupMember, rockContext );
                        break;
                    }

                    case church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_REGISTERED_FOR_NEIGHBORHOOD_GROUP:
                    {
                        StartWorkflow( "OptOutNeighborhoodWorkflow", groupMember.Person, rockContext );
                        break;
                    }

                    case church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_NO_LONGER_ATTENDING_CCV:
                    {
        ...
        private void SetControlVisibilities( )
        {
            ...
            dpFollowUpDate.Visible = ddlOptOutReason.SelectedValue == church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_FOLLOW_UP_LATER;
            ...
            tbReassignReason.Visible = ddlOptOutReason.SelectedValue == church.ccv.Utility.SystemGuids.DefinedValue.NEXT_STEPS_REASSIGN_TO_NEW_COACH;
