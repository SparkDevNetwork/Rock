...
namespace RockWeb.Plugins.church_ccv.Security
{
    ...
    public partial class CCVAccountEntry : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        private void DisplayDuplicates( Direction direction )
        {
            ...
            if ( Convert.ToBoolean( GetAttributeValue( "Duplicates" ) ) )
            {
                ...
                if ( matches.Count > 0 )
                {
                    // first populate the standard desktop sized grid
                    ...
                    // now also build a mobile duplicates "grid" that formats nicely on narrow devices
                    lMobileDuplicates.Text = string.Empty;
                    foreach( var match in matches )
                    {
                        lMobileDuplicates.Text +=
                        "<div class=\"mobile-duplicates-item\">" + 
