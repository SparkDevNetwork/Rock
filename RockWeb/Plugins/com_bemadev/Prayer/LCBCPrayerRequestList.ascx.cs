...
namespace RockWeb.Plugins.com_bemadev.Prayer
{
    [DisplayName( "LCBC Prayer Request List" )]
    [Category( "com_bemadev > Prayer" )]
    ...
    [CampusesField( "Campuses", "", false, "", "" )]
    [ContextAware( typeof( Rock.Model.Person ) )]
    public partial class LCBCPrayerRequestList : RockBlock, ICustomGridColumns
    {
        #region Fields
        ...
        private void BindGrid()
        {
            ...
            // Filter by block setting campus
            var campusGuidList = GetAttributeValue( "Campuses" ).Split( ',' ).AsGuidList();
            var campusIdList = CampusCache.All().Where( c => campusGuidList.Contains( c.Guid ) ).Select( c => c.Id ).ToList();
            if ( campusIdList.Any() )
            {
                prayerRequests = prayerRequests.Where( c => c.CampusId != null && campusIdList.Contains( c.CampusId.Value ) );
            }
