...
{
    [DisplayName( "Binary File Link List" )]
    [Category( "com_centralaz > Finance" )]
    [Description( "Shows a list of all binary files, with links." )]

    [LinkedPage( "Detail Page" )]
    [BinaryFileTypeField]
    public partial class BinaryFileLinkList : RockBlock
    {
        ...
        private void BindGrid()
        {
            ...
            var linkedQueryable = queryable.Select( q => new
            {
                Id = q.Id,
                FileName = q.FileName,
                FileLink = "<a href='GetFile.ashx?id=" + q.Id + "'>" + q.FileName + "</a>",
                MimeType = q.MimeType,
                ModifiedDateTime = q.ModifiedDateTime,
                IsSystem = q.IsSystem
            } );

            if ( sortProperty != null )
            {
                gBinaryFile.DataSource = linkedQueryable.Sort( sortProperty ).ToList();
            }
            else
            {
                gBinaryFile.DataSource = linkedQueryable.OrderBy( d => d.FileName ).ToList();
            }
