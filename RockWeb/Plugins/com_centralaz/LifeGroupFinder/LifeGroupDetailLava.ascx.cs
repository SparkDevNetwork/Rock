...
namespace RockWeb.Plugins.com_centralaz.LifeGroupFinder
{
    ...
    /// </summary>
    [DisplayName( "Life Group Detail Lava" )]
    ...
    public partial class LifeGroupDetailLava : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        protected void btnMoveGroupMember_Click( object sender, EventArgs e )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( groupMember != null )
                {
                    ...
                    var trigger = group.GetGroupMemberWorkflowTriggers().FirstOrDefault( a => a.Id == hfPlaceElsewhereTriggerId.Value.AsInteger() );
                    if ( trigger != null )
                    {
                ...
                pnlMoveGroupMember.Visible = false;
                pnlGroupView.Visible = true;
                DisplayViewGroup();
            }
        }

        protected void btnCancelMoveGroupMember_Click( object sender, EventArgs e )
        {
            pnlMoveGroupMember.Visible = false;
            pnlGroupView.Visible = true;
            ...
            if ( Request.Form["__EVENTARGUMENT"] != null )
            {
                ...
                if ( eventArgs.Length == 2 )
                {
                    ...
                    switch ( action )
                    {
                        ...
                        case "MoveGroupMember":
                            groupMemberId = int.Parse( parameters );
                            DisplayMoveGroupMember( groupMemberId );
                            sm.AddHistoryPoint( "Action", "MoveMember" );
                            break;
        ...
        private void DisplayMoveGroupMember( int groupMemberId )
        {
            var rockContext = new RockContext();

            var groupMemberPerson = new GroupMemberService( rockContext ).GetPerson( groupMemberId );
            if ( groupMemberPerson != null )
            {
                hfPlaceElsewhereGroupMemberId.Value = groupMemberId.ToString();
                lPlaceElsewhereGroupMemberName.Text = groupMemberPerson.ToString();
                BindPlaceElsewhereTriggerButtons( true );

                pnlGroupEdit.Visible = false;
                pnlGroupView.Visible = false;
                pnlMoveGroupMember.Visible = true;
            }
        }
        ...
        {
            Group group = new GroupService( new RockContext() ).Get( _groupId );
            var sortedTriggerList = group.GetGroupMemberWorkflowTriggers().Where( a => a.TriggerType == GroupMemberWorkflowTriggerType.MemberPlacedElsewhere ).ToList();
