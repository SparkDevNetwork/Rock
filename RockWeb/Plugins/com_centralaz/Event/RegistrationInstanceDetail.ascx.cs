...
{
    /// <summary>
    /// Handles building a list of registrants for the given registration instance or registration Template Id
    /// </summary>
...
";
        ...
        {
            int? instanceId = hfRegistrationInstanceId.Value.AsIntegerOrNull();
            int? templateId = PageParameter( "RegistrationTemplateId" ).AsIntegerOrNull();

            if ( instanceId.HasValue || templateId.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    IQueryable<RegistrationRegistrant> qry;

                    if ( templateId.HasValue )
                    {
                        qry = new RegistrationRegistrantService( rockContext )
                        .Queryable( "PersonAlias.Person.PhoneNumbers.NumberTypeValue,Fees.RegistrationTemplateFee,GroupMember.Group" ).AsNoTracking()
                        .Where( r =>
                            r.Registration.RegistrationInstance.RegistrationTemplateId == templateId.Value &&
                            r.PersonAlias != null &&
                            r.PersonAlias.Person != null &&
                            r.OnWaitList == false );
                    }
