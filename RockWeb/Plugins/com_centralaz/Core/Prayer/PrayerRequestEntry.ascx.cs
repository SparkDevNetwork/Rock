...
namespace RockWeb.Plugins.com_centralaz.Core.Prayer
{
    ...
    [CodeEditorField( "Save Success Text", "Text to display upon successful save. (Only applies if not navigating to parent page on save.) <span class='tip tip-lava'></span><span class='tip tip-html'></span>", CodeEditorMode.Html, CodeEditorTheme.Rock, 200, false, "<p>Thank you for allowing us to pray for you.</p>", "On Save Behavior", 11 )]
    [WorkflowTypeField( "Workflow", "An optional workflow to start when prayer request is created. The PrayerRequest will be set as the workflow 'Entity' when processing is started.", false, false, "", "On Save Behavior", 12 )]
    [BooleanField( "Enable Debug", "Outputs the object graph to help create your liquid syntax.", false, "On Save Behavior", 13 )]
            ...
            if ( workflowTypeGuid.HasValue )
            {
                ...
                if ( workflowType != null )
                {
                    try
                    {
                        ...
                        workflow.SetAttributeValue( "PrayerRequest", prayerRequest.Guid.ToString() );
