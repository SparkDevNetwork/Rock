...
{
    [DisplayName( "Passion Family Registration" )]
    [Category( "Passion" )]
    [Description( "Allows families to pre-register for Bloom & Passion Kids for a quick check-in experience." )]
...
ORDER BY [Text]", false, "", "Child Relationship", 2, "CanCheckinRelationships" )]
    ...
    public partial class PassionChildPreRegistration : RockBlock
    {
        ...
        private const string CHILD_TYPE_KEY = "ChildType";
        ...
        /// The group members.
        /// </value>
        protected List<PassionPreRegistrationChild> Children { get; set; }
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            Children = ViewState["Children"] as List<PassionPreRegistrationChild> ?? new List<PassionPreRegistrationChild>();
...
",
        ...
        {
            var row = sender as PassionChildRow;
        ...
        private void SetCurrentFamilyValues()
        {
            ...
            if ( family != null )
            {
                ...
                {
                    var child = new PassionPreRegistrationChild( groupMember.Person );
        ...
        private void AddChild()
        {
            ...
            var child = new PassionPreRegistrationChild( person );
        ...
        private void CreateChildrenControls( bool setSelection )
        {
            ...
            foreach ( var child in Children )
            {
                if ( child != null )
                {
                    var childRow = new PassionChildRow();
        ...
        private void GetChildrenData()
        {
            ...
            {
                var passionRow = (PassionChildRow)childRow;
                var person = new Person();
                person.Id = passionRow.PersonId;
                person.Guid = passionRow.PersonGuid ?? Guid.NewGuid();
                person.NickName = passionRow.NickName;
                person.LastName = passionRow.LastName;
                person.SuffixValueId = passionRow.SuffixValueId;
                person.Gender = passionRow.Gender;
                person.SetBirthDate( passionRow.BirthDate );
                person.GradeOffset = passionRow.GradeOffset;

                person.LoadAttributes();

                var child = new PassionPreRegistrationChild( person );

                child.MobilePhoneNumber = passionRow.MobilePhone;
                child.MobileCountryCode = passionRow.MobilePhoneCountryCode;
                child.EmailAddress = passionRow.EmailAddress;

                child.RelationshipType = passionRow.RelationshipType;
