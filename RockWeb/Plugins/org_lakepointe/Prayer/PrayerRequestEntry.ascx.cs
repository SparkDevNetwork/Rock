...
namespace RockWeb.Plugins.org_lakepointe.Prayer
{
    [DisplayName( "LPC > Prayer Request Entry" )]
    [Category( "Prayer" )]
    ...
    [CampusesField(
        name: "Available Campuses",
        description: "Restrict which campuses are available in the campus selector.",
        required: false,
        category: "Features",
        key: "AvailableCampuses",
        includeInactive: false,  // must be specified to resolve ambiguity in the constructors
        order: 12)]
    ...
    [DefinedValueField(Rock.SystemGuid.DefinedType.PERSON_CONNECTION_STATUS, "Default Connection Status", "The connection status that is used for new people who are added through the Prayer Request Entry block", true, false, "76b06690-3109-44e1-b415-1dc82a84dc0a","On Save Behavior", 18 )]
    ...
    public partial class PrayerRequestEntry : RockBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            var selectedCampuses = GetAttributeValue("AvailableCampuses").Split(',').AsGuidList();
            cpCampus.Campuses = CampusCache.All(false).Where(c => selectedCampuses.Contains(c.Guid)).OrderBy(c => c.Order).ToList();
            ...
            if ( charLimit > 0 )
            {
                dtbRequest.Placeholder = string.Format("Por favor oren por... (hasta {0} caracteres)", charLimit );
                string scriptFormat = @"
