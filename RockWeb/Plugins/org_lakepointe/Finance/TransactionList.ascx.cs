...
namespace RockWeb.Plugins.org_lakepointe.Finance
{
    ...
    [BooleanField(
    	name: "Enable Mark as Exported", 
    	description: "Allows the user to select transactions and mark them as exported.", 
        defaultValue: false, 
    	order: 12,
    	key: AttributeKey.EnableMarkAsExported )]
    public partial class TransactionList : Rock.Web.UI.RockBlock, ISecondaryBlock, IPostBackEventHandler, ICustomGridColumns
    {
        ...
        private static class AttributeKey
        {
            ...
            public const string EnableMarkAsExported = "EnableMarkAsExported";
        }
        ...
        private string _transactionExportedAtttributeGuid = "A7588A9B-5B1E-4210-AB80-D90AB5620F57";

        private RockDropDownList _ddlMove = new RockDropDownList();
        private LinkButton _lbReassign = new LinkButton();
        private LinkButton _lbExport = new LinkButton();
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            if ( _canEdit )
            {
                ...
                _lbExport.ID = "lbMarkExported";
                _lbExport.CssClass = "btn btn-default btn-sm pull-left";
                _lbExport.Click += _lbExport_Click;
                _lbExport.Text = "Mark as Exported";
                gTransactions.Actions.AddCustomActionControl( _lbExport );
            }
...
", _ddlMove.ClientID, gTransactions.ClientID, Page.ClientScript.GetPostBackEventReference( this, "MoveTransactions" ), hfMoveToBatchId.ClientID);
        ...
        private void SetupGridActionControls()
        {
            ...
            var exportAttribute = AttributeCache.Get( _transactionExportedAtttributeGuid.AsGuid() );
            var enableBulkExport = GetAttributeValue( AttributeKey.EnableMarkAsExported ).AsBoolean();

            if (_canEdit && exportAttribute != null && enableBulkExport)
            {
                showSelectColumn = true;
                _lbExport.Visible = true;
            }
            else
            {
                _lbExport.Visible = false;
            }
        ...
        private void _lbExport_Click( object sender, EventArgs e )
        {
            var exportAttribute = AttributeCache.Get( _transactionExportedAtttributeGuid.AsGuid() );
            if (!_canEdit || exportAttribute == null)
            {
                return;
            }

            var transSelected = new List<int>();
            gTransactions.SelectedKeys.ToList().ForEach( b => transSelected.Add( b.ToString().AsInteger() ) );

            if (transSelected.Any())
            {
                lExportConfirmMessage.Text = string.Format( "This action will mark {0} {1} as exported.",
                    transSelected.Count(), "Transaction".PluralizeIf( transSelected.Count() != 1 ) );
                ShowDialog( "Export" );
            }
            else
            {
        ...
        protected void dlgExport_SaveClick( object sender, EventArgs e )
        {
            var rockContext = new RockContext();
            var exportedAttribute = AttributeCache.Get( _transactionExportedAtttributeGuid.AsGuid(), rockContext );
            var enableExport = GetAttributeValue( AttributeKey.EnableMarkAsExported ).AsBoolean();

            if (_canEdit && enableExport && exportedAttribute != null)
            {
                var transSelected = new List<int>();
                gTransactions.SelectedKeys.ToList().ForEach( t => transSelected.Add( t.ToString().AsInteger() ) );

                var transactions = new FinancialTransactionService( rockContext ).Queryable()
                    .Where( t => transSelected.Contains( t.Id ) )
                    .ToList();
                int updatedTx = 0;
                int previouslyUpdatedTx = 0;
                
                foreach (var transaction in transactions)
                {
                    transaction.LoadAttributes( rockContext );
                    var exported = transaction.GetAttributeValue( exportedAttribute.Key ).AsBoolean();

                    if (!exported)
                    {
                        transaction.SetAttributeValue( exportedAttribute.Key, bool.TrueString );
                        transaction.SaveAttributeValue( exportedAttribute.Key, rockContext );
                        updatedTx++;
                    }
                    else
                    {
                ...
                sb.AppendFormat( "{0} {1} have been marked as exported. ", updatedTx, "transaction".PluralizeIf( updatedTx != 1 ) );
                if (previouslyUpdatedTx > 0)
                {
                    sb.AppendFormat( "\n {0} {1} have previously been marked as exported.", previouslyUpdatedTx, "transaction".PluralizeIf( previouslyUpdatedTx != 1 ) );
                }
                nbResult.Title = "Transactions Exported";
                nbResult.Text = sb.ToString();
                nbResult.NotificationBoxType = NotificationBoxType.Success;
                nbResult.Visible = true;
            }
            gTransactions.SelectedKeys.Clear();
            HideDialog();
