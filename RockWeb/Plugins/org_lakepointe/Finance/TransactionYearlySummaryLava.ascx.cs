...
namespace RockWeb.Plugins.org_lakepointe.Finance
{
    ...
    [BooleanField( "Show Only Tax Deductible Transactions", "Only show transactons that are assoicated with Tax Deductable Accounts.", false, "", 6, "TaxDeductableOnly" )]
    public partial class TransactionYearlySummaryLava : RockBlock, ISecondaryBlock
    {
        ...
        private void BindGrid()
        {
            ...
            if(transactionTypeValueIdList.Count == 0)
            {
                transactionTypeValueIdList.Add( DefinedValueCache.Get( Rock.SystemGuid.DefinedValue.TRANSACTION_TYPE_CONTRIBUTION.AsGuid() ).Id );
            }
            ...
            if ( transactionTypeValueIdList.Count > 0  )
            {
                ...
                var qry = transactionDetailService.Queryable().AsNoTracking()
                    .Where( a =>
                        transactionTypeValueIdList.Contains( a.Transaction.TransactionTypeValueId ) && 
                ...
                if ( GetAttributeValue( "TaxDeductableOnly" ).AsBoolean( false ) )
                {
                    qry = qry.Where( t => t.Transaction.TransactionDetails.Any( d => d.Account.IsTaxDeductible ) );
                }
