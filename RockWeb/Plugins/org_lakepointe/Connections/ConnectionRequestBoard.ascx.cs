...
namespace RockWeb.Plugins.org_lakepointe.Connections
{
    ...
    /// </summary>
    [DisplayName("Weddings Connection Request Board")]
...
";
        ...
        #region LPC Custom Methods

        private void renderWeddingAttributeValues(Rock.Attribute.IHasAttributes connectionRequest)
        {
            connectionRequest.LoadAttributes();

            // Check for required attributes, show panels accordingly
            string bridePersonAliasIdString = "";
            string groomPersonAliasIdString = "";
            string backgroundApplicationURL = "";
            string sendLinkRouterURL = "";

            try
            {
                bridePersonAliasIdString = connectionRequest.AttributeValues.GetValueOrNull("BrideAliasId").Value;
                groomPersonAliasIdString = connectionRequest.AttributeValues.GetValueOrNull("GroomAliasId").Value;
                backgroundApplicationURL = connectionRequest.AttributeValues.GetValueOrNull("BackgroundApplicationURL").Value;
                sendLinkRouterURL = connectionRequest.AttributeValues.GetValueOrNull("SendFormRouterURL").Value;
            }
            catch (Exception)
            {
                // Everything should be set to empty string, continue with validation
            }

            // Have to set this to visible or first load doesn't work
            mdRequest.Visible = true;

            if (bridePersonAliasIdString.IsNullOrWhiteSpace() || groomPersonAliasIdString.IsNullOrWhiteSpace()
                || backgroundApplicationURL.IsNullOrWhiteSpace() || sendLinkRouterURL.IsNullOrWhiteSpace())
            {
                // Make visible standard panels
                pnlRegularContactInfo.Visible = true;
                lRequestModalViewModePersonFullName.Visible = true;
                aRequestModalViewModeProfileLink.Visible = true;
                pnlStandardConnector.Visible = true;
                pnlStandardAttributes.Visible = true;
                pnlStandardAttributeEdit.Visible = true;
                lRequestModalViewModeBadgeBar.Visible = true;
                // Hide custom panels / Attributes
                pnlWeddingAttributes.Visible = false;
                pnlCustomWeddingContactInfo.Visible = false;
                lWeddingTitle.Visible = false;
                pnlCustomConnector.Visible = false;
                return;
            }
            else
            {
                // Hide standard panels
                lRequestModalViewModePersonFullName.Visible = false;
                pnlRegularContactInfo.Visible = false;
                aRequestModalViewModeProfileLink.Visible = false;
                pnlStandardConnector.Visible = false;
                pnlStandardAttributes.Visible = false;
                pnlStandardAttributeEdit.Visible = false;
                lRequestModalViewModeBadgeBar.Visible = false;
                // Show custom panels / Attributes
                pnlCustomWeddingContactInfo.Visible = true;
                pnlWeddingAttributes.Visible = true;
                lWeddingTitle.Visible = true;
                pnlCustomConnector.Visible = true;

            }

            // Services and GUIDs required
            RockContext rc = new RockContext();
            PersonAliasService personAliasService = new PersonAliasService(rc);
            PersonService personService = new PersonService(rc);
            CampusService cs = new CampusService(rc);
            Guid mobilePhoneGuid = Rock.SystemGuid.DefinedValue.PERSON_PHONE_TYPE_MOBILE.AsGuid();
            Dictionary<string, string> wa = new Dictionary<string, string>();


            // Wedding Applications
            lbBackgroundApplication.Attributes["href"] = backgroundApplicationURL;
            lbSendWeddingInformationForm.Attributes["href"] = String.Format("{0}?form={1}&b={2}&g={3}&cr={4}", sendLinkRouterURL, "0", bridePersonAliasIdString, groomPersonAliasIdString, connectionRequest.Id.ToString());

            wa["weddingInformationFormLastSendDate"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingInformationFormLastSentDate").Value;
            if (!wa["weddingInformationFormLastSendDate"].IsNullOrWhiteSpace())
            {
                lWeddingInformationFormLastSentDate.Text = DateTime.Parse(wa["weddingInformationFormLastSendDate"]).ToShortDateTimeString();
                // Form was sent, so check if complete
                wa["weddingInformationFormCompletedDate"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingInformationFormCompletedDate").Value;
                wa["weddingInformationFormCompleteURL"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingInformationFormCompletedURL").Value;
                if (!wa["weddingInformationFormCompletedDate"].IsNullOrWhiteSpace() && !wa["weddingInformationFormCompleteURL"].IsNullOrWhiteSpace())
                {
                    lWeddingInformationFormCompletedDate.Text = DateTime.Parse(wa["weddingInformationFormCompletedDate"]).ToShortDateTimeString();
                    lbCompletedWeddingInformationForm.Attributes["href"] = wa["weddingInformationFormCompleteURL"];
                    lbCompletedWeddingInformationForm.Visible = true;
                }
                else
                {
                    lWeddingInformationFormCompletedDate.Text = "Not Yet Completed";
                }
            }
            else
            {
                lWeddingInformationFormLastSentDate.Text = "Never";
                lWeddingInformationFormCompletedDate.Text = "Not Yet Completed";
            }
            ...
            // Bride
            int bridePersonAliasId = Int32.Parse(bridePersonAliasIdString);
            Person bride = getPersonFromAliasId(bridePersonAliasId);
            lBrideFullName.Text = bride.FullName;
            aBridePersonProfileLink.Attributes["href"] = string.Format("/person/{0}", bride.Id);
            lBrideMembershipStatus.Text = bride.ConnectionStatusValue.ToString();

            // Groom
            int groomPersonAliasId = Int32.Parse(groomPersonAliasIdString);
            Person groom = getPersonFromAliasId(groomPersonAliasId);
            lGroomFullName.Text = groom.FullName;
            aGroomPersonProfileLink.Attributes["href"] = string.Format("/person/{0}", groom.Id);
            lGroomMembershipStatus.Text = groom.ConnectionStatusValue.ToString();

            // Custom Connection Request Heading
            lWeddingTitle.Text = string.Format("{0} - {1} Wedding", bride.LastName, groom.LastName);
            lWeddingTitle.Visible = true;
            pnlRegularContactInfo.Visible = false;
            pnlCustomWeddingContactInfo.Visible = true;
            try
            {
                lBrideMobile.Text = bride.GetPhoneNumber(mobilePhoneGuid).NumberFormatted;
                lBrideEmail.Text = GetEmailLinkMarkup(bride.Id, bride.Email);
            }
            catch
            {
                // Contact info was missing, continue
            }
            try
            {
                lGroomMobile.Text = groom.GetPhoneNumber(mobilePhoneGuid).NumberFormatted;
                lGroomEmail.Text = GetEmailLinkMarkup(groom.Id, groom.Email);
            }
            catch
            {
                // Contact info was missing, continue
            ...
            // All of these attributes should be populated - all required fields in initial application
            wa["brideDivorced"] = connectionRequest.AttributeValues.GetValueOrNull("BridePreviouslyDivorced").Value;
            wa["groomDivorced"] = connectionRequest.AttributeValues.GetValueOrNull("GroomPreviouslyDivorced").Value;
            wa["brideClass"] = connectionRequest.AttributeValues.GetValueOrNull("BrideCompletedPremaritalClass").Value;
            wa["groomClass"] = connectionRequest.AttributeValues.GetValueOrNull("GroomCompletedPremaritalClass").Value;
            if (wa["brideDivorced"] != null && wa["groomDivorced"] != null && wa["brideClass"] != null && wa["groomClass"] != null)
            {
                lBridePreviouslyDivorced.Text = wa["brideDivorced"];
                lGroomPreviouslyDivorced.Text = wa["groomDivorced"];
                lBrideAttendedClass.Text = wa["brideClass"];
                lGroomAttendedClass.Text = wa["groomClass"];
            }

            // All of these attributes should be populated - all required fields in initial application
            wa["desiredWeddingDate"] = connectionRequest.AttributeValues.GetValueOrNull("DesiredWeddingDate").Value;
            wa["desiredRehersalDate"] = connectionRequest.AttributeValues.GetValueOrNull("DesiredRehearsalDate").Value;
            wa["desiredCampus"] = connectionRequest.AttributeValues.GetValueOrNull("DesiredCampus").Value;
            wa["desiredWeddingType"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingType").Value;
            if (!wa["desiredWeddingDate"].IsNullOrWhiteSpace() && !wa["desiredRehersalDate"].IsNullOrWhiteSpace()
                && !wa["desiredCampus"].IsNullOrWhiteSpace() && !wa["desiredWeddingType"].IsNullOrWhiteSpace())
            {
                Guid campusGuid = new Guid(wa["desiredCampus"]);
                Campus c = cs.Get(campusGuid);

                lDesiredWeddingDate.Text = DateTime.Parse(wa["desiredWeddingDate"]).ToShortDateTimeString();
                lDesiredRehersalDate.Text = DateTime.Parse(wa["desiredRehersalDate"]).ToShortDateTimeString();
                lDesiredCampus.Text = c.Name;
                lDesiredWeddingType.Text = wa["desiredWeddingType"];
            }

            wa["discoveryMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("DiscoveryMeetingDate").Value;
            if (!wa["discoveryMeetingDate"].IsNullOrWhiteSpace())
            {
                lDiscoveryMeetingDate.Text = DateTime.Parse(wa["discoveryMeetingDate"]).ToShortDateTimeString();
            }
            else
            {
                lDiscoveryMeetingDate.Text = "Not Yet Scheduled";
            }


            // Discovery Meeting
            lbSendWeddinngPolicyPDF.Attributes["href"] = String.Format("{0}?form={1}&b={2}&g={3}&cr={4}", sendLinkRouterURL, "1", bridePersonAliasIdString, groomPersonAliasIdString, connectionRequest.Id.ToString());
            wa["weddingCoordinator"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingCoordinator").Value;
            wa["officiatingPastor"] = connectionRequest.AttributeValues.GetValueOrNull("OfficiatingPastor").Value;
            wa["weddingCoordinatorMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingCoordinatorMeetingDate").Value;
            wa["weddingPolicyPDFLastSentDate"] = connectionRequest.AttributeValues.GetValueOrNull("WeddingPolicyPDFLastSentDate").Value;
            if (!wa["weddingCoordinator"].IsNullOrWhiteSpace())
            {
                lWeddingCoordinator.Text = wa["weddingCoordinator"];
            }
            else
            {
                lWeddingCoordinator.Text = "";
            }
            if (!wa["officiatingPastor"].IsNullOrWhiteSpace())
            {
                lPastor.Text = wa["officiatingPastor"];
            }
            else
            {
                lPastor.Text = "";
            }
            if (!wa["weddingCoordinatorMeetingDate"].IsNullOrWhiteSpace())
            {
                lWeddingCoordinatiorMeetingDate.Text = DateTime.Parse(wa["weddingCoordinatorMeetingDate"]).ToShortDateTimeString();
            }
            else
            {
                lWeddingCoordinatiorMeetingDate.Text = "Not Yet Scheduled";
            }
            if (!wa["weddingPolicyPDFLastSentDate"].IsNullOrWhiteSpace())
            {
                lWeddingPolicyPDFLastSentDate.Text = DateTime.Parse(wa["weddingPolicyPDFLastSentDate"]).ToShortDateTimeString();
            }
            else
            {
                lWeddingPolicyPDFLastSentDate.Text = "Never";
            }


            // Wedding Coordinator Meeting
            wa["approvedWeddingDate"] = connectionRequest.AttributeValues.GetValueOrNull("ApprovedWeddingDate").Value;
            wa["approvedRehersalDate"] = connectionRequest.AttributeValues.GetValueOrNull("ApprovedWeddingRehearsalTime").Value;
            wa["selectedPackage"] = connectionRequest.AttributeValues.GetValueOrNull("SelectedPackage").Value;
            wa["facilityReservation"] = connectionRequest.AttributeValues.GetValueOrNull("FacilityReservationStatus").Value;
            if (!wa["approvedWeddingDate"].IsNullOrWhiteSpace())
            {
                lApprovedWeddingDate.Text = DateTime.Parse(wa["approvedWeddingDate"]).ToShortDateTimeString();
            }
            else
            {
                lApprovedWeddingDate.Text = "";
            }
            if (!wa["approvedRehersalDate"].IsNullOrWhiteSpace())
            {
                lApprovedRehearsalDate.Text = DateTime.Parse(wa["approvedRehersalDate"]).ToShortDateTimeString();
            }
            else
            {
                lApprovedRehearsalDate.Text = "";
            }
            if (!wa["selectedPackage"].IsNullOrWhiteSpace())
            {
                lSelectedPackage.Text = wa["selectedPackage"];
            }
            else
            {
                lSelectedPackage.Text = "";
            }
            if (!wa["facilityReservation"].IsNullOrWhiteSpace())
            {
                lReservationStatus.Text = wa["facilityReservation"];
            }
            else
            {
                lReservationStatus.Text = "";
            }

            lbSendWeddingPolicyAndDeposit.Attributes["href"] = String.Format("{0}?form={1}&b={2}&g={3}&cr={4}", sendLinkRouterURL, "2", bridePersonAliasIdString, groomPersonAliasIdString, connectionRequest.Id.ToString());
            wa["policyAndDepositLastSentDate"] = connectionRequest.AttributeValues.GetValueOrNull("PolicyandDepositLastSentDate").Value;
            if (!wa["policyAndDepositLastSentDate"].IsNullOrWhiteSpace())
            {
                lPolicyAndDepositLastSentDate.Text = DateTime.Parse(wa["policyAndDepositLastSentDate"]).ToShortDateTimeString();
                // Form was sent, so check if complete
                wa["policyAndDepositCompletedDate"] = connectionRequest.AttributeValues.GetValueOrNull("PolicyandDepositCompletedDate").Value;
                wa["policyAndDepositCompletedURL"] = connectionRequest.AttributeValues.GetValueOrNull("PolicyandDepositCompletedURL").Value;

                if (!wa["policyAndDepositCompletedDate"].IsNullOrWhiteSpace() && !wa["policyAndDepositCompletedURL"].IsNullOrWhiteSpace())
                {
                    lPolicyAndDepositCompletedDate.Text = DateTime.Parse(wa["policyAndDepositCompletedDate"]).ToShortDateTimeString();
                    lbCompletedWeddingPolicyAndDeposit.Visible = true;
                    lbCompletedWeddingPolicyAndDeposit.Attributes["href"] = wa["policyAndDepositCompletedURL"];
                }
                else
                {
                    lPolicyAndDepositCompletedDate.Text = "Not Yet Completed";
                }
            }
            else
            {
                lPolicyAndDepositLastSentDate.Text = "Never";
                lPolicyAndDepositCompletedDate.Text = "Not Yet Completed";
            }

            wa["pastorMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("PastorMeetingDate").Value;
            if (!wa["pastorMeetingDate"].IsNullOrWhiteSpace())
            {
                lPastorMeetingScheduledDate.Text = DateTime.Parse(wa["pastorMeetingDate"]).ToShortDateTimeString();
            }
            else
            {
                lPastorMeetingScheduledDate.Text = "Not Yet Scheduled";
            }


            // Pastor Meeting
            if (wa["brideDivorced"].Equals("No") && wa["groomDivorced"].Equals("No"))
            {
                lRemarriageFormNecessary.Text = "Bride, Groom";
            }
            else if (wa["brideDivorced"].Equals("No"))
            {
                lRemarriageFormNecessary.Text = "Bride";
            }
            else if (wa["groomDivorced"].Equals("No"))
            {
                lRemarriageFormNecessary.Text = "Groom";
            }
            else
            {
                lRemarriageFormNecessary.Text = "No";
            }

            wa["couplePremaritalClass"] = connectionRequest.AttributeValues.GetValueOrNull("CouplePremaritalClass").Value;
            wa["couplePremaritalClassCompeltedDate"] = connectionRequest.AttributeValues.GetValueOrNull("CouplePremaritalClassCompletedDate").Value;
            if (!wa["couplePremaritalClass"].IsNullOrWhiteSpace())
            {
                lPremaritalClass.Text = wa["couplePremaritalClass"];
            }
            else
            {
                lPremaritalClass.Text = "";
            }
            if (!wa["couplePremaritalClassCompeltedDate"].IsNullOrWhiteSpace())
            {
                lPremaritalClassCompletedDate.Text = DateTime.Parse(wa["couplePremaritalClassCompeltedDate"]).ToShortDateString();
            }
            else
            {
                lPremaritalClassCompletedDate.Text = "";
            }

            lbSendBrideRemarriageForm.Attributes["href"] = String.Format("{0}?form={1}&b={2}&cr={3}", sendLinkRouterURL, "3A", bridePersonAliasIdString, connectionRequest.Id.ToString());
            wa["brideRemarriageLastSentDate"] = connectionRequest.AttributeValues.GetValueOrNull("BrideRemarriageFormLastSentDate").Value;
            if (!wa["brideRemarriageLastSentDate"].IsNullOrWhiteSpace())
            {
                lBrideRemarriageFormLastSentDate.Text = DateTime.Parse(wa["brideRemarriageLastSentDate"]).ToShortDateTimeString();
                // Form was sent, so check if complete
                wa["brideRemarriageCompletedDate"] = connectionRequest.AttributeValues.GetValueOrNull("BrideRemarriageFormCompletedDate").Value;
                wa["brideRemarriageCompletedURL"] = connectionRequest.AttributeValues.GetValueOrNull("BrideRemarriageFormCompletedURL").Value;

                if (!wa["brideRemarriageCompletedDate"].IsNullOrWhiteSpace() && !wa["brideRemarriageCompletedURL"].IsNullOrWhiteSpace())
                {
                    lBrideRemarriageFormCompletedDate.Text = DateTime.Parse(wa["brideRemarriageCompletedDate"]).ToShortDateTimeString();
                    lbBrideCompletedRemarriageForm.Visible = true;
                    lbBrideCompletedRemarriageForm.Attributes["href"] = wa["brideRemarriageCompletedURL"];
                }
                else
                {
                    lBrideRemarriageFormCompletedDate.Text = "Not Yet Completed";
                }
            }
            else
            {
                lBrideRemarriageFormLastSentDate.Text = "Never";
                lBrideRemarriageFormCompletedDate.Text = "Not Yet Completed";
            }

            lbSendGroomRemarriageForm.Attributes["href"] = String.Format("{0}?form={1}&g={2}&cr={3}", sendLinkRouterURL, "3B", groomPersonAliasIdString, connectionRequest.Id.ToString());
            wa["groomRemarriageLastSentDate"] = connectionRequest.AttributeValues.GetValueOrNull("GroomRemarriageFormLastSentDate").Value;
            if (!wa["groomRemarriageLastSentDate"].IsNullOrWhiteSpace())
            {
                lGroomRemarriageFormLastSentDate.Text = DateTime.Parse(wa["groomRemarriageLastSentDate"]).ToShortDateTimeString();
                // Form was sent, so check if complete
                wa["groomRemarriageCompletedDate"] = connectionRequest.AttributeValues.GetValueOrNull("GroomRemarriageFormCompletedDate").Value;
                wa["groomRemarriageCompletedURL"] = connectionRequest.AttributeValues.GetValueOrNull("GroomRemarriageFormCompletedURL").Value;

                if (!wa["groomRemarriageCompletedDate"].IsNullOrWhiteSpace() && !wa["groomRemarriageCompletedURL"].IsNullOrWhiteSpace())
                {
                    lGroomRemarriageFormCompletedDate.Text = DateTime.Parse(wa["groomRemarriageCompletedDate"]).ToShortDateTimeString();
                    lbGroomCompletedRemarriageForm.Visible = true;
                    lbGroomCompletedRemarriageForm.Attributes["href"] = wa["groomRemarriageCompletedURL"];
                }
                else
                {
                    lGroomRemarriageFormCompletedDate.Text = "Not Yet Completed";
                }
            }
            else
            {
                lGroomRemarriageFormLastSentDate.Text = "Never";
                lGroomRemarriageFormCompletedDate.Text = "Not Yet Completed";
            }

            wa["finalMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("FinalMeetingDate").Value;
            if (!wa["finalMeetingDate"].IsNullOrWhiteSpace())
            {
                lFinalMeetingScheduledDate.Text = DateTime.Parse(wa["finalMeetingDate"]).ToShortDateTimeString();
            }
            else
            {
                lFinalMeetingScheduledDate.Text = "Not Yet Scheduled";
            }


            // Final Meeting
            lbSendFinalPayment.Attributes["href"] = String.Format("{0}?form={1}&b={2}&g={3}&cr={4}&def=none", sendLinkRouterURL, "4", bridePersonAliasIdString, groomPersonAliasIdString, connectionRequest.Id.ToString());
            wa["finalPaymentLastSentDate"] = connectionRequest.AttributeValues.GetValueOrNull("FinalPaymentLastSentDate").Value;
            if (!wa["finalPaymentLastSentDate"].IsNullOrWhiteSpace())
            {
                lFinalPaymentLastSentDate.Text = DateTime.Parse(wa["finalPaymentLastSentDate"]).ToShortDateTimeString();
                // Form was sent, so check if complete
                wa["finalPaymentCompletedDate"] = connectionRequest.AttributeValues.GetValueOrNull("FinalPaymentCompletedDate").Value;
                wa["finalPaymentCompletedURL"] = connectionRequest.AttributeValues.GetValueOrNull("FinalPaymentCompletedURL").Value;

                if (!wa["finalPaymentCompletedDate"].IsNullOrWhiteSpace() && !wa["finalPaymentCompletedURL"].IsNullOrWhiteSpace())
                {
                    lFinalPaymentCompletedDate.Text = DateTime.Parse(wa["finalPaymentCompletedDate"]).ToShortDateTimeString();
                    lbCompletedFinalPayment.Visible = true;
                    lbCompletedFinalPayment.Text = "View Completed Form";
                    lbCompletedFinalPayment.Attributes["href"] = wa["finalPaymentCompletedURL"];
                }
                else
                {
                    // Check if in-progress
                    wa["finalPaymentRegistrationId"] = connectionRequest.AttributeValues.GetValueOrNull("FinalPaymentRegistrationId").Value;
                    if (!wa["finalPaymentRegistrationId"].IsNullOrWhiteSpace())
                    {
                        lFinalPaymentCompletedDate.Text = "In Progress";
                        lbCompletedFinalPayment.Visible = true;
                        lbCompletedFinalPayment.Text = "View Payment Progress";
                        lbCompletedFinalPayment.Attributes["href"] = wa["finalPaymentCompletedURL"];

                    }
                    else
                    {
                        lFinalPaymentCompletedDate.Text = "Not Yet Completed";
                    }
                }
            }
            else
            {
                lFinalPaymentLastSentDate.Text = "Never";
                lFinalPaymentCompletedDate.Text = "Not Yet Started";
            }

            if (wa["selectedPackage"].Equals("Package 1 - Offsite") || wa["selectedPackage"].Equals("Package 1 - On Campus") || wa["selectedPackage"].Equals(""))
            {
                // Gray out event forms because only necessary for package B and C
                lbSendEventForms.Attributes["href"] = "";
                lbSendEventForms.Enabled = false;
            }
            else
            {
                lbSendEventForms.Enabled = true;
                if (wa["selectedPackage"].Equals("Package 2"))
                {
                    lbSendEventForms.Attributes["href"] = String.Format("{0}?form={1}&b={2}&g={3}&cr={4}", sendLinkRouterURL, "B", bridePersonAliasIdString, groomPersonAliasIdString, connectionRequest.Id.ToString());
                }
                else if (wa["selectedPackage"].Equals("Package 3"))
                {
                    lbSendEventForms.Attributes["href"] = String.Format("{0}?form={1}&b={2}&g={3}&cr={4}", sendLinkRouterURL, "C", bridePersonAliasIdString, groomPersonAliasIdString, connectionRequest.Id.ToString());
                } 
                wa["eventFormsLastSentDate"] = connectionRequest.AttributeValues.GetValueOrNull("EventFormsLastSentDate").Value;
                if (!wa["eventFormsLastSentDate"].IsNullOrWhiteSpace())
                {
                    lEventFormsLastSentDate.Text = DateTime.Parse(wa["eventFormsLastSentDate"]).ToShortDateTimeString();
                    // Form was sent, so check if complete
                    wa["eventFormsCompletedDate"] = connectionRequest.AttributeValues.GetValueOrNull("EventFormsCompletedDate").Value;
                    wa["eventFormsCompletedURL"] = connectionRequest.AttributeValues.GetValueOrNull("EventFormsCompletedURL").Value;

                    if (!wa["eventFormsCompletedDate"].IsNullOrWhiteSpace() && !wa["eventFormsCompletedURL"].IsNullOrWhiteSpace())
                    {
                        lEventFormsCompletedDate.Text = DateTime.Parse(wa["eventFormsCompletedDate"]).ToShortDateTimeString();
                        lbCompletedEventForms.Visible = true;
                        lbCompletedEventForms.Attributes["href"] = wa["eventFormsCompletedURL"];
                    }
                    else
                    {
                        lEventFormsCompletedDate.Text = "Not Yet Completed";
                    }
                }
                else
                {
                    lEventFormsLastSentDate.Text = "Never";
                    lEventFormsCompletedDate.Text = "Not Yet Completed";
                }
            }

            // Clear Key Attributes After Loading
            bridePersonAliasIdString = null;
            groomPersonAliasIdString = null;
            backgroundApplicationURL = null;
            wa = null;
        }

        private void renderEditableWeddingAttributeValues()
        {
            var rockContext = new RockContext();
            var connectionRequestService = new ConnectionRequestService(rockContext);
            var connectionRequest = connectionRequestService.Get(ConnectionRequestId.Value);
            connectionRequest.LoadAttributes();
            CampusService cs = new CampusService(rockContext);
            ...
            {
                // Continue with normal validation, strings will be empty
            }
            ...
            {
                // Hide custom panels / Attributes
                pnlEditWeddingAttributes.Visible = false;
                return;
            }
            pnlEditWeddingAttributes.Visible = true;
                ...
                && !wa["desiredCampus"].IsNullOrWhiteSpace() && !wa["desiredWeddingType"].IsNullOrWhiteSpace())
            {
                Guid campusGuid = new Guid(wa["desiredCampus"]);
                Campus c = cs.Get(campusGuid);
                dtpEditDesiredWeddingDate.SelectedDateTime = DateTime.Parse(wa["desiredWeddingDate"]);
                dtpEditDesiredRehersalDate.SelectedDateTime = DateTime.Parse(wa["desiredRehersalDate"]);
                cpEditDesiredCampus.SelectedCampusId = c.Id;
                rtbDesiredWeddingType.Text = wa["desiredWeddingType"];
            }

            wa["discoveryMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("DiscoveryMeetingDate").Value;
            if (!wa["discoveryMeetingDate"].IsNullOrWhiteSpace())
            {
                dtpEditDiscoveryMeetingDate.SelectedDateTime = DateTime.Parse(wa["discoveryMeetingDate"]);
            }
            ...
            if (!wa["weddingCoordinator"].IsNullOrWhiteSpace())
            {
                rtbEditWeddingCoordinator.Text = wa["weddingCoordinator"];
            }
            if (!wa["officiatingPastor"].IsNullOrWhiteSpace())
            {
                rtbEditOfficiatingPastor.Text = wa["officiatingPastor"];
            }
            if (!wa["weddingCoordinatorMeetingDate"].IsNullOrWhiteSpace())
            {
                dtpEditWeddingCoordinatorMeeting.SelectedDateTime = DateTime.Parse(wa["weddingCoordinatorMeetingDate"]);
            }
            ...
            if (!wa["approvedWeddingDate"].IsNullOrWhiteSpace())
            {
                dtpEditApprovedWeddingDate.SelectedDateTime = DateTime.Parse(wa["approvedWeddingDate"]);
            }
            if (!wa["approvedRehersalDate"].IsNullOrWhiteSpace())
            {
                dtpEditApprovedRehearsalDate.SelectedDateTime = DateTime.Parse(wa["approvedRehersalDate"]);
            }
            // Only populate empty list
            if (ddlEditSelectedPackage.Items.Count == 0)
            {
                ddlEditSelectedPackage.Items.Add("");
                ddlEditSelectedPackage.Items.Add("Package 1 - Offsite");
                ddlEditSelectedPackage.Items.Add("Package 1 - On Campus");
                ddlEditSelectedPackage.Items.Add("Package 2");
                ddlEditSelectedPackage.Items.Add("Package 3");
            }
            if (!wa["selectedPackage"].IsNullOrWhiteSpace() && ddlEditSelectedPackage.Items.Contains(new ListItem(wa["selectedPackage"])))
            {
                ddlEditSelectedPackage.SelectedValue = wa["selectedPackage"];
            }
            if (!wa["facilityReservation"].IsNullOrWhiteSpace() && wa["facilityReservation"].Equals("Complete"))
            {
                rcbEditFacilityReservation.Checked = true;
            }
            else
            {
                rcbEditFacilityReservation.Checked = false;
            }

            wa["pastorMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("PastorMeetingDate").Value;
            if (!wa["pastorMeetingDate"].IsNullOrWhiteSpace())
            {
                dtpEditPastorMeetingDate.SelectedDateTime = DateTime.Parse(wa["pastorMeetingDate"]);
            }
            ...
            if (!wa["couplePremaritalClass"].IsNullOrWhiteSpace())
            {
                rtbEditPremaritalClass.Text = wa["couplePremaritalClass"];
            }
            if (!wa["couplePremaritalClassCompeltedDate"].IsNullOrWhiteSpace())
            {
                dpEditPremartialClassDate.SelectedDate = DateTime.Parse(wa["couplePremaritalClassCompeltedDate"]);
            }
            wa["finalMeetingDate"] = connectionRequest.AttributeValues.GetValueOrNull("FinalMeetingDate").Value;
            if (!wa["finalMeetingDate"].IsNullOrWhiteSpace())
            {
                dtpEditFinalMeetingDate.SelectedDateTime = DateTime.Parse(wa["finalMeetingDate"]);
            }
        ...
        private void saveWeddingAttributes(ConnectionRequest cr)
        {
            cr.LoadAttributes();

            // Application Highlights
            cr.SetAttributeValue("DesiredWeddingDate", dtpEditDesiredWeddingDate.SelectedDateTime.ToString());
            cr.SetAttributeValue("DesiredRehearsalDate", dtpEditDesiredRehersalDate.SelectedDateTime.ToString());
            if (cpEditDesiredCampus.SelectedCampusId != null)
            {
                var rockContext = new RockContext();
                CampusService cs = new CampusService(rockContext);
                cr.SetAttributeValue("DesiredCampus", cs.Get(cpEditDesiredCampus.SelectedCampusId.Value).Guid);
            }
            cr.SetAttributeValue("WeddingType", rtbDesiredWeddingType.Text);
            cr.SetAttributeValue("DiscoveryMeetingDate", dtpEditDiscoveryMeetingDate.SelectedDateTime.ToString());

            // Discovery Meeting
            cr.SetAttributeValue("WeddingCoordinator", rtbEditWeddingCoordinator.Text);
            cr.SetAttributeValue("OfficiatingPastor", rtbEditOfficiatingPastor.Text);
            cr.SetAttributeValue("WeddingCoordinatorMeetingDate", dtpEditWeddingCoordinatorMeeting.SelectedDateTime.ToString());

            // Wedding Coordinator Meeting
            cr.SetAttributeValue("ApprovedWeddingDate", dtpEditApprovedWeddingDate.SelectedDateTime.ToString());
            cr.SetAttributeValue("ApprovedWeddingRehearsalTime", dtpEditApprovedRehearsalDate.SelectedDateTime.ToString());
            cr.SetAttributeValue("SelectedPackage", ddlEditSelectedPackage.SelectedValue.ToString());
            if (rcbEditFacilityReservation.Checked)
            {
                cr.SetAttributeValue("FacilityReservationStatus", "Complete");
            }
            else
            {
                cr.SetAttributeValue("FacilityReservationStatus", "");
            }
            cr.SetAttributeValue("PastorMeetingDate", dtpEditPastorMeetingDate.SelectedDateTime.ToString());

            // Pastor Meeting
            cr.SetAttributeValue("CouplePremaritalClass", rtbEditPremaritalClass.Text);
            cr.SetAttributeValue("CouplePremaritalClassCompletedDate", dpEditPremartialClassDate.SelectedDate.ToString());
            cr.SetAttributeValue("FinalMeetingDate", dtpEditFinalMeetingDate.SelectedDateTime.ToString());
            return;
        }

        // Helpers
        private Person getPersonFromAliasId(int personAliasId)
        {
            RockContext rc = new RockContext();
            PersonAliasService personAliasService = new PersonAliasService(rc);
            PersonService personService = new PersonService(rc);
            ...
            int? possiblePersonId = personAliasService.GetPersonId(personAliasId).Value;
            if (possiblePersonId.HasValue)
            {
                int personId = possiblePersonId.Value;
                personIds.Add(personId);

                IQueryable<Person> personResults = personService.GetByIds(personIds);
                return personResults.FirstOrDefault();

            }
            return null;
        }

        #endregion LPC Custom Methods
...
mdRequest.OnCancelScript = "Rock.controls.connectionRequestBoard.removeConnectionQuerystringParameters('ConnectionRequestId');";
	    ...
	    /* 
            ...
            */
            ...
            renderWeddingAttributeValues(connectionRequest);
	...
	internal string UpdateConnectionQuerystring( string connectionRequestId )
	    ...
	    // If a value is selected in the radio button list, use it, otherwise use "Active".
            ...
            if (pnlEditWeddingAttributes.Visible)
            {
                saveWeddingAttributes(connectionRequest);
            }
            else
            {
...
";
        ...
        protected void btnRequestModalViewModeEdit_Click( object sender, EventArgs e )
        {
            ...
            RequestModalViewModeSubMode = RequestModalViewModeSubMode_View;
            IsRequestModalAddEditMode = true;
            renderEditableWeddingAttributeValues();
            ShowRequestModal();
        }
