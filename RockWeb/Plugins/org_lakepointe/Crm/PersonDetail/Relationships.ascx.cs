...
";
        ...
        protected void bbSaveAndAdd_Click( object sender, EventArgs e )
        {
            if ( CanEdit )
            {
                if ( SaveRelationship() )
                {
                    ...
                    // and reset the modal dialog
                    ppPerson.SetValue( null );
                    //grpRole.GroupRoleId = null;
                    hfRoleId.Value = "0";
                }
            }
        }

        private bool SaveRelationship()
        {
                ...
                if ( roleId.HasValue )
                {
                    ...
                    if ( group != null )
                    {
                        ...
                        groupMember = memberService.Queryable( true )
                                .Where( m => m.PersonId == ppPerson.PersonId )
                        ...
                        if ( groupMember != null )
                        {
                            nbSaveSucess.Text = "The requested action would create a duplicate relationship. " +
                                "That is not allowed.";
                            nbSaveSucess.NotificationBoxType = NotificationBoxType.Danger;
                            nbSaveSucess.Visible = true;
                            return false;
                        }
                        ...
                        if ( IsInverseRelationshipsOwner )
                        {
                            // If inverse relationship does not yet exist, this line creates the inverse relationship
                            // Will not create a duplicate inverse relationship
