...
{
    [DisplayName( "LPC - New Family Pre Registration" )]
    [Category( "LPC > CRM" )]
    ...
    [TextField( "Adult Title", "How should adults be listed in the interface?", false, "Adult", "", 6, ADULT_TITLE_KEY )]
    [TextField( "Progeny Title", "How should progeny be listed in the interface?", false, "Children", "", 7, PROGENY_TITLE_KEY )]
    ...
    public partial class FamilyPreRegistration : RockBlock
    {
        ...
        public string Path
        {
            get
            {
                ...
                {
                    _path = MapPath( ResolveRockUrl( "~/Plugins/org_lakepointe/Crm/FamilyPreRegistration/ChildRow.en.json" ) );
                }
                return _path;
            }
        }
...
",
            ...
            if ( adultTitle != Translation.GetValueOrDefault( "Adult", "Adult" )) // don't bother changing if it is the value that matches the json file
            {
                string patchAdult = $@"
  $('body :not(script)').contents().filter( function() {{
                    return this.nodeType === 3;
                }}).replaceWith( function() {{
                    return this.nodeValue.replace( '{Translation.GetValueOrDefault( "Adult", "Adult" )}', '{adultTitle}' );
                }});
";
...
$('body :not(script)').contents().filter( function() {{
                ...
                return this.nodeValue.replace( 'Add Child', '{Translation.GetValueOrDefault( "LabelAddChild", "Add Child" )}' );
            }});");
        ...
        private void SetControls()
        {
            ...
            string script = $@"<script>var dropDowns = document.querySelectorAll(""[id*=dvpMaritalStatus]"");for (var x = 0; x < dropDowns.length; x++) {{var options = dropDowns[x].querySelectorAll(""option"");for (var y = 0; y < options.length; y++) {{if (options[y].innerText == ""Married"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionMarried", "Married" )}"";}}else if (options[y].innerText == ""Single"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionSingle", "Single" )}"";}}else if (options[y].innerText == ""Engaged"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionEngaged", "Engaged" )}"";}}else if (options[y].innerText == ""Divorced"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionDivorced", "Divorced" )}"";}}else if (options[y].innerText == ""Unknown"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionUnknown", "Unknown" )}"";}}else if (options[y].innerText == ""Widowed"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionWidowed", "Widowed" )}"";}}else if (options[y].innerText == ""Separated"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionSeparated", "Separated" )}"";}}}}}}var dropDowns = document.querySelectorAll(""[id*=ddlGender]"");for (var x = 0; x < dropDowns.length; x++) {{var options = dropDowns[x].querySelectorAll(""option"");for (var y = 0; y < options.length; y++) {{if (options[y].innerText == ""Male"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionMale", "Male" )}"";}}else if (options[y].innerText == ""Female"") {{options[y].innerText = ""{Translation.GetValueOrDefault( "OptionFemale", "Female" )}"";}}}}}}var serviceLabels = document.querySelectorAll(""[id*=attribute_field_39149]"");for (var x = 0; x < serviceLabels.length; x++) {{serviceLabels[x].parentElement.parentElement.querySelector(""label"").innerText = ""{Translation.GetValueOrDefault( "LabelService", "Service Attending" )}"";}}</script>";
            ClientScriptManager cs = Page.ClientScript;
            cs.RegisterStartupScript(GetType(), "OnLoad", script);
        ...
        private bool ValidateInfo()
        {
            ...
            {
                errorMessages.Add("Acknowledgement is required.");
            }
            ...
            foreach (var child in Children)
            {
                ...
                {
                    errorMessages.Add("Each child must have their allergy field filled out if they have an allergy.");
                }
            }
