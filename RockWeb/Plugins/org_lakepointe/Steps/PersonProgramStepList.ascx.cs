...
namespace RockWeb.Plugins.org_lakepointe.Steps
{
    [DisplayName( "Clickable Personal Step List" )]
    [Category( "LPC > Steps" )]
    ...
    [TextField(
        name: "Step Status for Start Button to Appear",
        description: "When a step has this status, the start button will appear when the user hovers over the step. This must match your step status name exactly.",
        required: false,
        order: 3,
        key: AttributeKey.StepStartStatus)]

    [LinkedPage(
        name: "Linked Page for Step 1",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage1 )]

    [LinkedPage(
        name: "Linked Page for Step 2",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage2 )]

    [LinkedPage(
        name: "Linked Page for Step 3",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage3 )]

    [LinkedPage(
        name: "Linked Page for Step 4",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage4 )]

    [LinkedPage(
        name: "Linked Page for Step 5",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage5 )]

    [LinkedPage(
        name: "Linked Page for Step 6",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage6 )]

    [LinkedPage(
        name: "Linked Page for Step 7",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage7 )]

    [LinkedPage(
        name: "Linked Page for Step 8",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage8 )]

    [LinkedPage(
        name: "Linked Page for Step 9",
        description: "The page where step records can be edited or added",
        required: false,
        order: 4,
        key: AttributeKey.StepPage9 )]
    ...
    public partial class PersonProgramStepList : RockBlock
    {
        ...
        private static class AttributeKey
        {
            ...
            /// The step page attribute key
            /// </summary>
            public const string StepStartStatus = "StepStartStatus";

            /// <summary>
            /// The attribute keys for linked pages
            /// </summary>
            public const string StepPage1 = "StepPage1";
            public const string StepPage2 = "StepPage2";
            public const string StepPage3 = "StepPage3";
            public const string StepPage4 = "StepPage4";
            public const string StepPage5 = "StepPage5";
            public const string StepPage6 = "StepPage6";
            public const string StepPage7 = "StepPage7";
            public const string StepPage8 = "StepPage8";
            public const string StepPage9 = "StepPage9";
        ...
        public void ScOnClick( object sender, CommandEventArgs e )
        {
            ...
            if (stepId.HasValue)
            {
                stepTypeOrder = GetStepTypeOrder(stepId.Value);
            }

            GoToLinkedPage(stepTypeOrder);
        ...
        protected void rSteps_ItemDataBound( object sender, RepeaterItemEventArgs e )
        {
            ...
            int stepTypeOrder = GetStepTypeOrder(cardStepViewModel.StepId);
            string page = GetPageForStepBasedOnOrder(stepTypeOrder);

            var tdStart = e.Item.FindControl("tdStart") as HtmlTableCell;

            var status = GetAttributeValue(AttributeKey.StepStartStatus);
            if (status != "")
            {
                if (cardStepViewModel.StatusHtml == status && page != "")
                {
...
@"<style>
    #" + upContent.ClientID + @" {
        ...
        private int GetStepTypeOrder( int stepId )
        {
            var rockContext = GetRockContext();
            var stepService = new StepService(rockContext);
            ...
            if (stepId >= 0)
            {
                stepTypeId = stepService.Queryable().AsNoTracking()
                    .Where(gm => gm.Id == stepId)
                    .Select(gm => gm.StepTypeId)
                    .FirstOrDefault();
            }
            ...
            if (stepTypeId != -1)
            {
                stepTypeOrder = stepTypeService.Queryable().AsNoTracking()
                    .Where(hm => hm.Id == stepTypeId)
                    .Select(hm => hm.Order)
                    .FirstOrDefault();
            }

            return stepTypeOrder;
        }

        private void GoToLinkedPage(int stepOrder)
        {
            var person = GetPerson();
            var page = GetPageAttributeKeyForStepBasedOnOrder(stepOrder);

            if (person != null && page != "")
            {
                NavigateToLinkedPage(page, new Dictionary<string, string> {
                    { "PersonId", person.Id.ToString() }
                });
            }
        }

        private string GetPageForStepBasedOnOrder(int stepOrder)
        {
            string page = "";

            if (stepOrder == 0)
            {
                page = GetAttributeValue(AttributeKey.StepPage1);
            }
            else if (stepOrder == 1)
            {
                page = GetAttributeValue(AttributeKey.StepPage2);
            }
            else if (stepOrder == 2)
            {
                page = GetAttributeValue(AttributeKey.StepPage3);
            }
            else if (stepOrder == 3)
            {
                page = GetAttributeValue(AttributeKey.StepPage4);
            }
            else if (stepOrder == 4)
            {
                page = GetAttributeValue(AttributeKey.StepPage5);
            }
            else if (stepOrder == 5)
            {
                page = GetAttributeValue(AttributeKey.StepPage6);
            }
            else if (stepOrder == 6)
            {
                page = GetAttributeValue(AttributeKey.StepPage7);
            }
            else if (stepOrder == 7)
            {
                page = GetAttributeValue(AttributeKey.StepPage8);
            }
            else if (stepOrder == 8)
            {
                page = GetAttributeValue(AttributeKey.StepPage9);
            }

            return page;
        }

        private string GetPageAttributeKeyForStepBasedOnOrder(int stepOrder)
        {
            string page = "";

            if (stepOrder == 0)
            {
                page = AttributeKey.StepPage1;
            }
            else if (stepOrder == 1)
            {
                page = AttributeKey.StepPage2;
            }
            else if (stepOrder == 2)
            {
                page = AttributeKey.StepPage3;
            }
            else if (stepOrder == 3)
            {
                page = AttributeKey.StepPage4;
            }
            else if (stepOrder == 4)
            {
                page = AttributeKey.StepPage5;
            }
            else if (stepOrder == 5)
            {
                page = AttributeKey.StepPage6;
            }
            else if (stepOrder == 6)
            {
                page = AttributeKey.StepPage7;
            }
            else if (stepOrder == 7)
            {
                page = AttributeKey.StepPage8;
            }
            else if (stepOrder == 8)
            {
                page = AttributeKey.StepPage9;
            }
