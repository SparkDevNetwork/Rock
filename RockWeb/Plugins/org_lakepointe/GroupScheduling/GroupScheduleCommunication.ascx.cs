...
namespace RockWeb.Plugins.org_lakepointe.GroupScheduling
{
    ...
    ///
    /// </summary>
    [DisplayName( "LPC - Group Schedule Communication" )]
    [Category( "LPC > Group Scheduling" )]
    [Description( "Allows an individual to create a communication based on group schedule criteria. LPC version can use GroupId and GroupIds query parameters." )]
    public partial class GroupScheduleCommunication : RockBlock
    {
        ...
        private void ShowDetails()
        {
            ...
            if ( groupIdParameter != "" || groupIdsParameter != "" )
            {
                var groupId = groupIdParameter.AsIntegerOrNull();
                groupIds = ( groupIdsParameter ?? string.Empty ).Split( ',' ).AsIntegerList();
            ...
            {
                groupIds = userPreferenceConfiguration.GroupIds.ToList() ?? new List<int>();
            }

            if ( groupIds.Any() )
            {
                gpGroups.SetValues( new GroupService( new RockContext() )
                    .Queryable()
                    .AsNoTracking()
