...
namespace RockWeb.Plugins.org_lakepointe.GroupScheduling
{
    ...
    /// </summary>
    [DisplayName( "LPC - Group Schedule Toolbox v2" )]
    ...
    [BooleanField("Decline Link Uses Person Token",
        Description = "If checked, the decline button will use a person token, meaning that if the user got to the group scheduler with a person token that is only valid on that page, they will not have to sign in to decline the event.",
        DefaultBooleanValue = false,
        Order = 3,
        Key = AttributeKey.DeclineUseToken)]
    ...
    public partial class GroupScheduleToolboxV2 : RockBlock
    {
        ...
        protected class AttributeKey
        {
            ...
            public const string DeclineUseToken = "DeclineUseToken";
...
",
        ...
        protected void btnDeclineAttend_Click( object sender, EventArgs e )
        {
            ...
            if ( attendanceId.HasValue )
            {
                ...
                if ( requiresDeclineReason )
                {
                    ...
                    if ( GetAttributeValue( AttributeKey.DeclineUseToken).AsBoolean() && PageParameter( "rckipid" ).IsNotNullOrWhiteSpace() )
                    {
                        var pageReference = new PageReference( GetAttributeValue( AttributeKey.DeclineReasonPage ) );
                        queryParams.Add( "rckipid", CurrentPerson.GetImpersonationToken( DateTime.Now.AddMinutes(20), 3, pageReference.PageId ) );
                    }
        ...
        private void BindScheduleRepeater()
        {
            ...
            lActionHeader.Visible = !lActionHeader.Text.IsNullOrWhiteSpace() && ( enableAdditionalTimeSignUp || enableScheduleUnavailability || enableUpdateSchedulePreferences );
        }
        ...
        private void UpdateScheduleSignUp( Control senderControl )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( attendanceId.HasValue )
                {
                    ...
                    attendanceService.ScheduledPersonClear( attendanceId.Value );
                }
