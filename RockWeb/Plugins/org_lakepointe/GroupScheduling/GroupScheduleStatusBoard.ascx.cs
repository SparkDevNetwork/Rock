...
namespace RockWeb.Plugins.org_lakepointe.GroupScheduling
{
    ...
    [DisplayName( "LPC - Group Schedule Status Board" )]
    ...
    [BooleanField(
        "Pass Only Primary Group to Roster Page",
        Key = AttributeKey.PassPrimaryGroup,
        Description = "When enabled, the link to the roster page will use the GroupId query parameter instead of using the GroupIds parameter.The group that it passes will be whatever group was passed to this page via query parameter or the first selected group in the list.",
    ...
    public partial class GroupScheduleStatusBoard : RockBlock
    {
        ...
        private static class AttributeKey
        {
            ...
            public const string PassPrimaryGroup = "PassPrimaryGroup";
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            selectableGroups = groupService.ExecuteQuery( @"/* Query running from RockWeb.Plugins.org_lakepointe.GroupScheduling.GroupScheduleStatusBoard.ascx.cs */
...
SELECT g.*
    ...
    AND g.DisableScheduling = 0", CurrentPerson.Id ).ToList();
        ...
        private void ApplyBlockSettings()
        {
            ...
            if ( GetAttributeValue( AttributeKey.PassPrimaryGroup ).AsBoolean() )
            {
                btnRosters.Text = "<i class=\"fa fa-calendar-check\"></i> Roster";
            }
        ...
        private void BuildStatusBoard()
        {
            ...
            selectedGroupIds.AddRange( GetSelectedGroupIds() );
...
<th colspan='{0}' class='position-relative'><div class='sticky-cell'>
    ...
    {0}
        ...
        protected void btnGroups_Click( object sender, EventArgs e )
        {
            ...
            foreach ( int groupId in GetSelectedGroupIds() )
            {
        ...
        protected void btnRosters_Click( object sender, EventArgs e )
        {
            ...
            if ( GetAttributeValue( AttributeKey.PassPrimaryGroup ).AsBoolean() )
            {
                // If only primary group should be passed to roster page
                ...
                if ( group != null )
                {
                    ...
                    // selectedGroupIds list.
                    bool isAuthorized = group.IsAuthorized( Authorization.SCHEDULE, CurrentPerson );
                    if ( isAuthorized && group.GroupType.IsSchedulingEnabled && !group.DisableScheduling )
                    {
                    ...
                    else
                    {
                        ...
                        {
                            queryParams.Add( "GroupId", selectedGroupIds[0].ToString() );
                        }
                    }
                }
                else
            ...
            {
                // If all selected groups should be passed to roster page
                queryParams.Add( "GroupIds", GetSelectedGroupIds().AsDelimited( "," ) );
            }
