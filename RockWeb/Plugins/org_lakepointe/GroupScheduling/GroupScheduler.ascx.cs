...
namespace RockWeb.Plugins.org_lakepointe.GroupScheduling
{
    ...
    ///
    /// </summary>
    [DisplayName( "LPC - Group Scheduler" )]
    [Category( "LPC > Group Scheduling" )]
    [Description( "Allows group schedules for groups and locations to be managed by a scheduler. LPC version adds toggle to disallow" +
        "users from scheduling people from other groups into the user's group. LPC version also swaps out group picker for multiselect" +
        "dropdown so that users can only select groups that they are allowed to schedule." )]
    ...
    [BooleanField(
        "Allow Scheduling From Outside Group",
        Description = "Allow users of the scheduler to schedule people that aren't in the check-in team",
        IsRequired = true,
        DefaultBooleanValue = true,
        Order = 1,
        Key = AttributeKey.AllowSchedulingFromOutsideGroup )]
    ...
    public partial class GroupScheduler : RockBlock
    {
        ...
        protected class AttributeKey
        {
            ...
            /// <summary>
            /// Whether scheduling from outside of the group should be allowed
            /// </summary>
            public const string AllowSchedulingFromOutsideGroup = "AllowSchedulingFromOutsideGroup";
        ...
        private List<Group> selectableGroups;
        ...
        private List<int> selectedGroupIds
        {
            get
            {
                ...
                foreach ( string groupName in rlbGroups.SelectedValues )
                {
                    foreach ( Group selectableGroup in selectableGroups )
                    {
                        if ( selectableGroup.Name == groupName )
                        {
                            selectedGroupIds.Add( selectableGroup.Id );
                            break;
                        }
                    }
                }
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            RockPage.AddStyleToHead( "group-scheduler-style-override",
                ".resource .btn { padding: 0 6px; margin-left: 4px; } .resource .btn:focus { color: #fff; }" );

            this.AddConfigurationUpdateTrigger( upnlContent );

            // Load selectable groups
            GroupService groupService = new GroupService( new RockContext() );
            selectableGroups = groupService.ExecuteQuery( @"/* Query running from RockWeb.Plugins.org_lakepointe.GroupScheduling.GroupScheduler.ascx.cs */
/* Query written by Jon Corey on 12/14/2022 */
/* Updated by Jon Corey on 3/20/2023 to remove verbose columns from select to allow for columns to change */
/* Purpose: Get all scheduled groups where person is a coach or a leader. */
/* Parameters: @p0 - The person's Id */
SELECT g.*
    FROM [dbo].[GroupMember] gm
    INNER JOIN [dbo].[Group] g ON g.Id = gm.GroupId
    INNER JOIN [dbo].[GroupType] gt ON gt.Id = g.GroupTypeId
    INNER JOIN [dbo].[GroupTypeRole] gr ON gr.Id = gm.GroupRoleId
    WHERE gm.PersonId = {0}
    AND gm.InactiveDateTime IS NULL
    AND gm.IsArchived = 0
    AND gt.IsSchedulingEnabled = 1
    AND ( gr.IsLeader = 1
    OR gr.[Name] LIKE '%Coach%' )
    AND g.DisableScheduling = 0", CurrentPerson.Id ).ToList();
...
btnCopyToClipboard.ClientID );
        ...
        private void LoadDropDowns()
        {
            ...
            rlbGroups.DataSource = selectableGroups;
            rlbGroups.DataBind();
        }
        ...
        private List<Group> GetListedGroups()
        {
            ...
            foreach ( string groupName in rlbGroups.SelectedValues )
            {
                foreach ( Group selectableGroup in selectableGroups )
                {
                    if ( selectableGroup.Name == groupName )
                    {
                        selectedGroups.Add( selectableGroup );
                        break;
                    }
                }
            }
        ...
        private void LoadFilterFromUserPreferencesOrURL()
        {
            ...
            if ( this.PageParameter( PageParameterKey.GroupIds ).IsNotNullOrWhiteSpace() || this.PageParameter( PageParameterKey.GroupId ).IsNotNullOrWhiteSpace() )
            {
                ...
                if ( pageParameterGroupId.HasValue )
                {
                     ...
                     */
                    pGroups.Visible= GetAttributeValue( AttributeKey.DisallowGroupSelectionIfSpecified ).AsBoolean() || pageParameterGroupIds.Any();
                    pGroups.InnerText = new GroupService( new RockContext() ).GetNoTracking( (int) pageParameterGroupId )?.Name;
                    rlbGroups.Visible = !GetAttributeValue( AttributeKey.DisallowGroupSelectionIfSpecified ).AsBoolean() || pageParameterGroupIds.Any();
                    btnGroupsGo.Visible = !GetAttributeValue( AttributeKey.DisallowGroupSelectionIfSpecified ).AsBoolean() || pageParameterGroupIds.Any();
            ...
            foreach ( int groupId in pickerGroupIds )
            {
                foreach ( Group selectableGroup in selectableGroups )
                {
                    if ( selectableGroup.Id == groupId )
                    {
                        selectedGroupNames.Add( selectableGroup.Name );
                        break;
                    }
                }
            }
            rlbGroups.SetValues( selectedGroupNames );
        ...
        private void ApplyFilter()
        {
            ...
            this.SetBlockUserPreference( UserPreferenceKey.PickerGroupIds, selectedGroupIds.AsDelimited( "," ), false );
            ...
            else
            {
                ...
                if ( GetAttributeValue( AttributeKey.AllowSchedulingFromOutsideGroup ).AsBoolean() )
                {
                ...
                {
                    schedulerResourceListSourceTypes = new List<GroupSchedulerResourceListSourceType> {
                    GroupSchedulerResourceListSourceType.GroupMembers,
                    GroupSchedulerResourceListSourceType.GroupMatchingPreference,
                    GroupSchedulerResourceListSourceType.GroupMatchingAssignment };
        ...
        private bool ValidateFilter( List<Group> authorizedListedGroups, Group selectedGroup, List<int> scheduleIds, List<Location> listedLocations, List<int> selectedLocationIds )
        {
            ...
            var pickedGroupCount = selectedGroupIds.Count;
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void btnGroupsGo_Click( object sender, EventArgs e )
        {
            UpdatePickedGroups();
        }
        ...
        private void UpdatePickedGroups()
        {
            ...
            var pickedGroupIds = selectedGroupIds;
