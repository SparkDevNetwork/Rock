...
namespace RockWeb.Plugins.org_lakepointe.Core
{
    /// <summary>
    ...
    public partial class DefinedValueList : RockBlock, ISecondaryBlock, ICustomGridColumns
    {
        ...
        {
            // Grid Filter IsActive 
            public const string GridFilter_Active = "Active";


            // Grid Filter Value
            public const string GridFilter_Value = "Value";
        }
        #endregion
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (_definedType != null)
            {
                ...
                gDefinedValues.Actions.ShowAdd = canAddEdit;
                gDefinedValues.IsDeleteEnabled = IsUserAuthorized(Authorization.DELETE) || IsUserAuthorized(Authorization.ADMINISTRATE);
            ...
            ddlActive.Items.Add(new ListItem());
            ddlActive.Items.Add(new ListItem("Yes", "Yes"));
            ddlActive.Items.Add(new ListItem("No", "No"));
        }
        ...
        protected void BindDefinedValuesFilter()
        {
            ddlActive.SetValue(fDefinedValues.GetUserPreference(UserPreferenceKeyBase.GridFilter_Active));
            tbValue.Text = fDefinedValues.GetUserPreference(UserPreferenceKeyBase.GridFilter_Value);
        }
        ...
        protected void BindDefinedValuesGrid()
        {
            if (_definedType != null)
            {
                var qry = new DefinedValueService(new RockContext()).Queryable().Where(a => a.DefinedTypeId == _definedType.Id);

                var isFiltered = false;

                if (ddlActive.SelectedValue.IsNotNullOrWhiteSpace())
                {
                    var isActive = ddlActive.SelectedValue.AsBoolean();
                    qry = qry.Where(a => a.IsActive == isActive);
                    isFiltered = true;
                }

                if (tbValue.Text.IsNotNullOrWhiteSpace())
                {
                    qry = qry.Where(a => a.Value.StartsWith(tbValue.Text));
                    isFiltered = true;
                }

                gDefinedValues.Columns[0].Visible = !isFiltered;

                var result = qry.OrderBy(a => a.Order).ToList();
        ...
        protected void fDefinedValues_DisplayFilterValue(object sender, GridFilter.DisplayFilterValueArgs e)
        {

        }

        protected void fDefinedValues_ClearFilterClick(object sender, EventArgs e)
        {
            fDefinedValues.DeleteUserPreferences();
        ...
        protected void fDefinedValues_ApplyFilterClick(object sender, EventArgs e)
        {
            fDefinedValues.SaveUserPreference(UserPreferenceKeyBase.GridFilter_Active, ddlActive.SelectedValue);
            fDefinedValues.SaveUserPreference(UserPreferenceKeyBase.GridFilter_Value, tbValue.Text);
