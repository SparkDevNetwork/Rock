<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Locations.ascx.cs" Inherits="RockWeb.Plugins.org_lakepointe.CheckIn.Manager.Locations" %>
<style>
    .attendeeIcon {
        margin-left: 10px;
        margin-right: 10px;
...
<script type="text/javascript">
    Sys.Application.add_load(function () {
        ...
        $('.js-checkout').click(function (event) {
            event.stopImmediatePropagation();
            var personName = $(this).closest(".list-group-item").find(".js-checkin-person-name").first().text();
            return Rock.dialogs.confirmPreventOnCancel(event, 'Are you sure you want to check out this person (' + personName + ')?');
        });
    });
    ...
    <ContentTemplate>
                        ...
                        <ItemTemplate>
                            <li id="liNavItem" runat="server" class="list-group-item clickable clearfix">
                                ...
                                <div class="pull-left margin-t-sm">
                                    ...
                                    <asp:Literal ID="lPager" Visible="false" runat="server"></asp:Literal>
                                    <%# Eval("ScheduleGroupNames") %>
                                </div>
                                <span class="pull-right margin-t-sm">
                                    <asp:Literal ID="lStatus" runat="server" />
                                    <asp:LinkButton ID="lbPager" runat="server" CssClass="btn btn-checkinPager" CommandName="AssignPager" CommandArgument='<%# Eval("Id") %>'><i class="far fa-rss"></i> ##PagerText##</asp:LinkButton>
        ...
        <Rock:ModalDialog ID="dlgMoveLocation" runat="server" Title="Select Move Location" OnSaveClick="dlgMoveLocation_SaveClick" OnCancelScript="clearActiveDialog();" ValidationGroup="MoveLocation">
            <Content>
                ...
                <Rock:LocationPicker ID="lpNewLocation" runat="server" ValidationGroup="MoveLocation" Required="true" Label="Selected Location" AllowedPickerModes="Named" Visible="false" />

                <Rock:RockDropDownList ID="rdlNewGroup" runat="server" ValidationGroup="MoveLocation" Label="Selected Group" Visible="false" AutoPostBack="true" OnSelectedIndexChanged="rdlNewGroup_SelectedIndexChanged" />
                <Rock:RockDropDownList ID="rdlGroupLocation" runat="server" ValidationGroup="MoveLocation" Label="Group Location" Visible="false"  />
            </Content>
        </Rock:ModalDialog>
