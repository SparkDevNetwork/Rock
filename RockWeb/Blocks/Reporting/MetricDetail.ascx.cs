...
";
        ...
        protected void btnSave_Click( object sender, EventArgs e )
        {
            ...
            if ( string.IsNullOrEmpty( hfMetricCategoryId.Value ) )
            {
                ...
                {
                    var metricCategory = new MetricCategoryService( RockApp.Current.CreateRockContext() ).Queryable()
        ...
        protected void btnDelete_Click( object sender, EventArgs e )
        {
            ...
            var metricCategory = new MetricCategoryService( RockApp.Current.CreateRockContext() ).Get( hfMetricCategoryId.Value, !disablePredictableIds );
            if ( metricCategory != null )
            {
...
The Lava can include Lava merge fields:";
        ...
        private void BindMetricPartitionsGrid()
        {
            ...
            if ( metric != null )
            {
                nbMetricValuesWarning.Visible = new MetricValueService( RockApp.Current.CreateRockContext() ).Queryable().Where( a => a.MetricId == metric.Id ).Any();
        ...
        private void CreateChart( Metric metric )
        {
            ...
            var legendColor = (rootStyles.getPropertyValue('--color-interface-strong') || '').trim() || '#0a0a0a';
            var gridColor = (rootStyles.getPropertyValue('--color-interface-soft') || '').trim() || '#D9D9E3';

            config.options = config.options || {{}};
            config.options.legend = config.options.legend || {{}};
            config.options.legend.labels = config.options.legend.labels || {{}};
            config.options.legend.labels.fontColor = legendColor;

            if (config.options.scales) {{
                // Time-series factory usually outputs xAxes/yAxes arrays
                if (Array.isArray(config.options.scales.xAxes)) {{
                    config.options.scales.xAxes.forEach(function (ax) {{
                        ax.ticks = ax.ticks || {{}};
                        ax.gridLines = ax.gridLines || {{}};
                        ax.ticks.fontColor = legendColor;
                        ax.gridLines.color = gridColor;
                        ax.gridLines.zeroLineColor = gridColor;
                    }});
                }}
                if (Array.isArray(config.options.scales.yAxes)) {{
                    config.options.scales.yAxes.forEach(function (ax) {{
                        ax.ticks = ax.ticks || {{}};
                        ax.gridLines = ax.gridLines || {{}};
                        ax.ticks.fontColor = legendColor;
                        ax.gridLines.color = gridColor;
            ...
            var barChart = new Chart(barCtx, config);",
        ...
        private static List<MetricValue> GetMetricValues( Metric metric, DateTime? startDate, DateTime? endDate )
        {
            ...
            var metricValuesQry = new MetricValueService( RockApp.Current.CreateRockContext() )
                .Queryable()
        ...
        private void LoadDropDowns()
        {
            ...
            etpMetricPartitionEntityType.EntityTypes = new EntityTypeService( RockApp.Current.CreateRockContext() ).GetEntities().OrderBy( t => t.FriendlyName ).Where( a => a.SingleValueFieldTypeId.HasValue ).ToList();
        ...
        protected void gMetricPartitions_ShowEdit( Guid? metricPartitionGuid )
        {
            ...
            var metricValueService = new MetricValueService( RockApp.Current.CreateRockContext() );
