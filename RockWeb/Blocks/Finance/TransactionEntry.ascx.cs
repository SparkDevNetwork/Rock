                        ...
                        if ( authorizedPersonAlias != null && authorizedPersonAlias.Person != null )
                        {
                            ...
                            else
                            {
                                if ( authorizedPersonAlias != null )
                                {
                                    ...
                                    {
                                        var ccInfo = (CreditCardPaymentInfo)paymentInfo;
                                        savedAccount.NameOnCardEncrypted = Rock.Security.Encryption.EncryptString( ccInfo.NameOnCard );
                                        savedAccount.ExpiryMonthEncrypted = Rock.Security.Encryption.EncryptString( ccInfo.ExpirationDate.Month.ToString() );
                                        savedAccount.ExpiryYearEncrypted = Rock.Security.Encryption.EncryptString( ccInfo.ExpirationDate.Year.ToString() );

                                        var billingAddress = new Location
                                        {
                                            Name = string.Format( "Billing address for {0}", savedAccount.Name ),
                                            Street1 = ccInfo.BillingStreet1,
                                            Street2 = ccInfo.BillingStreet2,
                                            City = ccInfo.City,
                                            State = ccInfo.State,
                                            PostalCode = ccInfo.PostalCode
                                        };

                                        new LocationService( rockContext ).Add( billingAddress );
                                        rockContext.SaveChanges();
                                        savedAccount.BillingLocationId = billingAddress.Id;
                                    }
