            ...
            if ( e.Row.RowType == DataControlRowType.DataRow )
            {
                ...
                if ( _displayValueEdit )
                {
                    ...
                    if ( lValue != null )
                    {
                        ...
                        {
                            lValue.Text = fieldType.Field.FormatValueAsHtml( lValue, attribute.EntityTypeId, _entityId, attributeValue.Value, attribute.QualifierValues, true );
                        }
                        else
                        {
                            lValue.Text = string.Format( "<span class='text-muted'>{0}</span>", fieldType.Field.FormatValueAsHtml( lValue, attribute.EntityTypeId, _entityId, attribute.DefaultValue, attribute.QualifierValues, true ) );
                        }
                    }
                }
                else
