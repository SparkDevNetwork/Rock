...
class Bar {
    ...
    show_popup() {
        ...
        const subtitle = 'Group Role: ' + this.task.Role + '<br>' + start_date + ' - ' + end_date;

        this.gantt.show_popup({
            target_element: this.$bar,
            title: '<a href="/group/' + this.task.GroupId + '">' + this.task.Name + '</a>', //
            subtitle: subtitle
        });
    }
    ...
    compute_y() {
        return (
            ...
            this.task._line * (this.height + this.gantt.options.padding)
        );
    }
...
class Popup {
    ...
    show(options) {
        ...
        this.parent.style.width = 200 + 'px';
...
class Swimlanes {
    constructor(wrapper, groups, options) {
        this.setup_wrapper(wrapper);
        this.setup_options(options);
        this.setup_groups(groups);
        this.setup_tasks(groups);
    ...
    setup_groups(groups) {
        ...
        this.groups = groups.map((group, i) => {
            const tasks = group.StartStopHistory;
            ...
            this.tasks = tasks.map((task, i) => {
                ...
                self._starts.push(startStopOutput);
            });
        });
    }
    ...
    setup_tasks(tasks) {
        ...
        this.tasks = self._starts.map((task, i) => {
            ...
            // swimlanes
            {
                const previousTask = self._starts[i - 1];
                const newGroup =
                    previousTask && task.GroupId !== previousTask.GroupId;
                if (!previousTask) {
                    task._line = 0;
                } else if (previousTask._end <= task._start && !newGroup) {
    ...
    make_grid_background() {
        ...
        $.attr(this.$svg, {
            height: grid_height + 100 + this.options.padding,
            width: '100%'
        });
    }
