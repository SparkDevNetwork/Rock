			...
			funcCalc = function ($fbItem) {
				...
				if($fbItem.hasClass('fluidbox')) {
					...
					function imageProp() {
						...
						if(vpRatio > data.thumbRatio) {
							data.imgScale = $w.height()*settings.viewportFill/data.imgHeight;
						} else {
							data.imgScale = $w.width()*settings.viewportFill/data.imgWidth;
						}
					}
