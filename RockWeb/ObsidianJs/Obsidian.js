 ...
 * import-map. Without this delay logic, there were occassional
...
(async function () {
    ...
    while (!window.System && hasMoreAttempts) {
        ...
        if (hasMoreAttempts) {
            console.log('SystemJS did not resolve. Waiting and then trying again.');
            await sleep();
        }
        else {
            console.log('SystemJS did not resolve. No more attempts will be made.');
        }
    }
    ...
    // Wait for the importmap to be loaded
    let resolvedAddress = '';
    ...
    while (!resolvedAddress && hasMoreAttempts) {
        ...
        try {
            ...
            if (hasMoreAttempts) {
                console.log('Import-Map did not resolve. Waiting and then trying again.');
                await sleep();
            }
            else {
                console.log('Import-Map did not resolve. No more attempts will be made.');
            }
        }
    }
