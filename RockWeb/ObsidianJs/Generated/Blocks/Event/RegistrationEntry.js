System.register(["../../Elements/RockButton.js", "../../Vendor/Vue/vue.js", "../../Store/Index.js", "../../Controls/Loading.js"], function (exports_1, context_1) {
    "use strict";
    var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
        function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
        return new (P || (P = Promise))(function (resolve, reject) {
            function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
            function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
            function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
            step((generator = generator.apply(thisArg, _arguments || [])).next());
        });
    };
    var __generator = (this && this.__generator) || function (thisArg, body) {
        var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
        return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
        function verb(n) { return function (v) { return step([n, v]); }; }
        function step(op) {
            if (f) throw new TypeError("Generator is already executing.");
            while (_) try {
                if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
                if (y = 0, t) op = [op[0] & 2, t.value];
                switch (op[0]) {
                    case 0: case 1: t = op; break;
                    case 4: _.label++; return { value: op[1], done: false };
                    case 5: _.label++; y = op[1]; op = [0]; continue;
                    case 7: op = _.ops.pop(); _.trys.pop(); continue;
                    default:
                        if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                        if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                        if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                        if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                        if (t[2]) _.ops.pop();
                        _.trys.pop(); continue;
                }
                op = body.call(thisArg, _);
            } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
            if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
        }
    };
    var RockButton_js_1, vue_js_1, Index_js_1, Loading_js_1, Step;
    var __moduleName = context_1 && context_1.id;
    return {
        setters: [
            function (RockButton_js_1_1) {
                RockButton_js_1 = RockButton_js_1_1;
            },
            function (vue_js_1_1) {
                vue_js_1 = vue_js_1_1;
            },
            function (Index_js_1_1) {
                Index_js_1 = Index_js_1_1;
            },
            function (Loading_js_1_1) {
                Loading_js_1 = Loading_js_1_1;
            }
        ],
        execute: function () {
            (function (Step) {
                Step[Step["Start"] = 0] = "Start";
            })(Step || (Step = {}));
            exports_1("default", vue_js_1.defineComponent({
                name: 'Event.RegistrationEntry',
                components: {
                    Loading: Loading_js_1.default,
                    RockButton: RockButton_js_1.default
                },
                setup: function () {
                    return {
                        blockSettings: vue_js_1.inject('blockSettings'),
                        blockAction: vue_js_1.inject('blockAction')
                    };
                },
                data: function () {
                    return {
                        isLoading: false,
                        currentStep: Step.Start,
                        instructionsMarkup: ''
                    };
                },
                methods: {
                    goToNext: function () {
                        this.currentStep++;
                    },
                    setLoadingWhileAwaiting: function (callback) {
                        return __awaiter(this, void 0, void 0, function () {
                            var result;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        this.isLoading = true;
                                        return [4 /*yield*/, callback()];
                                    case 1:
                                        result = _a.sent();
                                        this.isLoading = false;
                                        return [2 /*return*/, result];
                                }
                            });
                        });
                    },
                    showStartPanel: function () {
                        return __awaiter(this, void 0, void 0, function () {
                            var startPanelData;
                            var _this = this;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0: return [4 /*yield*/, this.setLoadingWhileAwaiting(function () { return __awaiter(_this, void 0, void 0, function () {
                                            return __generator(this, function (_a) {
                                                switch (_a.label) {
                                                    case 0: return [4 /*yield*/, this.blockAction('GetStartPanelData', {
                                                            slug: this.slug,
                                                            registrationInstanceId: this.registrationInstanceId,
                                                            registrationId: this.registrationId
                                                        })];
                                                    case 1: return [2 /*return*/, _a.sent()];
                                                }
                                            });
                                        }); })];
                                    case 1:
                                        startPanelData = (_a.sent()).data;
                                        if (startPanelData) {
                                            this.instructionsMarkup = startPanelData.InstructionsMarkup;
                                        }
                                        return [2 /*return*/];
                                }
                            });
                        });
                    }
                },
                computed: {
                    slug: function () {
                        return Index_js_1.default.getters.pageParameter('Slug') || null;
                    },
                    registrationInstanceId: function () {
                        return Index_js_1.default.getters.pageParameter('RegistrationInstanceId') || null;
                    },
                    registrationId: function () {
                        return Index_js_1.default.getters.pageParameter('RegistrationId') || null;
                    }
                },
                watch: {
                    currentStep: {
                        immediate: true,
                        handler: function () {
                            return __awaiter(this, void 0, void 0, function () {
                                var _a;
                                return __generator(this, function (_b) {
                                    switch (_b.label) {
                                        case 0:
                                            _a = this.currentStep;
                                            switch (_a) {
                                                case Step.Start: return [3 /*break*/, 1];
                                            }
                                            return [3 /*break*/, 3];
                                        case 1: return [4 /*yield*/, this.showStartPanel()];
                                        case 2:
                                            _b.sent();
                                            return [2 /*return*/];
                                        case 3: throw new Error(this.currentStep + " is not a valid step");
                                    }
                                });
                            });
                        }
                    }
                },
                template: "\n<Loading :isLoading=\"isLoading\">\n    <div v-if=\"currentStep === 0\" class=\"registrationentry-intro\">\n        <div v-if=\"instructionsMarkup\" v-html=\"instructionsMarkup\" class=\"text-left\"></div>\n\n        <!--asp:Panel ID=\"pnlHowMany\" runat=\"server\" Visible=\"false\" CssClass=\"registrationentry-intro\">\n            <h1>How many <asp:Literal ID=\"lRegistrantTerm\" runat=\"server\" /> will you be registering?</h1>\n            <Rock:NumberUpDown ID=\"numHowMany\"  runat=\"server\" CssClass=\"input-lg\" OnNumberUpdated=\"numHowMany_NumberUpdated\"  />\n        </asp:Panel-->\n\n        <div class=\"actions\">\n            <RockButton class=\"btn-primary pull-right\" @click=\"goToNext\">\n                Next\n            </RockButton>\n        </div>\n    </div>\n</Loading>"
            }));
        }
    };
});
//# sourceMappingURL=RegistrationEntry.js.map