...
function setActiveCampus(campusSC, saveCampus) {
  $('.multicampus-item.is-active').removeClass('is-active is-open').find('.multicampus-collapse').addClass('collapse').removeClass('in')
  $('[data-campus='+campusSC+']').addClass('is-active is-open').insertBefore('.multicampus-item:first').find('.multicampus-collapse').addClass('in')
  if (saveCampus) {
...
function findNearestCampus(trigger) {
  ...
  // ga('send', {
  //   hitType: 'event',
  //   eventCategory: 'Christmas Invite Page',
  //   eventAction: 'Find Nearest Campus'
  // });
}
