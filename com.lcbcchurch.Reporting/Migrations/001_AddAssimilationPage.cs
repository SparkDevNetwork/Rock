...
namespace com.lcbcchurch.Reporting.Migrations
{
    ...
    public class AddAssimilationPage : Migration
    {
        public override void Up()
        {
            ...
            RockMigrationHelper.AddBlockAttributeValue( "1D9DAD3B-2276-48A8-AD25-1F04F13C76C3", "71C8BA4E-8EF2-416B-BFE9-D1D88D9AA356", @"-- - First grab everyone for this campus and time period
            ...
            {% endfor %}
            {% if CampusIds != null %}
            Set @CampusIds = '{{CampusIds}}' {% endif %}
            {% if VisitorString != null %}
            Set @AttendanceNumbers = '{{VisitorString}}' {% endif %}
            Declare @EndDateTime datetime = DateAdd( day, 1, [dbo].ufnUtility_GetPreviousSundayDate() )
            Declare @StartDateTime datetime = DateAdd( day, -7, @EndDateTime )  {% if DateRange[0] != null %}
            Set @StartDateTime = '{{ DateRange[0] }} 12:00 AM' {% endif %}
            {% if DateRange[1] != null %}
            Set @EndDateTime = DateAdd( day, 1, '{{ DateRange[1] }} 12:00 AM' ) {% endif %}
            ...
            RockMigrationHelper.AddBlockAttributeValue( "37A9B4BC-FED2-4E09-BECA-2CAB9DE1D248", "CBE194E9-4F10-4505-93DE-31EABA918E85", @"--- First grab everyone for this campus and time period
                ...
                Declare @AttendedNowTable table(
	                PersonId int,
	                Age int,
                ...
                Select	p.Id, attendedTable.AttendanceDateTime, count(0), attendedTable.CampusName
                From Attendance a
                Join AttendanceOccurrence ao on a.OccurrenceId = ao.Id
                Join [Group] g on ao.GroupId = g.Id
                Join GroupType gt on g.GroupTypeId = gt.Id
                Join PersonAlias pa on a.PersonAliasId = pa.Id
                Join Person p on pa.PersonId = p.Id
                Join @AttendedNowTable attendedTable on p.Id = attendedTable.PersonId
                Where gt.AttendanceCountsAsWeekendService = 1
                and attendedTable.AttendanceDateTime >= a.StartDateTime
                Group By p.Id, attendedTable.AttendanceDateTime, CampusName
                ...
                Select	p.Id,
		                ...
		                countTable.AttendanceCount as 'Visit #'
                From @TotalAttendanceCountTable countTable
                Join Person p on countTable.PersonId = p.Id
                Join @AttendedNowTable attendedTable on attendedTable.PersonId = p.Id
                ...
                Where countTable.AttendanceCount in (Select * From ufnUtility_CsvToTable(@AttendanceNumbers))

                Order By LastName, [First Name], [Visit #]" );

            RockMigrationHelper.AddSecurityAuthForPage( "82A4A145-4E72-41DA-AA05-E4B31A3290FF", 0, "View", true, "628C51A8-4613-43ED-A18D-4A6FB999273E", 0, "9ECC858C-331C-469F-86F1-6B512910B5D9" ); // Page:Assimilation Report
            RockMigrationHelper.AddSecurityAuthForPage( "82A4A145-4E72-41DA-AA05-E4B31A3290FF", 1, "View", false, "", 1, "0EFB4D9D-07AB-457D-A889-5ADC110746C3" ); // Page:Assimilation Report
        }
        public override void Down()
        {
            RockMigrationHelper.DeleteSecurityAuth( "0EFB4D9D-07AB-457D-A889-5ADC110746C3" ); // Page:Assimilation Report Group: <all users>
            RockMigrationHelper.DeleteSecurityAuth( "9ECC858C-331C-469F-86F1-6B512910B5D9" ); // Page:Assimilation Report Group: 628C51A8-4613-43ED-A18D-4A6FB999273E ( RSR - Rock Administration ),
