...
// Buyer Beware: This dataview filter is hacked together and it works. It is probably _not_ a best-practice design pattern.
...
namespace org.lakepointe.Reporting.DataFilter.Person
{
    ...
    ///
    /// </summary>
    [Description( "Find people who are referenced in a specific group attribute." )]
    [Export( typeof( DataFilterComponent ) )]
    [ExportMetadata( "ComponentName", "Person Referenced In Group Attribute Filter" )]
    public class PersonInGroupAttributeFilter : EntityFieldFilter
    {
        #region Settings
        ...
        {
            return "Person In Group Attribute";
        }
        ...
        public override string GetSelectedFieldName( string selection )
        {
            ...
            if (!string.IsNullOrWhiteSpace( selection ))
            {
                ...
                var entityField = GetPersonGroupAttributes( settings.GroupTypeId ).FirstOrDefault( f => f.UniqueName == settings.AttributeKey );
                if (entityField != null)
                {
        ...
        public override Control[] CreateChildControls( Type entityType, FilterField filterControl, FilterMode filterMode )
        {
            ...
            groupTypePicker.EnhanceForLongLists = true;
            groupTypePicker.IsSortedByName = true;
            groupTypePicker.GroupTypes = new GroupTypeService( new RockContext() ).Queryable().OrderBy( gt => gt.Name ).ToList();
        ...
        private void EnsureSelectedGroupTypeControls( GroupTypePicker groupTypePicker )
        {
            ...
            var entityFields = GetPersonGroupAttributes( groupTypePicker.SelectedGroupTypeId );
            ...
            if (controls.Length > 0)
            {
                ...
                if (containerControl.Controls.Count >= 1)
                {
                    ...
                    if (containerControl.Controls.Count > 1)
                    {
                        ...
                        var entityFields = GetPersonGroupAttributes( groupTypeId );
        ...
        public override Expression GetExpression( Type entityType, IService serviceInstance, ParameterExpression parameterExpression, string selection )
        {
            ...
            {
                var entityFields = GetPersonGroupAttributes( settings.GroupTypeId );
                var entityField = entityFields.FindFromFilterSelection( settings.AttributeKey );
                if ( entityField != null && entityField.AttributeGuid.HasValue )
                {
                    // Performance could probably be improved by using linq expressions

                    var rockContext = serviceInstance.Context as RockContext;
                    var groupIds = new GroupService( rockContext ).Queryable()
                        .Where( g => g.GroupTypeId == settings.GroupTypeId && g.IsActive && !g.IsArchived )
                        .Select( g => g.Id );

                    var attributeId = AttributeCache.Get( entityField.AttributeGuid.Value ).Id;
                    var personIds = new AttributeValueService( rockContext ).Queryable()
                        .Where( av => av.AttributeId == attributeId && av.EntityId.HasValue && groupIds.Contains( av.EntityId.Value ) )
        ...
        /// <returns></returns>
        private List<EntityField> GetPersonGroupAttributes( int? groupTypeId )
        {
            List<EntityField> entityAttributeFields = new List<EntityField>();

            var fakeGroup = new Rock.Model.Group();
            fakeGroup.GroupTypeId = groupTypeId ?? 0;

            Rock.Attribute.Helper.LoadAttributes( fakeGroup );
            var attributeList = fakeGroup.Attributes
                .Where( a => a.Value.FieldTypeId == 18 ) // Person fields
