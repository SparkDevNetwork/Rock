...
internal class LavaEndpointDefinition
{
    ...
    /// Custom Component expects, and an existing endpoint keeps its stored
    /// content type.
    /// </summary>
    [Description( "The MIME content type of the endpoint's response. Defaults to application/json on create, which is what an authored Custom Component expects. Omit when updating to leave the stored content type unchanged. Only set this when the endpoint deliberately returns something other than JSON." )]
