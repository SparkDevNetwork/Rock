...
internal class ChatAgentImplementation : ChatAgent
{
    ...
    private static readonly string RawCoreSystemPrompt = """
        <system>
            <purposes>
    ...
    /// <inheritdoc/>
    public override Task StartNewSessionAsync( SessionType sessionType, CancellationToken cancellationToken )
    {
    ...
    async private Task SummarizeChatHistoryAsync( CancellationToken cancellationToken )
    {
        ...
        var result = await chat.GetChatMessageContentAsync(
            ...
            executionSettings: _agentConfiguration.Provider.GetChatCompletionPromptExecutionSettings( _context, ReasoningEffort.Low ),
            kernel: _kernel
        );
    ...
    public override async Task<ChatMessageResponse> GetChatMessageResponseAsync( CancellationToken cancellationToken )
    {
        ...
        try
        {
            result = await chat.GetChatMessageContentAsync(
                history,
                executionSettings: _agentConfiguration.Provider.GetChatCompletionPromptExecutionSettings( _context, _agentConfiguration.ReasoningEffort ),
                kernel: _kernel );
        }
        catch ( Exception ex )
        {
