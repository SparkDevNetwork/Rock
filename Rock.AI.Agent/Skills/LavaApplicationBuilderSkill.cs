...
[AgentUsage( "Create one application per block, named after the feature, and group all of the block's endpoints under it by passing the same applicationSlug each time. Use GetLavaApplication to see what an application already contains before adding to it." )]
...
[AgentUsage( "The endpoint runs as whoever views the page, not as you. Write the template for the least-privileged viewer. These tools create applications and endpoints but write no authorization rules; a new application answers 401 to everyone except Rock Administrators and Lava Application Developers until you secure it with the Core Administration skill's AddOrUpdateAuthorizationForEntity. Endpoint write modes and page or block security remain separate decisions." )]
[AgentUsage( "The audience is a security decision. When the user describes who the page is for in plain words ('the worship team leaders', 'staff and campus pastors'), call ResolveAudience with that description to map it onto security roles (each with its groupIdKey) or the AllUsers and AllAuthenticatedUsers special roles, confirm any match scored under 50 with the user, then grant them with AddOrUpdateAuthorizationForEntity. Never allow the AllUsers special role on the user's behalf." )]
[AgentUsage( "Security is set at two levels. The application's ExecuteView rules govern every endpoint left in ApplicationView mode, which is the default and right for most read endpoints: allow the intended roles then deny AllUsers on the application (entityTypeIdKey and idKey from AddOrUpdateLavaApplication or GetLavaApplication). When one endpoint must be restricted to fewer people than the application (a write endpoint only leaders may call), set that endpoint's securityMode to EndpointExecute and grant Execute on the endpoint itself (its entityTypeIdKey and endpointIdKey) to those roles, then deny AllUsers." )]
...
[AgentUsage( "Before writing any endpoint template, follow the coding guide route the Community Knowledge Base skill describes and retrieve only the material it assigns for a read or a write endpoint." )]
[AgentUsage( "Before authoring a modify block, follow the write procedure the coding guide route assigns and read the target entity's article in the version-matching data-model topic, then make a property evidence list with every property to be written, its exact case-sensitive schema name, whether it is required or nullable, and its server-side or user-input value source. Do not call AddOrUpdateLavaEndpoint while any proposed property is absent from that evidence. SearchKnowledge is not authoritative evidence for entity property names. The absence of a dedicated entity recipe or exhaustive contract is not a blocker; resolve domain defaults, save behavior, picker boundaries, and authorization through the sources the route assigns." )]
