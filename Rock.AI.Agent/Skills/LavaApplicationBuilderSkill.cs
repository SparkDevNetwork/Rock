...
[AgentUsage( "The endpoint runs as whoever views the page, not as you. Write the template for the least-privileged viewer. AddOrUpdateLavaApplication requires one or more audiences when creating an application and configures its read access from them; endpoint write modes and page or block security remain separate decisions." )]
[AgentUsage( "The audience is a security decision. When the user describes who the page is for in plain words ('the worship team leaders', 'staff and campus pastors'), call ResolveAudience with that description to map it onto 'Public', 'AllAuthenticatedPeople', or exact security role names, confirm any match scored under 50 with the user, then pass the chosen values to AddOrUpdateLavaApplication. Never choose 'Public' on the user's behalf." )]
[AgentUsage( "Security is set at two levels. The application's audiences govern every endpoint left in ApplicationView mode, which is the default and right for most read endpoints. When one endpoint must be restricted to fewer people than the application (a write endpoint only leaders may call), pass definition.audiences on that endpoint's AddOrUpdateLavaEndpoint call; it then answers for itself in EndpointExecute mode with its own Execute rules." )]
