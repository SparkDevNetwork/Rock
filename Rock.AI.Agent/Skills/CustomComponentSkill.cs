...
/*
    ...
    Control discovery is delegated to the Rock knowledge base
    (knowledge.rockrms.com) rather than reimplemented here. That service already
    indexes every Framework Controls .obs file with a semantic description, per
    release version scoping, and a raw-source URL, which is the search-then-fetch
    shape an agent needs and is expensive to reproduce. This is instruction-level
    composition across two MCP servers, not an integration: Rock cannot see or
    verify the knowledge base's tools, so the usages below name tools that only
    resolve when the client has both servers connected, and the fallback is
    stated explicitly so an agent without it says so rather than inventing
    control APIs.
...
[AgentUsage( "Use to read or replace the authored source of a Custom Component block the user is building. The block must already exist on a page; identify it by its block id." )]
[AgentUsage( "When the component needs data, create a Lava endpoint with the Lava Application skill's AddOrUpdateLavaEndpoint tool. Do not search Rock for an existing REST endpoint: writing Lava lets you return exactly the shape the component renders, with permissions decided when the endpoint is created." )]
[AgentUsage( "Group all of one block's endpoints under a single Lava application named after the dashboard, by passing the same applicationSlug to every AddOrUpdateLavaEndpoint call. Security and configuration rigging are then set once for the whole block." )]
