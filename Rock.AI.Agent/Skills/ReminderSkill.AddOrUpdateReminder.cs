...
internal sealed partial class ReminderSkill
{
    ...
    public AgentToolResult AddOrUpdateReminder(
        ...
        string assignToPersonIdKey = null,
        string note = null,
        DateTime? date = null,
        ...
        int? repeatAmount = null )
    {
        ...
        if ( reminderIdKey.IsNotNullOrWhiteSpace() )
        {
            ...
            if ( reminder.PersonAlias.PersonId != AgentRequestContext.CurrentPerson?.Id )
            {
                helper.AddError( "You can only update reminders that are assigned to you." );
            }
        }
        else
        {
            ...
            if ( assignToPersonIdKey.IsNotNullOrWhiteSpace() )
            {
                person = helper.GetRequiredEntity<Model.Person>( assignToPersonIdKey, checkSecurity: false );
            }
            else
            {
                person = AgentRequestContext.CurrentPerson;

                if ( person == null )
                {
                    helper.AddError( "You must be logged in to add a reminder for yourself." );
                }
            }

            // If person is null, then the error will be handled by the helper.
            reminder.PersonAliasId = person?.PrimaryAliasId.Value ?? 0;
            ...
            {
                helper.AddError( $"You must specify {nameof( entityIdKey )} when adding a reminder." );
            }
        }
        ...
        // Only use the date portion of the date parameter, since the time
        // portion is not relevant for reminders.
        date = date?.Date;
