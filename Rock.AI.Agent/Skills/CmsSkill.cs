...
[AgentUsage( "When the block type is not established by the user's request or by the agent's workflow, ask which one, then resolve it with ListBlockTypes." )]
...
[AgentUsage( "The mutating tools change site structure. Before creating, make sure the parent page, page name, block type, and zone are established, either by the user's explicit input or by a more specific agent workflow that names its defaults. Do not ask again for a decision that is already established." )]
