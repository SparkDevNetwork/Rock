...
[AgentUsage( "Use to read or replace the authored source of a Forge Content block the user is building. The block must already exist on a page; identify it by the IdKey returned by AddOrUpdateBlock or GetPage. To create one, use the Cms skill: resolve the block type with ListBlockTypes, place it with AddOrUpdateBlock, then author it here with AddOrUpdateForgeContent." )]
...
[AgentUsage( "Group all of one block's endpoints under that single Lava application, named after the feature, by passing the same applicationSlug to every AddOrUpdateLavaEndpoint call. Security and configuration rigging are then set once for the whole block." )]
[AgentUsage( "To call an endpoint from the component, follow the Lava Application Builder skill's useLavaApp guidance. Before reporting the integration as verified, compare every invoke call with the saved endpoint: application slug, endpoint slug, HTTP method, parameter location and names, and response shape must all match." )]
[AgentUsage( "Before writing a component, follow the coding guide route the Community Knowledge Base skill describes, and retrieve only the material it assigns. For control, Grid, field-type, configuration, or value-shape questions, verify only the focused Reference articles the route names." )]
[AgentUsage( "For a control or API the guide does not cover, find it with the Community Knowledge Base skill's SearchCode tool (sourceType 'obs') by concept, for example 'person picker' or 'grid with columns', then read its defineProps block with GetCodeLines for the smallest range that answers the question. Use GetCodeFile only when the whole file is genuinely needed. The defineProps block is the authoritative list of props, their types, and their defaults. Never infer a control's props from its name or from a different control." )]
