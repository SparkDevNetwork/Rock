...
internal sealed partial class ReminderSkill
{
    ...
    public AgentToolResult DeleteReminder( string reminderIdKey )
    {
        ...
        var currentPersonId = AgentRequestContext.CurrentPerson?.Id;

        var reminder = helper.GetRequiredEntity<Reminder>( reminderIdKey );

        if ( reminder.PersonAlias.PersonId != currentPersonId )
        {
            helper.AddError( "You can only delete your own reminders." );
        }
