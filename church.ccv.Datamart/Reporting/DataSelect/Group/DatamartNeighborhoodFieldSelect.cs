...
namespace church.ccv.Datamart.Reporting.DataSelect.Group
{
    /// <summary>
    /// 
    /// </summary>
    [Description( "Select a field for the group using the value from Datamart Neighborhood " )]
    [Export( typeof( DataSelectComponent ) )]
    [ExportMetadata( "ComponentName", "Select a field value from Datamart Neighborhood" )]
    public class DatamartNeighborhoodFieldSelect : DataSelectComponent
    {
        /// <summary>
        ...
        public override string ColumnPropertyName
        {
            get
            {
                return "DatamartNeighborhoodField";
            }
        }
        ...
        {
            var datamartParameterExpression = Expression.Parameter( typeof( DatamartNeighborhood ), "d" );
        ...
        public override System.Web.UI.Control[] CreateChildControls( System.Web.UI.Control parentControl )
        {
            ...
            var datamartFields = EntityHelper.GetEntityFields( typeof( DatamartNeighborhood ) ).Where( a => a.FieldKind == FieldKind.Property ).OrderBy( a => a.Name ).ToList();
        ...
        {
            var datamartNeighborhoodService = new Service<DatamartNeighborhood>( context );
            var groupService = new GroupService( context );
            var qryDatamartNeighborhood = datamartNeighborhoodService.Queryable().Where( a => a.NeighborhoodId.HasValue );
            var qryGroup = groupService.Queryable();
            ...
            {
                var projectionLamba = Expression.Lambda<Func<DatamartNeighborhood, bool>>( datamartMember, new ParameterExpression[] { datamartParameterExpression } );
                qryResult = qryGroup
                    .Select( p => qryDatamartNeighborhood.Where( d => d.NeighborhoodId == p.Id )
            ...
            {
                var projectionLamba = Expression.Lambda<Func<DatamartNeighborhood, DateTime?>>( datamartMember, new ParameterExpression[] { datamartParameterExpression } );
                qryResult = qryGroup
                    .Select( p => qryDatamartNeighborhood.Where( d => d.NeighborhoodId == p.Id )
                      .Select( projectionLamba ).FirstOrDefault() );
            }
            ...
            {
                var projectionLamba = Expression.Lambda<Func<DatamartNeighborhood, decimal?>>( datamartMember, new ParameterExpression[] { datamartParameterExpression } );
                qryResult = qryGroup
                    .Select( p => qryDatamartNeighborhood.Where( d => d.NeighborhoodId == p.Id )
                      .Select( projectionLamba ).FirstOrDefault() );
            }
            ...
            {
                var projectionLamba = Expression.Lambda<Func<DatamartNeighborhood, int?>>( datamartMember, new ParameterExpression[] { datamartParameterExpression } );
                qryResult = qryGroup
                    .Select( p => qryDatamartNeighborhood.Where( d => d.NeighborhoodId == p.Id )
                      .Select( projectionLamba ).FirstOrDefault() );
            }
            ...
            {
                var projectionLamba = Expression.Lambda<Func<DatamartNeighborhood, string>>( datamartMember, new ParameterExpression[] { datamartParameterExpression } );
                qryResult = qryGroup
                    .Select( p => qryDatamartNeighborhood.Where( d => d.NeighborhoodId == p.Id )
                      .Select( projectionLamba ).FirstOrDefault() );
            }
