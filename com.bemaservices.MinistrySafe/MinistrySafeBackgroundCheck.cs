...
namespace com.bemaservices.MinistrySafe
{
    ...
    public class MinistrySafeBackgroundCheck : BackgroundCheckComponent
    {
                    ...
                    AttributeCache billingCodeAttribute, out List<string> errorMessages )
        {
            ...
            try
            {
                ...
                lock ( lockObject )
                {
                    ...
                    if ( !CreateTraining( userId, level, packageCode, errorMessages ) )
                    {
        ...
        private static void UpdateWorkflow( int id, string recommendation, string documentId, string reportStatus, RockContext rockContext )
        {
            ...
            lock ( lockObject )
            {
                ...
                if ( workflow != null && workflow.IsActive )
                {
                    ...
                    {
                        int entityTypeId = EntityTypeCache.Get( typeof( MinistrySafeBackgroundCheck ) ).Id;
        ...
        private static bool UpdateBackgroundCheckAndWorkFlow( BackgroundCheckWebhook backgroundCheckWebhook )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( backgroundCheck.WorkflowId.HasValue && backgroundCheck.WorkflowId > 0 )
                {
                    ...
                    UpdateWorkflow( backgroundCheck.WorkflowId.Value, recommendation, backgroundCheckWebhook.ResultsUrl, reportStatus, rockContext );
                }
            }
        ...
        /// <returns>True/False value of whether the request was successfully sent or not.</returns>
        public static bool CreateTraining( string userId, string level, string packageCode, List<string> errorMessages )
        {
