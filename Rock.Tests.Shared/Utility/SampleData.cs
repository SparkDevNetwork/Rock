...
namespace Rock.Tests.Shared.Utility
{
    public class SampleData
    {
        ...
        private static int _currentPersonAliasId = new PersonService( new RockContext() ).Queryable().AsNoTracking().FirstOrDefault().PrimaryAliasId ?? 0;
        ...
        public bool Load( string xmlFileUrl )
        {
            var isSampleDataLoaded = !string.IsNullOrEmpty( Web.SystemSettings.GetValue( SystemKey.SystemSetting.SAMPLEDATA_DATE ) );
        ...
        private void ProcessXml( string sampleXmlFile )
        {
            ...
            var smallGroupTopicDefinedType = DefinedTypeCache.Get( Rock.SystemGuid.DefinedType.SMALL_GROUP_TOPIC.AsGuid() );
            ...
            try
            {
                ...
                foreach ( var elemGroup in elemGroups.Elements( "group" ) )
                {
                    if ( elemGroup.Attribute( "studyTopic" ) != null )
                    {
                        ...
                        var smallGroupTopicDefinedValue = smallGroupTopicDefinedType.DefinedValues.FirstOrDefault( a => a.Value == topic );

                        // add it as new if we didn't find it.
                        if ( smallGroupTopicDefinedValue == null )
                        {
                            smallGroupTopicDefinedValue = AddDefinedTypeValue( topic, smallGroupTopicDefinedType );
                        }
                    }
                }
                ...
                // Clear the objects that contains all security roles and auth rules (so that it will be refreshed)
        ...
        private List<GroupMember> BuildFamilyMembersFromXml( XElement elemMembers, RockContext rockContext )
        {
            ...
            foreach ( var personElem in elemMembers.Elements( "person" ) )
            {
                ...
                {
                    person = new Person();
                    person.CreatedByPersonAliasId = _currentPersonAliasId;
