...
- [ ] **SignalR person resolution (the original bug):** connect a real-time hub from a browser authenticated with a new-format `.ROCK` cookie; the hub resolves the current person (a `rock:person` claim is present and person-channel grouping works). This is the OWIN path that never reaches `Application_PostMapRequestHandler`.
- [ ] **SignalR anonymous:** an anonymous browser's hub connection proceeds with no current person and no error.
- [ ] **File upload while impersonating / user-token:** during an `Impersonation` or `UserToken` session (no `UserLogin`), upload via `FileUploader.ashx`; the upload is authorized (previously denied, treated as anonymous because no principal was set).
- [ ] **File upload anonymous still rejected:** an anonymous request to `FileUploader.ashx` is still denied.
- [ ] **API-key file upload (behavior change):** an `FileUploader.ashx` upload authenticated only by an API key (`Authorization-Token` header or `?apikey=`) is authorized as the key's person and stamps `CreatedByPersonAliasId` to that person. This is an intentional change — the handler now establishes the API-key identity on `RockRequestContext` / `HttpContext.Items` rather than on `Context.User`, so a binary-file upload that previously authorized and attributed as *anonymous* now runs as the key's user. Verify a binary-file upload the key's person is NOT authorized to `EDIT` is correctly denied.
- [ ] **REST cookie auth while impersonating / user-token:** a cookie-authenticated REST / Obsidian block action during an impersonation or user-token session authenticates as the effective person (previously anonymous via the `Thread.CurrentPrincipal` path).
- [ ] **REST cookie auth normal (regression):** a standard `Component`-session cookie REST call still authenticates after the `AuthenticateAttribute` cookie-path conversion.
- [ ] **OIDC authorize (OWIN):** the OIDC authorize endpoint resolves the current person for a logged-in user (another OWIN endpoint that bypasses `PostMapRequestHandler`).
- [ ] **Anonymous / static assets (no regression):** an anonymous page load and its static assets do NOT trigger a person-session DB lookup (lazy resolution fires only when identity is read); confirm no per-asset session queries.
- [ ] **`Context.User` plugin compatibility:** on a managed request for a `UserLogin`-backed session, `HttpContext.Current.User.Identity.Name` is still the username (the eager touch remains for plugins). For an impersonation / user-token session, `Context.User` is intentionally anonymous — confirm core features still work because they read `RockRequestContext.CurrentPerson`, not the principal.
- [ ] **Legacy cookie via lazy factory (regression):** a pre-deployment legacy `.ROCK` cookie still upgrades transparently through the lazy resolution path.
- [ ] **Request-context re-seat under async / load (regression):** a request whose handler awaits a real async operation (or the site under enough concurrent load to force thread switching between pipeline stages) completes without a "no RockRequestContext was attached" exception, and `CurrentPerson` still resolves in code that runs after the await. This exercises the `OnExecuteRequestStep` re-seat that flows the request-context accessor across ASP.NET pipeline stages. Reproduce as during development: a request that forces an async continuation (e.g. the query-string-driven repro) previously failed intermittently.
- [ ] **Migrated current-person readers under impersonation:** while impersonating (a session with `CurrentPerson` but no `UserLogin`), confirm the readers moved off `HttpContext.Items` onto `RockRequestContext` resolve the *impersonated* person, not anonymous: the "Set Attribute to Current Person" workflow action sets its attribute to the impersonated person, and a picker that defaults to / secures by the current person (e.g. the Data Views or System Phone Number picker) scopes to the impersonated person. Normal logged-in and anonymous cases are regression-covered by the audit-stamping and Lava items in the identity-resolution section above.
