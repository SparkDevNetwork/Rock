...
- [ ] Logout also clears the unsecured check-in person-identifier cookie (`.ROCK`-adjacent self-identification cookie), so a subsequent check-in self-service flow no longer recognizes the signed-out person.
