...
- [ ] **(Investigation) SaveChanges audit attribution from a SignalR hub call.** Determine how `DbContext.GetCurrentPersonAliasId` resolves the current person during a **SignalR hub method** invocation (not the initial connect), both pre-person-session and after these changes. Working assumption: during an established hub call there is no `HttpContext.Current` (so no `Items["CurrentPerson"]`) and the `RockRequestContextAccessor.Current` AsyncLocal is not set — the connecting request's `RockRequestContext` is captured into RealTime **connection state** (`AspNetEngine.ClientConnectedAsync` → `state.Request`), not the ambient accessor — so audit stamps as **anonymous** in both eras. Confirm, and decide whether hub-initiated saves *should* attribute to the connected person by flowing the connection-state `RockRequestContext` (or its person) into the ambient accessor for the duration of the hub call.
