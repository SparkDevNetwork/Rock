...
> **Post-implementation correction (as built):** new-format cookie *resolution* was later moved out of `Application_BeginRequest` and into `Application_PostMapRequestHandler` so that natively served static files (which map to a null handler) do NOT incur a per-asset database session lookup (an authenticated browser sends the `.ROCK` cookie on every one of the 70+ static assets in a page load). `Application_BeginRequest` still runs the legacy kill-switch, thread culture, and `RockRequestContext` attach; it no longer calls `ResolveSessionForRequest`. The `Deliverables` narrative below is preserved as the original phase intent; read it together with the spec's "Auth pipeline integration during the dual-reader window" (Design), which describes the shipped `PostMapRequestHandler` design. The identity population (`Context.Items["CurrentPerson"]` / `SetCurrentIdentity`) and the locked-out / unconfirmed eviction also moved as part of the "Identity-resolution refactor" (see Phase 18).
...
- **Pipeline correctness** is validated by the Phase 18 manual checklist (login, kill-switch, tampered-cookie behavior, anonymous request, reissue at half-life) plus code review of the shim.
...
### Tests
All tests target `PersonSessionService.ImpersonatePerson` directly with a mocked `RockRequestContext`. The block's shim is untested (same trade-off as Phase 5/6); the "no `rckipid` in URL" property is guaranteed by construction (no code path appends it) and observed manually in Phase 18.
...
- Static check (not a runtime test): `MarkOnlineUsersOffline()` no longer exists or has zero call sites, grep the codebase to confirm. (Booting `Global.asax` startup inside Rock.Tests is not feasible; see Guardrails. The cleanup is verified by absence at compile time and by the "App pool recycle does NOT mark all users offline" item in the Phase 18 manual checklist.)
- Mocked-database: a logout via `PersonSessionService.SignOut( requestContext )` marks the current `PersonSession` inactive (verify `IsActive == false` AND `InactiveDateTime != null`), clears the `.ROCK` cookie (verify via `TrackingResponseContext.RemovedCookies` like the existing Phase 6 legacy-upgrade tests do), and clears `requestContext.PersonSession`. New file: `Rock.Tests/Security/PersonSessionServiceSignOutTests.cs` (or fold into an existing file; the "Phase 18 follow-ups" already note that file consolidation is a pre-merge sweep).
...
## Phase 17, Lazy identity resolution and principal decoupling

### Goal
Make `RockRequestContext.CurrentPerson` / `CurrentUser` / `PersonSession` resolve **lazily** via a shared factory installed at `Application_BeginRequest`, so identity resolves for OWIN-terminated requests (SignalR, OIDC, plugin OWIN endpoints) that never reach `Application_PostMapRequestHandler`. Migrate the remaining core principal readers off `Context.User` so the eager principal population becomes legacy plugin-compat. See the spec's "Identity-resolution model" subsection (**Lazy, pipeline-agnostic resolution** and **The principal (`Context.User`) is a managed-pipeline convenience**) for the design decision and the audit results that back it.

> This phase was added after the original phase sequence, prompted by the SignalR bug where an OWIN request's `RockRequestContext.CurrentPerson` was never resolved. It is sequenced **before** manual testing because the Phase 18 "OWIN endpoints and lazy identity resolution" manual checklist below cannot be completed until this work lands.

### Deliverables (do in order; build + verify at each step — move slowly to avoid regressions)
- [x] **Convert `FileUploader.ashx`** (`RockWeb/App_Code/FileUploader.ashx.cs`) from `context.User.Identity.IsAuthenticated` to `RockRequestContext.CurrentPerson != null`. DONE (commit `5226af35b2`); `context.User` fully removed, and the inline API-key path now sets the identity on `RockRequestContext` + `HttpContext.Items` instead of the principal. Fixes the impersonation / user-token upload gap. Manual verification: Phase 18 "API-key file upload" item.
- [x] **Convert the REST cookie path** (`AuthenticateAttribute.TryAuthenticateFromCurrentPrincipal`, `Rock.Rest/Filters/AuthenticateAttribute.cs`). DONE (commit `66bb614814`); detects the session via `RockRequestContext.PersonSession` and forwards a Web API principal only for `UserLogin`-backed sessions (impersonation / user-token authenticate at the Rock layer via `CurrentPerson`). `AuthenticateAttributeTests` green. `TryAuthenticateFromOidcToken` left untouched (genuine OAuth bearer identity).
- [x] **Shared-lazy identity factory on `RockRequestContext`.** Added `SetPersonSessionFactory( Func<PersonSession> )`; `PersonSession` / `CurrentPerson` / `CurrentUser` resolve once from it on first read (person/user derived from the session); explicit `SetCurrentIdentity` / `SetPersonSession` still win (precedence). Thread-safe (`Lazy` execution-and-publication).
- [x] **`ResolveSessionForRequest` — best-effort reissue.** The factory calls the **full** `ResolveSessionForRequest` (decided after review, so OWIN both resolves *and* invalidates, and managed requests do not double-load). Its cookie reissue is **best-effort**: a `SetAuthCookie` failure (a late identity read after the response started, or an OWIN response that cannot take a `Set-Cookie`) is caught and logged, and the resolved session is still returned. (An interim commit split this into a read-only `EvaluateSessionForRequest` core + a read-only public wrapper for the factory; both were removed once the factory switched to the full resolver, leaving `ResolveSessionForRequest` a single linear method with one caller for the evaluation logic.)
- [x] **Install the factory in `Application_BeginRequest`** (runs for every request, managed and OWIN). Done: BeginRequest installs `SetPersonSessionFactory(() => ResolveSessionForRequest(ctx))`. `PostMapRequestHandler` no longer resolves separately - it reads `PersonSession` (triggering the factory, or returning the explicit legacy-upgrade session) and applies only the managed-only side effects (`Context.User`, `HttpContext.Items`). Reissue / invalidation live inside the factory's resolution.
- [x] **Migrate `HttpContext.Items["CurrentPerson"/"User"]` readers to `RockRequestContext`.** DONE, 20 files. Every core reader now reads `Rock.Net.RockRequestContextAccessor.Current?.CurrentPerson` **inline**; the `Items` writes are kept for plugin compat. `DbContext.GetCurrentPersonAliasId` reads RockRequestContext-first and keeps `Items` only as a last-resort fallback (with its disposal-race try/catch). Full Rock.Tests suite green (3170). Original planned text follows. Point the ~20 core reader sites at `RockRequestContextAccessor.Current?.CurrentPerson` **inline** (not a shared helper — decided), so `RockRequestContext` is the single read source and no two readers can disagree about who is logged in. This is required *because of* this changeset: once identity resolves lazily through `RockRequestContext` and OWIN requests never write `Items`, the two stores can diverge, so leaving `Items` readers in place would ship a new "who's logged in?" bug. **Keep the `Items` writes** (`PostMapRequestHandler`, `FileUploader`) as a write-only backward-compat shim for plugins during the deprecation window (few plugins likely read it directly). Sequence **after** the factory lands so `RockRequestContext` is authoritative for every request type first. Reader sites: audit `DbContext.GetCurrentPersonAliasId` (highest stakes — mis-attribution risk, test hardest; flip its Items-first branch to RockRequestContext-first, keep the disposal-race try/catch); Lava (`RenderLavaEndpoint`, `RockEntity{,Delete,Modify}Block`, `LavaFilters`, `LavaHelper`, `GroupFinder`/`MediaPlayer`/`ScheduledContent` shortcodes); workflow (`SetAttributeToCurrentPerson`, `Workflow.WebForms`); communication (`MediumComponent`, `SendLearningNotifications` — also run in background jobs where null is correct); reporting/UI (`PhoneNumberSelect`, `DataViews`/`SystemPhoneNumber(s)` pickers); entity WebForms partials (`Note.WebForms`, `GroupMemberService.WebForms`). The `RockRequestContextAccessor.Current` AsyncLocal is reliable here thanks to the committed pipeline-step re-seat fix.
- [x] **Fix `RegisterSignalRClaims`** (`Rock/RealTime/AspNet/AspNetEngineStartup.cs`). DONE: removed the `Identity.Name` gate; the `rock:person` claim is added from `RockRequestContext.CurrentPerson` (resolved via the factory for OWIN), falling back to the visitor cookie when there is no person. `ClientConnectedAsync` already reads `CurrentPerson` directly, so the factory populating it is sufficient there.
- [x] **Deprecation note on the eager touch.** DONE. Added an engineering note above the `Context.User` write in `Application_PostMapRequestHandler` (and a short pointer at the matching write in `Application_PostAuthenticateRequest`) marking it legacy plugin-compatibility, explaining that core no longer reads the principal (RockPage proxies `RockRequestContext`; REST cookie path, FileUploader, and the audit/Lava/workflow readers all migrated) and that it is incomplete for Impersonation/UserToken sessions, with a future-major-version removal target. Comment-only; the spec's "The principal (`Context.User`) is a managed-pipeline convenience" note is the design record.

### Tests
Automated coverage is bounded by the WebForms/OWIN pipeline (see Guardrails). Rely on the `RockRequestContextTests` factory-precedence tests, the `PersonSessionServiceTests` `ResolveSessionForRequest` lifecycle + best-effort-reissue tests, `AuthenticateAttributeTests` for the REST conversion, the full Rock.Tests suite as the regression guard for the `Items` reader migration, and the Phase 18 "OWIN endpoints and lazy identity resolution" manual checklist for the pipeline / SignalR behavior that cannot run in the harness.

### Verification
- `build` clean at each deliverable.
- Phase 18 OWIN / lazy-identity manual items exercised, with explicit attention to the impersonation / user-token edge cases.

### Spec references
- "Design / `RockRequestContext` integration / Identity-resolution model" (Lazy, pipeline-agnostic resolution; The principal is a managed-pipeline convenience)

---

## Phase 18, Manual testing
...
- [x] Standard login with username and password lands the user on the post-login page; the `.ROCK` cookie is new-format; a `PersonSession` row exists with `CreationSource = Component`.
- [x] "Remember me" checked → cookie persists across browser restart; same `PersonSession` is reused on first request post-restart.
...
- [x] Determine if any mocked-db tests should be promoted to full integration. Some tests in `Rock.Tests/Security/PersonSessionServiceTests.cs` use a generic `try { ... } catch { }` around the create / save leg and treat any exception as "the find leg returned null" (e.g. the orphan-not-resurrected and expired-not-reused tests). Decide whether these should move to `Rock.Tests.Integration/Security/PersonSessionTests.cs` so the create / save leg actually runs and the assertion is positive rather than absence-of-the-wrong-result. DONE (actual files are under `Model/`, not `Security/`). The bare `catch` swallowed even `AssertFailedException`, so five tests could never fail. All five were removed from `Rock.Tests/Model/PersonSessionServiceTests.cs` and re-created as real integration tests in `Rock.Tests.Integration/Model/PersonSessionTests.cs` (new region "FindOrCreate reuse / isolation invariants"), each asserting positively against the real save pipeline and filtered unique indexes: `FindOrCreateApiKeySession_OrphanedSession_IsNotResurrected`, `FindOrCreateApiKeySession_ExpiredButActiveSession_IsNotReused`, `FindOrCreateDeviceComponentSession_FreshLogin_DoesNotReuseAnotherClientSession`, `FindOrCreateDeviceComponentSession_DifferentUserLoginOnRequest_MarksPriorInactiveAndCreatesNew`, `FindOrCreateDeviceComponentSession_ExpiredButActiveSession_IsNotReused`. NOTE on the ApiKey-expired case: with an expired-but-active ApiKey row present, the filtered unique index `IX_ApiKey_UserLoginId` blocks inserting a replacement, so `FindOrCreateApiKeySession` returns null (a latent limitation, but the state is unreachable today since ApiKey sessions never get an ExpiresDateTime); the integration test therefore asserts the truthful invariant "never returns the expired row" (`resolved == null || resolved.Id != expired.Id`) and documents that a future expirable-ApiKey change must add deactivate-then-recreate. Good mocked-db unit tests that exercise the reuse/early-return path (no save leg) were kept: `FindOrCreateApiKeySession_SecondCall_ReturnsExistingActiveSession`, `FindOrCreateDeviceComponentSession_SameUserLoginOnRequest_LeavesPriorActive`, `FindOrCreateDeviceComponentSession_LegacyPriorSession_IsReused`. Verified: unit 85/85, and the 16 integration tests in `Model/PersonSessionTests.cs` + `Rest/Filters/AuthenticateAttributeTests.cs` pass under Docker.
- [x] Verify file name and location of new test files. Confirm each Phase-introduced test file lives under the right project (mocked-db → `Rock.Tests/`, full-integration → `Rock.Tests.Integration/`), matches the `*Tests.cs` naming convention, groups related tests under named `#region` blocks that align with the phase they cover, AND sits in the folder that mirrors the type-under-test's namespace (e.g. a test for `Rock.Model.X` belongs under `Rock.Tests/Model`, not another domain folder). VERIFIED CLEAN across all 10 new/changed test files: `Rock.Tests/{Model/PersonSessionServiceTests,Model/PersonSessionTests,Net/RockRequestContextTests,Tasks/UpdatePersonSessionLastActivityTests,Jobs/DataAutomationTests,Blocks/Cms/ActiveUsersTests,Blocks/Security/ChangePasswordTests,Blocks/Security/LoginTests}.cs` and `Rock.Tests.Integration/{Model/PersonSessionTests,Rest/AuthenticateAttributeTests}.cs`. Each file's declared namespace matches its folder and the SUT namespace; the convention mirrors the type's NAMESPACE (flat), not the deep source subfolder (confirmed by existing `Rock.Tests/Model/PersonTests.cs` testing `Rock.Model.Person` whose source is under `Rock/Model/CRM/Person/`). `*Tests.cs` naming holds throughout. Regions are optional in this repo (existing files range 0-N), so the region-less small files are consistent, not violations. Two same-named `PersonSessionTests.cs` (unit entity methods vs. integration InteractionSession linkage) are a normal unit/integration split, different assemblies. NOTE: the 861 item text below references stale paths (`Rock.Tests/Security/...`); the real files are correctly under `Model/`. FOLLOW-UP: the newly created `AuthenticateAttributeTests.cs` was moved from `Rock.Tests.Integration/Rest/` to `Rock.Tests.Integration/Rest/Filters/` (namespace `Rock.Tests.Integration.Rest.Filters`) to mirror its SUT namespace `Rock.Rest.Filters.AuthenticateAttribute`. The pre-existing `Rest/SecuredAttributeTests.cs` (same `Rock.Rest.Filters` SUT) is still at `Rest/`; the new file was placed correctly rather than matching the older file's location.
- [x] **`RockRequestContext.CurrentPerson` sources from `PersonSession`.** DONE, implemented as part of the "Identity-resolution refactor" below. `CurrentPerson` is no longer computed as `CurrentUser?.Person`; it is a stored value set together with `CurrentUser` via `SetCurrentIdentity( Person, UserLogin )`, populated from `personSession?.PersonAlias?.Person` at the auth-pipeline hook. This makes the correct person resolve for `UserToken` / `Impersonation` sessions that have no backing `UserLogin`. `ApiControllerBase.GetPerson` (simplified to read `RockRequestContext.CurrentPerson` in Phase 12) and other read-side callers pick this up automatically. Manual verification lives in the "Identity-resolution refactor" section below and the new `UserToken` personalized-Lava item in the "User-token (`rckipid`) email links" section above.
- [x] Look at combining the PersonSessionService...Tests.cs files into one. This is our pattern, but discuss if this is a bad idea (file size growth). RESEARCHED + ACTED: combining is the wrong direction. The unit tests already follow one-class-per-SUT correctly (`PersonSessionServiceTests`, `PersonSessionTests`, `UpdatePersonSessionLastActivityTests` are separate, split by SUT, not fragmented) and merging them would both mix SUTs and push toward the repo's ~3900-line single-class ceiling (`PersonSessionServiceTests` unit is already 3607; `AgentToolHelperTests` at 3905 is the repo max) - i.e., exactly the 7,000-line files to avoid. The real inconsistency was the opposite: the integration `PersonSessionTests.cs` mixed three SUTs. Split it (option 2) into `Rock.Tests.Integration/Model/PersonSessionTests.cs` (entity save hook), `Rock.Tests.Integration/Model/PersonSessionServiceTests.cs` (StartComponentSession, SignOut, FindOrCreate*, UA→DeviceType), and `Rock.Tests.Integration/Core/Model/InteractionServiceTests.cs` (the `GetInteractionSessionId` PersonSessionId upsert, whose SUT is InteractionService - placed next to the existing `Core/Model/InteractionTests.cs`). No `partial` used; each file mirrors its SUT's namespace and stays well under the size ceiling. Verified: integration builds clean and all 16 tests (14 across the three split files + 2 AuthenticateAttribute) still pass under Docker.
- [x] **(Investigation) SaveChanges audit attribution from a SignalR hub call.** Determine how `DbContext.GetCurrentPersonAliasId` resolves the current person during a **SignalR hub method** invocation (not the initial connect), both pre-person-session and after these changes. Working assumption: during an established hub call there is no `HttpContext.Current` (so no `Items["CurrentPerson"]`) and the `RockRequestContextAccessor.Current` AsyncLocal is not set — the connecting request's `RockRequestContext` is captured into RealTime **connection state** (`AspNetEngine.ClientConnectedAsync` → `state.Request`), not the ambient accessor — so audit stamps as **anonymous** in both eras. Confirm, and decide whether hub-initiated saves *should* attribute to the connected person by flowing the connection-state `RockRequestContext` (or its person) into the ambient accessor for the duration of the hub call. CONFIRMED: the assumption holds. `GetCurrentPersonAliasId` prefers `RockRequestContextAccessor.Current.CurrentPerson`, then falls back to `HttpContext.Current.Items["CurrentPerson"]`; during an established hub message neither is populated (the engine never seats the accessor with `state.Request`, and there is no managed `HttpContext.Current` for a WebSocket/SSE frame), so a bare `SaveChanges()` stamps anonymous in both eras — no regression, no improvement. DECISION: hub-initiated saves *should* attribute to the connected person, but this is deferred (not part of this feature). The chosen approach when it is done: per hub message, create a new `RockRequestContext` derived from `state.Request` and seat it on the ambient accessor for the duration of the call, configured with a **lazy identity factory** (like `Application_BeginRequest`) keyed off the scalar `CurrentPersonId`. Read-only messages (the majority) never trip the factory; a message that calls `SaveChanges()` resolves the person on a fresh, live context, so audit stamps correctly and the detached-entity hazard does not apply. Pairs naturally with a future message-scoped DI scope for RealTime.
...
- [x] **Audit stamping (ambient CurrentPerson):** save a record from a WebForms block while logged in; confirm `CreatedByPersonAliasId` / `ModifiedByPersonAliasId` are stamped (these read `HttpContext.Items["CurrentPerson"]`, now populated in `BeginRequest` instead of by `RockPage`). Repeat while impersonating, audit columns should reflect the impersonated person.
- [x] **Lava `CurrentPerson` on WebForms:** a Lava block/shortcode (e.g. `{{ CurrentPerson.FullName }}`) renders the correct person on a page load, and the impersonated person while impersonating.
- [x] **Locked-out eviction:** with an active logged-in session, mark the `UserLogin` locked out; the next request signs the user out (cookie cleared, redirected), verify on a WebForms page **and** a cookie-authenticated REST/mobile request (this check moved into `ResolveSessionForRequest`, so it now applies to all request types).
- [x] **Unconfirmed eviction:** same as above with `UserLogin.IsConfirmed = false`.
- [x] **Activity gating (no over-count):** load a page that pulls many static assets; confirm `PersonSession.LastActivityDateTime` advances once per real page load and NOT per static asset (activity is no longer fired from `BeginRequest`).
- [x] **Activity on API:** a cookie-authenticated REST / Obsidian block action advances `LastActivityDateTime` (fired from `ServiceScopeHandler`); an apikey request advances it (fired from `AuthenticateAttribute`); confirm a single request does not double-count.
- [x] **SignalR under impersonation:** connect a real-time hub while impersonating; the hub should see the impersonated person (the SignalR `RockRequestContext` now captures `CurrentPerson`, not just `CurrentUser`).
- [x] **SignalR detached identity:** confirm hub / real-time code reads only scalar identity (`CurrentPerson.Id`, names); the SignalR `RockRequestContext` is built from entities loaded on an already-disposed `RockContext`, so navigation properties will not lazy-load. (Audit current RealTime engine usage of `CurrentUser`/`CurrentPerson`.) AUDITED CLEAN: identity reaches topics as the scalar `Context.CurrentPersonId` (from the `rock:person` claim), and every core topic re-queries the person by Id on its own live `RockContext`. The detached `state.Request` is exposed only via `IContextExtensions.GetRequestContext()`, whose sole caller (`InteractiveExperienceParticipantTopic` → `CreateOrUpdateInteraction`) reads only `ClientInformation.UserAgent`/`.IpAddress` scalars. No navigation property is dereferenced off the detached `CurrentPerson`/`CurrentUser`. Note: `GetRequestContext()` hands back a context with detached identity entities, so a future topic/plugin reading e.g. `GetRequestContext().CurrentPerson.PrimaryAlias` would throw ObjectDisposedException; core is clean today.
- [x] **`RockPage.UserName`:** any page or plugin that reads `RockPage.UserName` still shows the person's full name (it is now repopulated from `CurrentPerson?.FullName`).
- [x] **`?Logout` control-flow:** logging out via `?Logout` clears the session/cookies and the next request is anonymous, for both an anonymous-viewable page and a non-anonymous page (re-verify given the OnInit early-return restructure).

### OWIN endpoints and lazy identity resolution

Covers the move to shared-lazy identity resolution on `RockRequestContext` (Phase 17) and the migration of the remaining principal readers off `Context.User`. **Test the edge cases explicitly**, especially impersonation / user-token sessions, which have a `CurrentPerson` but no `UserLogin`, so the old `Context.User` path treated them as anonymous.

- [x] **SignalR person resolution (the original bug):** connect a real-time hub from a browser authenticated with a new-format `.ROCK` cookie; the hub resolves the current person (a `rock:person` claim is present and person-channel grouping works). This is the OWIN path that never reaches `Application_PostMapRequestHandler`.
- [x] **SignalR anonymous:** an anonymous browser's hub connection proceeds with no current person and no error.
- [x] **File upload while impersonating / user-token:** during an `Impersonation` or `UserToken` session (no `UserLogin`), upload via `FileUploader.ashx`; the upload is authorized (previously denied, treated as anonymous because no principal was set).
- [x] **File upload anonymous still rejected:** an anonymous request to `FileUploader.ashx` is still denied.
- [x] **API-key file upload (behavior change):** an `FileUploader.ashx` upload authenticated only by an API key (`Authorization-Token` header or `?apikey=`) is authorized as the key's person and stamps `CreatedByPersonAliasId` to that person. This is an intentional change — the handler now establishes the API-key identity on `RockRequestContext` / `HttpContext.Items` rather than on `Context.User`, so a binary-file upload that previously authorized and attributed as *anonymous* now runs as the key's user. Verify a binary-file upload the key's person is NOT authorized to `EDIT` is correctly denied.
- [x] **Secured file DOWNLOAD while authenticated (`GetFile.ashx`, async handler):** download a binary file that requires VIEW security (a file whose parent entity or `BinaryFileType` restricts VIEW) while logged in as a person who IS authorized; confirm the file is served (HTTP 200), not a 403. This exercises the `IHttpAsyncHandler` async-completion path (`EndProcessRequest`) that resolves the current person — distinct from `FileUploader.ashx`, which is a synchronous handler, so the upload items above do NOT cover it. Also confirm: (a) a person NOT authorized to VIEW is still denied (403); (b) an anonymous request to a secured file is still denied; (c) a public file still downloads anonymously. The person is now read from the `RockRequestContext` on `HttpContext.Items` (not the AsyncLocal accessor, which is unreliable in the ADO.NET completion). Repro under concurrent load if possible, since the failure mode was thread/timing-dependent. Verified 2026-09-15: `GetFile.ashx?id=828` on a secured file type served for an authorized admin and denied for anonymous, and unsecured files download for both anonymous and logged-in users.
- [x] **Avatar cache refresh while authorized (`GetAvatar.ashx`, async handler):** as a person in an `AvatarHelper.AuthorizedRefreshCacheRoleGuids` role, hit `GetAvatar.ashx` with `?RefreshCache=true` (and `?RefreshItemCache=true`); confirm the cache actually refreshes. This is the same `IHttpAsyncHandler` completion-thread identity read (via `Delegate.BeginInvoke`), now sourced from `HttpContext.Items`. Confirm an unauthorized person's refresh request is a no-op (fails closed). Verified 2026-09-15: `?RefreshCache=true` refreshed the cache for an authorized administrator and was a no-op for an anonymous request.
- [x] **REST cookie auth while impersonating / user-token:** a cookie-authenticated REST / Obsidian block action during an impersonation or user-token session authenticates as the effective person (previously anonymous via the `Thread.CurrentPrincipal` path).
- [x] **REST cookie auth normal (regression):** a standard `Component`-session cookie REST call still authenticates after the `AuthenticateAttribute` cookie-path conversion.
- [x] **OIDC authorize (OWIN):** the OIDC authorize endpoint resolves the current person for a logged-in user (another OWIN endpoint that bypasses `PostMapRequestHandler`).
- [x] **Anonymous / static assets (no regression):** an anonymous page load and its static assets do NOT trigger a person-session DB lookup (lazy resolution fires only when identity is read); confirm no per-asset session queries.
- [x] **`Context.User` plugin compatibility:** on a managed request for a `UserLogin`-backed session, `HttpContext.Current.User.Identity.Name` is still the username (the eager touch remains for plugins). For an impersonation / user-token session, `Context.User` is intentionally anonymous — confirm core features still work because they read `RockRequestContext.CurrentPerson`, not the principal.
- [x] **Legacy cookie via lazy factory (regression):** a pre-deployment legacy `.ROCK` cookie still upgrades transparently through the lazy resolution path.
- [x] **Request-context re-seat under async / load (regression):** a request whose handler awaits a real async operation (or the site under enough concurrent load to force thread switching between pipeline stages) completes without a "no RockRequestContext was attached" exception, and `CurrentPerson` still resolves in code that runs after the await. This exercises the `OnExecuteRequestStep` re-seat that flows the request-context accessor across ASP.NET pipeline stages. Reproduce as during development: a request that forces an async continuation (e.g. the query-string-driven repro) previously failed intermittently.
- [x] **Migrated current-person readers under impersonation:** while impersonating (a session with `CurrentPerson` but no `UserLogin`), confirm the readers moved off `HttpContext.Items` onto `RockRequestContext` resolve the *impersonated* person, not anonymous: the "Set Attribute to Current Person" workflow action sets its attribute to the impersonated person, and a picker that defaults to / secures by the current person (e.g. the Data Views or System Phone Number picker) scopes to the impersonated person. Normal logged-in and anonymous cases are regression-covered by the audit-stamping and Lava items in the identity-resolution section above.
