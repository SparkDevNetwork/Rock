...
- **Campus filters:** MUST list campuses powered by configurable Campus Types and Campus Statuses. Each campus SHOULD show an identifying badge beside its name: the campus `ShortCode` when set, falling back to name-derived initials when it is blank.
- **Where filters:**
...
- **Location privacy (fuzzing):** exact group locations MUST be obscured server-side to protect privacy. The fuzzing is implemented in a reusable utility (`LocationObfuscator`) the block calls; the map control only renders the resulting circle from the supplied coordinates and radius. For each fuzzed group location:
  - Generate an offset vector (distance `X` up to a fixed maximum, hard-coded at ~475 ft, plus a direction) and shift the location's latitude and longitude by that vector. The offset MUST be generated server-side. The maximum is an opinionated hard-coded constant in v1 (0.95 of a fixed 500 ft algorithm radius), not an admin block setting (see Design).
  - The offset vector MUST be **pseudo-random but deterministic per group and location**: seeded by an HMAC-SHA256 over the group `Guid`, the group's quantized location, and a server-only secret salt, so the same group at the same location always produces the same offset across requests, sessions, and page loads. It MUST NOT be re-randomized per request. (Rationale: a per-request random offset could be averaged over many requests to triangulate the true location; a stable per-group vector prevents this.) Folding the quantized location into the seed means a group that relocates earns a fresh offset, so someone who once knew a group's fuzzed point cannot de-fuzz it after the group moves.
  - Send only the shifted coordinates to the browser. The true coordinates MUST NOT be transmitted to the client, and the salt MUST stay server-side.
  - Render a circle overlay centered on the shifted coordinates. The radius MUST be slightly larger than the maximum offset (offset up to ~475 ft, radius 500 ft) so the true location falls somewhere **within** the circle rather than on its edge. This communicates an approximate area rather than a precise point.
...
- Settings MUST include, at minimum: the four "Hide ... Filters" toggles; Campus Types and Campus Statuses; Enable Proximity Features; supported Meeting Styles; Display Day of Week Filter; Time of Day; Live Text Search; Featured Attributes and Display Attribute Filters (custom, Group-Type-scoped, mutually exclusive); Show Image; Show Average Age; Show Attribute on Card; Show Map, plus its Map Style (a Rock Map Style DefinedValue), Group Marker Color, and Current Location Marker Color; Group Card Template; and the Register Page linked-page setting. (The map location-fuzzing offset and circle radius are hard-coded opinionated constants, not block settings.)
...
**Visibility decision (resolved).** There is no formal "group finder service" today; the shortcode instantiates `GroupFinderHelper` directly and calls `GetGroupLocationQueryable(options)` + `ApplyFilters(...)`. That helper (plus `GroupFinderOptions` and `GroupFinderFilter`) IS the shared engine. Both the shortcode and the new block consume that one engine, rather than build a parallel service:

- The engine (`GroupFinderHelper`, `GroupFinderOptions`, `GroupFinderFilter`, and `GroupProximityResult`) stays `internal`. No public graduation was needed: the shortcode lives in the same `Rock` assembly, and the Obsidian block reaches the engine cross-assembly through the existing `[assembly: InternalsVisibleTo( "Rock.Blocks" )]` grant. A brief experiment that widened the surface to `[RockInternal( "20.0" )] public` was reverted once it was confirmed `InternalsVisibleTo` already covers every consumer.
- The existing shortcode keeps working, calling the same `internal` helper it always has.
- `GroupFinderHelper.cs` carries an author note: "This really shouldn't be in this namespace. Do not make it public without [thought]." That concern is preserved: the engine MAY later be relocated to a `GroupFinderService` in the Group model namespace, but that relocation was not done here and remains an optional, non-blocking cleanup.
...
The card attribute type is a private nested `GroupFinderCardAttributeInfo : LavaDataObject` in `GroupFinder.cs` (it feeds the `Attributes` merge field) and is never serialized to the client.
...
| `ShowImage` | bool | Whether the card image area is shown (the "Show Image" block setting). |
...
| `DrivingMinutes` | double? | Calculated driving time in minutes to the fuzzed marker, populated on the same rules as `DrivingDistance`. Null for a guessed origin, no origin, or a pair that could not be routed. |
| `DriveTime` | string | Friendly driving-time label (e.g. "1 hr 20 min"), or empty when the group cannot be routed. |
...
**Server-side filtering delegates to the field type.** Rather than a fixed string-operator set, the More Filters path builds each attribute's predicate from its own field type: convert the posted `ComparisonValue` into the field type's private filter value, then call `Rock.Utility.ExpressionHelper.GetAttributeExpression(...)`, which invokes `IFieldType.AttributeFilterExpression(...)` internally (no WebForms control required). This mirrors the legacy block (`GroupFinder.ascx.cs:1351`) and honors whatever comparisons each field type supports (contains, starts-with, is-blank, defined-value membership, and so on). Predicates from the same group type are AND'd and different group types OR'd (`GroupFinder.ascx.cs:1335`), so a filter that exists on only one group type does not exclude groups of other types. (Featured "What" pills take a lighter path: they still ride the shared engine's `in` operator rather than this per-field-type delegation, so only the More Filters modal supersedes the helper's narrow operators.)

**Engine integration.** These predicates are built over the `Group` entity, while the finder's engine query runs over `GroupLocation`. The block applies them by pre-filtering the group set upstream and feeding the surviving groups into the proximity/location query, keeping the shared `GroupFinderHelper` (also used by the legacy and mobile finders) untouched. The block's own attribute-filter path therefore supersedes the helper's narrow `con` / `sw` / `ew` / `eq` / `ne` operators for the More Filters modal; Featured "What" pills still go through the helper's `in` operator.
...
**Marker privacy (resolved).** Exact group locations are obscured with server-side coordinate fuzzing in the reusable `Rock.Core.Geography.LocationObfuscator` utility (so other blocks can reuse it), and the true coordinates are never sent to the browser (see the "Location privacy (fuzzing)" requirement under Map). For each fuzzed location the server derives a **pseudo-random but deterministic** offset vector seeded by an HMAC over the group `Guid`, the group's quantized location, and a server-only salt (distance up to a hard-coded max of ~475 ft, in a stable direction), shifts the lat/long by it, sends only the shifted point, and the control draws a circle overlay (radius 500 ft, slightly larger than the max offset) centered on the shifted point so the true location sits somewhere within the circle. The per-group seed keeps each group's fuzzed position stable across requests, which is a security property, not just cosmetics: a per-request random offset could be averaged over many samples to recover the true point. Because the seed also folds in the quantized location, a group that moves re-fuzzes rather than staying pinned to a now-stale offset. The maximum offset is an opinionated hard-coded constant in v1 (~475 ft), not an admin setting, since a church is unlikely to tune it; worth surfacing to the PO later in case they want it configurable. This supersedes the legacy `Location Precision Level` decimal-truncation approach (`GroupFinder.ascx.cs:1589`); the offset-plus-circle method both hides the exact point and visually signals the imprecision, which plain truncation does not.
...
- Legacy map zoom/marker knobs: `MapInfo`, `InitialZoomLevel`, `MinimumZoomLevel`, `MaximumZoomLevel`, `MarkerZoomLevel`, `MarkerZoomAmount`, `MapMarker` (the reusable map control owns its config with sensible defaults). The block does expose a **Map Style** setting (a Rock Map Style DefinedValue, which also carries the Google cloud Map ID that drives native light/dark; see the companion map-styling spec `260807-mapview-google-map-id-styling.md`) plus **Group Marker Color** and **Current Location Marker Color** for the results map.
...
| Engine (shared, `internal`) | Consumed in place from `Rock/Utility/GroupFinder/` via `InternalsVisibleTo( "Rock.Blocks" )` (optional later relocation to a `GroupFinderService` in the Group model namespace) |
...
- The cache is a client-side map keyed by an origin key (the resolved origin rounded to ~4 decimals) to a `groupGuid -> { miles, minutes, locationKey }` map. Each value carries both `miles` and `minutes` because a single Routes Matrix element returns them together, so caching the pair lets a card render distance and drive time without a second lookup; the `locationKey` is a signature of the group's location that invalidates the entry if the group later moves. It lives in the block component's memory only: not persisted to storage, and cleared on reload.
- Each results request carries the distances the client already holds for the current origin as `KnownDistances` (a `groupGuid -> { miles, minutes, locationKey }` map) paired with a `KnownDistancesOriginKey`. The server resolves the result page, then builds one distance map for it from **both** sources: each group's distance is taken from `KnownDistances` when present *and* still valid (the entry's `locationKey` still matches the group's current location, so a group that moved is re-routed), and the Routes Matrix is called only for the groups still missing. It resolves each card's Lava **once** from that merged map, so a card whose distance came from the client renders in the same single pass as a freshly looked-up one, and returns only the newly looked-up pairs as `NewDistances` (paired with the response's `OriginKey`) for the client to merge into its cache.
- Changing the origin (a new address) produces a new origin key, so the next search looks up distances against the new origin and the prior origin's entries simply go unused.
- `KnownDistances` is a plain `groupGuid -> { miles, minutes, locationKey }` object, which is small in practice: entries accumulate only for groups the visitor has actually displayed (one page at a time), and each serializes to roughly 60 bytes, so a heavily paged session of ~100 pairs is only ~6 KB on the request. To bound the payload under pathological paging, the client cache is capped (oldest entries evicted for the current origin) so it never grows without limit.
...
2. **Engine home.** Reuse the existing shared engine that the shortcode already uses (`GroupFinderHelper` / `GroupFinderOptions` / `GroupFinderFilter`) rather than building a parallel service. It stays `internal`; the block consumes it cross-assembly via `InternalsVisibleTo( "Rock.Blocks" )`, so no public graduation was needed. See the Visibility decision under Design. Optional non-blocking cleanup: relocate to a `GroupFinderService` in the Group model namespace.
...
### Flip `GroupFinderHelper` to public
Rejected. The class carries an explicit author warning against it (`GroupFinderHelper.cs`) and sits in a `Utility` namespace it should not permanently live in. The engine instead stays `internal` and is shared cross-assembly via `InternalsVisibleTo( "Rock.Blocks" )`, which keeps the shortcode and block on one engine without widening the public surface. (A short-lived `[RockInternal] public` widening was tried, then reverted once `InternalsVisibleTo` was confirmed to cover every consumer.)
