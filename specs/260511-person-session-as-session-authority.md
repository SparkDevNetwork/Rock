...
1. `Application_BeginRequest` calls `PersonSessionService.ResolveSessionForRequest( rockRequestContext )`. That method owns the full read-side cookie lifecycle: format detection (base64 + `"V2"` footer check, distinguishing new from legacy without a full crypto operation), decryption via `Encryption.DecryptString`, JSON parse, `PersonSession` load by Guid, `IsActive` and `ExpiresDateTime` validation, the `RejectAuthenticationCookiesIssuedBefore` kill switch against `PersonSession.IssuedDateTime`, and any required reissue (half-life, `OldDataEncryptionKey{n}` decryption, older payload `v`).
2. **New-format cookie with a resolvable session:** `ResolveSessionForRequest` returns the session. The hook sets `HttpContext.User` to an authenticated principal backed by `session.PersonAlias`. `FormsAuthenticationModule.OnEnter` short-circuits at its `Context.User != null && Context.User.Identity.IsAuthenticated` guard and does not attempt to decrypt the cookie with the machine key.
3. **New-format cookie, but invalid (tamper, inactive, expired, kill-switch fired):** `ResolveSessionForRequest` returns null and has already done any necessary cleanup (marked the session inactive, expired the cookie) internally. The hook does nothing further; the request proceeds unauthenticated.
4. **Legacy cookie:** `ResolveSessionForRequest` returns null (it does not touch legacy-format cookies — that is Phase 6's job). Before falling through to `FormsAuthenticationModule`, the BeginRequest hook still runs the existing `RejectAuthenticationCookiesIssuedBefore` check at `Global.asax.cs:582-604` against the legacy ticket's `IssueDate`. The new kill switch inside `ResolveSessionForRequest` cannot see legacy-format cookies, and `Application_PostAuthenticateRequest` (which performs the upgrade) only fires after `FormsAuthenticationModule` has already validated the principal — so without this fallback a pre-threshold legacy cookie would survive the current request and only get caught on the next one. The legacy block is dual-reader bridge code that retires alongside `FindOrCreateLegacyUpgradeSession`; the implementation MUST carry a code comment to that effect. If the legacy ticket survives the kill switch, `FormsAuthenticationModule` decrypts as it does today and sets `Context.User` to a `FormsIdentity`; `Application_PostAuthenticateRequest` detects the `FormsIdentity`, runs the upgrade path defined under "Cookie upgrade path" above (create the `PersonSession` from the ticket's `Name`, reissue the cookie in the new format), and replaces `Context.User` with the authenticated principal backed by the new `PersonSession`.
5. **No cookie:** `ResolveSessionForRequest` returns null. Anonymous request, no work to do (the legacy kill-switch block is a no-op when no legacy cookie is present).

The `Application_BeginRequest` hook is intentionally trivial — read context, call `ResolveSessionForRequest`, set principal if non-null, and (only when `ResolveSessionForRequest` returned null) run the existing legacy kill-switch block as a bridge. All cookie-format knowledge, validation rules, new-format kill-switch checks, and reissue policy live in `PersonSessionService`; the legacy fallback is the single intentional exception, sunset on the same v23-ish horizon as the rest of the legacy reader.
