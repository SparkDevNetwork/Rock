...
- The Person Bio block (`RockWeb/Blocks/Crm/PersonDetail/Bio.ascx.cs`) switches to a flow that performs the entire handoff server-side: call `PersonSessionService.StartImpersonationSession(targetAliasId, impersonatorSession)`, add the result to the rock context, save, call `PersonSessionService.SetAuthCookie(newSession, RockRequestContext)` to write the new `.ROCK` cookie value on the response, and then redirect to the target URL. The redirect target carries NO `rckipid` query parameter. On the next request the browser sends the new cookie, the standard cookie validation path resolves it to the impersonation `PersonSession`, and the user is the impersonated person from that point on.
