...
**Request resolution helper.** The service exposes a single read-side seam for the cookie lifecycle. The `Application_BeginRequest` hook in `Global.asax.cs` calls into it; every other caller reaches session state through `RockRequestContext.PersonSession` and never needs this method directly.
...
`ResolveSessionForRequest` owns the full read-side cookie lifecycle: read the cookie from `RockRequestContext`, decode (via an internal `TryDecodeCookie` helper that returns the parsed payload plus decryption metadata), load the `PersonSession` by Guid, validate `IsActive` and `ExpiresDateTime`, enforce the `RejectAuthenticationCookiesIssuedBefore` kill switch against `PersonSession.IssuedDateTime`, apply reissue triggers (half-life, `OldDataEncryptionKey{n}` decryption, older payload `v`), and return the resolved session — or null if anything fails. On kill-switch fire, the method marks the session inactive (so `InactiveDateTime` stamps in `PreSave`) and expires the cookie via `RockRequestContext` before returning null. On reissue, it calls `SetAuthCookie` internally to emit the fresh cookie. **Callers MUST NOT duplicate any of this logic.** Putting the whole read-side lifecycle behind one method keeps the auth pipeline (`Application_BeginRequest`) a thin shim, and means future changes to the cookie format, validation rules, or reissue policy land in one place.
...
- The full server-side handoff lives in `PersonSessionService.ImpersonatePerson( RockRequestContext context, int targetPersonAliasId )` (`internal static`, returns `void`). The method reads the admin's current `PersonSession` and `InteractionSession` from `RockRequestContext`, calls `StartImpersonationSession` (which stamps `PersonSessionAdminImpersonationSettings` with both Guids), saves the new row, calls `SetAuthCookie` to write the new `.ROCK` cookie, and writes the `HistoryLogin` audit row. If the request has no active `PersonSession` (e.g., the admin's session expired between rendering the Impersonate button and clicking it), the method throws `InvalidOperationException`. The Person Bio block (`RockWeb/Blocks/Crm/PersonDetail/Bio.ascx.cs`) becomes a thin shim: call `ImpersonatePerson`, then redirect to the configured target URL. The redirect target carries NO `rckipid` query parameter because no code path in this flow appends one. On the next request the browser sends the new cookie, the standard cookie validation path resolves it to the impersonation `PersonSession`, and the user is the impersonated person from that point on. (The orchestrator lives on `PersonSessionService` rather than in the WebForms code-behind so the logic is mocked-database testable; Bio.ascx.cs is compiled at runtime and cannot be exercised from Rock.Tests, the same WebForms-pipeline constraint the `Application_BeginRequest` and `PostAuthenticateRequest` shims work around.)
- The flow does NOT write a `PersonToken` row. Today's `PersonToken` write existed purely as the handoff mechanism — token created, redirect with `rckipid`, token consumed on next page load, token effectively disabled — and no consumer reads the row after that single consumption. With the cookie carrying the handoff directly, the `PersonToken` write is vestigial. `HistoryLogin` continues to capture the audit trail for the impersonation event (it is written by the impersonation start path, not by `PersonToken`). `PersonToken` is reserved exclusively for user-token (`rckipid` email link) flows under the new model.
...
1. The existing `RejectAuthenticationCookiesIssuedBefore` block at `Global.asax.cs:582-604` runs first, unconditionally, exactly as it did pre-rollout. This block decrypts the cookie as a legacy `FormsAuthenticationTicket`, compares `ticket.IssueDate` against the kill-switch threshold, and removes the cookie from `Request.Cookies` if the ticket pre-dates it. For a new-format cookie the `FormsAuthentication.Decrypt` call is a harmless no-op (the new format is not a `FormsAuthenticationTicket` and the result is dropped). Running this block first — rather than gating it on a null return from step 3 below — preserves the pre-Phase-5 ordering: a bad legacy cookie is removed from `Request.Cookies` before any downstream code (thread culture, the `RockRequestContext` cookie snapshot, `ResolveSessionForRequest`) reads the request. The legacy block is dual-reader bridge code that retires alongside `FindOrCreateLegacyUpgradeSession`; the implementation MUST carry a code comment to that effect.
2. `WebRequestHelper.SetThreadCultureFromRequest` runs next, exactly where it always did.
3. The BeginRequest hook builds and attaches the request-scoped `RockRequestContext`, then calls `PersonSessionService.ResolveSessionForRequest( rockRequestContext )`. That method owns the full read-side cookie lifecycle for new-format cookies: format detection (base64 + `"V2"` footer check, distinguishing new from legacy without a full crypto operation), decryption via `Encryption.DecryptString`, JSON parse, `PersonSession` load by Guid, `IsActive` and `ExpiresDateTime` validation, the `RejectAuthenticationCookiesIssuedBefore` kill switch against `PersonSession.IssuedDateTime`, and any required reissue (half-life, `OldDataEncryptionKey{n}` decryption, older payload `v`).
4. **New-format cookie with a resolvable session:** `ResolveSessionForRequest` returns the session. The hook sets `HttpContext.User` to an authenticated principal backed by `session.PersonAlias`. `FormsAuthenticationModule.OnEnter` short-circuits at its `Context.User != null && Context.User.Identity.IsAuthenticated` guard and does not attempt to decrypt the cookie with the machine key.
5. **New-format cookie, but invalid (tamper, inactive, expired, kill-switch fired):** `ResolveSessionForRequest` returns null and has already done any necessary cleanup (marked the session inactive, expired the cookie) internally. The hook does nothing further; the request proceeds unauthenticated.
6. **Legacy cookie:** Step 1's block has already removed the cookie if it pre-dated the kill-switch threshold. `ResolveSessionForRequest` in step 3 returns null (it does not touch legacy-format cookies — that is Phase 6's job). If the legacy ticket survived step 1, `FormsAuthenticationModule` decrypts as it does today and sets `Context.User` to a `FormsIdentity`; `Application_PostAuthenticateRequest` detects the `FormsIdentity`, runs the upgrade path defined under "Cookie upgrade path" above (create the `PersonSession` from the ticket's `Name`, reissue the cookie in the new format), and replaces `Context.User` with the authenticated principal backed by the new `PersonSession`.
7. **No cookie:** Step 1's block is a no-op; `ResolveSessionForRequest` in step 3 returns null. Anonymous request, no further work.

The `Application_BeginRequest` hook is intentionally trivial — preserve the legacy kill-switch block verbatim at the top, set thread culture, attach `RockRequestContext`, call `ResolveSessionForRequest`, set principal if non-null. All new-format cookie-format knowledge, validation rules, kill-switch checks, and reissue policy live in `PersonSessionService`; the top-of-hook legacy block is the single intentional exception, sunset on the same v23-ish horizon as the rest of the legacy reader.
