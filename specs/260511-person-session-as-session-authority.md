...
The gist: the current person is resolved from the *session* rather than from the *login*, and both WebForms and Obsidian read it from one place (`RockRequestContext`). The load-bearing reason is that a session can identify a person without a `UserLogin`. `UserToken` and `Impersonation` sessions point at a `PersonAlias.Person` but often have no `UserLogin` at all, so the old "`CurrentPerson` is just `CurrentUser?.Person`" derivation would return null for exactly those sessions.

Concretely:

- **One seam sets both values.** `RockRequestContext.CurrentPerson` and `CurrentUser` are set together through a single `SetCurrentIdentity( Person, UserLogin )` call, at the same auth-pipeline point where `PersonSession` is resolved. `CurrentPerson` is now a stored value, not a derived one.
- **Both are sourced from the session.** The pipeline sets `CurrentPerson` from `personSession?.PersonAlias?.Person` and `CurrentUser` from `personSession?.UserLogin`, so a session with no `UserLogin` still resolves a person.
- **Ambient consumers see it too.** The identity is also written into `HttpContext.Items["CurrentPerson"]` / `["CurrentUser"]` from the Global.asax hook, so audit-column stamping (via `Model<T>`) and WebForms Lava `CurrentPerson` see the resolved person (including the impersonated person while impersonating) without depending on `RockPage`.
- **`RockPage` becomes a proxy.** `RockPage.CurrentUser` / `CurrentPerson` (and `CurrentPersonId` / `CurrentPersonAlias` / `CurrentPersonAliasId`) are thin proxies over `RockRequestContext`, so WebForms and Obsidian resolve the same identity from the same source.
- **New setter entry point.** `SetCurrentUser( UserLogin )` is a `public` `[RockInternal]` method on `RockRequestContext` (the `CurrentUser` / `CurrentPerson` setters are otherwise `private`), needed because RockWeb's runtime-generated assembly name cannot be granted `InternalsVisibleTo`.
- **Consequent deprecations** (beyond the "Deprecations and removals" table): `UserLoginService.GetCurrentUser*` and `UserLogin.GetCurrentUserName` are `[Obsolete]` `[RockObsolete( "20.0" )]`, superseded by reading identity from `RockRequestContext`.

**Lazy, pipeline-agnostic resolution (further evolution).** The eager "resolve at `Application_PostMapRequestHandler`" model above only runs for *managed* requests. OWIN-terminated requests never reach that event: the managed handler-mapping stage does not run for them, so SignalR (`/rock-rt`), OIDC, and plugin OWIN endpoints had their `RockRequestContext` identity left unresolved (`CurrentPerson` null even though a valid `.ROCK` session existed). The resolution therefore moves to a **shared, lazily-initialized factory**: `Application_BeginRequest` (which *does* run for every request, managed and OWIN) installs a factory via `SetPersonSessionFactory( Func<PersonSession> )`, and `PersonSession` / `CurrentPerson` / `CurrentUser` resolve from it once, on first read (`CurrentPerson` / `CurrentUser` derive from the resolved session). This is pipeline-agnostic, so it fixes OWIN endpoints; it also resolves only when identity is actually read, which naturally skips static-asset requests and removes the need for the `Context.Handler != null` gate that had been added to avoid per-asset lookups. The legacy-cookie upgrade is folded into the factory so backward compatibility is preserved, and an explicit eager `SetCurrentIdentity` / `SetPersonSession` still wins over the factory for callers that set identity themselves (`RockPage`, the legacy upgrade in `Application_PostAuthenticateRequest`, `AspNetEngine.ClientConnectedAsync`). The factory runs the **full, side-effecting** resolution (`ResolveSessionForRequest`), so it both resolves *and* invalidates (reissue, kill-switch mark-inactive, locked-out sign-out) on any request type - this is what lets OWIN requests invalidate a bad session, not just fail closed. `Application_PostMapRequestHandler` triggers it by reading `PersonSession` (so the DB lookup runs exactly once) and then applies the managed-only side effects (`Context.User`, `HttpContext.Items`). Cookie reissue inside that resolution is **best-effort**: a write that cannot land (a late read after the response started, or an OWIN response that cannot take a `Set-Cookie`) is swallowed and never discards the resolved session.

**The principal (`Context.User`) is a managed-pipeline convenience, not the source of truth.** For managed requests (`Context.Handler != null`), the pipeline "touches" the now-lazy identity early and populates `Context.User` with a `GenericPrincipal( GenericIdentity( UserLogin.UserName ) )`, so framework and plugin readers of `HttpContext.Current.User` / `Thread.CurrentPrincipal` keep working and authorization runs before the handler. This principal is deliberately **incomplete**: it is written only when the session has a backing `UserLogin`, so `Impersonation` and `UserToken` sessions (which have a `CurrentPerson` but no `UserLogin`) get *no* principal. Core code therefore MUST read identity from `RockRequestContext`, never from the principal, or it will treat those sessions as anonymous. Once the remaining core principal readers are migrated to `RockRequestContext` — the `.ashx` upload handler (`FileUploader.ashx`) and the REST cookie-authentication path (`AuthenticateAttribute.TryAuthenticateFromCurrentPrincipal`) — the eager `Context.User` write benefits only plugins and becomes **legacy compatibility code**, to be marked as such and removed in a future major version. Verified during the audit: `RockPage.CurrentUser` / `CurrentPerson` already proxy `RockRequestContext`; no `web.config` `<authorization>` rules and no `[Authorize]` filters gate on the principal; and `FormsAuthenticationModule` is inert against the new `.ROCK` cookie (it is not a forms ticket). Readers that legitimately stay on a principal are reading a *different* one: the REST OAuth path (`TryAuthenticateFromOidcToken`) reads the OWIN bearer identity's claims, and `RealTimeHub` reads the SignalR connection's injected `rock:person` claim — neither consumes the managed `.ROCK` principal.

**`RockRequestContext` is the sole read source for the current person; `HttpContext.Items["CurrentPerson"/"User"]` is a write-only compatibility shim.** `Items["CurrentPerson"]` was historically a second ambient store read directly by ~20 core sites (the `Model<T>` audit path in `DbContext.GetCurrentPersonAliasId`, most Lava blocks/filters/shortcodes, workflow, communication mediums, reporting, and several UI pickers). Because identity now resolves lazily through `RockRequestContext` — and OWIN requests never write `Items` at all — `Items` and `RockRequestContext` can diverge, which would let one part of a request believe Person A is signed in while another sees no one. To keep a single source of truth, every core reader is migrated to read `RockRequestContextAccessor.Current?.CurrentPerson` (reliable across pipeline steps thanks to the `OnExecuteRequestStep` re-seat), and no core code reads `Items` any longer. The **writes** to `Items["CurrentPerson"/"User"]` are retained as a backward-compatibility shim for plugins that may still read them directly, and are removable in a future major version once that deprecation window passes.
