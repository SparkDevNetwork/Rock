...
summary: >-
  A single build plan for Forge Content authoring: an administrator describes a dashboard in
...
contributors: []
related_docs:
  - docs/ai/forge-content-architecture.md
  - docs/cms/forge-content.md
---

# Forge Content: AI-Authored Components
...
flowchart TD
    ...
    Client -->|"/api/v2/mcp/code-composer"| Agent["Code Composer MCP Agent<br/>AIAgent, AgentType.Mcp"]
    ...
    Agent --> VC["ForgeContent<br/>GetRockVersion, GetForgeContent, AddOrUpdateForgeContent"]
    PB --> Page["Page + Forge Content block"]
    LD --> Lava["Lava application + JSON endpoints"]
    VC --> Compile["Compile in headless Chromium"]
    Compile --> Table["[ForgeContent] row"]
...
| 9 | Internal chat agent | An administrator can build Forge Content from Rock's own chat, no external client |
...
`Rock/Model/CMS/ForgeContent/ForgeContent.cs`, EntityType `38F182A7-9FE4-4D7B-B483-59F615BDE41C`.
...
`Rock/Cms/ForgeContentCompiler.cs`. Runs the bundle in a page of the headless Chromium Rock already manages for PDF generation, via PuppeteerSharp. It serves both writers: the block's save action (layer 3) and the agent's `AddOrUpdateForgeContent` (layer 6).
...
**How the utility obtains the trace (decided 2026-08-24).** `parentTrace` lives on `ObsidianBlockConfigBag`, which `rockBlock.partial.ts` holds as a prop and does not expose: it provides only `configurationValues`, `blockActionUrl`, and the invoke functions to descendants. `rockBlock.partial.ts` additionally provides the traceparent string (for example `provide("blockParentTrace", ...)`), and `useLavaApp` injects it when called during component setup. Authored components mount inside the host block's tree, so injection reaches them, and every other framework utility gains access for free. Cost: `useLavaApp` becomes setup-only, which it effectively already is by convention. The alternative, taking the traceparent as an explicit option threaded by the caller, was rejected: authored components have no path to the value either, so it only relocates the problem.

### Layer 6: Agent skills

Three `AgentSkillComponent` subclasses. They compose by handing off identifiers: Cms returns a block `IdKey`, ForgeContent writes against it, LavaApplication feeds it.
...
**They are built to the established skill shape, not the exploration's.** The exploration placed them loose in the `Rock` project as monolithic files (the exploration-era component skill reached 1,243 lines) and diverged from the 21 shipped skills on nearly every convention. The rebuild follows the shipped pattern exactly:
...
| ForgeContent | `647770A9-F3D7-4924-B046-5C9C43959ECB` / `4C833FA4-A7EF-4D49-9549-B24CBB629A73` | `GetRockVersion`, `GetForgeContent`, `AddOrUpdateForgeContent` | EDIT of the block (`GetRockVersion` ungated) |
...
GetForgeContent          7D3A8200-3A90-44CC-9E30-B600383E835F
AddOrUpdateForgeContent  26FFEE94-4868-4DEC-BE40-68FBE30DAEB8
```
...
- **`AddOrUpdateForgeContent` stores nothing on compile failure** and returns the compiler's error text.

### Layer 7: Agent seeding

Seeding lives in the **same EF migration as layer 1**, after the table and block type. The content is modeled on `Rock/Plugin/HotFixes/309_EnableRockIntelligence.cs`, which established the seeding sequence, but it does not ship as a plugin hotfix. The exploration used a separate agent-seeding hotfix with a placeholder number; folding it into the one migration removes the numbering problem and the two-artifact deploy.
...
3. Insert the `AIAgent` rows **only when absent**: "Code Composer MCP Agent" (`AgentType.Mcp`, `AudienceType.Internal`, `AdditionalSettingsJson` of `{ "McpAgentSettings": { "Slug": "code-composer", "IsExcludingSystemSkills": false } }`, GUID `DC44435A-8900-4AB4-9EB3-1756FCC1B355`) and "Code Composer Chat Agent" (see layer 9). One narrow exception to create-only: an existing row still named "Vibe Agent", the pre-release default, is renamed to "Code Composer MCP Agent", since an untuned name is safe to bring in line.
4. Insert `AIAgentSkill` link rows, each carrying an **explicit `EnabledTools` allowlist** in `{ "AgentSkillSettings": { "EnabledTools": [...] } }`.
5. Grant VIEW to Rock Administrators, deny to all users. For the LavaApplication and ForgeContent skills this lands at the skill level; for the Cms skill it lands on its two mutating tools only, because its read tools serve any audience.
...
3. **Remote compile, agent path.** With the same configuration, call `AddOrUpdateForgeContent` through an MCP client. It compiles and stores. Both writers share one compile path, so a fix that only reaches one of them means the branch landed in the wrong place.
...
**What it is.** A second `AIAgent` row with `AgentType.Chat`, attached to the same skills (the `CmsSkill`, `ForgeContentBuilderSkill`, and `LavaApplicationBuilderSkill` plus the core `CommunityKnowledgeBaseSkill`), so an administrator runs the whole authoring loop from Rock's own chat UI with no external MCP client at all. The MCP agent stays; this is a sibling on a different transport, not a replacement. As built, the two agents are named **Code Composer MCP Agent** (renamed from the pre-release "Vibe Agent"; the migration renames an existing row only while it still carries that untuned default name) and **Code Composer Chat Agent**.
...
- **Resolved with it: the second-MCP-server dependency.** Because skills are transport-agnostic, the MCP agent carries the same knowledge base tools, so external clients no longer need to connect knowledge.rockrms.com separately; `ForgeContentBuilderSkill`'s usage guidance now names the in-Rock tools (`SearchCode`/`GetCodeFile`) instead of the external server's (`search_code`/`file_url`).

**Seeding.** Same single migration, honoring the one-migration requirement. The branch is unmerged, so amending `202608172102127_AddForgeContent` is legitimate; any dev database that already applied it must re-run the new seeding method by hand or roll the migration back and forward. Pinned values:

| Item | Value |
|---|---|
| Agent name | Code Composer Chat Agent |
...
3. Configure an AI provider. As an administrator, ask it to build a small dashboard. It walks the build order (`SearchPages`, `AddOrUpdatePage`, `AddOrUpdateBlock`, `AddOrUpdateLavaApplication`, `AddOrUpdateLavaEndpoint`, `AddOrUpdateForgeContent`), and the resulting page renders for a normal member.
...
5. As a person who is not a Rock administrator, confirm the agent is not visible in chat at all (the deny auth), and separately, as a person who can see the agent but lacks EDIT on a target block, confirm `AddOrUpdateForgeContent` refuses with the authorization message. The transport changed; the gates must not have.
...
**The Coding Guide topic comes first (2026-08-24 meeting; production routing updated 2026-09-03).** Both agents' instructions MUST call `GetKnowledgeBaseOverview`, locate the Rock Coding Guide topic, pass the returned topic key unchanged to the topic tool, and open the root article listed by that topic before authoring. They MUST follow the guide's own routing for the requested outcome while retrieving only the Contracts, Reference articles, Examples, schema details, and source ranges assigned by the selected Playbook. The agent instructions MUST NOT hardcode the guide's topic key, article keys, Playbook names, or internal paths because the guide owns that structure. This lands as instruction-text edits to both seeded `AIAgent` rows in the migration; because agent rows are create-only, instances that already seeded the agents pick it up only by re-running the seeding or editing the agent by hand.
...
1. The single migration applies; `[ForgeContent]` exists with the cascade FK; the block type, skills, tools, and agent are all registered. No other migration or hotfix is involved.
...
5. Through an MCP client connected to `/api/v2/mcp/code-composer`: `GetRockVersion` returns this instance's version.
6. `AddOrUpdatePage` with a route, then `AddOrUpdateBlock` with the Forge Content block type resolved through `ListBlockTypes`. The friendly URL resolves.
7. `AddOrUpdateForgeContent` with a valid component. It compiles server side and stores. The page renders it.
...
- `specs/260722-mcp-driven-obsidian-content-Forge Content.md` (lifting authoring into MCP)
...
- `specs/260803-mcp-Forge Content-skill-candidates.md`
- `specs/260804-Forge Content-findings.md`
...
- [docs/ai/forge-content-architecture.md](../docs/ai/forge-content-architecture.md)
- [docs/cms/forge-content.md](../docs/cms/forge-content.md)
