...
**They are built to the established skill shape, not the exploration's.** The exploration placed them loose in the `Rock` project as monolithic files (`CodeBuilderSkill.cs` reached 1,243 lines) and diverged from the 21 shipped skills on nearly every convention. The rebuild follows the shipped pattern exactly:
