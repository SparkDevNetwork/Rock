...
namespace Rock.Rest.Filters
{
    ...
    /// </para>
    /// </summary>
    public class SecuredFilter : IActionFilter, IFilter
    {
        /// <inheritdoc/>
        ...
        Task<HttpResponseMessage> IActionFilter.ExecuteActionFilterAsync( HttpActionContext actionContext, CancellationToken cancellationToken, Func<Task<HttpResponseMessage>> continuation )
        {
            ...
            if ( actionContext.ActionDescriptor is ReflectedHttpActionDescriptor reflectedActionDescriptor )
            {
                ...
                if ( methodInfo.GetCustomAttribute<Rest.SecuredAttribute>() is Rest.SecuredAttribute securedAttribute )
                {
                    AuthorizeRequest( actionContext, securedAttribute.SecurityActions );
        ...
        /// <summary>
        /// Verifies the current person is authorized to at least one of the
        /// <paramref name="securedActions"/> specified.
        /// </summary>
        /// <param name="actionContext">The action context.</param>
        /// <param name="securedActions">The actions that the user must be authorized for. Only one action needs to be authorized.</param>
        internal static void AuthorizeRequest( HttpActionContext actionContext, IReadOnlyCollection<string> securedActions )
        {
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            if ( securedActions.Count == 0 )
            {
                ...
                securedActions = [action];
            }

            bool authorized = false;

            if ( securedActions.Any( action => item.IsAuthorized( action, person ) ) )
            {
                authorized = true;
            }
            else if ( actionContext.Request.Headers.Contains( "X-Rock-App-Id" ) && actionContext.Request.Headers.Contains( "X-Rock-Mobile-Api-Key" ) )
            {
                ...
                if ( appId.HasValue )
                {
                    ...
                    if ( appUser != null && securedActions.Any( action => item.IsAuthorized( action, appUser.Person ) ) )
                    {
                        authorized = true;
                    }
                }
