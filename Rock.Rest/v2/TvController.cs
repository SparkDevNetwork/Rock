        ...
        private IActionResult GetCommonLaunchPacket()
        {
            ...
            try
            {
                ...
                if ( person != null )
                {
                    ...
                    // Identity.Name) is the legacy pattern; this should resolve the current
                    // person / user from RockRequestContext / PersonSession instead. Left as-is
                    // for now because the passwordless AuthToken generated below for the TV client
                    // is keyed off the principal's user name, so switching the source also means
