...
namespace com.lcbcchurch.Care.Migrations
{
    ...
    public class AddCarePages : Migration
    {
        public override void Up()
        {
            ...
            RockMigrationHelper.AddBlock( true, "19D2AEA1-7A31-4980-93C6-D2CA7286D5A4","","3F69E04F-F966-4CAE-B89D-F97DFEF6407A","My Connection Opportunities","Main","<script type='text/javascript'>  function renameConnectionTextAtOnload() {     $('.checkbox-inline:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });          $('.panel-title:contains(\"Connection Request\")').each(function(){         var replacementText = $(this).text().replace(\"Connection Request\",'Care Request');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });          $('.label-success:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });      $('.col-md-3:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });          $('.grid-select-cell:contains(\"Connected\")').each(function(){         var replacementText = $(this).html().replace(\"Connected\",'Completed');     $(this).html(replacementText);     });          $('label:contains(\"Connector\")').each(function(){         var replacementText = $(this).text().replace(\"Connector\",'Assigned PRT');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });              $('a:contains(\"Connector\")').each(function(){         var replacementText = $(this).text().replace(\"Connector\",'Assigned PRT');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     }); }  function startListener(){   var mutationObserver = new MutationObserver(function(mutations) {   mutations.forEach(function(mutation) {     for (var i = 0; i < mutation.addedNodes.length; i++){         renameConnectionTextAtOnload();     }   }); });      mutationObserver.observe(document.documentElement, {   childList: true,   subtree: true  }); }    $( document ).ready(function() {     startListener(); });  </script>","",0,"A02BD97D-E78A-4A51-95C1-C9730A53D9B7");               RockMigrationHelper.AddBlockAttributeValue( "A02BD97D-E78A-4A51-95C1-C9730A53D9B7", "70C77D07-4D28-4E71-BD0F-3DF0F61CE12D", @"9cc19684-7ad2-4d4e-a7c4-10dae56e7fa6" );
            ...
            // Add Block to Page: Care Request Detail, Site: Rock RMS
            RockMigrationHelper.AddBlock( true, "0AFB6A46-50F3-4E82-939B-88D4248A657B","","FF3B07E0-D6F5-4874-B7A3-A0D5B770AF8A","LCBC Connection Request Detail","Main","{% assign isRockAdmin = false %} {% groupmember where:'GroupId == 2 && PersonId == {{CurrentPerson.Id}}'%}     {% for gm in groupmemberItems %}         {% assign isRockAdmin = true %}     {% endfor %} {% endgroupmember %}  {% assign personId = 'Global' | PageParameter:'PersonId' %} {% assign connectionRequestId = 'Global' | PageParameter:'ConnectionRequestId' %}  <script type='text/javascript'>  function renameConnectionTextAtOnload() {     $('.radio-inline:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);     });          $('.btn-success:contains(\"Connect\")').each(function(){         var replacementText = $(this).text().replace(\"Connect\",'Complete');         $(this).contents().last().replaceWith(replacementText);     });          $('.label-success:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);     });          $('.grid-select-cell:contains(\"Connected\")').each(function(){         var replacementText = $(this).html().replace(\"Connected\",'Completed');   $(this).html(replacementText);     });          $('label:contains(\"Connector\")').each(function(){         var replacementText = $(this).text().replace(\"Connector\",'Assigned PRT');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });              $('th:contains(\"Connector\")').each(function(){         var replacementText = $(this).text().replace(\"Connector\",'Assigned PRT');         $(this).contents().last().replaceWith(replacementText);       // $(this).text(replacementText);     });              $('.form-group:contains(\"Placement Group\")').each(function(){         $(this).hide();       // $(this).text(replacementText);     });           {% if isRockAdmin == false %}     $('.grid-delete-button').each(function(){         $(this).hide();     });     {% endif %}      {% if connectionRequestId == 0 %}         {% person id:'{{personId}}' %}             $('.picker-selectedperson').each(function(){                 $(this).html(\"{{person.FullName}}\");             });               $('.js-person-name').each(function(){                 $(this).val(\"{{person.FullName}}\");             });                $('.js-person-id').each(function(){                 $(this).val({{person.Id}});             });         {% endperson %}     {% endif %} }  function startListener(){   var mutationObserver = new MutationObserver(function(mutations) {   mutations.forEach(function(mutation) {     for (var i = 0; i < mutation.addedNodes.length; i++){         renameConnectionTextAtOnload();     }   }); });      mutationObserver.observe(document.documentElement, {   childList: true,   subtree: true  }); }    $( document ).ready(function() {     startListener(); });  </script>","",1,"10D902F6-1F8C-4AFE-A033-0BD4ED8CE23C");
            ...
            RockMigrationHelper.AddBlockAttributeValue( "BAFAE24E-3BEC-4823-8F3E-9B124EC97D06", "B9163A35-E09C-466D-8A2D-4ED81DF0114C", @"~/page/536?ConnectionRequestId={ConnectionRequestId}&ConnectionOpportunityId={ConnectionOpportunityId}" );
