...
namespace com.lcbcchurch.Care.Migrations
{
    ...
    public class AddCarePages : Migration
    {
        public override void Up()
        {
            ...
            RockMigrationHelper.AddBlock( true, "19D2AEA1-7A31-4980-93C6-D2CA7286D5A4", "", "3F69E04F-F966-4CAE-B89D-F97DFEF6407A", "My Connection Opportunities", "Main", "<script type='text / javascript'>  function renameConnectionTextAtOnload() {     $('.checkbox-inline:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);             });          $('.panel-title:contains(\"Connection Request\")').each(function(){         var replacementText = $(this).text().replace(\"Connection Request\",'Care Request');         $(this).contents().last().replaceWith(replacementText);             });          $('.label-success:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);             });      $('.col-md-3:contains(\"Connected\")').each(function(){         var replacementText = $(this).text().replace(\"Connected\",'Completed');         $(this).contents().last().replaceWith(replacementText);             });          $('.grid-select-cell:contains(\"Connected\")').each(function(){         var replacementText = $(this).html().replace(\"Connected\",'Completed');     $(this).html(replacementText);     });          $('label:contains(\"Connector\")').each(function(){         var replacementText = $(this).text().replace(\"Connector\",'Assigned PRT');         $(this).contents().last().replaceWith(replacementText);             });              $('a:contains(\"Connector\")').each(function(){         var replacementText = $(this).text().replace(\"Connector\",'Assigned PRT');         $(this).contents().last().replaceWith(replacementText);             }); }  function startListener(){   var mutationObserver = new MutationObserver(function(mutations) {   mutations.forEach(function(mutation) {     for (var i = 0; i < mutation.addedNodes.length; i++){         renameConnectionTextAtOnload();     }   }); });      mutationObserver.observe(document.documentElement, {   childList: true,   subtree: true  }); }    $( document ).ready(function() {     startListener(); });  </script>", "", 0, "A02BD97D-E78A-4A51-95C1-C9730A53D9B7" );
            // Attrib Value for Block:My Connection Opportunities, Attribute:Configuration Page Page: Care, Site: Rock RMS
