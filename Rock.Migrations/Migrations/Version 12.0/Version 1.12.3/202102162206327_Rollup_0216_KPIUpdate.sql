UPDATE TOP(1) [LavaShortcode] SET [Name]=N'KPI', [Description]=N'Create quick key performance indicators.', [Documentation]=N'<p>Basic Usage:</p>
<pre><code>{[kpis]}
...
<li><b>style</b>(default) - The display style of the KPI. Options include <code>default</code> and <code>card</code>.
<ul>
...
<li><b>iconbackground</b> (true) - Set the value to true to display a background behind the FontAwesome icon.
...
<div class="my-2">
<img src="https://rockrms.blob.core.windows.net/documentation/Lava/Shortcodes/kpi-default.jpg" class="img-responsive">
</div>
...
<div class="my-2">
<img src="https://rockrms.blob.core.windows.net/documentation/Lava/Shortcodes/kpi-advanced.jpg" class="img-responsive">
</div>
...
</ul>', [IsSystem]='1', [IsActive]='1', [TagName]=N'kpis', [Markup]=N'{%- if columncount != '''' -%}
...
<div class="kpi-container" {% if columncount != '''' %}style="--kpi-col-lg: {{ 100 | DividedBy:columncountlg }}%;--kpi-col-md: {{ 100 | DividedBy:columncountmd }}%;--kpi-col-sm: {{ 100 | DividedBy:columncountsm }}%;{% if columnmin != '''' %}--kpi-min-lg:{{ columnmin }};{% endif %}{% if columnminmd != '''' %}--kpi-min-md:{{ columnminmd }};{% endif %}{% if columnminsm != '''' %}--kpi-min-sm:{{ columnminsm }};{% endif %} {{ cssstyle }}"{% endif %}>
    {% for item in kpis %}
        ...
        <div class="kpi {{ kpisize }} {% if style == ''card'' %}kpi-card{% endif %} {% if iconbackground %}has-icon-bg{% endif %} {% if colorSplitLength == 2 %}text-{{ color }} border-{{ colorSplit | First }}-{{ colorSplit | Last | Minus:200 | AtLeast:100 }}{% endif %}{{ class }}" {% if height != '''' or colorSplitLength != 2 %}style="{% if height != '''' %}min-height: {{ height }};{% endif %}{% if color != '''' and colorSplitLength != 2 %}color:{{ color }};border-color:{{ color | FadeOut:''50%'' }}{% endif %}"{% endif %} {% if itemDescription != '''' %}data-toggle="tooltip" title="{{ itemDescription }}"{% endif %}>
            {%- if itemIcon != '''' -%}
            <div class="kpi-icon">
    ...
    {% endfor %}
</div>', [Parameters]=N'title^|subtitle^|showtitleseparator^false|icontype^fa|columncount^|columncountmd^|columncountsm^|size^|style^|iconbackground^true|columnmin^|columnminmd^|columnminsm^' WHERE ([Guid]='8A49FD01-D59E-4611-8FF4-9E226C99FB22');