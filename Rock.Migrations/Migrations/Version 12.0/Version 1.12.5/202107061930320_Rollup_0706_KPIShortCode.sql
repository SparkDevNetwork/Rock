...
<div class="kpi-container" {% if columncount != '''' %}style="--kpi-col-lg:{{ 100 | DividedBy:columncountlg,4 }}%;--kpi-col-md:{{ 100 | DividedBy:columncountmd,4 }}%;--kpi-col-sm:{{ 100 | DividedBy:columncountsm,4 }}%;{% if columnmin != '''' %}--kpi-min-width:{{ columnmin }};{% endif %} {{ cssstyle }}"{% endif %}>
    ...
    {% endfor %}
</div>', [Parameters]=N'title^|subtitle^|showtitleseparator^false|icontype^fa|columncount^|columncountmd^|columncountsm^|size^|style^card|tooltipdelay^|iconbackground^true|columnmin^|columnminmd^|columnminsm^' WHERE ([Guid]='8A49FD01-D59E-4611-8FF4-9E226C99FB22');
