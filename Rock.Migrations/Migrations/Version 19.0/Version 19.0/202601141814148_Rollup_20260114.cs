...
namespace Rock.Migrations
{
    ...
    ///
    /// </summary>
    public partial class Rollup_20260114 : Rock.Migrations.RockMigration
    {
        /// <summary>
        ...
        {
            // v18.2 HotFix data-migration moved to this EF migration (v19)
            NA_UpdateLavaChartShortcode_Issue6579_Up();

            // v19.0 data-migration
            NA_UpdateServiceJobsToHigherHistoryCount();
        }
        ...
        {
            // v18.2 HotFix down data-migration moved to this EF migration (v19)
            NA_UpdateLavaChartShortcode_Issue6579_Down();
        }

        /// <summary>
        /// Update LavaShortcode 'Chart' to fix issue #6579 "Chart Grid Lines Not Visible in Dark Mode"
        /// </summary>
        private void NA_UpdateLavaChartShortcode_Issue6579_Up()
        {
        ...
        {% if xaxistype == ''time'' %}
            ,scales: {
                xAxes: [{
                    ...
                    display: {{ xaxisshow }},
                    gridLines: {
                         color: getComputedStyle(document.documentElement).getPropertyValue(''--color-interface-soft'') 
                    },
                    scaleLabel: {
                        display: true,
                        labelString: ''Date''
...
WHERE [GUID] = '43819A34-4819-4507-8FEA-2E406B5474EA'" );
        ...
        /// <summary>
        /// Update ServiceJob HistoryCount values to reflect expected job frequency.
        /// </summary>
        public void NA_UpdateServiceJobsToHigherHistoryCount()
        {
            Sql( @"
    -- Update these to 10000 if they are still only 500
    UPDATE [ServiceJob] SET HistoryCount = 10000 WHERE [Guid] IN (
     'CB24FF2A-5AD3-4976-883F-DAF4EFC1D7C7' -- Job Pulse
    ,'11900FEC-B5D4-4CF8-8B48-136F5BF06CB0' -- Update Persisted DataViews
    ,'67CFE1FE-7C64-4328-8576-F1A4BFD0EA8B' -- Update Personalization Data
    ) AND HistoryCount = 500

    -- Update these to 10000 if they are still only 0
    UPDATE [ServiceJob] SET HistoryCount = 10000 WHERE [Guid] IN (
     'B6D3B48A-039A-4A1C-87BE-3FC0152AB5DA' -- Update Persisted Datasets
    ) AND HistoryCount = 0

    -- Update these to 2000 if they are still only 500
    UPDATE [ServiceJob] SET HistoryCount = 2000 WHERE [Guid] IN (
     '36FA38CA-9DB0-40A8-BABD-5411121B4809' -- Collect Hosting Metrics
    ,'8D79FB3B-F0B1-4B57-A8DA-EEE46B38A08B' -- Process Communication Flows
    ) AND HistoryCount = 500

    -- Update these to 1000 if they are still only 500
    UPDATE [ServiceJob] SET HistoryCount = 1000 WHERE [Guid] IN (
     '123ADD3C-8A58-4A4D-9370-5E9C6CD3760B' -- Charge Future Transactions
    ,'35EABBDB-1EFA-46F1-86D4-4199FFA2D9A7' -- Process Workflows
    ,'F1FB3E17-93A6-42E5-BE1A-8FA3AAD2B3AB' -- Send Communications
    ) AND HistoryCount = 500

    -- Update these to 1000 if they are still only 0
    UPDATE [ServiceJob] SET HistoryCount = 1000 WHERE [Guid] IN (
     'C0A57A17-1FE9-4C52-96B9-2F6EA1433D00' -- Populate Interaction Session Data
    ) AND HistoryCount = 0

    -- Update these to 700 if they are still only 500
    UPDATE [ServiceJob] SET HistoryCount = 700 WHERE [Guid] IN (
     '3425AE05-354C-4C0E-AF5E-50CD6A7F8740' -- Calculate Metrics
    ,'DFBDD4F9-1DB3-4DE0-8FBA-3B9B613D879D' -- Communication Queue Alert
    ,'A554B0EC-D439-4E3D-8462-517B69FDE5B1' -- Send Group Attendance Reminders
    ) AND HistoryCount = 500
" );
        }

        /// <summary>
        /// Down migration for LavaShortcode 'Chart' to fix issue #6579 "Chart Grid Lines Not Visible in Dark Mode"
        /// </summary>
        public void NA_UpdateLavaChartShortcode_Issue6579_Down()
        {
            Sql( @"UPDATE [LavaShortcode] 
        SET [Markup]=N'{% javascript url:''~/Scripts/moment.min.js'' id:''moment''%}{% endjavascript %}
        {% javascript url:''~/Scripts/Chartjs/Chart.min.js'' id:''chartjs''%}{% endjavascript %}
