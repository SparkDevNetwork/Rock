...
BEGIN
	...
	BEGIN
        ...
        -- PersonPreviousName
		-----------------------------------------------------------------------------------------------
        -- Add the old person's last name to the new person's last name if different and not
        -- already in the [PersonPreviousName].
        DECLARE @OldLastName NVARCHAR(100) = ( SELECT LTRIM(RTRIM([LastName])) FROM [Person] WHERE [Id] = @OldId );
        DECLARE @NewLastName NVARCHAR(100) = ( SELECT LTRIM(RTRIM([LastName])) FROM [Person] WHERE [Id] = @NewId );
        DECLARE @NewPrimaryAliasId INT = ( SELECT [PrimaryAliasId] FROM [Person] WHERE [Id] = @NewId );

        -- Fallback if PrimaryAliasId is NULL (should be super rare or maybe impossible?)
        IF @NewPrimaryAliasId IS NULL
        BEGIN
            SELECT TOP (1) @NewPrimaryAliasId = PA.[Id]
            FROM [PersonAlias] AS PA
            WHERE PA.[PersonId] = @NewId
            ORDER BY PA.[Id];
        END

        IF @OldLastName IS NOT NULL AND @OldLastName <> '' AND @OldLastName <> @NewLastName
        BEGIN
            IF NOT EXISTS
            (
                SELECT 1
                FROM [PersonPreviousName] AS PN
                INNER JOIN [PersonAlias] AS PA ON PA.[Id] = PN.[PersonAliasId]
                WHERE PA.[PersonId] = @NewId
                  AND LTRIM(RTRIM(PN.[LastName])) = @OldLastName
            )
            BEGIN
