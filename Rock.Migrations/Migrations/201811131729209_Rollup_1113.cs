...
{% stylesheet id:'contentcomponent-hero' %}
    .contentComponent-hero {   margin-left: -15px;   margin-right: -15px;   padding: 120px 30px; }
...
{%- for item in Items -%}
    ...
    <section class=""contentComponent contentComponent - hero bg - cover"" style=""{% - if imageGuid != '' -%}
            background - image: url( '/GetImage.ashx?Guid={{ imageGuid }}' );
            {% -endif -%}
            { { contentItemStyle }
            }
            "">
...
class=""{{ textAlignment }}"">{{ item.Title }}</{{ channelTitleSize }}> 
...
<section class=""contentComponent contentComponent-card"">
    <div class=""row bg-cover"" style=""background-image:url('/GetImage.ashx?Guid={{ channelBackgroundImage }}');"">
        {%- for item in Items -%}
            ...
            <div class=""{{ cardWidth }}"">
                <div class=""card panel"" {% if contentItemStyle != '' %}style='{{ contentItemStyle }}'{% endif %}>
