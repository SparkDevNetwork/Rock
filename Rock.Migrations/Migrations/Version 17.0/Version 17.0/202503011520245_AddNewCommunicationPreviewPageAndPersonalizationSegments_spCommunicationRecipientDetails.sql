...
CREATE PROCEDURE [dbo].[spCommunicationRecipientDetails]
...
BEGIN
	...
	IF @ListType = 1 
		BEGIN
			...
			INSERT INTO #RecipientList
				SELECT DISTINCT [PersonId] FROM [GroupMember] gm 
	...
	ELSE
		BEGIN
			INSERT INTO #RecipientList
				SELECT pa.[PersonId] FROM [CommunicationRecipient] cr 
	...
	SELECT 
		...
		, CAST(CASE WHEN (pn.[NumberFormatted] IS NULL OR pn.[IsMessagingEnabled] = 0 OR pn.[IsMessagingOptedOut] = 0) THEN 0 ELSE 1 END AS BIT) AS [IsSmsEnabled]
	...
	WHERE p.[Id] IN (SELECT [PersonId] FROM #RecipientList)
        AND p.[PrimaryAliasGuid] IS NOT NULL AND p.[PrimaryAliasId] IS NOT NULL
