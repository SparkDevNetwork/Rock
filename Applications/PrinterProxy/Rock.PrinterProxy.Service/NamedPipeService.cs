        ...
        try
        {
            while ( !stoppingToken.IsCancellationRequested )
            {
                ...
                if ( OperatingSystem.IsWindows() )
                {
                    ...
                    pipeServer = NamedPipeServerStreamAcl.Create( "RockPrintProxyService", PipeDirection.InOut, 1, PipeTransmissionMode.Byte, PipeOptions.Asynchronous | PipeOptions.WriteThrough, 0, 0, pipeSecurity );
                }
                else
                {
                    pipeServer = new NamedPipeServerStream( "RockPrintProxyService", PipeDirection.InOut, 1 );
                }
