...
namespace Rock.PrinterProxy.Shared
{
    ...
    /// between client and server for the printer proxy system.
    /// </summary>
    internal abstract class ProxyWebSocket
    {
        ...
        /// <param name="cancellationToken">A cancellation token that can be used to cancel the asynchronous operation.</param>
        /// <returns>A <see cref="Task"/> that indicates when the operation has completed.</returns>
        public Task PostMessageAsync( PrinterProxyMessage message, CancellationToken cancellationToken )
        {
        ...
        /// <returns>A <see cref="Task"/> that indicates when the operation has completed.</returns>
#if NET
        public Task PostMessageAsync( PrinterProxyMessage message, byte[]? extraData, CancellationToken cancellationToken )
#else
        public Task PostMessageAsync( PrinterProxyMessage message, byte[] extraData, CancellationToken cancellationToken )
#endif
        {
        ...
        /// <returns>A <see cref="Task"/> that indicates when the operation has completed.</returns>
#if NET
        protected Task PostResponseAsync( PrinterProxyMessage message, object? response, CancellationToken cancellationToken )
#else
        protected Task PostResponseAsync( PrinterProxyMessage message, object response, CancellationToken cancellationToken )
#endif
        {
            var messageResponse = new PrinterProxyMessageResponse( message )
            {
                Result = response
            };
        ...
        /// <param name="cancellationToken">A cancellation token that can be used to cancel the asynchronous operation.</param>
        /// <returns>A <see cref="Task"/> that indicates when the operation has completed.</returns>
        public Task<T> SendMessageAsync<T>( PrinterProxyMessage message, CancellationToken cancellationToken )
        {
        ...
        /// <returns>A <see cref="Task"/> that indicates when the operation has completed.</returns>
#if NET
        public async Task<T> SendMessageAsync<T>( PrinterProxyMessage message, byte[]? extraData, CancellationToken cancellationToken )
#else
        public async Task<T> SendMessageAsync<T>( PrinterProxyMessage message, byte[] extraData, CancellationToken cancellationToken )
#endif
        {
        ...
        /// <summary>
        /// Returns a subclass of <see cref="PrinterProxyMessageType"/> based
        ...
        private static (PrinterProxyMessage, int) GetMessageObject( ReadOnlySpan<byte> data, PrinterProxyMessageType messageType )
        {
            if ( !PrinterProxyMessage.MessageLookup.TryGetValue( messageType, out var type ) || type == null )
            {
                throw new Exception( "Invalid message received." );
            }

            var reader = new Utf8JsonReader( data );
            var message = JsonSerializer.Deserialize( ref reader, type ) as PrinterProxyMessage
                ?? throw new Exception( "Invalid message received." );
        ...
        /// <returns>A <see cref="Task"/> that indicates when the operation has completed.</returns>
        protected abstract Task OnMessageAsync( PrinterProxyMessage message, ReadOnlyMemory<byte> extraData, CancellationToken cancellationToken );
