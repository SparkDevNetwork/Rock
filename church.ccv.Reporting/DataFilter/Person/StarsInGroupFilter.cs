...
{
    [Description( "Filter STARS people on whether they are in the specified group or groups.  Any Children found in the result set will be replaced by their parents." )]
    [Export( typeof( DataFilterComponent ) )]
    [ExportMetadata( "ComponentName", "STARS Person In Group(s) Filter" )]
    public class StarsInGroupFilter : DataFilterComponent
    {
        #region Properties
        ...
        {
            return string.Format( @"Rock.reporting.formatFilterForGroupFilterField('{0}', $content)", not ? "Not in STARS groups:" : "In STARS groups:" );
        }
        ...
        public virtual string GroupFilterFormatSelection( string selection, bool not )
        {
            ...
            if ( selectionValues.Length >= 2 )
            {
                ...
                if ( groups != null )
                {
                    result = string.Format( not ? "Not in STARS groups: {0}" : "In STARS groups: {0}", groups.Select( a => a.Name ).ToList().AsDelimited( ", ", " or " ) );
        ...
        public override Expression GetExpression( Type entityType, IService serviceInstance, ParameterExpression parameterExpression, string selection )
        {
            ...
            if ( selectionValues.Length >= 2 )
            {
                ...
                // get a list of people from the group query above
                var people = new PersonService( ( RockContext ) serviceInstance.Context ).Queryable()
                    .Where( p => groupMemberServiceQry.Any( xx => xx.PersonId == p.Id ) );

                // get the parents of any children in the group query
                var parents = new PersonService( ( RockContext ) serviceInstance.Context ).Queryable()
                    .Where( p => p.Members.Where( a => a.GroupRoleId == adultRoleId )
                        .Any( a => a.Group.Members
                            .Any( c => c.GroupRoleId == childRoleId && people.Contains( c.Person ) ) ) );

                // get a list of adult group members
                var adults = new GroupMemberService( ( RockContext ) serviceInstance.Context ).Queryable()
                    .Where( g => g.GroupRoleId == adultRoleId );

                // combined the adults with the parents of children from the group query
                var qry = new PersonService( ( RockContext ) serviceInstance.Context ).Queryable()
                    .Where( p => people.Where( pp => adults.Any( xx => xx.PersonId == pp.Id ) )
                        .Union( parents ).Any( xx => xx.Id == p.Id ) );
