...
namespace org.lakepointe.Workflow.Action.CheckIn
{
    ...
    [Description( "Custom Calculates and updates the LastCheckIn property on check-in objects. Custom excludes hard limit attendances from preselection and allows a second scan to go further back when evaluating previous check ins." )]
    [Export( typeof( ActionComponent ) )]
    [ExportMetadata( "ComponentName", "Custom Calculate Last Attended" )]
    [IntegerField("AutoSelectDaysBack", "The number of days to look back for previous check-in location.", true)]
    public class CustomCalculateLastAttended : CheckInActionComponent
    {
        private const string ABSOLUTE_LIMITS_NOTE = "Using Absolute Limits.";
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            if ( checkInState != null )
            {
                ...
                DateTime autoSelectDaysBack = RockDateTime.Today.AddDays(0 - GetAttributeValue(action, "AutoSelectDaysBack").AsInteger());
                ...
                foreach ( var family in checkInState.CheckIn.GetFamilies( true ) )
                {
                    foreach ( var person in family.People.Union( family.OriginalPeople ) )
                    {
                        ...
                        if ( personAttendance.Any() )
                        {
                            ...
                            // CUSTOM: This list doesn't contain attendance marked as absolute limit so when this list is used to identify checkin preselections groups that are overflow don't get suggested
                                 ...
                                 person.LastCheckIn.Value.CompareTo( preSelectCutoff  ) >= 0 )
                            {
                                ...
                                {
                                    if ( item.ScheduleId.HasValue && !previousCheckins.Any( i => i.ScheduleId == item.ScheduleId ) && item.Note != ABSOLUTE_LIMITS_NOTE )
                                    {
                            ...
                            {
                                if (person.LastCheckIn.HasValue &&
                                     autoSelectDaysBack.CompareTo(RockDateTime.Today) < 0 &&
                                     person.LastCheckIn.Value.CompareTo(autoSelectDaysBack) >= 0)
                                {
                                    foreach (var item in personAttendance
                                        .Where(a => a.StartDateTime.Date == person.LastCheckIn.Value.Date)
                                        .OrderBy(a => a.Schedule.StartTimeOfDay)
