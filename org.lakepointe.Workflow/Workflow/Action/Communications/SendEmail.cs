...
namespace org.lakepointe.Workflow.Action.Communications
{
    /// <summary>
    /// Sends email
    /// </summary>
    [ActionCategory( "LPC Communications" )]
    ...
    [WorkflowTextOrAttribute( "From Name",
        "From Name Attribute",
        "The name or an attribute that contains the person or name that email should be sent from (will default to the from person name). <span class='tip tip-lava'></span>",
        false,
        "",
        "",
        0,
        AttributeKey.FromName,
    ...
    public class SendEmail : ActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, object entity, out List<string> errorMessages )
        {
            ...
            {
                var nameAttribute = AttributeCache.Get( fromNameGuid.Value, rockContext );
                if ( nameAttribute != null )
                {
                    ...
                    {
                        if ( nameAttribute.FieldType.Class == "Rock.Field.Types.PersonFieldType" )
                        {
                            Guid namePersonAliasGuid = fromNameAttributeValue.AsGuid();
                            if ( !namePersonAliasGuid.IsEmpty() )
                            {
                                var namePerson = new PersonAliasService( rockContext ).Queryable()
                                    .Where( a => a.Guid.Equals( namePersonAliasGuid ) )
                                    .Select( a => a.Person )
                                    .FirstOrDefault();
                                if ( namePerson != null )
                                {
                                    fromName = namePerson.FullName;
                                }
                            }
                        }
                        else
            ...
            {
                fromName = fromNameValue.ResolveMergeFields( mergeFields );
            }
