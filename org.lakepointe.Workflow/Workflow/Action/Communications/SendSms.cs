...
namespace org.lakepointe.Workflow.Action.Communications
{
    ...
    [Description( "Sends an SMS message. The recipient can either be a person or group attribute or a phone number entered in the 'To' field." )]
    [Export( typeof( ActionComponent ) )]
    [ExportMetadata( "ComponentName", "SMS Send From Attribute" )]

    [WorkflowAttribute( "From", "The number to originate message from (configured under Admin Tools > Communications > SMS Phone Numbers).",
        true, "", "", 0, null,
    ...
    [WorkflowAttribute( "Attachment",
        "Workflow attribute that contains the attachment to be added. Note that when sending attachments with MMS; jpg, gif, and png images are supported for all carriers. Support for other file types is dependent upon each carrier and device. So make sure to test sending this to different carriers and phone types to see if it will work as expected.",
        false, "", "", 3, null,
    ...
    public class SendSms : ActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            {
                var a = AttributeCache.Get( fromGuid.Value, rockContext );
                if ( a != null )
                {
                    string fromAttributeValue = action.GetWorkflowAttributeValue( fromGuid.Value );
                    var fromValue = DefinedValueCache.Get(fromAttributeValue.AsGuid(), rockContext);
                    if (fromValue != null)
                    {
                        fromId = fromValue.Id;
                    }
