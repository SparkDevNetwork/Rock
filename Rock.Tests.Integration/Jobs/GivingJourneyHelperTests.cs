...
namespace Rock.Tests.Integration.Jobs
{
    ...
    public class GivingJourneyHelperTests
    {
        ...
        [TestMethod]
        [DataRow( GivingJourneyStage.FormerGiver, 376, 383, 390, 397 )]
        [DataRow( GivingJourneyStage.FormerGiver, 390, 420, 450 )]
        [DataRow( GivingJourneyStage.FormerGiver, 400, 414, 428 )]

        [DataRow( GivingJourneyStage.LapsedGiver, 151, 200, 260 )]
        [DataRow( GivingJourneyStage.LapsedGiver, 161, 170, 190 )]

        [DataRow( GivingJourneyStage.NewGiver, 7 )]
        [DataRow( GivingJourneyStage.NewGiver, 149, 14, 1 )]

        // gives consistently, but first transaction was less than 150 days ago
        [DataRow( GivingJourneyStage.NewGiver, 7, 14, 21, 28 )]

        [DataRow( GivingJourneyStage.OccasionalGiver, 40, 120, 190 )]
        [DataRow( GivingJourneyStage.OccasionalGiver, 40, 120, ( 120 + 93 ) )]

        // Started recently, but gave more than 5 times, and has been giving consistently
        [DataRow( GivingJourneyStage.ConsistentGiver, 7, 14, 21, 28, 35, 42 )]

        // Has been giving consistently starting more than 150 days ago, and gave three times, and hasn't lapsed more than 149 days
        [DataRow( GivingJourneyStage.ConsistentGiver, 151, 144, 137 )]

        // Has been giving consistently every 2 weeks for over a year
        [DataRow( GivingJourneyStage.ConsistentGiver, 14, 28, 42, 56, 70, 84, 98, 112, 126, 140, 154, 168, 182, 196, 210, 224, 238, 252, 266, 280, 294, 308, 322, 336, 350, 364, 378, 392, 406, 420, 434, 448, 462 )]

        // Has been giving consistently every 1 weeks for over a year
        [DataRow( GivingJourneyStage.ConsistentGiver, 7, 14, 21, 28, 35, 42, 49, 56, 63, 70, 77, 84, 91, 98, 105, 112, 119, 126, 133, 140, 147, 154, 161, 168, 175, 182, 189, 196, 203, 210, 217, 224, 231, 238, 245, 252, 259, 266, 273, 280, 287, 294, 301, 308, 315, 322, 329, 336, 343, 350, 357, 364, 371, 378, 385, 392, 399, 406 )]
        ...
        // they look like a new giver since their first gift was 7 days ago, but NewGiverFirstGiftInTheLastDays is null and they only gave once, so they wouldn't fall into a stage
        [DataRow( null, 7 )]

        // they look like a new giver since their first gift was less than 150 days ago and gave 3 times, but NewGiverFirstGiftInTheLastDays is null, they would fall into OccasionalGiver instead of FirstTime giver
        [DataRow( GivingJourneyStage.OccasionalGiver, 149, 14, 1 )]

        // Just started giving, and gives consistently, but less than 4 times. So they might look like a new giver. However, if NewGiverFirstGiftInTheLastDays is null, 
        // they would fall into Consistent giver instead of FirstTime giver since they are giving consistently every week
        [DataRow( GivingJourneyStage.ConsistentGiver, 7, 14, 21, 28 )]

        // Just started giving, and started giving consistently, but only 2 times. So they might look like a new giver. However, if NewGiverFirstGiftInTheLastDays is null,
        // They would fall into Consistent Giver instead of FirstTime giver.
        // This is a situation where they really shouldn't be called a consistent giver, because they only gave 2 times.  But since NewGiver isn't configured, they end up here.
        [DataRow( GivingJourneyStage.ConsistentGiver, 7, 14)]
