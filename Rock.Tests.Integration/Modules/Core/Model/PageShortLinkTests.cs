...
namespace Rock.Tests.Integration.Modules.Core.Model
{
    ...
    public class PageShortLinkTests : DatabaseTestsBase
    {
        ...
        public void Interaction_SetUtmFieldsWithUnspecifiedValues_ResetsEmptyFieldValue()
        {
            ...
            // Verify that fields with existing values are reset by a subsequent update.
            interaction.SetUTMFieldsFromURL( $"http://www.rocksolidchurchdemo.com/signup?utm_source=google&utm_medium=organic&utm_campaign={_testCampaignNameA}" );
            interaction.SetUTMFieldsFromURL( $"http://www.rocksolidchurchdemo.com/signup" );

            Assert.That.IsEmpty( $"{interaction.SourceValueId}{interaction.MediumValueId}{interaction.CampaignValueId}" );

            // Verify that fields with existing values are reset by a subsequent update.
            interaction.SetUTMFieldsFromURL( $"http://www.rocksolidchurchdemo.com/signup?utm_source=new_source&utm_medium=new_medium&utm_campaign=new_campaign" );
            interaction.SetUTMFieldsFromURL( $"http://www.rocksolidchurchdemo.com/signup" );

            Assert.That.IsEmpty( $"{interaction.Source}{interaction.Medium}{interaction.Campaign}" );
        }
        ...
        [DataTestMethod]
        public void PageShortlink_WithInvalidUrl_ReturnsUrlWithUtmAsEmptyString()
        {
            var shortlink = CreateTestPageShortlinkWithUtmValues( "invalid_url", "google", "website", "campaign 1", "my+search+terms", "banner_image" );

            Assert.That.AreEqual( "invalid_url", shortlink.Url );
            Assert.That.IsTrue( shortlink.UrlWithUtm.IsNullOrWhiteSpace() );
        }
