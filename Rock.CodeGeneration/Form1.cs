...
" );
        ...
        private void WriteViewModelTypeScriptFile( string rootFolder, Type type, List<Type> viewModelTypes )
        {
            ...
            var typeProperties = GetEntityProperties( type, false, true, true );
            var viewModelProperties = GetEntityProperties( viewModelType, false, true, true )
                .Select( p =>
                {
                    var originalType = typeProperties.GetValueOrNull( p.Key );
                    ...
                    {
                        tsType = GetTypescriptType( propertyType, isRequired, imports );
                    }
                    else if ( !typeAttribute.ImportStatement.IsNullOrWhiteSpace() )
                    {
                        imports.Add( typeAttribute.ImportStatement );
                    }
        ...
        private static string GetTypescriptType( Type type, bool isRequired, HashSet<string> imports )
        {
            ...
            switch ( Type.GetTypeCode( type ) )
            {
                case TypeCode.Object:
                    ...
                    {
                        var itemType = GetTypescriptType( type.GetElementType(), false, imports );
                        tsType = $"({itemType})[]";
                    }
                    else
                    {
