...
{
    /// <summary>
    /// A derivation of the Fluid LowerThanExpression that implements the comparsion operators "<" and "<=".
    ...
    /// </summary>
    public class LavaLessThanExpression : Expression
    {
        public LavaLessThanExpression( Expression left, Expression right, bool failIfEqual )
        {
            Left = left;
            Right = right;
        ...
        private FluidValue Evaluate( FluidValue leftValue, FluidValue rightValue )
        {
            ...
            if ( rightObject is IComparable rightComparable )
            {
                if ( FailIfEqual )
                {
                    return rightComparable.CompareTo( rightObject ) > 0 ? BooleanValue.True : BooleanValue.False;
                }
                else
                {
                    return rightComparable.CompareTo( rightObject ) >= 0 ? BooleanValue.True : BooleanValue.False;
