...
namespace rocks.pillars.Jobs
{
    ...
    /// </summary>
    [DisplayName( "Send Attendance Reminders Based on Attribute" )]
    [Description( "Sends a reminder to group leaders about entering attendance for their group meeting if their group has a specific attribute set to true." )]
    ...
    [AttributeField( 
        Rock.SystemGuid.EntityType.GROUP, 
        "Group Boolean Attribute", 
        Description = "An attribute on groups that indicates if group should be sent attendance reminder.", 
        IsRequired = true,
        Key = AttributeKey.GroupBooleanAttribute,
        Order = 4 )]
    ...
    public class SendAttendanceReminderBasedOnAttribute : IJob
    {
        ...
        private static class AttributeKey
        {
            ...
            public const string GroupBooleanAttribute = "GroupBooleanAttribute";
        }
        ...
        /// </summary>
        public SendAttendanceReminderBasedOnAttribute() { }
        ...
        public virtual void Execute( IJobExecutionContext context )
        {
            ...
            // Filter groups that do not have attribute value.
            var groupBooleanAttr = AttributeCache.Get( dataMap.GetString( "GroupBooleanAttribute" ).AsGuid() );
            occurrences = FilterGroupsWithoutAttributeValue( occurrences, rockContext, groupBooleanAttr );
        ...
        private Dictionary<int, List<DateTime>> FilterGroupsWithoutAttributeValue( Dictionary<int, List<DateTime>> occurrences, RockContext rockContext, AttributeCache groupBooleanAttr )
        {
            var filteredOccurrences = new Dictionary<int, List<DateTime>>();

            var boolFieldType = FieldTypeCache.Get( Rock.SystemGuid.FieldType.BOOLEAN );

            if ( groupBooleanAttr != null && boolFieldType != null && groupBooleanAttr.FieldTypeId == boolFieldType.Id )
            {
                var groupIds = occurrences.Keys.ToList();
                    ...
                    .ToList() )
                {
                    ...
                    if ( group.GetAttributeValue( groupBooleanAttr.Key ).AsBoolean( true ) )
                    {
                        filteredOccurrences.AddOrIgnore( group.Id, occurrences[group.Id] );
                    }
                }
