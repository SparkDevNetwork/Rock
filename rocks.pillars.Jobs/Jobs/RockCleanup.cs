    ...
    public class RockCleanup : IJob
    {
        ...
        private int CleanupOrphanedAttributes( JobDataMap dataMap )
        {
            ...
            using ( RockContext rockContext = new RockContext() )
            {
                ...
                var orphanedAttributeMatrices = GetOrphanedAttributeMatrices(attributeMatrixService).ToList();
...
" );
        ...
        private int EnsureScheduleEffectiveStartEndDates()
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                {
                    if ( EnsureEffectiveStartEndDates(schedule))
                    {
                        rowsUpdated++;
                    }
                }
        ...
        private int UpdateMedianPageLoadTimes()
        {
            ...
            var serviceJob = serviceJobService.Get( Rock.SystemGuid.ServiceJob.ROCK_CLEANUP.AsGuid() );
        ...
        private bool EnsureEffectiveStartEndDates(Schedule schedule)
        {
            /*
             * 12/6/2019 BJW
             *
            ...
            var originalEffectiveEndDate = schedule.EffectiveEndDate;
            var originalEffectiveStartDate = schedule.EffectiveStartDate;
            schedule.EffectiveStartDate = calEvent.DtStart?.Value.Date;
            ...
            if ( hasRuleWithEndDate )
            {
                ...
                {
                    schedule.EffectiveEndDate = DateTime.MaxValue;
                }
                else
                {
                    schedule.EffectiveEndDate = endDateRules.Max( rule => rule.Until );
                }
            }
            ...
            if ( adjustEffectiveDateForLastOccurrence
                 && schedule.EffectiveEndDate != DateTime.MaxValue )
            {
                var occurrences = schedule.GetICalOccurrences( DateTime.MinValue, DateTime.MaxValue );
                ...
                if ( occurrences.Any() )
                {
                    ...
                    if ( schedule.EffectiveEndDate == null
                         || lastOccurrenceDate > schedule.EffectiveEndDate )
                    {
                        schedule.EffectiveEndDate = lastOccurrenceDate;
                    }
                }
            }

            return ( schedule.EffectiveEndDate?.Date != originalEffectiveEndDate?.Date || schedule.EffectiveStartDate?.Date != originalEffectiveStartDate?.Date );
        }

        private IEnumerable<AttributeMatrix> GetOrphanedAttributeMatrices(AttributeMatrixService matrixService)
        {
            string sql = @"
            ...
            return matrixService.ExecuteQuery(sql);
        }
