...
namespace Rock.Tests.UnitTests.Rock.CheckIn.v2.Labels.Renderers
{
    ...
    public class ZplImageHelperTests
    {
        ...
        [TestMethod]
        public void CreateImage_WithWideResizeAndCropRequest_ReturnsCenterCropArea()
        {
            var expectedWidth = 8;
            var expectedHeight = 1;
            var expectedRowValue = 255;
            var image = new Image<Rgba32>( 24, 1, new Rgba32( 255, 255, 255, 255 ) );
            var options = new ZplImageOptions
            {
                Width = expectedWidth,
                Height = expectedHeight
            ...
            for ( int i = 8; i < 16; i++ )
            {
                image[i, 0] = new Rgba32( 0, 0, 0, 255 );
            }
            ...
            using ( var ms = new MemoryStream() )
            {
                ...
                Assert.That.AreEqual( expectedWidth, imageCache.Width );
                Assert.That.AreEqual( expectedHeight, imageCache.Height );
                Assert.That.AreEqual( expectedRowValue, imageCache.ImageData[0] );
            }
        }

        [TestMethod]
        public void CreateImage_WithTallResizeAndCropRequest_ReturnsCenterCropArea()
        {
            var expectedWidth = 1;
            var expectedHeight = 8;
            var expectedRowValue = 1 << 7;
            var image = new Image<Rgba32>( 1, 24, new Rgba32( 255, 255, 255, 255 ) );
            var options = new ZplImageOptions
            {
                Width = expectedWidth,
                Height = expectedHeight
            ...
            for ( int i = 8; i < 16; i++ )
            {
                image[0, i] = new Rgba32( 0, 0, 0, 255 );
            }
            ...
            using ( var ms = new MemoryStream() )
            {
                ...
                var isAllExpectedBits = imageCache.ImageData.All( d => d == expectedRowValue );

                Assert.That.AreEqual( expectedWidth, imageCache.Width );
                Assert.That.AreEqual( expectedHeight, imageCache.Height );
                Assert.That.IsTrue( isAllExpectedBits );
            }
        }
