...
namespace Rock.Model
{
    ...
    public partial class BinaryFileService : Service<BinaryFile>
    {
        ...
        public bool CanDelete( BinaryFile item, out string errorMessage )
        {
            ...
            if ( new Service<Contact>( Context ).Queryable().Any( a => a.PhotoId == item.Id ) )
            {
                errorMessage = string.Format( "This {0} is assigned to a {1}.", BinaryFile.FriendlyTypeName, Contact.FriendlyTypeName );
                return false;
            }
            ...
            if ( new Service<Group>( Context ).Queryable().Any( a => a.PhotoId == item.Id ) )
            {
                errorMessage = string.Format( "This {0} is assigned to a {1}.", BinaryFile.FriendlyTypeName, Group.FriendlyTypeName );
                return false;
            }
