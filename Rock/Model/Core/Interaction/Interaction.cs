...
namespace Rock.Model
{
    ...
    public partial class Interaction : Model<Interaction>
    {
        ...
        {
            // Reset the current UTM values.
            this.Source = null;
            this.SourceValueId = null;
            this.Medium = null;
            this.MediumValueId = null;
            this.Campaign = null;
            this.CampaignValueId = null;
            this.Content = null;
            this.Term = null;
            ...
            try
            {
                ...
                {
                    this.SourceValueId = DefinedTypeCache.Get( SystemGuid.DefinedType.UTM_SOURCE )
                        .DefinedValues
                        .FirstOrDefault( v => v.Value.Equals( source, StringComparison.OrdinalIgnoreCase ) )?.Id;

                    if ( this.SourceValueId == null )
                    {
                        this.Source = source.Truncate( 25 );
                    }
                }

                // Get the UTM Medium.
                var medium = urlParams.Get( "utm_medium" );
                if ( medium.IsNotNullOrWhiteSpace() )
                {
                    this.MediumValueId = DefinedTypeCache.Get( SystemGuid.DefinedType.UTM_MEDIUM )
                        .DefinedValues
                        .FirstOrDefault( v => v.Value.Equals( medium, StringComparison.OrdinalIgnoreCase ) )?.Id;
                    if ( this.MediumValueId == null )
                    {
                        this.Medium = medium.Truncate( 25 );
                    }
                }

                // Get the UTM Campaign.
                var campaign = urlParams.Get( "utm_campaign" );
                if ( campaign.IsNotNullOrWhiteSpace() )
                {
                    this.CampaignValueId = DefinedTypeCache.Get( SystemGuid.DefinedType.UTM_CAMPAIGN )
                        .DefinedValues
                        .FirstOrDefault( v => v.Value.Equals( campaign, StringComparison.OrdinalIgnoreCase ) )?.Id;
                    if ( this.CampaignValueId == null )
                    {
                        this.Campaign = campaign.Truncate( 50 );
                    }
                }
