...
namespace Rock.Model
{
    ...
    /// </summary>
    /// <remarks>
    /// This is the storage behind the <c>ForgeContentDetail</c> block. An
    /// administrator (or an AI agent over MCP) writes Vue source; Rock compiles
    /// it server side to a browser-loadable SystemJS module and both the source
    /// and the compiled output are persisted here. Visitors are served only the
    /// precompiled output. The nullable <see cref="BlockId"/> reserves room for
    /// a future reusable-library record that is not tied to a block.
    /// </remarks>
    /*
        8/14/2026 - CLAUDE

        CodeGenerateRest is intentionally omitted. CompiledContent must only ever
        be written by the server-side compile path, and Source is only readable
        with EDIT authorization of the owning block. A generated REST surface
        would bypass both rules; the block action and the agent skill are the
        only intended writers.

        Reason: Generated REST endpoints would bypass the compile-on-save contract.
    */
    [RockDomain( "CMS" )]
    [Table( "ForgeContent" )]
    [DataContract]
    [Rock.SystemGuid.EntityTypeGuid( "38F182A7-9FE4-4D7B-B483-59F615BDE41C" )]
    public partial class ForgeContent : Model<ForgeContent>
    {
    ...
    /// <summary>
    /// ForgeContent Configuration class.
    /// </summary>
    public partial class ForgeContentConfiguration : EntityTypeConfiguration<ForgeContent>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ForgeContentConfiguration"/> class.
        /// </summary>
        public ForgeContentConfiguration()
        {
            /*
                7/22/2026 - CLAUDE
