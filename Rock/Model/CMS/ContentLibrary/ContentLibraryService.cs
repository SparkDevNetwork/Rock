...
{
    public partial class ContentLibraryService
    {
        /// <summary>
        /// Gets a globally unique slug for a content library.
        /// </summary>
        /// <param name="slug">The slug requested by the person.</param>
        /// <param name="existingLibraryId">The identifier of the existing library to ignore when ensuring uniqueness.</param>
        /// <returns>A slug that is unique in the system or <c>null</c> if it wasn't valid.</returns>
        public string GetUniqueSlug( string slug, int? existingLibraryId = null )
        {
            bool isValid = false;
            ...
            var existingSlugs = Queryable()
                .Where( cl => !existingLibraryId.HasValue || cl.Id != existingLibraryId.Value )
                .Select( cl => cl.LibraryKey )
                .ToList()
                .Select( s => s.ToLower() )
                .ToList();
