    ...
    public partial class PageShortLink
    {
        ...
        private void ParseUrl( string url, out string protocolDomainAndPath, out NameValueCollection queryParameters, out string fragment )
        {
            ...
            else
            {
                ...
                {
                    fragment = url.Substring( fragmentStartIndex + 1 );
        ...
        public virtual string UrlWithUtm
        {
            get
            {
                ...
                {
                    queryParameters.Add( "utm_term", utmSettings.UtmTerm.Trim().ToLower().UrlEncode() );
                    hasUtmValues = true;
                }

                if ( utmSettings.UtmContent.IsNotNullOrWhiteSpace() )
                {
                    queryParameters.Add( "utm_content", utmSettings.UtmContent.Trim().ToLower().UrlEncode() );
                    hasUtmValues = true;
                }
                ...
                var urlWithUtm = protocolDomainAndPath + "?" + queryParameters.ToQueryString();
