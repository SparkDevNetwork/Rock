...
public partial class PersonSessionService
{
    ...
    internal PersonSession FindOrCreateDeviceComponentSession( RockRequestContext requestContext, UserLogin userLogin )
    {
        ...
        /*
            ...
            Reuse is scoped to the device's OWN session - the one the current
            request resolved from the cookie IT presented
            (RockRequestContext.PersonSession) - never a UserLogin-wide lookup.
            Mobile / TV and web share the same Database UserLogin, so a
            UserLogin-wide "find any active Component session" reused a session
            belonging to a different client (e.g. the person's web session, even
            one created 30+ minutes earlier). Keying reuse on the presented
            cookie's PersonSession Guid mirrors the web Login flow, so each
            client keeps its own session. Component sessions carry no uniqueness
            constraint (see the AddPersonSessionModel migration), so multiple
            concurrent sessions per UserLogin - one per device / browser - are
            legitimate.

            Reason: A device login reuses only THIS device's session, never
            another client's.
        */
        var requestSession = requestContext?.PersonSession;
        var priorSession = ( requestSession != null && requestSession.Guid != Guid.Empty )
    ...
    public PersonSession ResolveSessionForRequest( RockRequestContext requestContext )
    {
        ...
        {
            // Legacy-format cookie, tampered, or otherwise undecodable. Left alone
            // here; the legacy upgrade path picks up legacy-format cookies.
            return null;
        }
        ...
        {
            // Mark inactive (SaveHook stamps InactiveDateTime) and clear the cookie.
            session.IsActive = false;
            ( Context as RockContext ).SaveChanges();
            ExpireAuthCookie( requestContext );
            return null;
        ...
        // If the session's backing UserLogin is locked out or no longer confirmed,
        // sign out. The session is passed explicitly because it has not yet been
        // attached to the request context at this point, so the parameterless
        ...
        // PersonSession.IssuedDateTime - only the cookie's iat changes. It is
        // best-effort: a cookie-write failure (a late identity read after the
        // response has started, or an OWIN response that cannot take a Set-Cookie)
        // must NOT discard the resolved session, so the refresh is swallowed on
        // failure and happens on a later request instead.
