...
namespace Rock.Cache
{
    ...
    public class CacheGlobalAttributes : ItemCache<CacheGlobalAttributes>
    {
        ...
        private string GetValue( string key, CacheAttribute attributeCache, RockContext rockContext )
        {
            ...
            if ( saveValue )
            {
                ...
                if ( attributeValue == null )
                {
                    ...
                    if ( attribute == null )
                    {
                        ...
                        {
                            FieldTypeId = CacheFieldType.Get( new Guid( SystemGuid.FieldType.TEXT ) ).Id,
        ...
        /// </summary>
        /// <returns></returns>
        public static CacheGlobalAttributes Get()
        {
            return GetOrAddExisting( "All", Load );
        }

        private static CacheGlobalAttributes Load()
        {
            var globalAttributes = new CacheGlobalAttributes
            {
        ...
        internal static Dictionary<string, object> GetLegacyMergeFields( Person currentPerson )
        {
            ...
            var globalAttributeValues = globalAttributes.Attributes
                ...
                .ToDictionary<CacheAttribute, string, object>( attributeCache => attributeCache.Key, attributeCache => globalAttributes.GetValueFormatted( attributeCache.Key ) );
