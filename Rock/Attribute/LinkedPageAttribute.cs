...
namespace Rock.Attribute
{
    ...
    public class LinkedPageAttribute : FieldAttribute
    {
        ...
        public LinkedPageAttribute( string name, string description = "", bool required = true, string defaultValue = "", string category = "", int order = 0, string key = null )
            : base( SystemGuid.FieldType.PAGE_REFERENCE.AsGuid(), name, description, required, defaultValue, category, order, key )
        {
        }
        ...
        /// </summary>
        /// <param name="name">The name.</param>
        public LinkedPageAttribute( string name )
            : base( SystemGuid.FieldType.PAGE_REFERENCE.AsGuid(), name )
        {
        }
    }
}
