...
namespace Rock.Attribute
{
    ...
    public class WorkflowActivityTypeAttribute : FieldAttribute
    {
        ...
        public WorkflowActivityTypeAttribute( string name, string description = "", bool required = true, string defaultValue = "", string category = "", int order = 0, string key = null )
            : base( SystemGuid.FieldType.WORKFLOW_ACTIVITY.AsGuid(), name, description, required, defaultValue, category, order, key )
        {
        }
        ...
        /// </summary>
        /// <param name="name">The name.</param>
        public WorkflowActivityTypeAttribute( string name )
            : base( SystemGuid.FieldType.WORKFLOW_ACTIVITY.AsGuid(), name )
        {
        }
    }
}
