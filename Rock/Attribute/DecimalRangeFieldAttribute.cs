...
namespace Rock.Attribute
{
    ...
    public class DecimalRangeFieldAttribute : FieldAttribute
    {
        ...
        public DecimalRangeFieldAttribute( string name, string description = "", bool required = true, string defaultValue = ",", string category = "", int order = 0, string key = null )
            : base( SystemGuid.FieldType.DECIMAL_RANGE.AsGuid(), name, description, required, defaultValue, category, order, key )
        {
        }
        ...
        /// </summary>
        /// <param name="name">The name.</param>
        public DecimalRangeFieldAttribute( string name )
            : base( SystemGuid.FieldType.DECIMAL_RANGE.AsGuid(), name )
        {
        }
    }
}
