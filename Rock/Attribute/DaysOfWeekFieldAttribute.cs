...
namespace Rock.Attribute
{
    ...
    public class DaysOfWeekFieldAttribute : FieldAttribute
    {
        ...
        public DaysOfWeekFieldAttribute( string name = "Days of Week", string description = "", bool required = true, string category = "", int order = 0, string key = null )
            : base( SystemGuid.FieldType.DAYS_OF_WEEK.AsGuid(), name, description, required, null, category, order, key )
        {
        }
        ...
        /// </summary>
        /// <param name="name">The name.</param>
        public DaysOfWeekFieldAttribute( string name )
            : base( SystemGuid.FieldType.DAYS_OF_WEEK.AsGuid(), name )
        {
        }
    }
}
