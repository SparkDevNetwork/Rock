...
namespace Rock.Attribute
{
    ...
    public class GenderFieldAttribute : FieldAttribute
    {
        ...
        public GenderFieldAttribute( string name = "Gender", string description = "", bool required = true, Gender defaultValue = Gender.Unknown, string category = "", int order = 0, string key = null )
            : base( SystemGuid.FieldType.GENDER.AsGuid(), name, description, required, defaultValue.ConvertToInt().ToString(), category, order, key )
        {
        }
        ...
        /// </summary>
        /// <param name="name">The name.</param>
        public GenderFieldAttribute( string name )
            : base( SystemGuid.FieldType.GENDER.AsGuid(), name )
        {
            DefaultValue = Gender.Unknown.ConvertToInt().ToString();
        }
