...
namespace Rock.Web.v2
{
    ...
    internal class LavaPageLayoutFactory
    {
        ...
        private void ProcessContentNodes( IDocument document, IElement container, LavaPageLayoutContext context )
        {
            ...
            {
                var nodes = context.Parser.ParseFragment( "{% if Page.PageDisplayIcon == true and PageIconCssClass != null and PageIconCssClass != empty %}<div class=\"page-icon\"><i class=\"{{ PageIconCssClass | Escape }}\"></i></div>{% endif %}", document.Body ).ToArray();
            ...
            {
                var nodes = context.Parser.ParseFragment( "{% if Page.PageDisplayTitle and PageTitle != null and PageTitle != empty %}{{ PageTitle | Escape }}{% endif %}", document.Body ).ToArray();
            ...
            {
                var nodes = context.Parser.ParseFragment( "{% if Page.PageDisplayDescription and Page.Description != null and Page.Description != empty %}<div class=\"pageoverview-description\">{{ Page.Description | Escape }}</div>{% endif %}", document.Body ).ToArray();
