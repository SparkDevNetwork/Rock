            ...
            try
            {
                if ( this.PageParameter( "ShowDebugTimings" ).AsBoolean() )
                {
                    ...
                    if ( lblShowDebugTimings != null )
                    {
                        ...
                        lblShowDebugTimings.Text += string.Format( "<tr><td class='debug-timestamp'>{1} ms</td><td style='padding-left: 24px;'>{0} <small><span style='color:#A4A4A4'>(Block Id: {2})</span></small></td><td class='debug-timestamp'><span data-duration-replace/></td><td class='debug-waterfall'><span class='debug-chart-bar' data-start-location='{1}' data-duration=''> </td></tr>", this.BlockName, Math.Round( tsDuration.TotalMilliseconds, 2 ), BlockCache.Id );
