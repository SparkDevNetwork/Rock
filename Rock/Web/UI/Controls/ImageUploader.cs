    ...
    public class ImageUploader : CompositeControl, IRockControl, IPostBackEventHandler
    {
        ...
        public void RenderBaseControl( HtmlTextWriter writer )
        {
            ...
            if ( IsBinaryFile && BinaryFileId.HasValue )
            {
                ...
                {
                    thumbnailImageUrl = string.Format( "{0}GetImage.ashx?isBinaryFile=T&fileIdKey={1}", System.Web.VirtualPathUtility.ToAbsolute( "~" ), IdHasher.Instance.GetHash( BinaryFileId.Value ) );
                }
                else
                {
                    thumbnailImageUrl = string.Format( "{0}GetImage.ashx?isBinaryFile=T&id={1}", System.Web.VirtualPathUtility.ToAbsolute( "~" ), BinaryFileId.Value );
                }
