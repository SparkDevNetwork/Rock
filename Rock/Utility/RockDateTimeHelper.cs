...
" );
        ...
        internal static DateRange CalculateDateRangeFromDelimitedValues( string value, DateTime? currentDateTime = null )
        {
            ...
            {
                var slidingDateRangeMode = splitValues[0].ConvertToEnum<Enums.Controls.SlidingDateRangeType>();
                var numberOfTimeUnits = splitValues[1].AsIntegerOrNull() ?? 1;
                var timeUnit = splitValues[2].ConvertToEnumOrNull<Enums.Controls.TimeUnitType>();
                var startDateTime = splitValues[3].AsDateTime();
                var endDateTime = splitValues[4].AsDateTime();

                return CalculateDateRangeFromDelimitedValues( slidingDateRangeMode, numberOfTimeUnits, timeUnit, startDateTime, endDateTime, currentDateTime );
            }
        ...
        /// Calculates the date range from delimited values in format SlidingDateRangeType|Number|TimeUnitType|StartDate|EndDate
        /// </summary>
        /// <param name="slidingDateRangeMode">The sliding date range mode to use with the calculation.</param>
        /// <param name="numberOfTimeUnits">The number of time units to use with the calculation; this is only used for certain <see cref="Enums.Controls.SlidingDateRangeType"/> values.</param>
        /// <param name="timeUnit">The time unit to use with the calculation; this is only used for certain <see cref="Enums.Controls.SlidingDateRangeType"/> values.</param>
        /// <param name="startDateTime">The start date to use with the calculation; this is only used when <see cref="Enums.Controls.SlidingDateRangeType.DateRange"/> is specified.</param>
        /// <param name="endDateTime">The end date to use with the calculation; this is only used when <see cref="Enums.Controls.SlidingDateRangeType.DateRange"/> is specified.</param>
        /// <param name="currentDateTime">If set then this value will be used for any calculations based on the current date and time; otherwise RockDateTime.Now will be used.</param>
        /// <returns>A <see cref="DateRange"/> object that contains the start and end dates.</returns>
        internal static DateRange CalculateDateRangeFromDelimitedValues( Enums.Controls.SlidingDateRangeType slidingDateRangeMode, int? numberOfTimeUnits, Enums.Controls.TimeUnitType? timeUnit, DateTime? startDateTime, DateTime? endDateTime, DateTime? currentDateTime = null )
        {
            DateRange result = new DateRange();

            // Set some sane defaults.
            currentDateTime = currentDateTime ?? RockDateTime.Now;
            numberOfTimeUnits = numberOfTimeUnits ?? 1;

            if ( slidingDateRangeMode == Enums.Controls.SlidingDateRangeType.Current )
            {
                switch ( timeUnit )
                {
                    case Enums.Controls.TimeUnitType.Hour:
                    ...
                    case Enums.Controls.TimeUnitType.Month:
                    ...
                    case Enums.Controls.TimeUnitType.Year:
                        result.Start = new DateTime( currentDateTime.Value.Year, 1, 1 );
                        result.End = new DateTime( currentDateTime.Value.Year + 1, 1, 1 );
                        break;
                }
            }
            else if ( ( Enums.Controls.SlidingDateRangeType.Last | Enums.Controls.SlidingDateRangeType.Previous ).HasFlag( slidingDateRangeMode ) )
            {
                // Last X Days/Hours. NOTE: addCount is the number of X that it go back (it'll actually subtract)
                int addCount = numberOfTimeUnits.Value;

                // if we are getting "Last" round up to include the current day/week/month/year
                int roundUpCount = slidingDateRangeMode == Enums.Controls.SlidingDateRangeType.Last ? 1 : 0;
            ...
            else if ( ( Enums.Controls.SlidingDateRangeType.Next | Enums.Controls.SlidingDateRangeType.Upcoming ).HasFlag( slidingDateRangeMode ) )
            {
                // Next X Days,Hours,etc
                int addCount = numberOfTimeUnits.Value;

                // if we are getting "Upcoming", round up to exclude the current day/week/month/year
                int roundUpCount = slidingDateRangeMode == Enums.Controls.SlidingDateRangeType.Upcoming ? 1 : 0;
            ...
            else if ( slidingDateRangeMode == Enums.Controls.SlidingDateRangeType.DateRange )
            {
                result.Start = startDateTime;
            ...
            if ( result.End.HasValue && timeUnit != Enums.Controls.TimeUnitType.Hour )
            {
        ...
        internal static string ConvertDateRangeDelimitedValueToRoundTripFormat( string value )
        {
            if ( value.IsNotNullOrWhiteSpace() )
            {
                ...
                if ( splitValues.Length == 5 )
                {
                    ...
                    if ( slidingDateRangeMode == Enums.Controls.SlidingDateRangeType.DateRange )
                    {
                        var startDateTime = splitValues[3].AsDateTime();
                        var endDateTime = splitValues[4].AsDateTime();
        ...
        /// <summary>
        /// Synchronizes the timezone configuration for the current database with the Rock application configuration.
        /// </summary>
        public static void SynchronizeTimeZoneConfiguration( string applicationTimeZoneId )
        {
            // Get the new timezone configuration value.
            applicationTimeZoneId = applicationTimeZoneId?.Trim();

            // Check that the requested timezone is available on the target database server.
            // The comparison uses an inefficient case conversion, but the number of rows is small so query performance
            // is not an issue here.
            var sqlValidateTimezone = $@"
SELECT [name]
FROM   sys.time_zone_info
WHERE  UPPER([name]) = UPPER('{applicationTimeZoneId}')
";
            var verifiedTimeZoneId = DbService.ExecuteScalar( sqlValidateTimezone ).ToStringSafe();
            if ( string.IsNullOrWhiteSpace( verifiedTimeZoneId ) )
            {
                throw new Exception( $"{nameof( SynchronizeTimeZoneConfiguration )} failed. The specified timezone is invalid. [TimezoneId={applicationTimeZoneId}]" );
            }

            // Get the configuration setting recorded in the database.
            var dbTimeZone = GetOrgTimeZoneForCurrentDatabase();

            // If the application configuration and database configuration are aligned, verify that the function exists
            // and is working correctly.
            if ( verifiedTimeZoneId.ToUpper() == dbTimeZone.ToUpper() )
            {
                // Verify that the function exists.
                var sqlFunctionExists = $@"
IF EXISTS (SELECT *
           FROM   sys.objects
           WHERE  object_id = OBJECT_ID(N'[dbo].[{RockGetDateFunctionName}]')
...
";
                ...
                var functionExists = DbService.ExecuteScalar( sqlFunctionExists ).ConvertToBooleanOrDefault() ?? false;

                if ( functionExists )
                {
                    // Verify that the function returns the expected timezone offset.
                    var actualOffset = DbService.ExecuteScalar( $"SELECT DATEDIFF( hour, GETUTCDATE(), [dbo].[{RockGetDateFunctionName}]() )" );

                    var expectedOffset = RockDateTime.OrgTimeZoneInfo.GetUtcOffset( RockDateTime.Now ).TotalHours;

                    if ( expectedOffset == Convert.ToDouble( actualOffset ) )
                    {
                        return;
                    }
                }
            ...
            // Recreate the GetDate user function using the specified timezone.
            CodeGen_SqlFunction_RockGetDate( verifiedTimeZoneId );

            // Update the database setting to synchronize with the application configuration.
            SetOrgTimeZoneForCurrentDatabase( verifiedTimeZoneId );
        }

        private const string RockGetDateFunctionName = "ROCKGETDATE";

        /// <summary>
        /// Generates the Sql Server Database Function "ROCKGETDATE".
        /// </summary>
        private static void CodeGen_SqlFunction_RockGetDate( string timeZoneName )
        {
            var functionSQL = $@"
/*
<doc>
    ...
    <remarks>
        This code is auto-generated by the Rock application ({nameof( RockDateTimeHelper )}.{nameof( CodeGen_SqlFunction_RockGetDate )}).
        The named timezone is synchronized with the current 'Time Zone' System Setting at application startup.
    </remarks>
    <code>
        SELECT [dbo].[{RockGetDateFunctionName}]()
    </code>
</doc>
*/
CREATE OR ALTER FUNCTION {RockGetDateFunctionName} ()
RETURNS datetime
AS
BEGIN
    DECLARE @ReturnValue datetime = SYSDATETIMEOFFSET() AT TIME ZONE '{timeZoneName}'
    RETURN @ReturnValue
END
";

            var result = DbService.ExecuteCommand( functionSQL, System.Data.CommandType.Text );

            if ( result != -1 )
            {
                throw new Exception( $"{nameof( CodeGen_SqlFunction_RockGetDate )} failed." );
            }
        }

        /// <summary>
        /// Gets the organization timezone for the current database.
        /// </summary>
        /// <returns>A timezone name in Windows standard format.</returns>
        public static string GetOrgTimeZoneForCurrentDatabase()
        {
            // Get the configuration setting recorded in the database.
            var dbTimeZone = SystemSettings.GetValue( Rock.SystemKey.SystemSetting.ORG_TIME_ZONE ).ToStringSafe().Trim();
            return dbTimeZone;
        }

        /// <summary>
        /// Sets the organization timezone for the current database.
        /// </summary>
        /// <param name="timeZoneId">A timezone name in Windows standard format.</param>
        public static void SetOrgTimeZoneForCurrentDatabase( string timeZoneId )
        {
            SystemSettings.SetValue( Rock.SystemKey.SystemSetting.ORG_TIME_ZONE, timeZoneId );
        }
