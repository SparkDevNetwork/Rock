...
namespace Rock
{
    ...
    internal static class CommunicationRecipientResponseExtensions
    {
        ...
        internal static ConversationMessageBag ToMessageBag( this CommunicationRecipientResponse response, bool loadAttachments )
        {
            ...
            {
                bag.PhotoUrl = MobileHelper.BuildPublicApplicationRootUrl( FileUrlHelper.GetFileUrl( response.RecipientPhotoId.Value, "GetImage", null, null, 256, 256 ) );
            }
                ...
                using ( var rockContext = new RockContext() )
                {
                    ...
                    foreach ( var attachment in attachments )
                    {
                        ...
                        bag.Attachments.Add( new ConversationAttachmentBag
                        {
                            ...
                            Url = MobileHelper.BuildPublicApplicationRootUrl( FileUrlHelper.GetFileUrl( attachment.Guid, "GetImage" ) ),
                            ThumbnailUrl = isImage ? MobileHelper.BuildPublicApplicationRootUrl( FileUrlHelper.GetFileUrl( attachment.Guid, "GetImage", null, null, 512, 512 ) ) : null
                        } );
                    }
                }
            }
