...
{
    /// <summary>
    /// Modifies an existing entity in Rock or creates a new entity from scratch.
    /// </summary>
    public class RockEntityModifyBlock : LavaBlockBase, ILavaSecured
    {
        #region Fields
        ...
        /// <summary>
        /// The type of entity being modified or created.
        /// </summary>
        private Type _entityType = null;

        /// <summary>
        /// The result of the modify entity operation. This will be set by
        /// various methods as the block processes.
        /// </summary>
        private readonly ModifyEntityResult _result = new ModifyEntityResult();
        ...
        public override void OnRender( ILavaRenderContext context, TextWriter result )
        {
            ...
            // Get the child property and attribute configuration elements
            var childElementsAreValid = ExtractBlockChildElements( context, _blockContent, out var updateElements, out var residualMarkup );

            if ( !childElementsAreValid || updateElements.Count == 0 )
            {
                result.Write( "Invalid list of update fields provided." );
                return;
            }
            ...
            {
                entity = ModifyOrAddEntity( context, entity, updateElements, IsSecurityEnabled( parms ), service, rockContext );
            }
            ...
            returnObject[_entityName] = entity;
        ...
        /// <summary>
        /// Modifies the entity based on the blocks configuration.
        /// </summary>
        /// <param name="context">The Lava render context that is used to provide additional information while processing the entity.</param>
        /// <param name="entity">The entity to be modified or created.</param>
        /// <param name="updateFields">The fields (properties and attributes) that should be updated.</param>
        /// <param name="isSecurityEnabled"><c>true</c> if security should be checked on the entity.</param>
        /// <param name="service">The service instance used to access the entity in the database.</param>
        /// <param name="rockContext">The context to use when accessing the database.</param>
        private IEntity ModifyOrAddEntity( ILavaRenderContext context, IEntity entity, Dictionary<string, object> updateFields, bool isSecurityEnabled, IService service, RockContext rockContext )
        {
            ...
            {
                var properties = updateFields["properties"];

                foreach ( var property in properties as List<object> )
                {
                    SetEntityProperty( property as Dictionary<string, object>, entity );
                }
            }
            ...
            if ( updateFields.ContainsKey( "attributes" ) )
            {
                attributesWereUpdated = true;
                entityAsIHasAttributes.LoadAttributes();

                var attributes = updateFields["attributes"];

                foreach ( var attribute in attributes as List<object> )
                {
                    SetEntityAttribute( attribute as Dictionary<string, object>, entityAsIHasAttributes );
                }
            }
            ...
            if ( _result.Success )
            {
                try
                {
                    // New people are difficult. We need to run some custom logic to ensure we get a valid person.
                    if ( isAdd && _entityName.ToLower() == "person" )
                    {
                        PersonService.SaveNewPerson( ( Person ) entity, rockContext );
                    }
                    ...
                    // If this is an add then reload the entity so that we have access to navigation properties.
                    if ( isAdd )
                    {
                        // This is dirty, but necessary as reloading the entity in the same rock context won't load the navigation properties.
        ...
        /// </summary>
        /// <param name="propertyInformation">The field configuration describing the property and value.</param>
        /// <param name="entity">The entity whose property is to be set.</param>
        private void SetEntityProperty( Dictionary<string, object> propertyInformation, IEntity entity )
        {
            var fieldConfiguration = ModifyEntityField.Property( propertyInformation );

            // Ensure we found the name
            if ( fieldConfiguration.Name.IsNullOrWhiteSpace() )
            {
                _result.Success = false;
                _result.ErrorMessage = "Property missing name field";
                return;
            }

            // Ensure value matches validation rules
            if ( !fieldConfiguration.IsValid )
            {
                _result.Success = false;
                _result.ValidationErrors.Add( new ValidationError( fieldConfiguration.ValidationMessage, fieldConfiguration.ControlName ) );
                _result.ErrorMessage = "A validation error occurred.";
            }
            ...
            try
            {
                ...
                PropertyInfo pi = entityType.GetProperty( fieldConfiguration.Name );

                if ( pi == null )
                {
                    _result.Success = false;
                    _result.ErrorMessage = $"Could not find property {fieldConfiguration.Name}";
                    return;
                }

                pi.SetValue( entity, ChangeType( fieldConfiguration.Value, pi.PropertyType ) );
            }
            catch ( Exception ex )
            {
                _result.Success = false;
                _result.ErrorMessage = $"Error setting property {fieldConfiguration.Name} to '{fieldConfiguration.Value}'. Error: {ex.Message}";
            }
        }

        /// <summary>
        /// Changes values type. This is needed to help address nullable types
        /// that Convert.ChangeType(...) can't handle itself. 🤷‍
        /// </summary>
        /// <param name="value">The value to be converted.</param>
        /// <param name="type">The target <see cref="Type"/> to convert the value into.</param>
        /// <returns>The value converted to the target type.</returns>
        private object ChangeType( object value, Type type )
        {
            // Special logic for nullable types
            if ( type.IsGenericType && type.GetGenericTypeDefinition().Equals( typeof( Nullable<> ) ) )
            {
                // If the type is nullable and the value is null or blank then return null.
                var valueAsString = value.ToString().Trim();
                if ( valueAsString.IsNullOrWhiteSpace() || valueAsString == "null" )
                {
                    return null;
                }
        ...
        /// </summary>
        /// <param name="attributeInformation">The field configuration describing the attribute and value.</param>
        /// <param name="entity">The entity whose property is to be set.</param>
        private void SetEntityAttribute( Dictionary<string, object> attributeInformation, IHasAttributes entity )
        {
            var fieldConfiguration = ModifyEntityField.Attribute( attributeInformation );

            if ( fieldConfiguration.Name.IsNullOrWhiteSpace() )
            {
                _result.Success = false;
                _result.ErrorMessage = "Attribute missing key field";
                return;
            }
            ...
            try
            {
                if ( !entity.Attributes.ContainsKey( fieldConfiguration.Name ) )
                {
                    _result.Success = false;
                    _result.ErrorMessage = $"No attribute found with the key of : {fieldConfiguration.Name}";
                }

                entity.SetAttributeValue( fieldConfiguration.Name, fieldConfiguration.Value );
            }
            catch ( Exception ex )
            {
                _result.Success = false;
                _result.ErrorMessage = $"Error setting attribute {fieldConfiguration.Name} to '{fieldConfiguration.Value}'. Error: {ex.Message}";
            }
        }
        ...
        private IEntity GetEntityFromIdAsGuid( Guid id, IService service )
        {
            ...
            return getMethod?.Invoke( service, new object[] { id } ) as IEntity;
        }
        ...
        private static LavaElementAttributes GetAttributesFromMarkup( string markup, ILavaRenderContext context )
        {
            ...
            {
                throw new Exception( "No parameters were found in your entity modify command. The syntax for a parameter is parmName:'' (note that you must use single quotes)." );
            }

            settings.AddOrIgnore( "securityenabled", "true" );
            settings.AddOrIgnore( "clientvalidationenabled", "false" );
            settings.AddOrIgnore( "return", "ModifyResult" );
        ...
        #region Start-up Functionality

        /// <inheritdoc/>
        public override void OnStartup( ILavaEngine engine )
        {
            RegisterEntityModifyCommands( engine );
        }

        /// <summary>
        /// Helper method to register the entity commands.
        /// </summary>
        private static void RegisterEntityModifyCommands( ILavaEngine engine )
        {
            ...
            engine.RegisterBlock( "modifybusiness", ( name ) => CreateEntityBlockInstance( "business" ) );
        ...
        /// <summary>
        /// Registers the modify command for the entity type.
        /// </summary>
        /// <param name="engine">The engine to register the command block into.</param>
        /// <param name="entityType">The entity type whose modify command will be registered.</param>
        private static void RegisterEntityCommand( ILavaEngine engine, EntityTypeCache entityType )
        {
            if ( entityType != null )
            {
                string blockName = $"modify{entityType.FriendlyName.RemoveSpaces().ToLower()}";
        ...
        private static RockEntityModifyBlock CreateEntityBlockInstance( string entityName )
        {
            // Return a block having a tag name corresponding to the entity name.
            return new RockEntityModifyBlock
            {
                SourceElementName = entityName,
        ...
        /// <inheritdoc/>
        public string RequiredPermissionKey => "RockEntityModify";
