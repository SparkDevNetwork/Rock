...
namespace Rock.Workflow.Action.Communications
{
    ...
    [Description( "Sends a chat message to a specified channel." )]
    [Export( typeof( ActionComponent ) )]
    [ExportMetadata( "ComponentName", "Chat Channel Message Send" )]

    #region Attributes

    [WorkflowAttribute( "Group",
        Description = "The group that represents the channel that should be used for the communication.",
    ...
    [Rock.SystemGuid.EntityTypeGuid( "834A69EE-E31E-4122-9D50-A613161C5A51" )]
    public class ChatChannelMessageSend : ActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, object entity, out List<string> errorMessages )
        {
            ...
            var groupGuid = GetAttributeValue( action, AttributeKey.Group, true ).AsGuidOrNull();
            if ( !groupGuid.HasValue )
            {
                errorMessages.Add( "A valid group was not provided." );
                return false;
            }

            var groupCache = GroupCache.Get( groupGuid.Value );
            if ( groupCache == null )
            {
            ...
            var sendTask = Task.Run( async () =>
            {
                ...
                {
                    var sendChatMessageResult = await chatHelper.SendChatChannelMessageAsync(
