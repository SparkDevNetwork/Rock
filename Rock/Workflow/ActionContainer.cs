...
namespace Rock.Workflow
{
    ...
    public class ActionContainer : Container<ActionComponent, IComponentData>
    {
        ...
        {
            var entityType = CacheEntityType.Get<Rock.Model.WorkflowActionType>( false );
            if ( entityType != null )
            {
                foreach ( var component in this.Components )
                {
                    using ( var rockContext = new Rock.Data.RockContext() )
                    {
                        ...
                        Rock.Attribute.Helper.UpdateAttributes( type, entityType.Id, "EntityTypeId", Rock.Cache.CacheEntityType.GetId( type.FullName ).ToString(), rockContext );
                    }
                }
            }
        }
