    ...
    public class WorkflowFieldType : FieldType, IEntityFieldType, IEntityReferenceFieldType, ISecurityGrantFieldType
    {
        ...
        internal override FieldTypeHints GetFieldHints( Dictionary<string, string> privateConfigurationValues )
        {
            return new FieldTypeHints
            {
                ...
                ValueFormat = "The guid of a single row in the Workflow table, not its id or idKey and not its name. Only one value is stored, so a comma separated list is not valid here. This is a workflow that has been started, not the workflow type it was launched from.",
                Instructions = "To find the correct value, read the workflows and take the guid of the one you want."
            };
        }
        ...
        public override void SetEditValue( System.Web.UI.Control control, Dictionary<string, ConfigurationValue> configurationValues, string value )
        {
            ...
            if ( workflowPicker != null )
            {
                ...
                if ( guid.HasValue )
                {
                    var workflowId = new WorkflowService( RockApp.Current.CreateRockContext() ).GetId( guid.Value );
                    if ( workflowId != null )
                    {
        ...
        public int? GetEditValueAsEntityId( Control control, Dictionary<string, ConfigurationValue> configurationValues )
        {
            ...
            return new WorkflowService( RockApp.Current.CreateRockContext() ).GetId( guid );
        }
        ...
        {
            var itemGuid = new WorkflowService( RockApp.Current.CreateRockContext() ).GetGuid( id ?? 0 );
