        ...
        public override void SetEditValue( Control control, Dictionary<string, ConfigurationValue> configurationValues, string value )
        {
            ...
            if ( pnlMatrixEdit != null && configurationValues.ContainsKey( ATTRIBUTE_MATRIX_TEMPLATE ) )
            {
                ...
                if ( attributeMatrixTemplateId.HasValue )
                {
                    ...
                    {
                        // create a clone of the orig to work with. That way if they Cancel on the parent object, changes to the orig won't be saved
                        var origAttributeMatrix = attributeMatrix;
                        var origItems = origAttributeMatrix.AttributeMatrixItems.ToList();
                        attributeMatrix = new AttributeMatrix();
                        attributeMatrix.AttributeMatrixTemplateId = origAttributeMatrix.AttributeMatrixTemplateId;
                        attributeMatrix.Id = 0;
                        attributeMatrix.Guid = Guid.NewGuid();
                        attributeMatrix.AttributeMatrixItems = new List<AttributeMatrixItem>();
                        foreach ( var attributeMatrixItem in origItems )
                        {
                            attributeMatrixItem.LoadAttributes();
                            var clonedItem = attributeMatrixItem.Clone( true );
                            clonedItem.Id = 0;
                            clonedItem.Guid = Guid.NewGuid();
                            clonedItem.AttributeMatrix = attributeMatrix;
                            attributeMatrix.AttributeMatrixItems.Add( clonedItem );
                        }

                        attributeMatrixService.Add( attributeMatrix );
                        rockContext.SaveChanges();
                        foreach ( var item in attributeMatrix.AttributeMatrixItems )
                        {
