...
namespace Rock.Cms.ContentLibrary
{
    ...
    {
        private static readonly ConcurrentQueue<ValueItem> _libraryQueue = new ConcurrentQueue<ValueItem>();

        private static readonly object _processLock = new object();
        private static int _taskRunning = 0;

        /// <summary>
        /// Add a new attribute field value for the content library. This will
        ...
        /// value is indexed.
        /// </summary>
        /// <param name="contentLibraryId">The identifier of the content library this field value belongs to.</param>
        ...
        public static void AddFieldValue( int contentLibraryId, string key, string value, string formattedValue )
        {
            ...
            _libraryQueue.Enqueue( new ValueItem
            {
        ...
        /// <param name="value">The value of the attribute that was indexed.</param>
        /// <param name="formattedValue">The formatted value of the attribute that was indexed.</param>
        public static void AddAttributeFieldValue( int contentLibraryId, string key, string value, string formattedValue )
        {
            if ( value.IsNullOrWhiteSpace() )
            {
                return;
        ...
        private static async Task ProcessQueueAsync()
        {
            ...
            while ( _libraryQueue.TryDequeue( out var item ) )
            {
            ...
            // This keeps two tasks from updating the same content library
            // at the same time.
            lock ( _processLock )
            {
        ...
        {
            // Process each content library as single update.
            var libraryItems = items.GroupBy( i => i.ContentLibraryId );

            using ( var rockContext = new RockContext() )
            {
                var contentLibraryService = new ContentLibraryService( rockContext );

                foreach ( var itemGroup in libraryItems )
                {
                    var contentLibrary = contentLibraryService.Get( itemGroup.Key );
                    ...
                    var filterSettings = contentLibrary.FilterSettings.FromJsonOrNull<ContentLibraryFilterSettingsBag>() ?? new ContentLibraryFilterSettingsBag();
                    bool modified = false;
                    ...
                    if ( modified )
                    {
                        contentLibrary.FilterSettings = filterSettings.ToJson();
                    }
                }
        ...
        {
            /// <summary>
            /// Gets or sets the content library identifier.
            /// </summary>
            /// <value>The content library identifier.</value>
