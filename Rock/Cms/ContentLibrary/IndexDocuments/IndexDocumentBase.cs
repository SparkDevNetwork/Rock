...
namespace Rock.Cms.ContentLibrary.IndexDocuments
{
    /// <summary>
    /// Base index document for the content library search index.
    /// </summary>
    internal class IndexDocumentBase : DynamicObject, ILavaDataDictionary, Lava.ILiquidizable
    {
        ...
        */
        ...
        /// <param name="source">The library source this model is associated with.</param>
        internal void AddIndexableAttributes( Attribute.IHasAttributes sourceModel, ContentLibrarySourceCache source )
        {
            ...
            foreach ( var attributeValue in sourceModel.AttributeValues )
            {
                ...
                FieldValueHelper.AddAttributeFieldValue( source.ContentLibraryId, key, value, formattedValue );
            }
        }
        ...
        /// </summary>
        internal async Task AddExistingTrendingDataAsync( ContentLibrarySourceCache source )
        {
            if ( !source.ContentLibrary.TrendingEnabled )
            {
                return;
            }
        ...
        /// <param name="source">The library source this model is associated with.</param>
        internal void AddPersonalizationData( IEntity entity, ContentLibrarySourceCache source )
        {
            // Check if personalization is enabled.
            if ( !source.ContentLibrary.EnableSegments && !source.ContentLibrary.EnableRequestFilters )
            {
                return;
            }
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( source.ContentLibrary.EnableSegments )
                {
                ...
                if ( source.ContentLibrary.EnableRequestFilters )
                {
