...
{
    /// <summary>
    /// Content Library Indexer for <see cref="ContentChannelItem"/> entities.
    /// </summary>
    internal class ContentChannelItemIndexer : IContentLibraryIndexer
    {
        /// <inheritdoc/>
        ...
        public async Task<int> IndexAllContentLibrarySourceDocumentsAsync( int sourceId, IndexDocumentOptions options )
        {
            ...
            if ( contentLibrarySourceCache == null || contentLibrarySourceCache.EntityTypeId != contentChannelEntityTypeId )
            {
                return 0;
            }
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                items = new ContentChannelItemService( rockContext ).Queryable()
                    .AsNoTracking()
                    .Where( cci => cci.ContentChannelId == contentLibrarySourceCache.EntityId
                ...
                trending = options.IsTrendingEnabled && contentLibrarySourceCache.ContentLibrary.TrendingEnabled
                    ? GetTrendingRanksLookup( contentLibrarySourceCache, rockContext )
                    : null;
            }
            ...
            {
                var document = await ContentChannelItemDocument.LoadByModelAsync( contentChannelItem, contentLibrarySourceCache );
        ...
        private static Dictionary<int, int> GetTrendingRanksLookup( ContentLibrarySourceCache source, RockContext rockContext )
        {
            var daysBack = source.ContentLibrary.TrendingWindowDay;
            var gravity = source.ContentLibrary.TrendingGravity;
            var maxItems = source.ContentLibrary.TrendingMaxItems;
            ...
            return ContentLibrarySourceService.CalculateTrendingRanksLookup( viewCounts, cutOffDate, gravity, maxItems );
        }
    }
}
