...
namespace Rock.Jobs
{
    ...
    public class ConnectionRequestWorkflowTriggers : RockJob
    {
...
#else
                        ExceptionLogService.LogException( new Exception( $"Exception occurred trying to trigger future follow-up workflow: {connectionRequestId}.", ex ) );
#endif
                        recordsWithError++;
                    }
                }
        ...
        private string SetJobResultSummaryForTriggerFutureFollowup( int recordsUpdated, int workflowsTriggered, int recordsWithError )
        {
            ...
            jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-success'></i> {recordsUpdated:N0} connection {"request".PluralizeIf( recordsUpdated != 1 )} updated" );
            jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-success'></i> {workflowsTriggered:N0} {"workflow".PluralizeIf( workflowsTriggered != 1 )} triggered" );

            if ( recordsWithError > 0 )
            {
                jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-danger'></i> {recordsWithError:N0} {"record".PluralizeIf( recordsWithError != 1 )} logged an exception" );
            }
