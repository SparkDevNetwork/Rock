...
namespace Rock.Jobs
{
    ...
    public class SendCreditCardExpirationNotices : RockJob
    {
        ...
        public override void Execute()
        {
            ...
            {
                jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-success'></i> {sendExpiredCreditCardNoticesResult.NoticesSentCount } notice(s) sent." );
            }
            ...
            {
                jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-danger'></i> {sendExpiredCreditCardNoticesResult.EmailSendExceptions.Count()} error(s) occurred when sending expired credit card notice(s). See exception log for details." );
                this.UpdateLastStatusMessage( jobSummaryBuilder.ToString() );
            ...
            {
                jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-success'></i> {removeExpiredSavedAccountsResult.AccountsDeletedCount} saved account(s) that expired before {removeExpiredSavedAccountsResult.DeleteIfExpiredBeforeDate.ToShortDateString()} removed." );
                this.UpdateLastStatusMessage( jobSummaryBuilder.ToString() );
            }
            ...
            {
                jobSummaryBuilder.AppendLine( $"<i class='ti ti-circle-filled text-danger'></i> {removeExpiredSavedAccountsResult.AccountRemovalExceptions.Count()} error(s) occurred when removing saved, expired account(s). See exception log for details." );
                this.UpdateLastStatusMessage( jobSummaryBuilder.ToString() );
        ...
        /// </summary>
        /// <returns>FinancialPersonSavedAccountService.RemoveExpiredSavedAccountsResult.</returns>
        private FinancialPersonSavedAccountService.RemoveExpiredSavedAccountsResult RemoveExpiredSavedAccounts()
        {
