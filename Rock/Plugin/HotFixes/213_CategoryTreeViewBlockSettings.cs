...
namespace Rock.Plugin.HotFixes
{
    ...
    {
        private const string allChurchCategoryGuid = "5A94E584-35F0-4214-91F1-D72531CC6325";
        private const string categoryTreeViewBlockTypeGuid = "ADE003C7-649B-466A-872B-B8AC952E7841";
        private const string categoryTreeViewInstanceGuid = "42E90A50-D8EC-4370-B970-83E48518BC26";
        private const string entityTypeFriendlyNameAttributeGuid = "07213E2C-C239-47CA-A781-F7A908756DC2";
        private const string generalCategoryGuid = "4B2D88F5-6E45-4B4B-8776-11118C8E8269";
        private const string pageParameterKeyAttributeGuid = "AA057D3E-00CC-42BD-9998-600873356EDB";
        private const string showOnlyCategoriesAttributeGuid = "845AC4E4-ACD1-40CC-96F6-8D22136C30CC";
        ...
        {
            Sql( $@"
DECLARE @categoryTreeViewBlockId INT = (SELECT [Id] FROM [Block] WHERE [Guid] = '{categoryTreeViewInstanceGuid}');
DECLARE @showOnlyCategoriesAttributeGuid NVARCHAR(40) = '{showOnlyCategoriesAttributeGuid}';
DECLARE @showOnlyCategoriesAttributeId INT = (SELECT [Id] FROM [dbo].[Attribute] WHERE [Guid] = @showOnlyCategoriesAttributeGuid);
DECLARE @pageParameterKeyAttributeId INT = (SELECT [Id] FROM [dbo].[Attribute] WHERE [Guid] = '{pageParameterKeyAttributeGuid}');
...
UPDATE [dbo].[Category] SET
    IsSystem = 1
WHERE [Guid] IN ('{generalCategoryGuid}', '{allChurchCategoryGuid}')
" );
        }
...
DECLARE @categoryTreeViewBlockTypeId INT = (SELECT Id FROM [BlockType] WHERE [Guid] = '{categoryTreeViewBlockTypeGuid}');
DECLARE @categoryTreeViewBlockId INT = (SELECT [Id] FROM [Block] WHERE [Guid] = '{categoryTreeViewInstanceGuid}');

DECLARE @showOnlyCategoriesAttributeGuid NVARCHAR(40) = '{showOnlyCategoriesAttributeGuid}';
DECLARE @showOnlyCategoriesAttributeId INT = (SELECT [Id] FROM [dbo].[Attribute] WHERE [Guid] = @showOnlyCategoriesAttributeGuid);
DECLARE @pageParameterKeyAttributeId INT = (SELECT [Id] FROM [dbo].[Attribute] WHERE [Guid] = '{pageParameterKeyAttributeGuid}');
DECLARE @entityTypeFriendlyNameAttributeId INT = (SELECT TOP 1 Id FROM Attribute WHERE [Guid] = '{entityTypeFriendlyNameAttributeGuid}');
...
BEGIN
	...
	SELECT 
		...
		@showOnlyCategoriesAttributeGuid[Guid]
