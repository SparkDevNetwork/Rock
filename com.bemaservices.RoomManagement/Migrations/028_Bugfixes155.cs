...
From _com_bemaservices_RoomManagement_ReservationResource
Group By [Guid]
) t1
Where GuidCount > 1
...
From (
Select [Guid], count(0) as GuidCount
From _com_bemaservices_RoomManagement_ReservationLocation
Group By [Guid]
) t1
Where GuidCount > 1

Update _com_bemaservices_RoomManagement_ReservationResource
Set Guid = NewId()
Where Guid in ( Select DuplicateGuid from @DuplicateGuids)

Update _com_bemaservices_RoomManagement_ReservationLocation
Set Guid = NewId()
Where Guid in ( Select DuplicateGuid from @DuplicateGuids)
