    ...
    public class StepParticipantList : RockEntityListBlockType<Step>
    {
        ...
        private StepParticipantListOptionsBag GetBoxOptions()
        {
            ...
            var options = new StepParticipantListOptionsBag()
            {
                ...
                StepStatusBackgroundColors = GetStepStatusBackgroundColors()
            };
            return options;
        }
        ...
        /// </summary>
        /// <returns></returns>
        private Dictionary<string, string> GetStepStatusBackgroundColors()
        {
        ...
        protected override IQueryable<Step> GetListQueryable( RockContext rockContext )
        {
            ...
            {
                queryable = queryable.Where( m => m.StartDateTime >= FilterDateStartedLower.Value );
            }

            if ( FilterDateStartedUpper.HasValue )
            {
                var exclusiveEndDate = FilterDateStartedUpper.Value.Date.AddDays( 1 );
            ...
            {
                queryable = queryable.Where( m => m.CompletedDateTime >= FilterDateCompletedLower.Value );
            }

            if ( FilterDateCompletedUpper.HasValue )
            {
                var exclusiveEndDate = FilterDateCompletedUpper.Value.Date.AddDays( 1 );
