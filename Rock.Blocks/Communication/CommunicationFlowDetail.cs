...
namespace Rock.Blocks.Communication
{
    ...
    public class CommunicationFlowDetail : RockBlockType, IBreadCrumbBlock
    {
        ...
        private CommunicationFlowDetailCommunicationTemplateBag GetCommunicationTemplateAsBag( CommunicationTemplate communicationTemplate )
        {
            ...
            {
                bag.PushOpenAction = ( PushOpenAction )( int )communicationTemplate.PushOpenAction.Value;
            }
        ...
        private CommunicationFlowDetailConversionGoalSettingsBag GetConversionGoalSettingsBag( CommunicationFlow entity )
        {
            ...
            if ( settings.TookStepSettings != null )
            {
                ...
                {
                    StepProgram = StepProgramCache.Get( settings.TookStepSettings.StepProgramGuid ).ToListItemBag(),
        ...
        private CommunicationFlow.ConversionGoalSettings GetConversionGoalSettings( CommunicationFlowDetailConversionGoalSettingsBag bag )
        {
            ...
            if ( bag.EnteredDataViewSettings != null )
            {
                // Ensure a real Data View GUID is used.
                var dataView = DataViewCache.Get( bag.EnteredDataViewSettings.DataView?.Value );

                if ( dataView != null )
                {
                    settings.EnteredDataViewSettings = new CommunicationFlow.EnteredDataViewConversionGoalSettings
                    {
            ...
            if ( bag.TookStepSettings != null )
            {
                // Ensure a real StepProgram GUID and StepType GUID is used.
                var stepProgram = StepProgramCache.Get( bag.TookStepSettings.StepProgram?.Value );
                var stepType = StepTypeCache.Get( bag.TookStepSettings.StepType?.Value );

                if ( stepProgram != null && stepType != null )
                {
                    settings.TookStepSettings = new CommunicationFlow.TookStepConversionGoalSettings
                    {
        ...
        private bool UpdateEntityFromBag( RockContext rockContext, CommunicationFlow entity, CommunicationFlowBag bag )
        {
            ...
            if ( entity.TriggerType == CommunicationFlowTriggerType.Recurring || entity.TriggerType == CommunicationFlowTriggerType.OneTime )
            {
            ...
            else if ( entity.TriggerType == CommunicationFlowTriggerType.OnDemand )
            {
                entity.TargetAudienceDataViewId = null;
                entity.TargetAudienceDataView = null;
