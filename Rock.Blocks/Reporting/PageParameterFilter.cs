...
namespace Rock.Blocks.Reporting
{
    ...
    public class PageParameterFilter : RockBlockType, IHasCustomActions
    {
        ...
        /// </summary>
        /// <remarks>
        /// This will be called when the block is operating in <see cref="FilterSelectionAction.UpdateFilters"/> mode,
        /// which means the administrator has indicated that some filters' AVAILABLE values are dependent upon the
        /// SELECTED values of other filters. We need to set the current filter private values on this request context's
        /// parameters collection (so that low-level Lava processes have access to them) and instruct all filters to
        /// rebuild their available and selected values, which we'll then send back to the client. Filters will not be
        ...
        /// </summary>
        /// <remarks>
        /// Consider this to be a translation method: when the individual makes filter selections within the public
        /// attribute edit controls, those "public" values need to be translated to the corresponding "private" values
        /// that can be used within the query string. Each filter attribute's field type knows how to do this
        /// translation for us.
        /// </remarks>
