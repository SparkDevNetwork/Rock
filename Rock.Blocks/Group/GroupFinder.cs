...
namespace Rock.Blocks.Group
{
    ...
    [DefinedValueField( Rock.SystemGuid.DefinedType.MAP_STYLES,
        "Map Style",
...
<div class=""group-finder-card-content"">
    ...
    <div class=""group-finder-card-body"">
        ...
        /// <summary>
        /// The offensive short codes a name-derived campus badge must not display, taken from the check-in
        /// security-code list. Held as a set so each badge screen is an O(1) lookup rather than a scan of
        /// the underlying list.
        /// </summary>
        private static readonly HashSet<string> _noGoodBadgeCodes = new HashSet<string>( AttendanceCodeService.NoGood, StringComparer.OrdinalIgnoreCase );
        ...
        public override object GetObsidianBlockInitialization()
        {
            ...
            return new GroupFinderInitializationBox
            {
                ...
                ConfigurationWarning = GetConfigurationWarning( isMapConfigured && !isMapAvailable, proximityEnabled && !isGeocodingAvailable ),
                PageSize = ResultsPageSize,
                IsCampusFilterShown = !GetAttributeValue( AttributeKey.HideCampusFilters ).AsBoolean(),
                IsWhereFilterShown = !GetAttributeValue( AttributeKey.HideWhereFilters ).AsBoolean(),
                IsWhenFilterShown = !GetAttributeValue( AttributeKey.HideWhenFilters ).AsBoolean(),
                IsWhatFilterShown = !GetAttributeValue( AttributeKey.HideWhatFilters ).AsBoolean(),
                IsMeetingStyleFilterShown = meetingStyles.Any(),
        ...
        /// <summary>
        /// Builds the administrator-only configuration warning for missing Google keys, or null when nothing is missing or the visitor cannot administrate the block.
        /// </summary>
        ...
        /// <returns>The warning message, or <c>null</c>.</returns>
        private string GetConfigurationWarning( bool isMapKeyMissing, bool isGeocodingKeyMissing )
        {
            if ( !isMapKeyMissing && !isGeocodingKeyMissing )
            {
                return null;
            }

            // Only surface configuration problems to someone who can fix them; visitors just get the
            // graceful degradation (list-only, or current-location-only).
            ...
            return $"Group Finder is not fully configured: {string.Join( "; and ", issues )}. Only administrators see this message.";
        }
        ...
        /// </summary>
        /// <returns>The campus items, each with its name and an avatar badge.</returns>
        private List<GroupFinderCampusBag> GetFilterCampuses()
        {
            ...
            return CampusCache.All()
                ...
                .OrderBy( c => c.Order )
                .Select( c => new GroupFinderCampusBag
                {
                    Value = c.Guid.ToString(),
                    Text = c.Name,
                    Badge = GetCampusBadge( c )
                } )
                .ToList();
        }

        /// <summary>
        /// Builds the short avatar badge for a campus: the admin's short code when set, otherwise a
        /// badge derived from the first letters of the campus name.
        /// </summary>
        /// <param name="campus">The campus.</param>
        /// <returns>The admin short code as entered, or an uppercase name-derived badge that is never an offensive combination.</returns>
        private static string GetCampusBadge( CampusCache campus )
        {
            // An admin-configured short code is the intended label, so show it exactly as entered.
            if ( campus.ShortCode.IsNotNullOrWhiteSpace() )
            {
                return campus.ShortCode;
            }

            var letters = new string( ( campus.Name ?? string.Empty ).Where( char.IsLetterOrDigit ).ToArray() ).ToUpper();
            if ( letters.Length == 0 )
            {
                return string.Empty;
            }
            ...
            /*
                ...
                Only the name-derived fallback is screened; an explicit short code is trusted as typed.
                Slide a three-character window across the name's alphanumeric characters and take the first
                window that is not in the check-in security-code NoGood list (the offensive combinations Rock
                avoids when generating attendance codes), so an unlucky run of letters never renders a bad
                word in the avatar.

                Reason: Never auto-generate an offensive campus badge.
            */
            for ( var i = 0; i + 3 <= letters.Length; i++ )
            {
                var candidate = letters.Substring( i, 3 );
                if ( !_noGoodBadgeCodes.Contains( candidate ) )
                {
                    return candidate;
                }
            }

            // Every three-character window was offensive (extremely unlikely); the first two letters
            // cannot match a three-character entry.
        ...
        private (GeographyPoint Point, GeographyBounds Viewport) ResolveOrigin( string origin )
        {
            ...
            */
            ...
            {
                RockLogger.Log.Warning( RockLogDomains.Group, ex, "Group Finder could not geocode the origin '{Origin}'.", origin );
                return (null, null);
            }
        }
        ...
        private Dictionary<int, (double Miles, double Minutes)> GetDriveMatrixByGroup( GeographyPoint origin, List<int> groupIds, Dictionary<int, (double Latitude, double Longitude)> fuzzedByGroup )
        {
            ...
            */
            ...
            {
                RockLogger.Log.Warning( RockLogDomains.Group, ex, "Group Finder could not compute drive distances; straight-line distance stands in." );
            }
